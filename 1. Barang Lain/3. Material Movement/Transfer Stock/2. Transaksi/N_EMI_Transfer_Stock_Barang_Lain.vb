...
Public Class N_EMI_Transfer_Stock_Barang_Lain

	Dim Flag_Opname As Boolean = False

	Public Property MenuAsal As String = "TRANSFER_STOCK"
	...
	Dim dgv_Rekap_lokasi, dgv_Rekap_KdBarang, dgv_Rekap_NmBarang, dgv_Rekap_Jumlah, dgv_Rekap_JumlahBags, dgv_Rekap_JumlahBersih, dgv_Rekap_Satuan, dgv_Rekap_SatuanKecil, dgv_Rekap_Oto, dgv_Rekap_JnsTransfer, dgv_rekap_otoReqGen, dgv_rekap_UrutKeep,
		dgv_rekap_JnsGudang, dgv_rekap_urut_pr_dept, dgv_rekap_ket_pr_dept, dgv_rekap_ket_no_pr_dept As String
	...
	Dim dgv_detail_Warna, dgv_detail_JenisKemasan, dgv_detail_TglProduksi, dgv_detail_TglExpired, dgv_detail_JenisKualitas, dgv_detail_Barcode, dgv_detail_JnsTransfer,
	dgv_detail_no_pr, dgv_detail_keterangan_pr As String
	...
	Dim itemDgvRekap_UrutKeep As Integer = 11
	Dim itemDgvRekap_JnsGudang As Integer = 12
	Dim itemDgvRekap_Urut_Prdept As Integer = 13
	Dim itemDgvRekap_KetPrDept As Integer = 14
	Dim itemDgvRekap_KetNoPR As Integer = 15
	...
	Dim itemDgvDetail_No_Pr As Integer = 18
	Dim itemDgvDetail_Keterangan_Pr As Integer = 19

	'Dim itemDgvIDWareHouseTujuan As Integer = 10

	Dim index_waste As Integer = -1

	Public isProduction As Boolean = False
	Dim isKeepStock As Boolean = False

	Dim Kategori_Gudang_Asal, Kategori_Gudang_Tujuan As String
	Dim Jenis_Gudang_Asal, Jenis_Gudang_Tujuan As String

	Dim Selected_Id_Kategori_Gudang As String = ""

	Private Sub N_EMI_Transfer_Stock_Barang_Lain_Activated(sender As Object, e As EventArgs) Handles Me.Activated
		My.Application.ChangeCulture("en-us")
		My.Application.ChangeUICulture("en-us")
	End Sub

	Private Sub N_EMI_Transfer_Stock_Barang_Lain_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		My.Application.ChangeCulture("en-us")
		My.Application.ChangeUICulture("en-us")
	...
	Private Sub get_grid_view_Rekap(ByVal index As Integer)
		...
		dgv_rekap_UrutKeep = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_UrutKeep).Value
		dgv_rekap_JnsGudang = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_JnsGudang).Value
		dgv_rekap_urut_pr_dept = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_Urut_Prdept).Value
		dgv_rekap_ket_pr_dept = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_KetPrDept).Value
		dgv_rekap_ket_no_pr_dept = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_KetNoPR).Value
	...
	Private Sub get_grid_view_Detail(ByVal index As Integer)
		...
		dgv_detail_no_pr = Dgv_DataDetail.Rows(index).Cells(itemDgvDetail_No_Pr).Value
		dgv_detail_keterangan_pr = Dgv_DataDetail.Rows(index).Cells(itemDgvRekap_KetNoPR).Value
	...
	Public Sub kosong()
		...
		Try
			...
			asal = "N_EMI_Transfer_Stock_Barang_Lain"
			...
			Cmb_Warna.SelectedIndex = 2 : arrSO.Clear()
		...
		Selected_Id_Kategori_Gudang = ""

		Cmb_MenuAsal.Items.Clear()
		Cmb_MenuAsal.Items.Add("TRANSFER STOCK")
		Cmb_MenuAsal.Items.Add("TRANSFER WASTE")

		'Btn_List_Keep_Stock.Enabled = False
		isKeepStock = False

		Kategori_Gudang_Asal = ""
		Kategori_Gudang_Tujuan = ""
		Jenis_Gudang_Asal = ""
		Jenis_Gudang_Tujuan = ""

		If MenuAsal Is Nothing Then
			MessageBox.Show("Terjadi Kesalahan Saat Membuka Menu", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)

			' Gunakan BeginInvoke untuk menunda penutupan sampai Form selesai tampil
			Me.BeginInvoke(New MethodInvoker(Sub()
												 Me.Close()
											 End Sub))
			'Exit Sub
		Else
			If MenuAsal.ToUpper = "TRANSFER_STOCK" Then
				Cmb_MenuAsal.SelectedIndex = 0
			ElseIf MenuAsal.ToUpper = "TRANSFER_WASTE" Then
				Cmb_MenuAsal.SelectedIndex = 1

				CmbJnsTransfer.SelectedIndex = 1

				CmbSo_Tujuan.SelectedIndex = index_waste

				CmbJnsTransfer.Enabled = False
				CmbSo_Tujuan.Enabled = True
				Button1.Enabled = False
				Btn_ListGeneral.Enabled = False
				TxtjmlPermintaanDisplay.Enabled = False
				TxtjmlPermintaanBersih.Enabled = False
			Else
				MessageBox.Show("Terjadi Kesalahan Saat Membuka Menu", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Exit Sub
				Me.Close()
		...
		If MenuAsal = "TRANSFER_STOCK" Then
			CmbJnsTransfer.DroppedDown = True
			CmbJnsTransfer.Focus()
		ElseIf MenuAsal = "TRANSFER_WASTE" Then
			TxtKeterangan.Focus()
		End If

	End Sub

	Private Sub hapus_sementara()
		SQL = "delete from N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara where kode_perusahaan = '" & KodePerusahaan & "' "
	...
	Private Sub DGV_Data_TF_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellEndEdit
		...
		If DGV_Data_TF.CurrentRow.Cells(itemDgvCheckBox).Value = "True" Then
			...
			If MenuAsal <> "TRANSFER_WASTE" Then
			...
			'Dim Nilai_Toleransi_Min As Double = 0
			'Dim Nilai_Toleransi_Max As Double = 0
			...
			'        SQL = SQL & "and Kode_Stock_Owner = '" & arrSO(CmbSO_Asal.SelectedIndex) & "' and Kode_Barang = '" & DGV_Data_TF.CurrentRow.Cells(itemDgvKodeBarang).Value & "' "
			'        Using Dr = OpenTrans(SQL)
			'            If Dr.Read Then
			...
			'        Nilai_Toleransi_Min = Val(HilangkanTanda(TxtjmlPermintaanBersih.Text)) - (Val(HilangkanTanda(TxtjmlPermintaanBersih.Text)) * (Persen_Toleransi_Min / 100))
			'        Nilai_Toleransi_Max = Val(HilangkanTanda(TxtjmlPermintaanBersih.Text)) + (Val(HilangkanTanda(TxtjmlPermintaanBersih.Text)) * (Persen_Toleransi_Max / 100))
			...
			If DGV_Data_TF.CurrentRow.Cells(itemJenisKemasan).Value.ToString.ToUpper = "ORIGINAL BAGS" Then
				...
				If Not DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = "" Then
					...
					If jumlahInputBags < 0 Then
						MessageBox.Show("Jumlah transfer tidak boleh kurang dari 1 ", "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
						DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
						DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
						Exit Sub
					End If
					...
					'If isProduction Then
					'    If currentColumn = itemDgvBags OrElse currentColumn = itemDgvJumlah Then
					'        If valueJumlah < Nilai_Toleransi_Min Then
					'            MessageBox.Show("Jumlah Kurang Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
					'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
					'            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
					'            Exit Sub
					'        ElseIf valueJumlah > Nilai_Toleransi_Max Then
					'            MessageBox.Show("Jumlah Lebih Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
					'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
					'            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
					'            Exit Sub
					'        End If
					...
					DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(valueJumlah, "N4")
					If valueJumlah > jumlahStock Then
						DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(jumlahStock, "N4")
			...
			Else
				...
				'If isProduction Then
				'    If currentColumn = itemDgvBags OrElse currentColumn = itemDgvJumlah Then
				'        If jumlahInput < Nilai_Toleransi_Min Then
				'            MessageBox.Show("Jumlah Kurang Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
				'            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
				'            Exit Sub
				'        ElseIf jumlahInput > Nilai_Toleransi_Max Then
				'            MessageBox.Show("Jumlah Lebih Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
				'            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
				'            Exit Sub
		...
		If currentColumnIndex = itemDgvBags OrElse currentColumnIndex = itemDgvJumlah Then
			...
			If jumlahValue IsNot Nothing AndAlso IsNumeric(jumlahValue) Then
				DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Convert.ToDecimal(jumlahValue).ToString("N4")
			Else
				DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
			End If
			...
			If bagsValue IsNot Nothing AndAlso IsNumeric(bagsValue) Then
				DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Convert.ToDecimal(bagsValue).ToString("N4")
			Else
				DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
			End If
		End If
	...
	Private Sub get_no_faktur()
		Dim FPro_Results As String = "TL-"
		TxtNo_Transaksi.Text = FPro_Results & arrInisialFaktur.Item(CmbSO_Asal.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy") & "-" &
									  General_Class.Get_Last_Number2("N_EMI_Transfer_Stock_Barang_Lain", "no_faktur", JumlahDigit,
	...
	Private Sub get_no_faktur_sementara()
		Dim FPro_Results As String = "TF-"
		txtNoFaktur_Sementara.Text = FPro_Results & "TM-" & Format(tgl_skg, "MM/yy") & "-" &
									  General_Class.Get_Last_Number2("N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara", "no_faktur", JumlahDigit,
									  "Kode_perusahaan", KodePerusahaan,
									  "And", "substring(no_faktur,1," & Len(FPro_Results) + Len("TM") + 6 & ")", FPro_Results & "TM" & "-" & Format(tgl_skg, "MM/yy"))
	...
	Private Sub TxtKd_Barang_TextChanged(sender As Object, e As EventArgs) Handles TxtKd_Barang.TextChanged, Txt_SO.TextChanged, Txt_SatuanPermintaan.TextChanged, Txt_JumlahPermintaan.TextChanged, Txt_OtoMaterial_req.TextChanged
		If asal <> "Emi_Display_Request_Material" Then
			...
			Else
				Lv_DetBarang.Location = New Point(1278, 252)
			...
			Try
				...
				SQL = $"
                    ...
                    select top(20) a.kode_stock_owner, a.kode_barang, a.nama, dbo.Ubah_Satuan_Lain(a.kode_Perusahaan, 'masa', a.kode_barang, a.satuan,
	                    b.satuan, a.good_stock) as Good_Stock, a.Satuan, b.satuan as satuan_display, ISNULL(a.Jumlah_Bags, 0) as Jumlah_Bags,
	                    a.Satuan_Isi_Bags, a.Metode_Pengeluaran_Stok, a.Jenis_Kemasan
                    from Barang_Lain a, Barang_Detail_Satuan_Lain b, View_Kategori_Turunan c, cte d
                    where a.Kode_Perusahaan='{KodePerusahaan}'
	                    and a.Kode_Stock_Owner='{arrSO(CmbSO_Asal.SelectedIndex)}'
	                    and a.kode_barang like '%{TxtKd_Barang.Text}%' and a.Kode_Barang=b.kode_barang
	                    And a.kode_Perusahaan = b.kode_Perusahaan And b.flag_tampil_display ='Y'
	                    and a.Kode_Perusahaan = c.Kode_Perusahaan and c.Id_Sub_Kategori_Jenis_3 = a.Id_Sub_Kategori_Jenis_3
	                    and c.Kode_Perusahaan = d.Kode_Perusahaan and c.Id_Kategori_Jenis = d.Id_Kategori_Jenis and c.Id_Sub_Kategori_Jenis = d.Id_Sub_Kategori_Jenis and a.Kode_Stock_Owner = d.Kode_Stock_Owner_Gudang
				...
				Using Dr = OpenTrans(SQL)
					Do While Dr.Read
						...
						If Flag_Opname Then
							Lv.SubItems.Add(0)
						Else
							Lv.SubItems.Add(If(General_Class.CekNULL(Dr("Good_Stock")) = "", "", Format(Dr("Good_Stock"), "N4")))
						End If
	...
	Private Sub Lv_DetBarang_DoubleClick(sender As Object, e As EventArgs) Handles Lv_DetBarang.DoubleClick
		...
		TxtStockDisplay.Text = Format(Val(HilangkanTanda(lv_DetGoodStock)), "N4") + " " + lv_DetSatuanDIsplay
	...
	Public Sub Btn_Insert_Click(sender As Object, e As EventArgs) Handles Btn_GetData.Click
		...
		Try
			...
			SQL = $"
                ...
                from Barang_Lain a
	                inner join View_Kategori_Turunan b on a.kode_perusahaan = b.kode_perusahaan and a.Id_Sub_Kategori_Jenis_3 = b.Id_Sub_Kategori_Jenis_3
	                inner join cte c on b.Kode_Perusahaan = c.Kode_Perusahaan and b.Id_Kategori_Jenis = c.Id_Kategori_Jenis and b.Id_Sub_Kategori_Jenis = c.Id_Sub_Kategori_Jenis and a.Kode_Stock_Owner = c.Kode_Stock_Owner_Gudang
                where a.Kode_Perusahaan = '{KodePerusahaan}'
	                and a.Kode_Stock_Owner = '{arrSO(CmbSO_Asal.SelectedIndex)}'
	                and a.Kode_Barang = '{TxtKd_Barang.Text.Trim}'
	                and c.user_id = '{UserID}'
            "
			Using Dr = OpenTrans(SQL)
				If Not Dr.Read Then
					CloseConn()
					MessageBox.Show($"User Tidak Memiliki Akses Untuk Transaksi kode Barang {TxtKd_Barang.Text.Trim}", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
					Exit Sub
				End If
			End Using
			...
			For index = 0 To Dgv_DataRekap.Rows.Count - 1
				...
				If TxtKd_Barang.Text = dgv_Rekap_KdBarang And Urut_Pr_Dept.Text = dgv_rekap_urut_pr_dept Then
			...
			SQL = "Select a.Kode_Stock_Owner, a.Kode_Barang, 'X' as Serial_Number, b.Nama, a.Id_Warehouse, c.Keterangan As kode_rak, a.Id_Nametag_pallet, "

			SQL = SQL & "dbo.Ubah_Satuan_Lain(a.kode_Perusahaan, 'masa', a.kode_barang, b.satuan, '" & TxtSatuan.Text & "', sum(a.jumlah)) as jumlah, "

			SQL = SQL & "b.satuan, 0 As nomor_pallet, isNull(sum(a.Jumlah_Bags), 0) As stock_bags, a.warna, b.Metode_Pengeluaran_Stok, "
			SQL = SQL & "b.Jenis_Kemasan, isNull(b.Isi_Per_Bags, 0) As Isi_Per_Bags, b.Satuan_Isi_Bags, a.Tgl_Expired, a.Tgl_Produksi, a.Tgl_Masuk "
			...
			SQL = SQL & "isnull((select sum(x.total) from N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara x "
			SQL = SQL & "where a.Kode_Perusahaan= x.Kode_Perusahaan  and  a.Qr_Code +'-' + a.Kode_Unik_Berjalan = x.qr_code "
			SQL = SQL & "), 0) as jumlah_tf_sementara, "

			SQL = SQL & "isnull((select sum(x.total_bags) from N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara x "
			SQL = SQL & "where a.Kode_Perusahaan= x.Kode_Perusahaan  and  a.Qr_Code +'-' + a.Kode_Unik_Berjalan = x.qr_code "
			SQL = SQL & "), 0) as jumlah_tf_bag_sementara, "

			SQL = SQL & "ISNULL((select sum(x.jumlah) from N_EMI_Transfer_Stock_Barang_Lain_Det x, "
			SQL = SQL & "N_EMI_Transfer_Stock_Barang_Lain y, barang_lain_sn z "
			SQL = SQL & "where x.No_Faktur = y.No_Faktur "
			SQL = SQL & "and y.Status is null and x.Selesai is null "
			SQL = SQL & "and a.kode_perusahaan = y.kode_perusahaan "
			SQL = SQL & "and x.Serial_Number_Awal = z.serial_number "
			SQL = SQL & "and z.Kode_Perusahaan = a.Kode_Perusahaan "
			SQL = SQL & "and z.Serial_Number = a.Serial_Number "
			SQL = SQL & "), 0) as Jumlah_TF_Belum_Validasi, "

			SQL = SQL & "ISNULL((select sum(x.jumlah_bags) from N_EMI_Transfer_Stock_Barang_Lain_Det x, "
			SQL = SQL & "N_EMI_Transfer_Stock_Barang_Lain y, barang_lain_sn z "
			SQL = SQL & "where x.No_Faktur = y.No_Faktur "
			SQL = SQL & "and y.Status is null and x.Selesai is null "
			SQL = SQL & "and a.kode_perusahaan = y.kode_perusahaan "
			...
			SQL = SQL & "and z.Kode_Perusahaan = a.Kode_Perusahaan "
			SQL = SQL & "and z.Serial_Number = a.Serial_Number "
			SQL = SQL & "), 0) as Jumlah_Bag_TF_Belum_Validasi "
			...
			SQL = SQL & "a.Id_Warehouse = c.Id_WMS_Warehouse_Position And a.Jumlah <> 0 And "
			SQL = SQL & "a.Kode_Perusahaan ='" & KodePerusahaan & "' And b.Kode_Stock_Owner ='" & arrSO(CmbSO_Asal.SelectedIndex) & "' "
			SQL = SQL & "And b.Kode_Barang ='" & TxtKd_Barang.Text & "' "

			SQL = SQL & "group by a.serial_number, b.kode_perusahaan, a.Kode_Perusahaan, a.Kode_Stock_Owner, a.Kode_Barang, b.Nama, b.Satuan, a.Id_Warehouse, "
			SQL = SQL & "c.Keterangan, a.Id_Nametag_pallet, a.warna, b.Metode_Pengeluaran_Stok, b.Jenis_Kemasan, "
			SQL = SQL & "b.Isi_Per_Bags, b.Satuan_Isi_Bags, a.Tgl_Expired, a.Tgl_Produksi, a.warna, "
			SQL = SQL & "a.Qr_Code, a.Kode_Unik_Berjalan, a.Blok_SN, a.Tgl_Masuk "

			SQL = SQL & "order by case when Metode_Pengeluaran_Stok='FIFO' then a.Tgl_Masuk "
			...
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					If Dr("jumlah") - Dr("jumlah_tf_sementara") - Dr("Jumlah_TF_Belum_Validasi") < 1 Then
						Continue Do
					End If
					...
					DGV_Data_TF.Rows(rows).Cells(itemDgvGoodStock).Value = If(General_Class.CekNULL(Dr("jumlah")) = "", "", Format(Dr("jumlah") - Dr("jumlah_tf_sementara"), "N0"))
					DGV_Data_TF.Rows(rows).Cells(itemDgvStockBags).Value = If(General_Class.CekNULL(Dr("stock_bags")) = "", "", Format(Dr("stock_bags") - Dr("jumlah_tf_bag_sementara"), "N0"))
	...
	Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
		...
		Try
			...
			'==================================
			SQL = "insert into N_EMI_Transfer_Stock_Barang_Lain (kode_perusahaan, No_faktur, SO_Awal, SO_Tujuan, Tanggal, Jam, UserID, Jenis_Transfer, Lokasi, Keterangan) Values "
			...
			SQL = "Select isnull(Flag_Timbang, 'T') as Timbang from stock_owner_gudang_Lain where "
			SQL = SQL & "Kode_stock_owner='" & arrSO(CmbSO_Asal.SelectedIndex) & "' and kode_perusahaan='" & KodePerusahaan & "' "
			Using dr = OpenTrans(SQL)
				If dr.Read Then
					Lokasi_Timbang = dr("Timbang")
				Else
					dr.Close()
					CloseTrans()
					CloseConn()
			...
			For i As Integer = 0 To Dgv_DataRekap.Rows.Count - 1
				...
				If CmbSo_Tujuan.SelectedIndex = index_waste Then
					Flag_Timbang = "T"
				End If

				Dim urutKeepStokIsnullOrNot As String

				If dgv_Rekap_JnsTransfer = "KEEP STOCK" Then

					'====
					' cek apakah sudah pernah di tf lalu mau di tf ulang
					' jika ia maka harus di tolak
					SQL = "select  a.jumlah, isnull(( "
					SQL = SQL & "select sum(y.total) From N_EMI_Transfer_Stock_Barang_Lain x, N_EMI_Transfer_Stock_Barang_Lain_Detail y "
					SQL = SQL & "where x.Kode_Perusahaan = y.Kode_Perusahaan and x.No_Faktur = y.No_Faktur and x.Status is null "
					SQL = SQL & "and y.Urut_Keep_Stock = a.Urut_Oto "
					SQL = SQL & "),0) as Jumlah_Tf From N_EMI_Keep_Stock_Barang_Lain_Departement a where  Kode_Perusahaan = '" & KodePerusahaan & "' "
					SQL = SQL & "and Urut_Oto = '" & dgv_rekap_UrutKeep & "' and a.Status is null "
					Using Dr = OpenTrans(SQL)

						If Dr.Read Then

							If Dr("jumlah") < Dr("jumlah_tf") + HilangkanTanda(dgv_Rekap_JumlahBersih) Then
								Dr.Close()
								CloseTrans()
								CloseConn()
								MessageBox.Show("Terjadi perubahan pada Jumlah Stok Barang, silahkan refresh dan coba lagi", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
								Exit Sub
							End If
						Else
							CloseTrans()
							CloseConn()
							MessageBox.Show("Data keep stok tidak ditemukan, silahkan refresh dan coba lagi", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
							Exit Sub
						End If
					End Using

					urutKeepStokIsnullOrNot = "'" & dgv_rekap_UrutKeep & " '"
				ElseIf dgv_Rekap_JnsTransfer = "PR DEPT" Then

					'====
					' cek apakah sudah pernah di tf lalu mau di tf ulang
					' jika ia maka harus di tolak
					SQL = "select  b.jumlah, isnull(( "
					SQL = SQL & "select sum(y.total) From N_EMI_Transfer_Stock_Barang_Lain x, N_EMI_Transfer_Stock_Barang_Lain_Detail y "
					SQL = SQL & "where x.Kode_Perusahaan = y.Kode_Perusahaan and x.No_Faktur = y.No_Faktur and x.Status is null "
					SQL = SQL & "and y.urut_pr_dept = b.no_urut "
					SQL = SQL & "),0) as Jumlah_Tf  From N_EMI_Purchase_Requisition_Barang_Lain_Departement a, N_EMI_Purchase_Requisition_Barang_Lain_Departement_Detail b "
					SQL = SQL & "where a.kode_perusahaan = b.Kode_Perusahaan and a.No_Faktur = b.No_Faktur and a.Status is null  "
					SQL = SQL & "and b.No_Urut = " & dgv_rekap_urut_pr_dept & " and a.kode_perusahaan  ='" & KodePerusahaan & "'"
					Using Dr = OpenTrans(SQL)
						...
						Else
							CloseTrans()
							CloseConn()
							MessageBox.Show("Data PR DEPT tidak ditemukan, silahkan refresh dan coba lagi", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
							Exit Sub
						End If
					End Using

					urutKeepStokIsnullOrNot = "NULL"
				Else
					CloseTrans()
					CloseConn()
					MessageBox.Show("Jenis request transfer tidak valid . . ! ! ")
					Exit Sub

				End If

				SQL = "insert into N_EMI_Transfer_Stock_Barang_Lain_Detail (Kode_Perusahaan, No_faktur, Kode_Barang, Total, Satuan, "
				SQL = SQL & "Total_Barang, Satuan_Barang, Total_Bags, urut_material_requisition_convert, Flag_Timbang, Flag_Jenis_Request, Urut_Keep_Stock, urut_pr_dept) values "
				...
				SQL = SQL & "'" & HilangkanTanda(dgv_Rekap_JumlahBags) & "', '" & dgv_Rekap_Oto & "', '" & Flag_Timbang & "', '" & dgv_Rekap_JnsTransfer & "' , " & urutKeepStokIsnullOrNot & ", '" & dgv_rekap_urut_pr_dept & "' )"
				ExecuteTrans(SQL)

				'awal

				Dim jumlahStockSkrng As Double
				Dim jumlahDiKeepStok As Double
				Dim jumlahBelumvalidasi As Double
				'SQL = "select isnull(sum(a.jumlah) - b.jumlah,0) as Jumlah_Stock_Sekarang, b.jumlah From barang_lain_sn a "
				'SQL = SQL & "inner join N_EMI_Keep_Stock_Barang_Lain_Departement b on "
				'SQL = SQL & "a.kode_perusahaan = b.kode_perusahaan and a.kode_barang = b.kode_barang "
				'SQL = SQL & "and a.kode_stock_owner = b.kode_stock_owner and b.status is null  "
				'SQL = SQL & "and b.flag_selesai_pengeluaran_barang is null where a.kode_barang = '" & TxtKd_Barang.Text.Trim & "' "
				'SQL = SQL & "and a.kode_stock_owner = '" & arrSO(CmbSO_Asal.SelectedIndex) & "' "
				...
				SQL = $"
                        ...
                        FROM barang_lain_sn
                        WHERE kode_barang ='{dgv_Rekap_KdBarang}'
                          and kode_perusahaan = '{KodePerusahaan}'
                          AND kode_stock_owner ='{arrSO(CmbSO_Asal.SelectedIndex)}'
                    ),
                    stok_keep AS (
                        SELECT
                            SUM(jumlah) AS total_keep
                        FROM N_EMI_Keep_Stock_Barang_Lain_Departement
                        WHERE kode_barang = '{dgv_Rekap_KdBarang}'
                           and kode_perusahaan = '{KodePerusahaan}'
                          AND kode_stock_owner =  '{arrSO(CmbSO_Asal.SelectedIndex)}'
                          AND status IS NULL
                          AND flag_selesai_pengeluaran_barang IS NULL
                    ),
                    total_transfer as (
                         select sum(y.Jumlah) as Total_Transfer
                    ...
                    and z.so_awal = '{arrSO(CmbSO_Asal.SelectedIndex)}'
                    and x.Kode_Barang = '{dgv_Rekap_KdBarang}'

                    )
                    SELECT
                        ISNULL((SELECT total_barang FROM stok_barang), 0)
                        - ISNULL((SELECT Total_Transfer FROM total_transfer), 0)

                       AS jumlah_stock_sekarang,
                    ISNULL((SELECT total_keep FROM stok_keep), 0) AS total_keep,
                     ISNULL((SELECT Total_Transfer FROM total_transfer), 0) AS total_transfer

                   "

				Using dr = OpenTrans(SQL)
					If dr.Read Then
						If dr("Jumlah_Stock_Sekarang") < HilangkanTanda(dgv_Rekap_JumlahBersih) Then

							jumlahStockSkrng = Format(dr("jumlah_stock_sekarang"), "N4")
							jumlahDiKeepStok = Format(dr("total_keep"), "N2")
							jumlahBelumvalidasi = Format(dr("total_transfer"), "N2")
							dr.Close()
							CloseConn()
							MessageBox.Show(
									"Stok untuk " & dgv_Rekap_KdBarang & " tidak mencukupi" & vbCrLf &
									"Stok Tersedia : " & jumlahStockSkrng & vbCrLf &
									"Keep Stok : " & jumlahDiKeepStok & vbCrLf &
									 "Belum validasi Transfer Stok : " & jumlahBelumvalidasi & vbCrLf &
									"Jumlah ingin ditransfer : " & dgv_Rekap_JumlahBersih,
									Judul,
				...
				'If dgv_Rekap_JnsTransfer = "KEEP STOCK" Then
				'    SQL = "update N_EMI_Keep_Stock_Barang_Lain_Departement set "
				'    SQL = SQL & "jmlh_transfer = '" & HilangkanTanda(dgv_Rekap_JumlahBersih) & "'"
				'Else
				'    CloseTrans()
				'    CloseConn()
				'    MessageBox.Show("Jenis transfer tidak valid ! !", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				'    Exit Sub
				'End If

				Dim x_ident_current As Integer = 0
				SQL = "select IDENT_CURRENT('N_EMI_Transfer_Stock_Barang_Lain_Detail') as urutan"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
				...
				SQL = "select kode_Perusahaan from N_EMI_Transfer_Stock_Barang_Lain_Detail where "
				...
				For j As Integer = 0 To Dgv_DataDetail.Rows.Count - 1
					...
					If Not (dgv_Rekap_KdBarang = dgv_detail_KdBarang And dgv_rekap_ket_no_pr_dept = dgv_detail_no_pr) Then
						Continue For
					End If

					Dim jenis_bags As String = ""
					Dim isi_per_bags As Double = 0
					SQL = "select Jenis_Kemasan, isnull(Isi_Per_Bags,0) as Isi_Per_Bags from barang_lain where "
					SQL = SQL & "kode_perusahaan='" & KodePerusahaan & "' and "
					SQL = SQL & "kode_barang='" & dgv_KodeBarang & "' and "
					SQL = SQL & "kode_stock_owner='" & dgv_detail_lokasi & "'"
					Using Dr = OpenTrans(SQL)
						If Dr.Read Then
					...
					If dgv_detail_lokasi = dgv_Rekap_lokasi And dgv_detail_KdBarang = dgv_Rekap_KdBarang Then
						...
						SQL = "select a.Kode_Perusahaan from N_EMI_Transfer_Stock_Barang_Lain a, N_EMI_Transfer_Stock_Barang_Lain_Det b, Barang_Lain_SN c  where "
						...
						SQL = SQL & "c.qr_Code+'-'+kode_unik_berjalan = '" & dgv_detail_Barcode & "' and a.no_faktur <> '" & TxtNo_Transaksi.Text & "' and "
						...
						Using Dr = OpenTrans(SQL)
							If Dr.Read Then
								...
								CloseTrans()
								CloseConn()
								MessageBox.Show("Barang pada Rak " & dgv_detail_KodeRakAwal & " belum melalui proses pencetakan barcode Validasi.", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
								Exit Sub
							Else
								Dr.Close()
							End If
						...
						Using Dr1 = OpenTrans(SQL)
							If Dr1.Read Then
								...
								If General_Class.CekNULL(Dr1("hasil")) = 0 Then
									Dr1.Close()
									CloseTrans()
									CloseConn()
									MessageBox.Show("data konversi satuan kirim tidak ada ")
						...
						SQL = SQL & "from Barang_Lain_SN a where "
						SQL = SQL & "a.Kode_Perusahaan = '" & KodePerusahaan & "' "
						SQL = SQL & "and a.qr_Code+'-'+a.kode_unik_berjalan = '" & dgv_detail_Barcode & "' "
						SQL = SQL & "and a.Kode_stock_owner = '" & dgv_detail_lokasi & "' and a.jumlah<>0 "
							...
							With Ds.Tables("MyTable")
								If .Rows.Count <> 0 Then
									...
									For Index As Integer = 0 To .Rows.Count - 1
										...
										SQL = "insert into N_EMI_Transfer_Stock_Barang_Lain_Det(Kode_Perusahaan, No_faktur, Id_Wms_Awal, No_Pallet_Awal, Id_Wms_Tujuan, "
										...
										SQL = SQL & "'" & dgv_detail_IDPalletAwal & "', '" & dgv_detail_IDWarehouseTujuan & "', '" & Data_SN & "', "
...
#End Region
		...
		Try
			...
			SQL = SQL & "from N_EMI_View_Transfer_Stock_Barang_Lain a where "
			SQL = SQL & "a.Kode_Perusahaan = '" & KodePerusahaan & "' "
			SQL = SQL & "and a.No_Faktur = '" & Trim(TxtNo_Transaksi.Text) & "' "
			Using Ds = BindingTrans(SQL)
				If Ds.Tables("MyTable").Rows.Count <> 0 Then

					CrDoc = New N_EMI_CR_Faktur_Transfer_Stock_Barang_Lain
					kertas = "Faktur"
					...
					'    CrDoc.RecordSelectionFormula = "{N_EMI_View_Transfer_Stock_Barang_Lain.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Vw_Tf_Stock.No_Faktur}='" & Trim(TxtNo_Transaksi.Text) & "' "
					...
					'    CrDoc.RecordSelectionFormula = "{N_EMI_View_Transfer_Stock_Barang_Lain.Kode_Perusahaan} = '" & KodePerusahaan & "' and {N_EMI_View_Transfer_Stock_Barang_Lain.No_Faktur}='" & TxtNo_Transaksi.Text & "' "
					...
					CrDoc.RecordSelectionFormula = "{N_EMI_View_Transfer_Stock_Barang_Lain.Kode_Perusahaan} = '" & KodePerusahaan & "' and {N_EMI_View_Transfer_Stock_Barang_Lain.No_Faktur}='" & Trim(TxtNo_Transaksi.Text) & "' "
					'CrDoc.SummaryInfo.ReportTitle = "Halaman : " & min & "/" & max
	...
	Private Sub CmbSO_Asal_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbSO_Asal.SelectedIndexChanged
		...
		If MenuAsal.ToUpper = "TRANSFER_WASTE" Then
			If arrSO(CmbSO_Asal.SelectedIndex) = arrSO(CmbSo_Tujuan.SelectedIndex) Then
				MessageBox.Show("Lokasi Awal Tidak Boleh Sama Dengan Lokasi Tujuan", "Warning!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				CmbSO_Asal.SelectedIndex = -1
				Exit Sub
			End If
		End If

		If MenuAsal.ToUpper = "TRANSFER_STOCK" Then CmbSo_Tujuan.SelectedIndex = -1
		...
		Try
			...
			Dim IdKategoriGudang As String = ""
			SQL = $"
                select distinct a.Kode_Stock_Owner, a.Keterangan, b.Urut_Oto, b.Jenis_Gudang
                from Stock_Owner_Gudang_lain a
	                inner join N_EMI_Master_Kategori_Gudang_Barang_Lain b on a.kode_perusahaan = b.kode_perusahaan and a.kode_stock_owner = b.kode_stock_owner_gudang
	                inner join N_EMI_Master_Kategori_Gudang_Binding_User_Barang_Lain c on b.kode_perusahaan = c.kode_perusahaan and b.Urut_Oto = c.id_kategori_gudang
                where a.Kode_Perusahaan = '{KodePerusahaan}'
                and b.Status is null and c.Status is null
                and c.User_ID = '{UserID}'
            "
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					If Dr("Kode_Stock_Owner") = arrSO(CmbSO_Asal.SelectedIndex) Then
						HasAccess = True
						IdKategoriGudang = Dr("Urut_Oto")

						If Dr("Jenis_Gudang") = "Warehouse" Then
							Btn_List_Keep_Stock.Enabled = True
						Else
							Btn_List_Keep_Stock.Enabled = False
						End If
			...
			If Not HasAccess Then
				CloseConn()
				MessageBox.Show($"User Tidak Memiliki Akses untuk di gudang {CmbSO_Asal.Text}", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Btn_List_Keep_Stock.Enabled = False
				CmbSO_Asal.SelectedIndex = -1
				Exit Sub
			End If

			Selected_Id_Kategori_Gudang = IdKategoriGudang

			Kategori_Gudang_Asal = Get_Kategori_Gudang(arrSO(CmbSO_Asal.SelectedIndex)).Kode_Kategori_Gudang
			Jenis_Gudang_Asal = Get_Kategori_Gudang(arrSO(CmbSO_Asal.SelectedIndex)).Jenis_Gudang
			hapus_sementara()
			CloseConn()
		Catch ex As Exception
			CloseConn()
	...
	Private Sub CmbSo_Tujuan_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbSo_Tujuan.SelectedIndexChanged
		...
		If MenuAsal.ToUpper = "TRANSFER STOCK" Then
		...
		Try
			...
			'isProduction = False
			'SQL = "select kode_stock_owner from Stock_Owner_Gudang_Lain where kode_perusahaan = '" & KodePerusahaan & "' and flag_produksi = 'Y' "
			'Using Dr = OpenTrans(SQL)
			'    Do While Dr.Read
			'        If arrSO(CmbSo_Tujuan.SelectedIndex).ToString.ToUpper = Dr("kode_stock_owner").ToString.ToUpper Then
			'            isProduction = True
			'            Exit Do
			'        End If
			'    Loop
			...
			'If isProduction Then
			'    TxtKd_Barang.Enabled = False
			'Else
			'    TxtKd_Barang.Enabled = True
			'End If
			'TxtKd_Barang.Text = ""

			Kategori_Gudang_Tujuan = Get_Kategori_Gudang(arrSO(CmbSo_Tujuan.SelectedIndex)).Kode_Kategori_Gudang
			Jenis_Gudang_Tujuan = Get_Kategori_Gudang(arrSO(CmbSo_Tujuan.SelectedIndex)).Jenis_Gudang

			'If Kategori_Gudang_Asal.Trim.ToUpper = Kategori_Gudang_Tujuan.Trim.ToUpper Then
			'    Btn_List_Keep_Stock.Enabled = True
			'Else
			'    Btn_List_Keep_Stock.Enabled = False
			'End If
	...
	Private Sub GetGrandTotal()
		...
		TxtTotalTransfer.Text = Format(total, "N4")
		TxtTotalTransferBags.Text = Format(totalBags, "N4")

		If Not TxtjmlPermintaanDisplay.Text.Trim.Length = 0 Then
			request = Val(HilangkanTanda(TxtjmlPermintaanBersih.Text))
			TxtsisaRequest.Text = Format((request - total), "N4") & " KG"
		Else
			TxtsisaRequest.Text = 0
		End If
...
#Region "Transfer baru"
		...
		Try
			OpenConn()
			Cmd.Transaction = Cn.BeginTransaction
			get_no_faktur_sementara()

			'==========================================-================================================
			'=========== Cek jumlah stok yang sudah di transfer , yang di keep dan stok skrng ==========
			...
			SQL = $"
                            ...
                            SELECT
                                SUM(jumlah) AS total_barang
                            FROM barang_lain_sn
                            WHERE kode_barang ='{TxtKd_Barang.Text.Trim}'
                              and kode_perusahaan = '{KodePerusahaan}'
                              AND kode_stock_owner ='{arrSO(CmbSO_Asal.SelectedIndex)}'
                        ...
                        stok_keep AS (
                            ...
                            WHERE kode_barang = '{TxtKd_Barang.Text.Trim}'
                              ...
                              AND flag_selesai_pengeluaran_barang IS NULL
                               ...
                               stok_transfer_sementara AS (
                            SELECT
                                SUM(TOTAL) AS total_transfer_sementara
                            FROM N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara
                            WHERE kode_barang = '{TxtKd_Barang.Text.Trim}'
                          AND kode_stock_owner ='{arrSO(CmbSO_Asal.SelectedIndex)}'
                          and kode_perusahaan = '{KodePerusahaan}'
                    ),
                        ...
                        and y.selesai is  null
                        and z.so_awal = '{arrSO(CmbSO_Asal.SelectedIndex)}'
                        and x.Kode_Barang = '{TxtKd_Barang.Text.Trim}'
                        ...
                        ISNULL((SELECT total_keep FROM stok_keep), 0) AS total_keep,
                         ISNULL((SELECT Total_Transfer FROM total_transfer), 0) AS total_transfer,
                           ISNULL((select total_transfer_sementara from stok_transfer_sementara), 0) AS total_simpan_sementara

                   "

			Using dr = OpenTrans(SQL)
				If dr.Read Then
					Dim stockSekarang As Double = dr("Jumlah_Stock_Sekarang")
					Dim totalKeep As Double = dr("total_keep")
					Dim totalTransfer As Double = TxtTotalTransfer.Text

					Dim totalSudahTransferBelumValidasi As Double = dr("total_transfer")
					Dim total_simpan_sementara As Double = dr("total_simpan_sementara")

					Dim stockAvailable As Double
					If Txt_Jenis_Transfer.Text = "KEEP STOCK" Then

						stockAvailable = stockSekarang - total_simpan_sementara
					ElseIf Txt_Jenis_Transfer.Text = "PR DEPT" Then

						stockAvailable = stockSekarang - totalKeep - total_simpan_sementara - totalSudahTransferBelumValidasi
					Else
						CloseTrans()
						CloseConn()
						MessageBox.Show("Jenis transfer salah/ tidak ditemukan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
						Exit Sub

					End If

					If stockAvailable < totalTransfer Then
						...
						CloseConn()
						MessageBox.Show(
									"Stok untuk " & TxtNm_Barang.Text & " tidak mencukupi" & vbCrLf &
									"Stok Tersedia : " & jumlahStockSkrng & vbCrLf &
									"Keep Stok : " & jumlahDiKeepStok & vbCrLf &
									"Belum validasi Transfer Stok : " & jumlahBelumvalidasi & vbCrLf &
									  "Jumlah disimpan sementara : " & total_simpan_sementara & vbCrLf &
									"Jumlah ingin ditransfer : " & TxtTotalTransfer.Text,
									Judul,
									MessageBoxButtons.OK,
									MessageBoxIcon.Exclamation
								)
			...
			If isProduction Then
				...
				Dim Jumlah_Kebutuhan As Double = 0
				Dim Jumlah_sdh_transfer As Double = 0
				...
				SQL = $"
                    ...
                    and a.Kode_Stock_Owner = '{arrSO(CmbSO_Asal.SelectedIndex)}'
                    and a.Urut_Oto = '{Txt_Urut_Request.Text}'
                "
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
						Jumlah_Kebutuhan = Dr("Jumlah")
						Jumlah_sdh_transfer = Dr("Jumlah_TF")
					Else
						CloseConn()
				...
				'Nilai_Toleransi_Min = Val(HilangkanTanda(Jumlah_Kebutuhan)) - (Val(HilangkanTanda(Jumlah_Kebutuhan)) * (Persen_Toleransi_Min / 100))
				'Nilai_Toleransi_Max = Val(HilangkanTanda(Jumlah_Kebutuhan)) + (Val(HilangkanTanda(Jumlah_Kebutuhan)) * (Persen_Toleransi_Max / 100))

				If (Jumlah_sdh_transfer + Val(HilangkanTanda(TxtTotalTransfer.Text))) > Jumlah_Kebutuhan Then
					MessageBox.Show("Total Transfer Lebih Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
					Exit Sub
				End If
...
#Region "insert data"
			...
			'    SQL = "select sum(b.Total) as total from N_EMI_Keep_Stock_Barang_Lain_Departement a where  "

			'    SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' and a.Status is null  and a.urut_oto = " & Txt_Urut_Request.Text.Trim & ""
			'    Using Dr = OpenTrans(SQL)
			'        If Dr.Read Then
			'            If Dr("total") < tt Then
			'            End If
			...
			'    SQL = "select sum(b.Total) from N_EMI_Transfer_Stock_Barang_Lain a, N_EMI_Transfer_Stock_Barang_Lain_Detail b "
			'    SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan and a.No_Faktur = b.No_Faktur "
			'    SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' and a.Status is null  and b.Urut_PR_Dept = " & Txt_Urut_Request.Text.Trim & ""
			'    Using Dr = OpenTrans(SQL)
			'        If Dr.Read Then
			...
			For row As Integer = 0 To DGV_Data_TF.RowCount - 1
				...
				If Not CmbSo_Tujuan.SelectedIndex = index_waste Then
					If dgv_FlagBlokSN = "Y" Then
						CloseTrans()
						CloseConn()
						MessageBox.Show("SN pada barang " & dgv_Nama & " di blok, barang Tidak Bisa di Transfer " & vbNewLine & " Barcode : " & dgv_Barcode, "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
						Exit Sub
					End If
				End If
				...
				If Val(HilangkanTanda(TxtTotalTransfer.Text)) > Val(HilangkanTanda(TxtjmlPermintaanBersih.Text)) Then
					CloseTrans()
					CloseConn()
					MessageBox.Show(
							"Jumlah untuk " & TxtNm_Barang.Text & " tidak boleh melebihi Permintaan" & vbCrLf &
							"Jumlah Permintaan : " & TxtjmlPermintaanDisplay.Text & vbCrLf &
							"Jumlah Transfer : " & TxtTotalTransfer.Text,
							Judul,
							MessageBoxButtons.OK,
							MessageBoxIcon.Exclamation
						)
				...
				'SQL = "insert into N_EMI_Transfer_Stock_Barang_Lain_Det_Sementara(Kode_Perusahaan, No_faktur, Id_Wms_Awal, No_Pallet_Awal, Id_Wms_Tujuan, "
				'SQL = SQL & "Serial_Number_Awal, Jumlah, Jumlah_Barang, Jumlah_Bags, Warna, Urut_TF) values( "
				'SQL = SQL & "'" & KodePerusahaan & "', '" & Trim(txtNoFaktur_Sementara.Text) & "', '" & dgv_IDWareHouse & "', "
				'SQL = SQL & "'" & dgv_IDPallet & "', '" & arr2RakTujuan(row)(selectedIndex) & "', '" & dgv_SerialNumber & "', "
				'SQL = SQL & "'" & dgv_Jumlah & "', '" & dgv_Jumlah & "', "
				'SQL = SQL & "'" & dgv_JmlhBags & "', "
				'SQL = SQL & "'" & dgv_Warna & "', '" & x_ident_current1 & "')"
				'ExecuteTrans(SQL)
				...
				Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JnsTransfer).Value = Txt_Jenis_Transfer.Text
				Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_No_Pr).Value = txtDariPR.Text
				Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Keterangan_Pr).Value = TxtDariPRDepartement.Text

				'SQL = "select IDENT_CURRENT('N_EMI_Transfer_Stock_Barang_Lain_Detail_sementara') as urutan"
				'Using Dr = OpenTrans(SQL)
				'    If Dr.Read Then
				'        x_ident_current1 = Dr("urutan")
				'    End If
				'End Using

				SQL = "insert into N_EMI_Transfer_Stock_Barang_Lain_Detail_sementara (Kode_Perusahaan, No_faktur, Kode_Barang, Total, Satuan, "
				SQL = SQL & "Total_Barang, Satuan_Barang, Total_Bags, urut_material_requisition_convert, Flag_Timbang, Flag_Jenis_Request, Urut_Keep_Stock, urut_pr_dept, userid,tanggal,jam, id_wms_awal,id_wms_tujuan,kode_stock_owner, qr_code) values "
				SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(txtNoFaktur_Sementara.Text) & "', '" & TxtKd_Barang.Text & "', "
				SQL = SQL & "'" & dgv_Jumlah & "', '" & TxtSatuan.Text & "', "
				SQL = SQL & "'" & dgv_Jumlah & "', '" & TxtSatuanKecil.Text & "', "
				SQL = SQL & "'" & dgv_JmlhBags & "', '" & Txt_OtoMaterial_req.Text & "', null, '" & Txt_Jenis_Transfer.Text & "' , " & Txt_Urut_Request.Text & ", '" & Urut_Pr_Dept.Text & "',"
				SQL = SQL & "'" & UserID & "', '" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & Format(tgl_skg, "HH:mm:ss") & "','" & dgv_IDWareHouse & "', '" & arr2RakTujuan(row)(selectedIndex) & "', '" & dgv_Lokasi & "' ,'" & dgv_Barcode & "')"
				ExecuteTrans(SQL)

				If hasDataSelected Then
					Dim sudahAdaData As Boolean = False
					For j As Integer = 0 To Dgv_DataRekap.Rows.Count - 1

						get_grid_view_Rekap(j)

						If (dgv_Rekap_KdBarang = dgv_KodeBarang And txtDariPR.Text = dgv_detail_no_pr) Then
							sudahAdaData = True
							Exit For
						End If
					Next

					If Not sudahAdaData Then
						...
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_JnsTransfer).Value = Txt_Jenis_Transfer.Text
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_OtoGen).Value = ""
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_UrutKeep).Value = Txt_Urut_Request.Text
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_Urut_Prdept).Value = Urut_Pr_Dept.Text
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_KetPrDept).Value = TxtDariPRDepartement.Text
						Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_KetNoPR).Value = txtDariPR.Text
						'tab 2
...
#End Region
		...
		If isProduction Then
			...
			Try
				...
				Using dr = OpenTrans(SQL)
					If dr.Read Then
						Dim stockSekarang As Integer = CInt(dr("Jumlah_Stock_Sekarang"))
						Dim totalKeep As Integer = CInt(dr("total_keep"))
						Dim totalTransfer As Integer = CInt(TxtTotalTransfer.Text)

						Dim stockAvailable As Integer = stockSekarang - totalKeep
			...
			'If (Jumlah_sdh_transfer + Val(HilangkanTanda(TxtTotalTransfer.Text))) < Nilai_Toleransi_Min Then
			'    MessageBox.Show("Total Transfer Kurang Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			'    Exit Sub
			'ElseIf (Jumlah_sdh_transfer + Val(HilangkanTanda(TxtTotalTransfer.Text))) > Nilai_Toleransi_Max Then
			'    MessageBox.Show("Total Transfer Lebih Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			'    Exit Sub
			'End If
	...
	Private Sub Dgv_DataRekap_KeyDown(sender As Object, e As KeyEventArgs) Handles Dgv_DataRekap.KeyDown
		...
		If e.KeyCode = Keys.Delete Then
			...
			Try
				...
				SQL = "delete from N_EMI_Transfer_Stock_Barang_Lain_Detail_Sementara where kode_perusahaan = '" & KodePerusahaan & "' "
				SQL = SQL & "and userid = '" & UserID & "' and "
				SQL = SQL & "kode_barang = '" & dgv_Rekap_KdBarang & "' "
				SQL = SQL & "and urut_pr_dept = '" & dgv_rekap_urut_pr_dept & "' "
				ExecuteTrans(SQL)
	...
	Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Btn_List_Keep_Stock.Click

		If CmbSO_Asal.SelectedIndex = -1 Then
			MessageBox.Show("Harap Pilihl Dahulu Lokasi Awal", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			CmbSO_Asal.DroppedDown = True
			CmbSO_Asal.Focus()
			Exit Sub
		ElseIf CmbSo_Tujuan.SelectedIndex = -1 Then
			MessageBox.Show("Harap Pilihl Dahulu Lokasi Tujuan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			CmbSo_Tujuan.DroppedDown = True
			CmbSo_Tujuan.Focus()
			Exit Sub
		End If

		N_EMI_SD_List_Keep_Stock_Barang_Lain.Kosong()
		N_EMI_SD_List_Keep_Stock_Barang_Lain._tabIndex = 0

		N_EMI_SD_List_Keep_Stock_Barang_Lain.lokasi_kirim = arrSO(CmbSO_Asal.SelectedIndex)

		N_EMI_SD_List_Keep_Stock_Barang_Lain.lokasi_Terima = arrSO(CmbSo_Tujuan.SelectedIndex)
		N_EMI_SD_List_Keep_Stock_Barang_Lain.asal = "TF_Stock"
		N_EMI_SD_List_Keep_Stock_Barang_Lain.ShowDialog()
	...
	Private Sub CmbJnsTransfer_KeyPress(sender As Object, e As KeyPressEventArgs) Handles CmbJnsTransfer.KeyPress
	...
	Private Sub CmbSo_Tujuan_KeyPress(sender As Object, e As KeyPressEventArgs) Handles CmbSo_Tujuan.KeyPress
		If e.KeyChar = Chr(13) Then
			TabControl1.SelectedIndex = 0
			TxtKd_Barang.Focus()
		End If
	...
	Private Function Get_Kategori_Gudang(ByVal Kode_Stock_Owner As String) As (Kode_Kategori_Gudang As String, Jenis_Gudang As String)

		Dim Kategori As String = ""
		Dim Jns_Gudang As String = ""
		SQL = $"
            select Kode_Kategori_Gudang, Jenis_Gudang
            from N_EMI_Master_Kategori_Gudang_Barang_Lain
            where Kode_Perusahaan = '{KodePerusahaan}'
            and status is null
            and Kode_Stock_Owner_Gudang = '{Kode_Stock_Owner}'
        "
		Using Dr9 = OpenTrans(SQL)
			If Dr9.Read Then
				Kategori = Dr9("Kode_Kategori_Gudang")
				Jns_Gudang = Dr9("Jenis_Gudang")
			End If
		End Using

		Return (Kategori, Jns_Gudang)

Public Class N_EMI_Laporan_Barang_Masuk_Asset
    ...
    Private Sub Kosong()
        ...
        Cmb_Filter_Mobil.Items.Add("No Faktur") : arrFilterMobil.Add("No_Faktur") : arrFilterSFMobil.Add("{View_Laporan_Barang_Masuk_Asset_Loading.No_Faktur}")
        Cmb_Filter_Mobil.Items.Add("No PO") : arrFilterMobil.Add("No_PO") : arrFilterSFMobil.Add("{View_Laporan_Barang_Masuk_Asset_Loading.No_PO}")
        Cmb_Filter_Mobil.Items.Add("Mobil") : arrFilterMobil.Add("Mobil") : arrFilterSFMobil.Add("{View_Laporan_Barang_Masuk_Asset_Loading.Mobil}")
    ...
    Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
        ...
        Try
            ...
            If Cmb_FilterBy.SelectedIndex = 0 Then
                ...
                SQL = "select * from View_Laporan_Barang_Masuk_Asset a "
                SQL = SQL & "where a.kode_perusahaan = '" & KodePerusahaan & "' "
                SQL = SQL & "and a.tanggal_masuk between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

                SF = "{View_Laporan_Barang_Masuk_Asset.kode_perusahaan} = '" & KodePerusahaan & "' "
                SF = SF & "and {View_Laporan_Barang_Masuk_Asset.tanggal_masuk} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
                SF = SF & "{View_Laporan_Barang_Masuk_Asset.tanggal_masuk} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

                If Not Txt_KdSupplier.Text = "--- SELURUH ---" Then
                    SQL = SQL & "and a.kode_supplier = '" & Txt_KdSupplier.Text & "' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.kode_supplier} = '" & Txt_KdSupplier.Text & "'"
                End If

                If Not Txt_KdBarang.Text = "--- SELURUH ---" Then
                    SQL = SQL & "and a.kode_barang = '" & Txt_KdBarang.Text & "' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.kode_barang} = '" & Txt_KdBarang.Text & "'"
                End If

                If Not Txt_NoSJ.Text = "--- SELURUH ---" Then
                    SQL = SQL & "and a.No_SJ = '" & Txt_NoSJ.Text & "' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.No_SJ} = '" & Txt_NoSJ.Text & "'"
                End If

                If Cmb_FlagValidasiWarehouse.SelectedIndex = 1 Then
                    SQL = SQL & "and a.status_Warehouse = 'Validasi' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.status_Warehouse} = 'Validasi'"
                ElseIf Cmb_FlagValidasiWarehouse.SelectedIndex = 2 Then
                    SQL = SQL & "and a.status_Warehouse = 'Belum Di Validasi' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.status_Warehouse} = 'Belum Di Validasi'"
                End If

                If Cmb_FlagValidasiAcc.SelectedIndex = 1 Then
                    SQL = SQL & "and a.status_ACC = 'Validasi' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.status_ACC} = 'Validasi'"
                ElseIf Cmb_FlagValidasiAcc.SelectedIndex = 2 Then
                    SQL = SQL & "and a.status_ACC = 'Belum Di Validasi' "
                    SF = SF & "And {View_Laporan_Barang_Masuk_Asset.status_ACC} = 'Belum Di Validasi'"
                End If
                ...
                Using DS = BindingTrans(SQL)
                    With DS.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            ...
                            Dim CrDoc As New N_EMI_CR_Laporan_Barang_Masuk_Asset

                            CrDoc.SetDataSource(DS.Tables("MyTable"))
            ...
            ElseIf Cmb_FilterBy.SelectedIndex = 1 Then
                ...
                'SQL = "select kode_perusahaan from View_Laporan_Barang_Masuk_Asset_Loading "
                'SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
                'SQL = SQL & "and tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

                'SF = "{View_Laporan_Barang_Masuk_Asset_Loading.kode_perusahaan} = '" & KodePerusahaan & "' "
                'SF = SF & "and {View_Laporan_Barang_Masuk_Asset_Loading.tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
                'SF = SF & "{View_Laporan_Barang_Masuk_Asset_Loading.tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

                'If Not Txt_KdSupplier.Text = "--- SELURUH ---" Then
                '    SQL = SQL & "and kode_supplier = '" & Txt_KdSupplier.Text & "' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.kode_supplier} = '" & Txt_KdSupplier.Text & "'"
                'End If

                'If Not Txt_KdBarang.Text = "--- SELURUH ---" Then
                '    SQL = SQL & "and kode_barang = '" & Txt_KdBarang.Text & "' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.kode_barang} = '" & Txt_KdBarang.Text & "'"
                'End If

                'If Not Txt_NoSJ.Text = "--- SELURUH ---" Then
                '    SQL = SQL & "and No_SJ = '" & Txt_NoSJ.Text & "' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.No_SJ} = '" & Txt_NoSJ.Text & "'"
                'End If

                'If Cmb_FlagValidasiWarehouse.SelectedIndex = 1 Then
                '    SQL = SQL & "and status_Warehouse = 'Validasi' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.status_Warehouse} = 'Validasi'"
                'ElseIf Cmb_FlagValidasiWarehouse.SelectedIndex = 2 Then
                '    SQL = SQL & "and status_Warehouse = 'Belum Di Validasi' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.status_Warehouse} = 'Belum Di Validasi'"
                'End If

                'If Cmb_FlagValidasiAcc.SelectedIndex = 1 Then
                '    SQL = SQL & "and status_acc = 'Validasi' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.status_ACC} = 'Validasi'"
                'ElseIf Cmb_FlagValidasiAcc.SelectedIndex = 2 Then
                '    SQL = SQL & "and status_acc = 'Belum Di Validasi' "
                '    SF = SF & "And {View_Laporan_Barang_Masuk_Asset_Loading.status_ACC} = 'Belum Di Validasi'"
                'End If
    ...
    Private Sub Txt_NoSJ_TextChanged(sender As Object, e As EventArgs) Handles Txt_NoSJ.TextChanged
        ...
        Try
            ...
            SQL = SQL & "from EMI_Pembelian_Loading_Barang_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and status is null and No_SJ like '%" & Txt_NoSJ.Text & "%' "
    ...
    Private Sub Txt_NoSJ_Leave(sender As Object, e As EventArgs) Handles Txt_NoSJ.Leave
        ...
        Try
            ...
            If Not Txt_KdSupplier.Text = "--- SELURUH ---" Then
                ...
                SQL = "select Distinct No_Faktur, No_SJ, ( No_Plat + ' - ' + Driver) as Mobil "
                SQL = SQL & "from EMI_Pembelian_Loading_Barang_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and status is null and No_SJ = '" & Txt_NoSJ.Text & "' "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
    ...
    Private Sub Txt_NmSJ_TextChanged(sender As Object, e As EventArgs) Handles Txt_NmSJ.TextChanged
        ...
        Try
            ...
            SQL = "select Distinct No_Faktur, No_SJ, (No_Plat + ' - ' + Driver) as Mobil "
            SQL = SQL & "from EMI_Pembelian_Loading_Barang_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and status is null and (No_Plat + ' - ' + Driver) like '%" & Txt_NmSJ.Text & "%' "

...
Public Class N_EMI_Laporan_Mutasi_Bahan_Asset
    ...
    Private Sub N_EMI_Laporan_Mutasi_Bahan_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            If CekButtonRole("Laporan_Saldo_Mutasi_Bahan_Asset") = "Y" Then
        ...
        Try
            ...
            SQL = "select kode_Stock_Owner from Stock_Owner_Gudang_Lain where kode_perusahaan = '" & KodePerusahaan & "' "
            ...
            SQL = "select Id_Group_Jenis, Kode_Group_Jenis from EMI_Group_Jenis_lain where Kode_Perusahaan = '" & KodePerusahaan & "'"
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
        ...
        Try
            ...
            Try
                ...
                If Cmb_Filter_by.SelectedIndex = 0 Then
                    SQL = "exec EMI_Mutasi_Bahan_Lain '" & KodePerusahaan & "', '" & Format(Tgl1.Value, "yyyy-MM-dd") & "', '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                    ExecuteTrans(SQL)
                ElseIf Cmb_Filter_by.SelectedIndex = 1 Then
                    SQL = "exec EMI_Mutasi_Bahan_lain2 '" & KodePerusahaan & "', '" & Format(Tgl1.Value, "yyyy-MM-dd") & "', '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                    ExecuteTrans(SQL)
                End If
                ...
                If Cmb_Jenis_Laporan.SelectedIndex = 0 Then
                    ...
                    SQL = SQL & "isnull((select round(sum(masuk-Keluar),2) from Data_CutOffTracking_lain x "
                    ...
                    SQL = SQL & "from Data_CutOffTracking_lain a, barang_lain b, EMI_Group_Jenis_lain c  "
                    ...
                    'SQL = SQL & "FROM Data_CutOffTracking a, barang b, EMI_Group_Jenis c "
                    'SQL = SQL & "where a.kode_perusahaan = b.Kode_Perusahaan and b.Kode_Perusahaan = c.Kode_Perusahaan "
                    ...
                    SQL = SQL & "and b.Id_Group_Jenis = c.Id_Group_Jenis) "
                    SQL = SQL & ",cte_b as( SELECT kode_perusahaan, Kode_Group_Jenis, Lokasi, kode_barang, Nama, QR, ISNULL([Saldo Awal],0) AS [Saldo Awal], ISNULL([Barang Masuk],0) AS [Barang Masuk], "
                    SQL = SQL & "ISNULL([Pengeluaran Stock],0) AS [Pengeluaran Stock] "
                    SQL = SQL & "FROM ( SELECT kode_perusahaan, Kode_Group_Jenis, Lokasi, kode_barang, Nama, QR, Jenis  AS JenisKolom, Nilai FROM cte ) src "
                    SQL = SQL & "PIVOT ( SUM(Nilai) FOR JenisKolom IN ( [Saldo Awal], [Barang Masuk],[Pengeluaran Stock] ) ) p ) "
                    SQL = SQL & "select *, Round( [Saldo Awal]+ [Barang Masuk]+[Pengeluaran Stock],4) as Stock from cte_b a "
                        ...
                        With Ds.Tables("MyTable")
                            If .Rows.Count <> 0 Then
                                For i As Integer = 0 To .Rows.Count - 1
                                    ...
                                    Dgv_Detail.Rows(i).Cells(7).Value = Format(.Rows(i).Item("Pengeluaran Stock"), "N2")
                                    Dgv_Detail.Rows(i).Cells(8).Value = Format(.Rows(i).Item("Stock"), "N2")
                                Next
                            End If
                        End With
                    End Using
                ...
                ElseIf Cmb_Jenis_Laporan.SelectedIndex = 1 Then
                    ...
                    SQL = SQL & "isnull((select round(sum(Masuk_Saldo-Keluar_Saldo),2) from Data_CutOffTracking_lain x "
                        ...
                        With Ds.Tables("MyTable")
                            If .Rows.Count <> 0 Then
                                For i As Integer = 0 To .Rows.Count - 1
                                    ...
                                    Dgv_Rekap.Rows(i).Cells(11).Value = Format(.Rows(i).Item("Keluar_Saldo"), "N2")
    ...
    Private Sub Btn_Cetak_Rekap_Click(sender As Object, e As EventArgs) Handles Btn_Cetak_Rekap.Click
        ...
        Try
            ...
            If Cmb_Filter_by.SelectedIndex = 0 Then
                SQL = "exec EMI_Mutasi_Bahan2_rekap '" & KodePerusahaan & "', '" & Format(Tgl1.Value, "yyyy-MM-dd") & "', '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                ExecuteTrans(SQL)
            ElseIf Cmb_Filter_by.SelectedIndex = 1 Then
                SQL = "exec EMI_Mutasi_Bahan2 '" & KodePerusahaan & "', '" & Format(Tgl1.Value, "yyyy-MM-dd") & "', '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                ExecuteTrans(SQL)
            End If
            ...
            If Cmb_Jenis_Laporan.SelectedIndex = 0 Then
                ...
                SQL = "select * from N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain "
                SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
                SQL = SQL & "and tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                SF = "{N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.kode_perusahaan} = '" & KodePerusahaan & "' "
                SF = SF & "AND CDate({N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.tanggal}) >= Date(" & Year(Tgl1.Value) & "," & Month(Tgl1.Value) & "," & DateAndTime.Day(Tgl1.Value) & ") "
                SF = SF & "AND CDate({N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.tanggal}) <= Date(" & Year(Tgl2.Value) & "," & Month(Tgl2.Value) & "," & DateAndTime.Day(Tgl2.Value) & ")"



                If Cmb_Lokasi.SelectedIndex > 0 Then
                    SQL = SQL & "and lokasi = '" & Cmb_Lokasi.Text & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.lokasi} = '" & Cmb_Lokasi.Text & "' "
                End If

                If Not Txt_KdBarang.Text.ToUpper = OpsiSeluruh.ToUpper Then
                    SQL = SQL & "and kode_barang = '" & Txt_KdBarang.Text & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.kode_barang} = '" & Txt_KdBarang.Text & "' "
                End If

                If Cmb_Barcode.SelectedIndex > 0 Then
                    If Not Txt_Barcode.Text.Trim.Length = 0 Then
                        SQL = SQL & "and QR = '" & Txt_Barcode.Text & "' "
                        SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.QR} = '" & Txt_Barcode.Text & "' "
                    Else
                        CloseConn()
                        MessageBox.Show("Barcode Tidak Boleh Kosong")
                        Txt_Barcode.Focus() : Exit Sub
                ...
                If Cmb_Group_Jenis.SelectedIndex > 0 Then
                    SQL = SQL & "and Id_Group_Jenis = '" & arr_Id_Group_Jenis(Cmb_Group_Jenis.SelectedIndex) & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_lain.Id_Group_Jenis} = " & arr_Id_Group_Jenis(Cmb_Group_Jenis.SelectedIndex) & " "
                End If
                Using DS = BindingTrans(SQL)
                    With DS.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            ...
                            Dim CrDoc As New N_EMI_CR_Laporan_Mutasi_Bahan_Rekap_Asset
            ...
            ElseIf Cmb_Jenis_Laporan.SelectedIndex = 1 Then
                SQL = "select * from N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain "
                SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
                SQL = SQL & "and tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "
                SF = "{N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.kode_perusahaan} = '" & KodePerusahaan & "' "
                SF = SF & "AND CDate({N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.tanggal}) >= Date(" & Year(Tgl1.Value) & "," & Month(Tgl1.Value) & "," & DateAndTime.Day(Tgl1.Value) & ") "
                SF = SF & "AND CDate({N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.tanggal}) <= Date(" & Year(Tgl2.Value) & "," & Month(Tgl2.Value) & "," & DateAndTime.Day(Tgl2.Value) & ")"



                If Cmb_Lokasi.SelectedIndex > 0 Then
                    SQL = SQL & "and lokasi = '" & Cmb_Lokasi.Text & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.lokasi} = '" & Cmb_Lokasi.Text & "' "
                End If

                If Not Txt_KdBarang.Text.ToUpper = OpsiSeluruh.ToUpper Then
                    SQL = SQL & "and kode_barang = '" & Txt_KdBarang.Text & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.kode_barang} = '" & Txt_KdBarang.Text & "' "
                End If

                If Cmb_Barcode.SelectedIndex > 0 Then
                    If Not Txt_Barcode.Text.Trim.Length = 0 Then
                        SQL = SQL & "and QR = '" & Txt_Barcode.Text & "' "
                        SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.QR} = '" & Txt_Barcode.Text & "' "
                    Else
                        CloseConn()
                        MessageBox.Show("Barcode Tidak Boleh Kosong")
                        Txt_Barcode.Focus() : Exit Sub
                ...
                If Cmb_Group_Jenis.SelectedIndex > 0 Then
                    SQL = SQL & "and Id_Group_Jenis = '" & arr_Id_Group_Jenis(Cmb_Group_Jenis.SelectedIndex) & "' "
                    SF = SF & "And {N_EMI_View_Laporan_Mutasi_Bahan_Rekap_Saldo_Lain.Id_Group_Jenis} = " & arr_Id_Group_Jenis(Cmb_Group_Jenis.SelectedIndex) & " "
                End If
                ...
                Using DS = BindingTrans(SQL)
                    With DS.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            ...
                            Dim CrDoc As New N_EMI_CR_Laporan_Mutasi_Bahan_Rekap_Saldo_Asset
    ...
    Private Sub Generate_excel_detail()
        Try
            ...
            Dim JudulLaporan As String = "LAPORAN MUTASI BAHAN DETAIL"
...
#Region "Generate Coloms"
            ...
            For i As Integer = 0 To tot_kolom - 1
                ...
                If currentIdentifier = "Main" OrElse currentIdentifier = "Loss" Then
                ...
                If Not groupInfo.ContainsKey(currentIdentifier) Then
                    Dim groupTitle As String = If(currentIdentifier = "Main", "", currentIdentifier)
            ...
            For Each kvp In groupInfo
                ...
                If identifier = "Main" OrElse identifier = "Loss" Then
                    For c As Integer = startCol To endCol
                        Dim specificColor = CInt(dataKoloms(c - 1)("Warna"))

                        With xlWorkSheet.Range(xlWorkSheet.Cells(3, c), xlWorkSheet.Cells(4, c))
                            .Merge()
                            .Interior.Color = specificColor
                        End With
                    Next
                Else
                    Dim groupColor = CInt(dataKoloms.First(Function(k) k("Identifier").ToString() = identifier)("Warna"))
                    ...
                    topRowRange.Merge()
                    topRowRange.Interior.Color = groupColor
                    bottomRowRange.Interior.Color = groupColor
                End If
            Next
...
#Region "KODE LAMA"
                ...
                'SQL = ";WITH cte AS ( SELECT  a.No_Faktur, Tanggal, Jam, Jenis, Keterangan, Kode_stock_owner AS Lokasi, Kode_barang, Nama, QR, ROUND(Masuk,4) - ROUND(Keluar,4) AS Nilai FROM Data_CutOffTracking a ) "
...
#End Region
    ...
    Private Sub Generate_excel_detail_Saldo()
        Try
            ...
            Dim JudulLaporan As String = "LAPORAN MUTASI BAHAN DETAIL SALDO"
...
#Region " Kode LAMA"
                ...
                'Dim row As Integer = 0
                'SQL = ";WITH cte AS ( SELECT  a.No_Faktur, Tanggal, Jam, Jenis, Keterangan, Kode_stock_owner AS Lokasi, Kode_barang, Nama, QR, ROUND(Masuk_Saldo,4) - ROUND(Keluar_Saldo,4) AS Nilai FROM Data_CutOffTracking a ) "
                'SQL = SQL & ",cte_b as( SELECT  Lokasi, kode_barang, Nama, QR, ISNULL([Saldo Awal],0) AS [Saldo Awal], ISNULL([Barang Masuk],0) AS [Barang Masuk], "
                'SQL = SQL & "ISNULL([Terima Transfer Stock],0) AS [Terima Transfer Stock], ISNULL([Terima Split Stock],0) AS [Terima Split Stock], "
                'SQL = SQL & "ISNULL([Terima Transfer Material],0) AS [Terima Transfer Material], ISNULL([Penambahan Stock],0) AS [Penambahan Stock], "
                'SQL = SQL & "ISNULL([Retur Produksi],0) AS [Retur Produksi], ISNULL([Penerimaan Produksi 1],0) AS [Penerimaan Produksi 1], "

Public Class EMI_PO_Pembelian_Barang_Lain
    ...
    Private Sub get_no_faktur_submit_Po()
        fakturSubmitPO = FSubmitPO & arrInisialFakturSubmitPO & "-" & Format(tgl_skg, "MM/yy") & "-" &
                          General_Class.Get_Last_Number2("Submit_PO_Barang_Lain", "No_Faktur", JumlahDigit,
                          "Kode_perusahaan", KodePerusahaan,
    ...
    Private Sub Get_Isi_Listview(ByVal No_Index As Integer)
        ...
        lvPO_TempoPembayaran = LvPO_DataPO.Items(No_Index).SubItems(cellPO_TempoPembayaran).Text
        lvPO_JatuhTempo = LvPO_DataPO.Items(No_Index).SubItems(cellPO_JatuhTempo).Text
        lvPO_FakPenawaran = LvPO_DataPO.Items(No_Index).SubItems(cellPO_FakPenawaran).Text
        lvPO_JnsKategori = LvPO_DataPO.Items(No_Index).SubItems(cellPO_JnsKategori).Text
    End Sub
    ...
    Public Sub HitungGrandTotal()
        ...
        If LvPO_DataPO.Items.Count <> 0 Then
            ...
            Try
                ...
                SQL = "select isnull(pph, 0) as pph "
                SQL = SQL & "from suppliers "
                SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
                SQL = SQL & "and Kode_Supplier = '" & TxtPO_KdSupplier.Text & "' "
                Using Dr = OpenTrans(SQL)
                    ...
                    Else
                        Dr.Close()
                        CloseConn()
                        MessageBox.Show("Terjadi Kesalahan, Supplier Tidak Ditemukan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    End If
                End Using
        ...
        Try
            ...
            For index = 0 To LvPO_DataPO.Items.Count - 1
                ...
                SQL = "select isnull(dbo.Ubah_Satuan_lain('" & KodePerusahaan & "','MASA',Kode_Barang,'Gram','Gram', berat),0) as hasil "
            ...
            SQL = "Select dbo.Ubah_Satuan_lain('" & KodePerusahaan & "','MASA','" & lvPO_KdBarang & "',"
            SQL = SQL & "'Gram','KG',"
    ...
    Public Sub kosong()
        ...
        LvPO_DataPO.Columns.Add("Harga", 160, HorizontalAlignment.Right) '3
        LvPO_DataPO.Columns.Add("Jumlah", 160, HorizontalAlignment.Right) '4
        ...
        Try
            ...
            SQL = "select Kode_Stock_Owner_gudang from Binding_Lokasi_Gudang_Lain where kode_perusahaan = '" & KodePerusahaan & "' and Gudang_Default = 'Y' and Kode_Stock_Owner='" & CmbPO_Lokasi.Text & "' "
            Using dr = OpenTrans(SQL)
                If dr.Read Then
        ...
        Try
            ...
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    ...
                    SQL = "select Satuan, flag_tampil_display from barang_Detail_Satuan_lain where Kode_Barang= '" & Trim(TxtPO_KdBrg.Text) & "' "
    ...
    Private Sub BtnPO_Simpan_Click(sender As Object, e As EventArgs) Handles BtnPO_Simpan.Click
        ...
        Try
            ...
            If BtnPO_Simpan.Tag = "&Simpan" Then
                ...
                SQL = SQL & "Tgl_Jatuh_Tempo,ekspedisi,biaya, Flag_Import, tempo_pembayaran, Lama_Pembayaran, Grand_Total_Terbilang, Grand_PPH, Eta_Simulasi ) values( "
                ...
                SQL = "insert into N_EMI_LOG_PO_INDUK_ETA_ETD_Barang_lain(kode_perusahaan,no_faktur,tanggal,jam,userid,eta,etd) "
                SQL = SQL & "values( "
                SQL = SQL & "'" & KodePerusahaan & "', '" & TxtPO_NoFaktur.Text & "', '" & Format(tgl_skg, "yyyy-MMM-dd") & "',"
                SQL = SQL & "'" & Format(tgl_skg, "HH:mm:ss") & "', '" & UserID & "',"
                SQL = SQL & "'" & Format(DTPPO_ETA.Value, "yyyy-MM-dd") & "', '" & Format(DtpPO_ETD.Value, "yyyy-MM-dd") & "' )"
                ExecuteTrans(SQL)
                ...
                For i As Integer = 0 To tempDataPajak.Count - 1
                    ...
                    SQL = "insert into EMI_Detail_PPH_PO_Induk_Barang_Lain (Kode_Perusahaan, No_faktur, Persentase, Nilai, Kode_Tarif, Flag_PPN, Kode_Akun ) values "
            ...
            Else
                ...
                SQL = SQL & "eta_simulasi = '" & Format(DTPPO_ETA.Value, "yyyy-MM-dd") & "',"
    ...
    Private Sub BtnPO_Ok_Click(sender As Object, e As EventArgs) Handles BtnPO_Ok.Click
        ...
        Try
            OpenConn()
            Dim harga_satuan_besar As Double = 0
            SQL = "Select dbo.Ubah_Satuan_Lain('" & KodePerusahaan & "','UANG','" & TxtPO_KdBrg.Text & "',"
            ...
            SQL = "select dbo.Ubah_Satuan_lain('" & KodePerusahaan & "','MASA','" & TxtPO_KdBrg.Text & "',"
            ...
            SQL = SQL & "from EMI_Purchase_Requisition_Barang_Lain_Detail a, emi_detail_proses_pengiriman_po_Barang_Lain b, Suppliers c "
            ...
            'If Not LvPO_DataPO.Items(0).SubItems(cellPO_JnsKategori).Text = Dr("Jenis_Kategori") ThenS
            '                Dr.Close()
            '                CloseConn()
            '                MessageBox.Show("Jenis Kategori Besar Tidak Boleh Berbeda!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            '                Exit Sub
            ...
            lvw.SubItems.Add(Format(Val(TxtPO_Jml.Text), "N2")) '4
            lvw.SubItems.Add(CmbPO_Satuan.Text) '5
    ...
    Public Sub TxtPO_NoFaktur_Leave(sender As Object, e As EventArgs) Handles TxtPO_NoFaktur.Leave
        ...
        Try
            ...
            SQL = SQL & "Total_IDR,Grand_Sebelum_PPN,a.ppn,Grand,ETD_Simulasi,eta_simulasi, ekspedisi,biaya, flag_release "
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    ...
                    If General_Class.CekNULL(Dr("etd_simulasi")) = "" Then
                        DtpPO_ETD.Format = DateTimePickerFormat.Custom
                        DtpPO_ETD.CustomFormat = " "
                    Else
                        'DtpPO_ETD.Format = DateTimePickerFormat.Custom

                        'DtpPO_ETD.Value = Format(Dr("etd_simulasi"), "dd MMMM yyyy")
                        DtpPO_ETD.Value = Dr("etd_simulasi")
                        DtpPO_ETD.CustomFormat = "dd MMMM yyyy"
                    End If


                    If General_Class.CekNULL(Dr("eta_simulasi")) = "" Then
                        DTPPO_ETA.Format = DateTimePickerFormat.Custom
                        DTPPO_ETA.CustomFormat = " "
                    Else
                        'DtpPO_ETD.Format = DateTimePickerFormat.Custom
                        'DTPPO_ETA.Value = Format(Dr("eta_simulasi"), "dd MMMM yyyy")
                        DTPPO_ETA.Value = Dr("eta_simulasi")
                ...
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            lvw.SubItems.Add(Format(.Rows(i).Item("nilai_barang") * Val(.Rows(i).Item("harga_barang")), "N2")) '11
                            lvw.SubItems.Add("") '12
                            ...
                            '=     GET DATA PENAWARAN     =
                            '==============================
                            SQL = "select top 1 b.Tempo_Pembayaran, b.Lama_Pembayaran "
                            ...
                            Using Ds2 = BindingTrans(SQL)
                                If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                                    ...
                                    lvw.SubItems.Add(Ds2.Tables("MyTable").Rows(0).Item("Lama_Pembayaran")) '14
                            ...
                            'Using Ds2 = BindingTrans(SQL)
                            '    If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                            '        lvw.SubItems.Add(Ds2.Tables("MyTable").Rows(0).Item("Jenis_Kategori")) '14
                            '    Else
                            lvw.SubItems.Add("-")
                            '    End If
                            'End Using
    ...
    Private Sub Txt_GrandPPH_DoubleClick(sender As Object, e As EventArgs) Handles Txt_GrandPPH.DoubleClick
        ...
        SD_Detail_PajakPO_Barang_Lain.TxtPO_GrandTotal.Text = TxtPO_Total.Text
        SD_Detail_PajakPO_Barang_Lain.KdSupplier = TxtPO_KdSupplier.Text
        SD_Detail_PajakPO_Barang_Lain.No_Fak = TxtPO_NoFaktur.Text
        SD_Detail_PajakPO_Barang_Lain.tempDataPajak = tempDataPajak
        SD_Detail_PajakPO_Barang_Lain.ShowDialog()

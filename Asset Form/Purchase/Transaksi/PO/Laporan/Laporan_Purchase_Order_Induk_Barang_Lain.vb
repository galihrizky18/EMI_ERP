Public Class Laporan_Purchase_Order_Induk_Barang_Lain

    Dim arrParamLain, arrParamLainSF, arrTanggal As New ArrayList

    Dim JudulForm As String = "Laporan Purchase Order"

    Private Sub Emi_Laporan_PO_Activated(sender As Object, e As EventArgs) Handles Me.Activated
    ...
    Private Sub Kosong()
        ...
        CmbJenis.Items.Add("BERJALAN")
        CmbJenis.Items.Add("SELESAI")
        ...
        Cmb_ParamLain.Items.Add("User Created") : arrParamLain.Add("User_Created") : arrParamLainSF.Add("{View_Laporan_PO_barang_lain.User_Created}")
        Cmb_ParamLain.Items.Add("User Release") : arrParamLain.Add("User_Release") : arrParamLainSF.Add("{View_Laporan_PO_barang_lain.User_Release}")
    ...
    Private Sub Txt_Faktur_TextChanged(sender As Object, e As EventArgs) Handles Txt_Faktur.TextChanged
        ...
        Try
            ...
            Lv.SubItems.Add("--- SELURUH ---")
            SQL = "select No_Faktur, No_Nota, Tanggal from EMI_Pembelian_PO_Induk where Kode_Perusahaan ='" & KodePerusahaan & "' and No_Faktur like '%" & Txt_Faktur.Text & "%' and Status is null "
    ...
    Private Sub Txt_Faktur_Leave(sender As Object, e As EventArgs) Handles Txt_Faktur.Leave
        ...
        Try
            ...
            If Not Txt_Faktur.Text = "--- SELURUH ---" Then
                ...
                SQL = "select No_Faktur, No_Nota, Tanggal from EMI_Pembelian_PO_Induk_barang_lain where Kode_Perusahaan ='" & KodePerusahaan & "' and No_Faktur = '" & Txt_Faktur.Text & "' and Status is null "
                Using Dr = Open(SQL)
                    If Dr.Read Then
    ...
    Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
        ...
        Try
            ...
            SQL = "select kode_perusahaan from View_Laporan_PO_barang_lain  "
            ...
            SF = "{View_Laporan_PO_barang_lain.kode_perusahaan} = '" & KodePerusahaan & "' "
            SF = SF & "and {View_Laporan_PO_barang_lain." & arrTanggal(Cmb_Periode.SelectedIndex) & "} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
            SF = SF & "{View_Laporan_PO_barang_lain." & arrTanggal(Cmb_Periode.SelectedIndex) & "} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

            If Not Cmb_Status.SelectedIndex = 0 Then

                If Cmb_Status.SelectedIndex = 1 Then
                    SQL = SQL & "and Status_PO = 'SUBMITED' "
                    SF = SF & "And {View_Laporan_PO_barang_lain.Status_PO} = 'SUBMITED' "

                ElseIf Cmb_Status.SelectedIndex = 2 Then
                    SQL = SQL & "and Status_PO = 'UNSUBMITED' "
                    SF = SF & "And {View_Laporan_PO_barang_lain.Status_PO} = 'UNSUBMITED' "
                End If

            End If
            If CmbJenis.SelectedIndex = 1 Then
                SQL = SQL & "and Flag_selesai = 'T' "
                SF = SF & "And {View_Laporan_PO_barang_lain.Flag_selesai} = 'T' "
            ElseIf CmbJenis.SelectedIndex = 2 Then
                SQL = SQL & "and Flag_selesai = 'Y' "
                SF = SF & "And {View_Laporan_PO_barang_lain.Flag_selesai} = 'Y' "

            End If

            If Not Txt_Faktur.Text = "--- SELURUH ---" Then
                SQL = SQL & "and no_faktur = '" & Txt_Faktur.Text & "' "
                SF = SF & "And {View_Laporan_PO_barang_lain.no_faktur} = '" & Txt_Faktur.Text & "'"
            End If

            If Not Txt_KdSupplier.Text = "--- SELURUH ---" Then
                SQL = SQL & "and Kode_Supplier = '" & Txt_KdSupplier.Text & "' "
                SF = SF & "And {View_Laporan_PO_barang_lain.Kode_Supplier} = '" & Txt_KdSupplier.Text & "'"
            End If

            If Not Txt_KdBarang.Text = "--- SELURUH ---" Then
                SQL = SQL & "and Kode_Barang = '" & Txt_KdBarang.Text & "' "
                SF = SF & "And {View_Laporan_PO_barang_lain.Kode_Barang} = '" & Txt_KdBarang.Text & "'"
            End If
            ...
            Using DS = BindingTrans(SQL)
                With DS.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        Dim CrDoc As New Rpt_Laporan_PO_Barang_Lain

...
Public Class Master_Penawaran_SD_Barang_Lain
    ...
    Private Sub TxtKode_TextChanged(sender As Object, e As EventArgs) Handles TxtPilihBarang_KodeBarang.TextChanged
        If TxtPilihBarang_KodeBarang.Text.Length >= 3 Then
            ...
            Try
                ...
                SQL = SQL & "from Barang_Lain a, EMI_Group_Jenis_Lain b where "
    ...
    Private Sub TxtKode_Leave(sender As Object, e As EventArgs) Handles TxtPilihBarang_KodeBarang.Leave
        ...
        Try
            ...
            SQL = "select a.Kode_Barang, a.Kode_Stock_Owner,a.Nama, a.Satuan, b.Kode_Group_Jenis, a.Flag_PPN "
            SQL = SQL & "from Barang_Lain a, EMI_Group_Jenis_Lain b where "
            SQL = SQL & "a.Kode_Perusahaan=b.Kode_Perusahaan and a.Id_Group_Jenis=b.Id_Group_Jenis "
            SQL = SQL & "and a.Kode_Perusahaan='" & KodePerusahaan & "' and a.Kode_Stock_Owner='" & CmbPilihBarang_Lokasi.Text & "' "
                    ...
                    Using Ds = BindingTrans(SQL)
                        With Ds.Tables("MyTable")
                            For i As Integer = 0 To .Rows.Count - 1
                                CmbPilihBarang_Satuan.Items.Add(.Rows(i).Item("satuan"))

                                If General_Class.CekNULL(.Rows(i).Item("flag_tampil_display")) = "Y" Then
                                    indexTampilDisplay = i
                                End If
    ...
    Private Sub Btn_Simpan_Click_1(sender As Object, e As EventArgs) Handles BtnPilihBarang_Simpan.Click
        ...
        If asal = "Master_Penawaran_Lain" Then

            For index = 0 To Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows.Count - 1
                If Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(index).Cells(Transaksi_Penawaran_Barang_Lain.cellKdBrg).Value = TxtPilihBarang_KodeBarang.Text Then
                    MessageBox.Show("Barang Lain Sudah ada")
                    Exit Sub
                End If
            Next


            Dim rows As Integer = Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows.Count - 1
            Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows.Add(1)
            'Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(rows).Cells(Transaksi_Penawaran_Barang_Lain.cellCheckbox).Value = True
            Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(rows).Cells(Transaksi_Penawaran_Barang_Lain.cellKdBrg).Value = TxtPilihBarang_KodeBarang.Text
            Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(rows).Cells(Transaksi_Penawaran_Barang_Lain.cellNmBrg).Value = TxtPilihBarang_NamaBarang.Text
            ...
            Try
                ...
                Using dr2 = OpenTrans(SQL)
                    Do While dr2.Read
                        Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(rows).Cells(Transaksi_Penawaran_Barang_Lain.cellSatuan).Value = dr2("satuan")
                    Loop
                End Using
            ...
            Transaksi_Penawaran_Barang_Lain.DgvMaster_Penawaran.Rows(rows).Cells(Transaksi_Penawaran_Barang_Lain.cellHrgSatuan).Value = 0

            Transaksi_Penawaran_Barang_Lain.Lbl_GetKdBrg.Text = TxtPilihBarang_KodeBarang.Text
            Transaksi_Penawaran_Barang_Lain.Lbl_NmBrg.Text = TxtPilihBarang_NamaBarang.Text
            Transaksi_Penawaran_Barang_Lain.Lbl_SatuanBrg.Text = CmbPilihBarang_Satuan.Text
        Else

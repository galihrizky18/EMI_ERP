...
Public Class N_EMI_Pemakaian_Stock_Barang_Lain
    Public Property asal As String
    Public Property MenuAsal As String

    Dim arrSO, arrInisialFaktur, arrIdWMSWarehouse, WarehosePosition, arrJenisPengeluaran, arrPengguna As New ArrayList
    ...
    '            Lv_DetBarang.Location = New Point(37, 302)
    '            Lv_DetBarang.Visible = True
    '        Else
    '            Lv_DetBarang.Location = New Point(1278, 302)
    ...
    Public Sub kosong()
        get_jam()
        Dim x As New Point(153, 160)
        LvCost.Location = x
        LvCost.Visible = False


        LvPengguna.Columns.Clear()
        LvPengguna.Columns.Add("ID", 0, HorizontalAlignment.Center)
        LvPengguna.Columns.Add("Keterangan", 400, HorizontalAlignment.Left)

        LvPengguna.Location = New Point(153, 210)
        LvPengguna.Visible = False

        CmbPengguna.Items.Clear() : arrPengguna.Clear()
        CmbPengguna.Items.Add("PIC") : arrPengguna.Add("A")
        CmbPengguna.Items.Add("RUANGAN") : arrPengguna.Add("B")
        ...
        Cmb_JenisPengeluaran.Items.Add("Pemakaian Stock") : arrJenisPengeluaran.Add("Pemakaian_Stock")
        ...
        Else
            ...
            ElseIf MenuAsal.ToUpper = "PEMAKAIAN_STOCK" Then
                Cmb_JenisPengeluaran.SelectedIndex = 1
            Else
                MessageBox.Show("Menu Tidak Ditemukan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Me.Close()
    ...
    Private Sub get_no_faktur()
        ...
        TxtNo_Transaksi.Text = FPro_Results & arrInisialFaktur.Item(CmbSO_Asal.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy") & "-" &
                                      General_Class.Get_Last_Number2("N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain", "no_faktur", JumlahDigit,
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        Try
            ...
            '==================================
            SQL = "insert into N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain (kode_perusahaan, No_faktur, Kode_Stock_Owner, Tanggal, Jam, UserID, Lokasi, Keterangan, Id_Cost_Center, Flag_Stock_Rejected) Values "
            ...
            For i As Integer = 0 To Dgv_DataRekap.Rows.Count - 1
                ...
                SQL = "insert into N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain_Detail (Kode_Perusahaan, No_faktur, Kode_Barang, Total, Satuan, "
                ...
                SQL = "select IDENT_CURRENT('N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain_Detail') as urutan"
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                ...
                SQL = "select kode_Perusahaan from N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain_Detail where "
                ...
                For j As Integer = 0 To Dgv_DataDetail.Rows.Count - 1
                    ...
                    If dgv_detail_lokasi = dgv_Rekap_lokasi And dgv_detail_KdBarang = dgv_Rekap_KdBarang Then
                        ...
                        '======================================
                        SQL = "select a.Kode_Perusahaan from N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain a, N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain_Det b  where "
                        ...
                        SQL = "insert into N_EMI_Transaksi_Pemakaian_Stock_Barang_Lain_Det(Kode_Perusahaan, No_faktur, Id_Wms_Awal, No_Pallet_Awal, "
...
#End Region
    ...
    Private Sub TxtPengguna_KeyDown(sender As Object, e As KeyEventArgs) Handles TxtPengguna.KeyDown
        If e.KeyCode = Keys.Down Then LvPengguna.Focus()
    End Sub

    Private Sub TxtPengguna_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtPengguna.KeyPress
        If e.KeyChar = Chr(13) Then
            TxtPengguna_Leave(TxtPengguna, e)
            LvPengguna.Visible = False
        End If
    End Sub

    Private Sub TxtPengguna_Leave(sender As Object, e As EventArgs) Handles TxtPengguna.Leave
        If TxtPengguna.Text.Trim.Length = 0 Or CmbPengguna.SelectedIndex = -1 Then Exit Sub
        If LvPengguna.Focused = True Then Exit Sub

        Try
            OpenConn()


            If arrPengguna.Item(CmbPengguna.SelectedIndex) = "A" Then
                SQL = "Select a.Id_Karyawan As ID, a.Nama+' - '+b.Kode_Jabatan+' - '+c.Kode_Divisi as Keterangan "
                SQL = SQL & "from Emi_Karyawan a,Emi_Jabatan_Internal b, Emi_Divisi_Internal c "
                SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan And a.Id_Jabatan = b.Id_Jabatan "
                SQL = SQL & "And b.Kode_Perusahaan=c.Kode_Perusahaan And b.Id_Divisi=c.Id_Divisi "
                SQL = SQL & "And a.kode_perusahaan ='" & KodePerusahaan & "' and a.Id_Karyawan= '" & TxtIdPengguna.Text & "' "
                SQL = SQL & "order by Nama "
            Else
                SQL = "Select a.id_area As ID, b.keterangan+' - '+a.keterangan as Keterangan "
                SQL = SQL & "From N_EMI_Master_Area_Barang_Lain a, N_EMI_Master_Gedung_Barang_Lain b "
                SQL = SQL & "Where a.kode_Perusahaan = b.kode_perusahaan And a.id_gedung = b.id_gedung "
                SQL = SQL & "And a.kode_perusahaan ='" & KodePerusahaan & "' and a.id_area= '" & TxtIdPengguna.Text & "' "
                SQL = SQL & "order by  b.keterangan, a.keterangan "
            End If


            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    TxtIdPengguna.Text = Dr("ID")
                    TxtPengguna.Text = Dr("Keterangan")
                    TxtKd_Barang.Focus()
                Else
                    MessageBox.Show("Data Pengguna tidak ditemukan . . ! !", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    TxtIdPengguna.Text = "" : TxtPengguna.Text = ""
                    TxtPengguna.Focus()
                End If
                LvPengguna.Visible = False
            End Using
    ...
    Private Sub TxtPengguna_TextChanged(sender As Object, e As EventArgs) Handles TxtPengguna.TextChanged
        If TxtPengguna.Text.Trim.Length = 0 Or CmbPengguna.SelectedIndex = -1 Then
            LvPengguna.Visible = False : Exit Sub
        Else
            LvPengguna.Visible = True
        End If

        Try
            OpenConn()

            LvPengguna.Items.Clear()
            Dim lv As New ListViewItem
            ...
            If arrPengguna.Item(CmbPengguna.SelectedIndex) = "A" Then
                ...
                SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan And a.Id_Jabatan = b.Id_Jabatan "
                SQL = SQL & "And b.Kode_Perusahaan=c.Kode_Perusahaan And b.Id_Divisi=c.Id_Divisi "
                SQL = SQL & "And a.kode_perusahaan ='" & KodePerusahaan & "' and a.nama+' '+b.Kode_Jabatan+' '+c.Kode_Divisi like '%" & TxtPengguna.Text & "%' "
                SQL = SQL & "order by Nama "
            Else
                SQL = "Select a.id_area As ID, b.keterangan+' - '+a.keterangan as Keterangan "
                SQL = SQL & "From N_EMI_Master_Area_Barang_Lain a, N_EMI_Master_Gedung_Barang_Lain b "
                SQL = SQL & "Where a.kode_Perusahaan = b.kode_perusahaan And a.id_gedung = b.id_gedung "
                SQL = SQL & "And a.kode_perusahaan ='" & KodePerusahaan & "' and b.keterangan+' '+a.keterangan like '%" & TxtPengguna.Text & "%' "
                SQL = SQL & "order by  b.keterangan, a.keterangan "
            End If
    ...
    Private Sub LvPengguna_DoubleClick(sender As Object, e As EventArgs) Handles LvPengguna.DoubleClick
        Dim Kode As String = LvPengguna.FocusedItem.Text

        TxtIdPengguna.Text = Kode

        LvPengguna.Visible = False
        TxtPengguna_Leave(LvPengguna, e)
        TxtKd_Barang.Focus()
    End Sub

    Private Sub LvPengguna_KeyDown(sender As Object, e As KeyEventArgs) Handles LvPengguna.KeyDown
        If e.KeyCode = Keys.Enter Then
            LvPengguna_DoubleClick(LvPengguna, e)
        End If
    End Sub

    Private Sub CmbPengguna_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbPengguna.SelectedIndexChanged
        TxtIdPengguna.Text = ""
        TxtPengguna.Text = ""
    End Sub

Public Class Emi_Selisih_Barang_Masuk2_Barang_Lain
    ...
    Private Sub kosong()
        ...
        Try
            ...
            Cmb_JenisSelisih.Items.Clear() : arrJenisSelisih.Clear() : arrAkunBahan.Clear() : arrAkunPerjalanan.Clear()

            SQL = "select id_jenis_selisih, keterangan, akun_bahan,akun_perjalanan from emi_master_jenis_selisih where "
    ...
    Private Sub get_no_faktur()
        Txt_NoFaktur.Text = fSBL & arrInisialFaktur.Item(Cmb_LokasiPO.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy") & "-" &
                                     General_Class.Get_Last_Number2("EMI_Pembelian_Selisih_Barang_Masuk_Barang_Lain", "no_faktur", Jumlah_Digit,
                                     "Kode_perusahaan", KodePerusahaan,
                                     "And", "substring(no_faktur,1," & Len(fSBL) + Len(arrInisialFaktur.Item(Cmb_LokasiPO.SelectedIndex)) + 6 & ")", fSBL & arrInisialFaktur.Item(Cmb_LokasiPO.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy"))
    End Sub
                ...
                With DS.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            Dim jmlhBm_Display As Double = ubah_satuan_lain(.Rows(i).Item("Kode_Barang"), .Rows(i).Item("jmlh_BM"), .Rows(i).Item("satuan_barang"), .Rows(i).Item("Satuan"), "MASA")
                            ...
                            'SQL = "select satuan from Barang_Detail_Satuan_Lain where kode_perusahaan = '" & KodePerusahaan & "' and Kode_Barang ='" & .Rows(i).Item("Kode_Barang") & "'"
    ...
    Private Sub BtnSelisihBrgMsk_Simpan_Click(sender As Object, e As EventArgs) Handles BtnSelisihBrgMsk_Simpan.Click
        ...
        Try
            ...
            For i As Integer = 0 To Dgv_DetailBarang.Rows.Count - 1
                ...
                'SQL = "select Satuan from Barang_Detail_Satuan_Lain where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_barang='" & Dgv_KdBarang & "' and Flag_Tampil_Display is null"
                'Using Dr = OpenTrans(SQL)
                '    If Dr.Read Then
                ...
                SQL = "select satuan From Barang_Lain where kode_perusahaan = '" & KodePerusahaan & "' "
                ...
                Dim Dgv_JmlhPL_Gram As Double = Ubah_Satuan_Lain(Dgv_KdBarang, HilangkanTanda(Dgv_JmlhPL), Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_JmlhBM_Gram As Double = Ubah_Satuan_Lain(Dgv_KdBarang, HilangkanTanda(Dgv_JmlhBM), Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_JmlhUtang_Gram As Double = Ubah_Satuan_Lain(Dgv_KdBarang, HilangkanTanda(Dgv_JmlhUtang), Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_Jmlhselisih As Double = Ubah_Satuan_Lain(Dgv_KdBarang, HilangkanTanda(Dgv_Selisih), Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_JmlhPenyPlus As Double = Ubah_Satuan_Lain(Dgv_KdBarang, nilaiQtyPlus, Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_JmlhPenyMin As Double = Ubah_Satuan_Lain(Dgv_KdBarang, nilaiQtyMin, Dgv_Satuan, satuan_Kecil, "MASA")
                Dim Dgv_JmlhselisihFix As Double = Ubah_Satuan_Lain(Dgv_KdBarang, HilangkanTanda(Dgv_Selisih_Fix), Dgv_Satuan, satuan_Kecil, "MASA")

                SQL = "insert into EMI_Pembelian_Selisih_Barang_Masuk_Det_Barang_Lain(Kode_Perusahaan, No_Faktur, Kode_Stock_Owner, Kode_Barang, Jumlah_PL, Jumlah_BM, jumlah_utang, Selisih, Harga, Selisih_Rp, No_Faktur_Barang_Masuk, Tgl_Produksi, Tgl_Expired, Satuan, Urut_Loading)values("
            ...
            SQL = "Update emi_pembelian_loading_barang_lain set Flag_Selisih_BM = 'Y' "
            SQL = SQL & "where No_Faktur = '" & Txt_FakturPO.Text & "' and KOde_Perusahaan='" & KodePerusahaan & "' "
            ExecuteTrans(SQL)


            SQL = "Select distinct b.No_PO from "
            SQL = SQL & "emi_pembelian_loading_barang_lain a, emi_pembelian_loading_detail_barang_lain b "
            ...
            Using ds = BindingTrans(SQL)
                With ds.Tables("MyTable")
                    For index = 0 To .Rows.Count - 1
                        ...
                        SQL = SQL & "From EMI_Pembelian_PO_Barang_Lain a Where a.nO_faktur ='" & no_Po & "' and a.Kode_Perusahaan ='" & KodePerusahaan & "' "
                        Using Dr = OpenTrans(SQL)
                            If Dr.Read Then
        ...
        Catch ex As Exception
            ...
            Exit Sub
        End Try
        Emi_Selisih_Barang_Masuk_Display_Barang_Lain.kosong()
        Me.Close()
    End Sub

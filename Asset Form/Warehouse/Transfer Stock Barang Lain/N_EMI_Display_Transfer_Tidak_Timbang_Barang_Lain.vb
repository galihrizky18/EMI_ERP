    ...
    Private Sub Btn_TimbangFloorScale_Click(sender As Object, e As EventArgs) Handles Btn_TimbangFloorScale.Click
        ...
        Try
            ...
            SQL = $"
                ...
                isnull((
		                select 'Y' from N_EMI_Keep_Stock_Barang_Lain_Departement z
		                where g.Kode_Perusahaan = z.Kode_Perusahaan
		                and z.Status is null
		                and a.SO_Awal = z.Kode_Stock_Owner and g.Kode_Barang = z.Kode_Barang
		                and g.Urut_Keep_Stock = z.Urut_Oto
	                ), 'T') as IsKeep
            ...
            SQL = "Select c.serial_number from N_EMI_Transfer_Stock_Barang_Lain a, N_EMI_Transfer_Stock_Barang_Lain_Det b, Barang_Lain_SN c where "
            ...
            "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        If Val(HilangkanTanda(Dr("Jumlah_TF"))) > Val(HilangkanTanda(Dr("Jumlah_TF"))) Then
                            Dr.Close()
                            CloseTrans()
                            CloseConn()
                            MessageBox.Show($"Terjadi Kesalahan. Total Transfer Lebih Besar Dari Keep Stock")
                            Exit Sub
                        ElseIf Val(HilangkanTanda(Dr("Jumlah_TF"))) = Val(HilangkanTanda(Dr("Jumlah_TF"))) Then
                            Completed = True
                        End If
        ...
        Try
            ...
            SQL = $"
                ...
                select a.No_Faktur, a.Lokasi, a.SO_Awal, a.SO_Tujuan, c.Urut_Oto, b.Kode_Barang, d.nama, b.Total, b.Satuan, b.Satuan_Barang,
	                c.Serial_Number_Awal, c.Jumlah, c.Jumlah_Bags, c.Id_Wms_Tujuan, c.Warna,
	                isnull((
		                select x.Labeling_WMS_Position 
		                from View_Warehouse_Position x 

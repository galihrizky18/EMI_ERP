Public Class N_EMI_SD_Procurement_Loading
        ...
        Try
            ...
            Dim searchFilter As String = "
                AND (
                    ...
                    OR di.Kode_Barang LIKE '%" & searchText & "%'
            ...
            SQL = $"
                ;WITH DO_Items AS (
                    SELECT
                        ...
                        TRY_CAST(spo.ETA AS DATE)     AS ETA_date,
                        ...
                        do_d.No_PO,
                        CASE
                            WHEN spo.ETA IS NULL
                                THEN 'Terlambat'
                            WHEN do.Tanggal_Masuk IS NOT NULL
                                 AND do.Tanggal_Masuk <= spo.ETA
                                THEN 'On Schedule'
                            WHEN do.Tanggal_Masuk IS NULL
                                 AND spo.ETA >= CAST(GETDATE() AS DATE)
                                THEN 'On Schedule'
                            ELSE 'Terlambat'
                        END AS Status_Group,
                ...
                Barang_Masuk_Summary AS (
                    ...
                    FROM EMI_Barang_Masuk_Perpallet bmp
                ...
                SELECT
                    ...
                    CASE 
                        ...
                        WHEN di.Flag_Timbang_Keluar = 'Y' THEN 'Timbang Keluar'
                        WHEN di.Flag_QC = 'Y' THEN 'QC 2'
                        WHEN di.Flag_Timbang_Masuk = 'Y' THEN 'Timbang Masuk'
                        WHEN di.Flag_QC_Pertama = 'Y' THEN 'QC 1'
                        ELSE '-'
    ...
    Handles Guna2DataGridView1.CellMouseDown
    ...
    Private Sub BatalkanDOToolStripMenuItem_Click(sender As Object, e As EventArgs) _
    Handles BatalkanDOToolStripMenuItem.Click
        ...
        Try
            ...
            If CekButtonRole("Batal_DO") = "T" Then
                CloseTransB2B()
                CloseConnB2B()
            ...
            ' Cek Flag_Timbang_Keluar di detail DO
            Cmd.Parameters.Clear()
            Cmd.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
            Cmd.Parameters.AddWithValue("@NoDO", NoDO)

            ' Query untuk mengecek apakah ada record dengan Flag_Timbang_Keluar = 'Y' di detail
            Cmd.CommandText = "SELECT COUNT(*) AS JumlahFlagY FROM emi_pembelian_loading_detail " &
                      "WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoDO AND Flag_Timbang_Keluar = 'Y'"

            Dim countFlagY As Integer = CInt(Cmd.ExecuteScalar())

            ' Jika ada 1 atau lebih Flag_Timbang_Keluar = 'Y', tolak pembatalan
            If countFlagY > 0 Then
                MessageBox.Show("Pembatalan tidak bisa dilakukan karena DO sudah ditimbang keluar (" & countFlagY & " item).", "Validasi Gagal", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If


            'Cek apakah DO ada di B2B Purchase Order
            Dim SQL_CheckB2B_DO As String = "
                SELECT COUNT(*) 
                FROM B2B_Purchase_Order 
            ...
            CmdB2B.Parameters.AddWithValue("@NoDO", NoDO)

            Dim B2B_PO As Integer = Convert.ToInt32(CmdB2B.ExecuteScalar())
            ...
            ' Update Flag_Retur, UserID_Retur, Tanggal_Retur
            Cmd.Parameters.Clear()
            Cmd.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
            Cmd.Parameters.AddWithValue("@NoDO", NoDO)
            Cmd.Parameters.AddWithValue("@UserID", UserID)
            Cmd.Parameters.AddWithValue("@TanggalRetur", Date.Now())
            Dim SQL_UpdateDO As String = "UPDATE emi_pembelian_loading SET Flag_Retur = 'Y', UserID_Retur = @UserID, Tanggal_Retur = @TanggalRetur WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoDO"
            ExecuteTrans(SQL_UpdateDO)

            'Update B2B Purchase Order Status = 'Y'
            CmdB2B.Parameters.Clear()
            CmdB2B.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
            CmdB2B.Parameters.AddWithValue("@NoDO", NoDO)
            Dim SQL_UpdateB2B_PO As String = "UPDATE B2B_Purchase_Order SET Status = 'Y' WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoDO"
            ExecuteTransB2B(SQL_UpdateB2B_PO)

            'Update emi_pembelian_po_detail di B2B
            Cmd.Parameters.Clear()
            Cmd.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
            Cmd.Parameters.AddWithValue("@NoDO", NoDO)
            Dim SQL_DO_Detail As String = "SELECT No_Faktur, No_PO, Kode_Barang FROM emi_pembelian_loading_detail WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoDO"

            ' Collect data first instead of processing while reading
            Dim dataList As New List(Of Dictionary(Of String, Object))
            Using Dr = OpenTrans(SQL_DO_Detail)
                While Dr.Read()
                    ...
                    dataList.Add(record)
                End While
            End Using ' DataReader is now closed

            ' Now process the collected data
            For Each record In dataList
                ' Reset Flag_Selesai_PO ke NULL
                Cmd.Parameters.Clear()
                Cmd.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
                Cmd.Parameters.AddWithValue("@NoSubPO", record("No_PO"))
                Dim SQL_UpdateSubPO As String = "UPDATE emi_pembelian_po SET Flag_Selesai_PO = NULL WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoSubPO"
                ExecuteTrans(SQL_UpdateSubPO)

                CmdB2B.Parameters.Clear()
                CmdB2B.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
                CmdB2B.Parameters.AddWithValue("@NoSubPO", record("No_PO"))
                CmdB2B.Parameters.AddWithValue("@KodeBarang", record("Kode_Barang"))
                Dim SQL_Cancel_Flag_Sudah_Kirim As String = "UPDATE emi_pembelian_po_detail SET Flag_Sudah_Kirim = NULL WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoSubPO AND Kode_Barang = @KodeBarang"
                ExecuteTransB2B(SQL_Cancel_Flag_Sudah_Kirim)

                CmdB2B.Parameters.Clear()
                CmdB2B.Parameters.AddWithValue("@KodePerusahaan", KodePerusahaan)
                CmdB2B.Parameters.AddWithValue("@NoSubPO", record("No_PO"))
                Dim SQL_UpdateSubPO_B2B As String = "UPDATE emi_pembelian_po SET Flag_Selesai_PO = NULL WHERE Kode_Perusahaan = @KodePerusahaan AND No_Faktur = @NoSubPO"
                ExecuteTransB2B(SQL_UpdateSubPO_B2B)
            Next

Public Class Emi_Validasi_HPP_Produksi
    ...
    Dim CellFGQrCode As Integer = 3
    Dim CellFGJumlah As Integer = 4
    Dim CellFGSatuan As Integer = 5
    Dim CellFGHPP As Integer = 6
    Dim CellFGTotal As Integer = 7
            ...
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
                    For i As Integer = 0 To .Rows.Count - 1
                        ...
                        DgvDosing.Rows(i).Cells(CellQty_Rw_Dosing).Value = Format(.Rows(i).Item("Jumlah_Dosing"), "N0")
...
#End Region
            ...
            SQL = SQL & "And round(jumlah_dosing,2)<>round(jumlah_terpakai,2) "
            SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and a.no_production_order = '" & Txt_NoSplitProduksi.Text & "' "
            ...
            Using dss = BindingTrans(SQL)
                For ind = 0 To dss.Tables("MyTable").Rows.Count - 1
                    ...
                    Dim sisa As Double = dss.Tables("MyTable").Rows(ind).Item("sisa")

                    Dim jumlah_dosing As Double = dss.Tables("MyTable").Rows(ind).Item("jumlah_dosing")
                    Dim jumlah_dosing_pcs As Double = dss.Tables("MyTable").Rows(ind).Item("jumlah_dosing_pcs")

                    Dim nilai_bahan_KG As Double = Math.Round(dss.Tables("MyTable").Rows(ind).Item("Total_bahan_baku") / jumlah_dosing, 2)
                    'Dim nilai_biaya_KG As Double = Math.Round(dss.Tables("MyTable").Rows(ind).Item("total_biaya_produksi") / jumlah_dosing, 2)
                    Dim nilai_loss_KG As Double = Math.Round(dss.Tables("MyTable").Rows(ind).Item("nilai_loss_production") / jumlah_dosing, 2)

                    Total_Dosing += Math.Round(sisa * nilai_bahan_KG, 0)
                    Total_loss += Math.Round(sisa * nilai_loss_KG, 0)
                    Total_nilai += Math.Round(sisa * nilai_bahan_KG, 0) + Math.Round(sisa * nilai_loss_KG, 0)
                Next
            End Using
...
#Region "Jurnal 1"
            ...
            SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_Loss_production, 1),
                     ...
                     KodePerusahaan, KodeProyek, "Biaya Loss Production " & Txt_NoSplitProduksi.Text, Total_Dosing, "0", pagenumber, Lokasi, Bahasa_Pilihan, Ket_Cost_Center_HO)
            ExecuteTrans(SQL)
            ...
            SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_persedian_brg_dlm_proses, 1),
                                  ...
                                  KodePerusahaan, KodeProyek, ket_persedian_brg_dlm_proses & Txt_NoSplitProduksi.Text, "0", Total_nilai, pagenumber, Lokasi, Bahasa_Pilihan, Ket_Cost_Center_HO)
            ExecuteTrans(SQL)
            pagenumber = pagenumber + 1

...
Public Class EMI_Transfer_Quality2

    Dim kategoriQuality As New ArrayList({"good_stock", "Warning_Stock", "Bad_Stock"})
    Dim kategoriQualityKet As New ArrayList({"Good Stock", "Warning Stock", "Bad Stock"})
    ...
    Private Sub kosong()
        ...
        Try
            ...
            Cmb_QualityFrom.Items.Clear() : Cmb_QualityTo.Items.Clear()

            For i As Integer = 0 To kategoriQualityKet.Count - 1
                Cmb_QualityFrom.Items.Add(kategoriQualityKet(i)) : Cmb_QualityTo.Items.Add(kategoriQualityKet(i))
            Next
    ...
    Private Sub get_no_faktur()
        ...
        TxtNo_Transaksi.Text = FAdj & arrInisialFaktur.Item(CmbSO_Asal.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy") & "-" &
                                      ...
                                      "And", "substring(no_faktur,1," & Len(FAdj) + Len(arrInisialFaktur.Item(CmbSO_Asal.SelectedIndex)) + 6 & ")", FAdj & arrInisialFaktur.Item(CmbSO_Asal.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy"))
    ...
    Private Sub TxtKd_Barang_TextChanged(sender As Object, e As EventArgs) Handles TxtKd_Barang.TextChanged, Txt_SO.TextChanged
        ...
        Try
            ...
            Dim jenis_stock As String = kategoriQuality.Item(Cmb_QualityFrom.SelectedIndex)
            SQL = "select a.kode_stock_owner, a.kode_barang, a.nama, isnull((dbo.ubah_satuan(a.kode_Perusahaan, 'masa', a.kode_barang, a.satuan, "
            SQL = SQL & "b.satuan, a." & jenis_stock & ")),0) as Good_Stock, a.Satuan, b.satuan as satuan_display, ISNULL(a.Jumlah_Bags, 0) as Jumlah_Bags, "
    ...
    Private Sub Btn_Insert_Click(sender As Object, e As EventArgs) Handles Btn_GetData.Click
        ...
        Try
            ...
            Dim jenis_stock As String = ""
            If Cmb_QualityFrom.SelectedIndex = 0 Then
                jenis_stock = "jumlah"
            Else
                jenis_stock = kategoriQuality.Item(Cmb_QualityFrom.SelectedIndex)
            End If
            ...
            SQL = SQL & "'" & TxtSatuan.Text & "', a." & jenis_stock & ") as jumlah, b.satuan, a.nomor_pallet, ISNULL(a.Jumlah_Bags, 0) as stock_bags, a.Batch_number, "
            ...
            SQL = SQL & "and a.Id_Warehouse=c.Id_WMS_Warehouse_Position and a." & jenis_stock & " <> 0 "
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        Try
            ...
            SQL = SQL & "" & nilai_kecil & ", '" & TxtSatuanKecil.Text & "')"
            ExecuteTrans(SQL)

            For row As Integer = 0 To DGV_Data_TF.RowCount - 1

                Dim jenis_stockAwal As String = ""
                Dim jenis_stockAwalSN As String = ""

                If Cmb_QualityFrom.SelectedIndex = 0 Then
                    jenis_stockAwalSN = "jumlah"
                Else
                    jenis_stockAwalSN = kategoriQuality.Item(Cmb_QualityFrom.SelectedIndex)
                End If

                jenis_stockAwal = kategoriQuality.Item(Cmb_QualityFrom.SelectedIndex)

                Dim jenis_stockAkhir As String = ""
                Dim jenis_stockAkhirSN As String = ""

                If Cmb_QualityTo.SelectedIndex = 0 Then
                    jenis_stockAkhirSN = "jumlah"
                Else
                    jenis_stockAkhirSN = kategoriQuality.Item(Cmb_QualityTo.SelectedIndex)
                End If

                jenis_stockAkhir = kategoriQuality.Item(Cmb_QualityTo.SelectedIndex)
                        ...
                        If .Rows.Count <> 0 Then
                            ...
                            For h As Integer = 0 To .Rows.Count - 1
                                ...
                                ElseIf sisa > .Rows(h).Item("jumlah") Then
                                    ...
                                    SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' and "
                                    SQL = SQL & "kode_stock_owner = '" & .Rows(h).Item("kode_stock_owner") & "' and kode_barang = '" & .Rows(h).Item("kode_barang") & "' and "
                                    SQL = SQL & "serial_number = '" & .Rows(h).Item("serial_number") & "'"
                                    ExecuteTrans(SQL)
                ...
                '=====================================================
                SQL = "insert into Emi_Transfer_Quality_Det (Kode_Perusahaan, No_Faktur, Serial_Number, Batch_Number, Id_Warehouse, id_Pallet, Jumlah_Stock, Jumlah_Bags) values "
                SQL = SQL & "('" & KodePerusahaan & "', '" & TxtNo_Transaksi.Text & "', '" & dgv_SerialNumber & "', "
                SQL = SQL & "'" & dgv_Batch & "', '" & dgv_IDWareHouse & "', '" & dgv_IDPallet & "', '" & dgv_Jumlah & "', '" & dgv_JmlhBags & "')"
                ExecuteTrans(SQL)

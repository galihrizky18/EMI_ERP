...
Public Class EMI_Timbang_Unloading
    ...
    Private Sub loadJenisMuatan()
        Try
            ...
            If jenisMasuk = "KELUAR" Then
                ...
                Txt_NoFaktur.Text = "TK1124-00001"
                SQL = "select timbang_masuk, id_jenis_muatan from EMI_Timbang_Unloading where "
            ...
            Dim id As Integer = 0
            SQL = "select distinct a.kode_supplier, D.nama, b.Kode_Barang, C.nama as Nama_Barang, a.Lokasi, a.No_SJ, a.No_Plat, "
            SQL = SQL & "a.Driver from "
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        Try
            ...
            If jenisMasuk = "MASUK" Then
                ...
                SQL = SQL & "Foto_Timbang_Masuk_2, id_jenis_muatan) values('" & KodePerusahaan & "', '" & Txt_NoFaktur.Text & "','" & TxtNo_Loading.Text & "', "
                SQL = SQL & "'" & HilangkanTanda(Txt_Timbang1.Text) & "', '" & Format(DTP_1.Value, "yyyy-MM-dd") & "', "
                SQL = SQL & "'" & Format(CDate(DTP_Bruto.Value), "HH:mm:ss") & "', '" & Nama_File_1 & "', '" & Nama_File_2 & "', '" & arrIdJenisMuatan.Item(CmbJenisMuatan.SelectedIndex) & "')"
                ExecuteTrans(SQL)
            ...
            ElseIf jenisMasuk = "KELUAR" Then
                ...
                SQL = SQL & "Jumlah_Bags = " & Format(Tot_Bags.Text, "N0") & " "
                ...
                For i As Integer = 0 To DgvPO.RowCount - 1
                    ...
                    SQL = SQL & "nilai_barang ='" & jumlah_masuk_Barang & "', Satuan_Barang ='" & Satuan_Barang & "' "
                ...
                SQL = "select Serial_Number, Flag_angkut, Selesai from EMI_Barang_Masuk_Perpallet a where "
                SQL = SQL & "Kode_Perusahaan='" & KodePerusahaan & "' and "
                SQL = SQL & "No_Pembelian_Loading='" & TxtNo_Loading.Text & "' and status is null "
                Using ds = BindingTrans(SQL)
                    If ds.Tables("MyTable").Rows.Count <> 0 Then
                        For index = 0 To ds.Tables("MyTable").Rows.Count - 1

                            If General_Class.CekNULL(ds.Tables("MyTable").Rows(index).Item("Flag_angkut")) = "" Then
                                ...
                                Exit Sub
                            ElseIf General_Class.CekNULL(ds.Tables("MyTable").Rows(index).Item("Selesai")) = "" Then
                            ...
                            If metodeTruckScale = "TRUCK SCALE" Then
                                ...
                                SQL = "Update Barang_SN_sementara set "

                                SQL = SQL & "jumlah = '" & jumlah_masuk_BarangTimbang & "' "

                                SQL = SQL & "Where Kode_Perusahaan = '" & KodePerusahaan & "' and "
                                SQL = SQL & "Serial_Number='" & ds.Tables("MyTable").Rows(index).Item("Serial_Number") & "' "
                                ExecuteTrans(SQL)

                            End If

                            SQL = "Select kode_perusahaan, kode_stock_owner, kode_barang, serial_number, jumlah, Tgl_Produksi, "
                            SQL = SQL & "Tgl_Expired, Id_Warehouse, id_Susunan, Nomor_Pallet, Kode_Unik_Asal, Kode_Unik_Berjalan, "
                            SQL = SQL & "Jumlah_Bags, Qr_Code, Batch_Number, warna from Barang_SN_sementara where "
                            SQL = SQL & "Kode_Perusahaan ='" & KodePerusahaan & "' and "
                            SQL = SQL & "Serial_Number='" & ds.Tables("MyTable").Rows(index).Item("Serial_Number") & "' "
                            SQL = SQL & "And validasi Is null "
                            Using ds2 = BindingTrans(SQL)
                                With ds2.Tables("MyTable")
                                    If .Rows.Count <> 0 Then
                                        For index2 = 0 To .Rows.Count - 1

                                            SQL = "Update barang Set "
                                            SQL = SQL & "good_stock = good_stock + " & .Rows(index2).Item("jumlah") & ", "
                                            SQL = SQL & "Jumlah_Bags = Jumlah_Bags +" & .Rows(index2).Item("Jumlah_Bags") & " where "
                                            SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' And "
                                            SQL = SQL & "kode_stock_owner = '" & .Rows(index2).Item("kode_stock_owner") & "' And "
                                            SQL = SQL & "kode_barang = '" & .Rows(index2).Item("kode_barang") & "' "
                                            ExecuteTrans(SQL)
                                            ...
                                            SQL = SQL & "Values( "
                                            SQL = SQL & "'" & KodePerusahaan & "','" & .Rows(index2).Item("kode_stock_owner") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("kode_barang") & "','" & .Rows(index2).Item("serial_number") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("jumlah") & "','" & .Rows(index2).Item("Tgl_Produksi") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("Tgl_Expired") & "','" & .Rows(index2).Item("Id_Warehouse") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("id_Susunan") & "','" & .Rows(index2).Item("Nomor_Pallet") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("Kode_Unik_Asal") & "','" & .Rows(index2).Item("Kode_Unik_Berjalan") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("Jumlah_Bags") & "','" & .Rows(index2).Item("Qr_Code") & "', "
                                            SQL = SQL & "'" & .Rows(index2).Item("Batch_Number") & "','" & .Rows(index2).Item("warna") & "') "
                                            ExecuteTrans(SQL)
                                            ...
                                            SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' And "
                                            SQL = SQL & "kode_stock_owner = '" & .Rows(index2).Item("kode_stock_owner") & "' And "
                                            SQL = SQL & "kode_barang = '" & .Rows(index2).Item("kode_barang") & "' and "
                                            SQL = SQL & "serial_number = '" & .Rows(index2).Item("serial_number") & "' "
                                            ExecuteTrans(SQL)
                                            ...
                                            SQL = SQL & "FROM barang a WHERE a.Kode_Stock_Owner = '" & .Rows(index2).Item("kode_stock_owner") & "' "
                                            SQL = SQL & "AND a.Kode_Barang = '" & .Rows(index2).Item("kode_barang") & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
    ...
    Private Sub getSumOfBerat()
        ...
        For i As Integer = 0 To DgvTimbang.RowCount - 1
            ...
            Dim nilai As Double = Val(HilangkanTanda(LvTimbangBeratBags)) + Val(HilangkanTanda(LvTimbangBeratBarang))

Public Class N_EMI_Laporan_Saldo_Akhir_Barang_Lain
    Dim CrDoc As Object
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Btn_Cetak.Click
        ...
        Try
            ...
            SQL = SQL & "from Get_Saldo_Barang_Lain "

            SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
            SF = "{Get_Saldo_Barang_Lain.Kode_Perusahaan} = '" & KodePerusahaan & "' "

            If CmbLokasi.SelectedIndex > 0 Then
                SQL = SQL & "and Kode_stock_owner = '" & CmbLokasi.Text & "' "
                SF &= "and {Get_Saldo_Barang_Lain.Kode_stock_owner} = '" & CmbLokasi.Text & "' "
            End If

            If Not Txt_IdGroupJenis.Text = "000" Then
                SQL = SQL & "and Id_Group_Jenis = " & Txt_IdGroupJenis.Text & " "
                SF &= "and {Get_Saldo_Barang_Lain.Id_Group_Jenis} = " & Txt_IdGroupJenis.Text & " "
            End If
            ...
            If Cmb_FlagInspection.SelectedIndex > 0 Then
                If Cmb_FlagInspection.SelectedIndex = 1 Then
                    ...
                    SF &= "and {Get_Saldo_Barang_Lain.Flag_FG} = 'T' or "
                    SF &= "({Get_Saldo_Barang_Lain.Flag_FG} = 'Y' and {Get_Saldo_Barang_Lain.Flag_QI} = 'Y') "
                ElseIf Cmb_FlagInspection.SelectedIndex = 2 Then
                    SQL = SQL & "and (Flag_FG = 'T' OR "
                    SQL = SQL & "(Flag_FG = 'Y' AND Flag_QI = 'T'))"
                    SF &= "and {Get_Saldo_Barang_Lain.Flag_FG} = 'T' or "
                    SF &= "({Get_Saldo_Barang_Lain.Flag_FG} = 'Y' and {Get_Saldo_Barang_Lain.Flag_QI} = 'T') "
                End If
            End If

            SQL = SQL & "and tahun = '" & Strings.Right("0" & CmbBulan.SelectedIndex + 1, 2) & CmbTahun.Text & "' "
            SF &= "and {Get_Saldo_Barang_Lain.Tahun} = '" & Strings.Right("0" & CmbBulan.SelectedIndex + 1, 2) & CmbTahun.Text & "' "

            Using Ds = Binding(SQL)
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    If CmbJenis.SelectedIndex = 0 Then
                        CrDoc = New N_EMI_CR_Laporan_Saldo_Akhir_Barang_Lain_Rekap
                    ElseIf CmbJenis.SelectedIndex = 1 Then
                        CrDoc = New N_EMI_CR_Laporan_Saldo_Akhir_Barang_Lain_Per_Barang
                    ElseIf CmbJenis.SelectedIndex = 2 Then
                        CrDoc = New N_EMI_CR_Laporan_Saldo_Akhir_Barang_Lain_Detail
                    Else
                        CloseConn()
    ...
    Private Sub Txt_NoSJ_TextChanged(sender As Object, e As EventArgs) Handles Txt_IdGroupJenis.TextChanged
        ...
        Try
            ...
            SQL = "select Id_Group_Jenis, Kode_Group_Jenis from EMI_Group_Jenis_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and Id_Group_Jenis like '%" & Txt_IdGroupJenis.Text & "%' "
    ...
    Private Sub Txt_NmGroupJenis_TextChanged(sender As Object, e As EventArgs) Handles Txt_NmGroupJenis.TextChanged
        ...
        Try
            ...
            SQL = "select Id_Group_Jenis, Kode_Group_Jenis from EMI_Group_Jenis_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Group_Jenis like '%" & Txt_NmGroupJenis.Text & "%' "
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
    ...
    Private Sub Txt_IdGroupJenis_Leave(sender As Object, e As EventArgs) Handles Txt_IdGroupJenis.Leave
        ...
        Try
            ...
            If Not Txt_IdGroupJenis.Text.ToUpper = "000" Then
                ...
                SQL = "select Id_Group_Jenis, Kode_Group_Jenis from EMI_Group_Jenis_Lain where Kode_Perusahaan = '" & KodePerusahaan & "' and Id_Group_Jenis = " & Txt_IdGroupJenis.Text & " "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then

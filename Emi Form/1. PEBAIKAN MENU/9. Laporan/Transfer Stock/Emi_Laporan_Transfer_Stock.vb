Public Class Emi_Laporan_Transfer_Stock

    Dim arrSO, arrParamLain, arrParamLainSF As New ArrayList
    Dim JudulForm As String = "Laporan Transfer Stock"

    Private Sub Emi_Laporan_Transfer_Stock_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Kosong()
        ...
        Try
            ...
            SQL = "select Kode_Group_Jenis  from emi_group_jenis where Kode_Perusahaan = '" & KodePerusahaan & "' order by Kode_Group_Jenis"
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
    ...
    Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
        ...
        Try
            ...
            SQL = "select Kode_Perusahaan from View_Laporan_Transfer_Stock "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and Tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

            SF = "{View_Laporan_Transfer_Stock.Kode_Perusahaan} = '" & KodePerusahaan & "' "
            SF = SF & "and {View_Laporan_Transfer_Stock.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
            SF = SF & "{View_Laporan_Transfer_Stock.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

            If Not Cmb_JenisTransfer.SelectedIndex = 0 Then
                SQL = SQL & "and Jenis_Transfer = '" & Cmb_JenisTransfer.SelectedItem & "' "
                SF = SF & "And {View_Laporan_Transfer_Stock.Jenis_Transfer} = '" & Cmb_JenisTransfer.SelectedItem & "' "
            End If

            If Not Cmb_SOAwal.SelectedIndex = 0 Then
                SQL = SQL & "and Lokasi_Awal = '" & Cmb_SOAwal.SelectedItem & "' "
                SF = SF & "And {View_Laporan_Transfer_Stock.Lokasi_Awal} = '" & Cmb_SOAwal.SelectedItem & "' "
            End If

            If Not Cmb_SOTujuan.SelectedIndex = 0 Then
                SQL = SQL & "and Lokasi_Tujuan = '" & Cmb_SOTujuan.SelectedItem & "' "
                SF = SF & "And {View_Laporan_Transfer_Stock.Lokasi_Tujuan} = '" & Cmb_SOTujuan.SelectedItem & "' "
            End If

            If Not Cmb_GroupJenis.SelectedIndex = 0 Then
                SQL = SQL & "and Kode_Group_Jenis = '" & Cmb_GroupJenis.SelectedItem & "' "
                SF = SF & "And {View_Laporan_Transfer_Stock.Kode_Group_Jenis} = '" & Cmb_GroupJenis.SelectedItem & "' "
            End If

            'If Not Txt_KdBarang.Text = "--- SELURUH ---" Then
            '    SQL = SQL & "and Kode_Barang = '" & Txt_KdBarang.Text & "' "
            '    SF = SF & "And {View_Laporan_Transfer_Stock.Kode_Barang} = '" & Txt_KdBarang.Text & "'"
            'End If
            ...
            Using DS = BindingTrans(SQL)
                With DS.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        Dim CrDoc As New Rpt_Laporan_Transfer_Stock

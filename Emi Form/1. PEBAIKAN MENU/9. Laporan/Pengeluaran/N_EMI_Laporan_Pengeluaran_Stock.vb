Public Class N_EMI_Laporan_Pengeluaran_Stock
    ...
    Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
        ...
        Try
            ...
            SQL = "select Kode_Perusahaan from N_EMI_Pengeluaran_Stock_View "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and Tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

            SF = "{N_EMI_Pengeluaran_Stock_View.Kode_Perusahaan} = '" & KodePerusahaan & "' "
            SF = SF & "and {N_EMI_Pengeluaran_Stock_View.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
            SF = SF & "{N_EMI_Pengeluaran_Stock_View.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

            If Cmb_Lokasi.SelectedIndex <> 0 Then
                SQL = SQL & "and Kode_Stock_Owner = '" & arrLokasi(Cmb_Lokasi.SelectedIndex) & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_View.Kode_Stock_Owner} = '" & arrLokasi(Cmb_Lokasi.SelectedIndex) & "'"
            End If

            If Not Txt_IdCostCenter.Text.ToUpper = "000" Then
                SQL = SQL & "and Id_Cost_Center = '" & Txt_IdCostCenter.Text & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_View.Id_Cost_Center} = '" & Txt_IdCostCenter.Text & "'"
            End If

            If Not Txt_KdBarang.Text.ToUpper = OpsiSeluruh.ToUpper Then
                SQL = SQL & "and Kode_Barang = '" & Txt_KdBarang.Text & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_View.Kode_Barang} = '" & Txt_KdBarang.Text & "'"
            End If

            If Cmb_ParamLain.SelectedIndex <> 0 Then
                SQL = SQL & "and " & arrLain(Cmb_ParamLain.SelectedIndex) & " like '%" & Txt_ParamLain.Text & "%' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_View." & arrLain(Cmb_ParamLain.SelectedIndex) & "} Like '*" & Txt_ParamLain.Text & "*' "
            End If
            ...
            Using DS = BindingTrans(SQL)
                With DS.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        Dim CrDoc As New N_EMI_CR_Laporan_Pengeluaran_Stock

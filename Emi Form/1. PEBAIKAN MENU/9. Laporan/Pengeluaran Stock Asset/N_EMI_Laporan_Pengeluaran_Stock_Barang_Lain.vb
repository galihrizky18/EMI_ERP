Public Class N_EMI_Laporan_Pengeluaran_Stock_Barang_Lain
    ...
    Private Sub kosong()
        ...
        Try
            ...
            Lv_CostCenter.Visible = False : Lv_Barang.Visible = False
    ...
    Private Sub Txt_IdCostCenter_TextChanged(sender As Object, e As EventArgs) Handles Txt_IdCostCenter.TextChanged
        ...
        If Txt_IdCostCenter.Text.Trim.Length = 0 Then
            ...
            Exit Sub
        Else
            Me.Size = New Size(645, 400)
            Lv_CostCenter.Visible = True
            Lv_CostCenter.Location = New Point(114, 148)
        End If
    ...
    Private Sub Txt_KdBarang_TextChanged(sender As Object, e As EventArgs) Handles Txt_KdBarang.TextChanged
        If Txt_KdBarang.Text.Trim.Length = 0 Then
            Me.Size = New Size(645, 310)
            Lv_Barang.Location = New Point(650, 175)
            Lv_Barang.Visible = False
            Txt_KdBarang.Text = ""
            Txt_NmBarang.Text = ""
            Exit Sub
        Else
            Me.Size = New Size(645, 430)
            Lv_Barang.Visible = True
            Lv_Barang.Location = New Point(114, 175)
        End If
        ...
        Try
            ...
            If Not Txt_IdCostCenter.Text = "000" Then
                ...
                Using Dr = Open(SQL)
                    ...
                    Me.Size = New Size(645, 310)
                    Lv_CostCenter.Location = New Point(645, 400)
                    Lv_CostCenter.Visible = False
                End Using
            Else
                Txt_KdBarang.Focus()
    ...
    Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
        ...
        Try
            ...
            SQL = "select Kode_Perusahaan from N_EMI_Pengeluaran_Stock_Lain_View "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and Tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

            SF = "{N_EMI_Pengeluaran_Stock_Lain_View.Kode_Perusahaan} = '" & KodePerusahaan & "' "
            SF = SF & "and {N_EMI_Pengeluaran_Stock_Lain_View.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
            SF = SF & "{N_EMI_Pengeluaran_Stock_Lain_View.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

            If Cmb_Lokasi.SelectedIndex <> 0 Then
                SQL = SQL & "and Kode_Stock_Owner = '" & arrLokasi(Cmb_Lokasi.SelectedIndex) & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_Lain_View.Kode_Stock_Owner} = '" & arrLokasi(Cmb_Lokasi.SelectedIndex) & "'"
            End If

            If Not Txt_IdCostCenter.Text.ToUpper = "000" Then
                SQL = SQL & "and Id_Cost_Center = '" & Txt_IdCostCenter.Text & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_Lain_View.Id_Cost_Center} = '" & Txt_IdCostCenter.Text & "'"
            End If

            If Not Txt_KdBarang.Text.ToUpper = OpsiSeluruh.ToUpper Then
                SQL = SQL & "and Kode_Barang = '" & Txt_KdBarang.Text & "' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_Lain_View.Kode_Barang} = '" & Txt_KdBarang.Text & "'"
            End If

            If Cmb_ParamLain.SelectedIndex <> 0 Then
                SQL = SQL & "and " & arrLain(Cmb_ParamLain.SelectedIndex) & " like '%" & Txt_ParamLain.Text & "%' "
                SF = SF & "And {N_EMI_Pengeluaran_Stock_Lain_View." & arrLain(Cmb_ParamLain.SelectedIndex) & "} Like '*" & Txt_ParamLain.Text & "*' "
            End If
            ...
            Using DS = BindingTrans(SQL)
                With DS.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        Dim CrDoc As New N_EMI_CR_Laporan_Pengeluaran_Stock_Barang_Lain

Public Class N_EMI_Laporan_Pembayaran_Di_Muka_Proyek
	...
	Private Sub Txt_Pengajuan_TextChanged(sender As Object, e As EventArgs) Handles Txt_Pengajuan.TextChanged
		...
		Try
			...
			SQL = "select Distinct No_Pengajuan from emi_transaksi_pembayaran_dimuka_Proyek where Kode_Perusahaan = '" & KodePerusahaan & "' and Status is null "
	...
	Private Sub Txt_No_PO_TextChanged(sender As Object, e As EventArgs) Handles Txt_No_PO.TextChanged
		...
		Try
			...
			SQL = "select no_faktur, tanggal, jam, no_nota from PO_Pembelian_Proyek "
	...
	Private Sub Txt_Pengajuan_Leave(sender As Object, e As EventArgs) Handles Txt_Pengajuan.Leave
		...
		Try
			...
			If Not Txt_Pengajuan.Text.ToUpper = OpsiSeluruh.ToUpper Then
				...
				SQL = "select Distinct top 1 No_Pengajuan from emi_transaksi_pembayaran_dimuka_Proyek where Kode_Perusahaan = '" & KodePerusahaan & "' and Status is null "
	...
	Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
		...
		Try
			...
			SQL = "select * from N_EMI_View_Laporan_Down_Payment_Proyek "
			SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
			SQL = SQL & "and Tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "' "

			SF = "{N_EMI_View_Laporan_Down_Payment_Proyek.Kode_Perusahaan} = '" & KodePerusahaan & "' "
			SF = SF & "and {N_EMI_View_Laporan_Down_Payment_Proyek.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
			SF = SF & "{N_EMI_View_Laporan_Down_Payment_Proyek.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

			If Not Txt_Pengajuan.Text.ToUpper = OpsiSeluruh.ToUpper Then
				SQL = SQL & "and No_Pengajuan = '" & Txt_Pengajuan.Text & "' "
				SF = SF & "And {N_EMI_View_Laporan_Down_Payment_Proyek.No_Pengajuan} = '" & Txt_Pengajuan.Text & "' "
			End If

			If Not Txt_No_PO.Text.ToUpper = OpsiSeluruh.ToUpper Then
				SQL = SQL & "and No_Fak_PO = '" & Txt_No_PO.Text & "' "
				SF = SF & "And {N_EMI_View_Laporan_Down_Payment_Proyek.No_Fak_PO} = '" & Txt_No_PO.Text & "' "
			End If

			If Not Txt_Kd_Supplier.Text.ToUpper = OpsiSeluruh.ToUpper Then
				SQL = SQL & "and kode_Supplier = '" & Txt_Kd_Supplier.Text & "' "
				SF = SF & "And {N_EMI_View_Laporan_Down_Payment_Proyek.kode_Supplier} = '" & Txt_Kd_Supplier.Text & "' "
			End If

			If Cmb_Status_DP.SelectedIndex > 0 Then
				If arr_Status_DP(Cmb_Status_DP.SelectedIndex) = "SISA" Then
					SQL = SQL & "and Sisa > 0 "
					SF = SF & "And {N_EMI_View_Laporan_Down_Payment_Proyek.Sisa} > 0 "
			...
			Using DS = BindingTrans(SQL)
				With DS.Tables("MyTable")
					If .Rows.Count <> 0 Then
						...
						Dim CrDoc As New N_EMI_CR_Laporan_Pembayaran_Di_Muka_Proyek
						...
						With A_Place_For_Printing2
							.Text = "Laporan Down Payment Proyek"

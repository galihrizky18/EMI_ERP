Public Class EMI_Transaksi_Work_Center_PerBulan
    ...
    Dim arrSelectedRouting As New ArrayList

    Dim ColDinamis As Integer = 2
    ...
    Dim item_DGVWork_IDWorkCenter As Integer = 0

    Dim item_DGVWork_Mesin As Integer = 1
    ...
    Dim CellKeterangan As Integer = 1
    ...
    Private Sub Kosong()
        ...
        Try
            ...
            DataGridView1.Columns.Add("Id_Work_center", "Id Work Center")
            DataGridView1.Columns(0).Width = 0
            DataGridView1.Columns(0).Visible = False

            DataGridView1.Columns.Add("Mesin", "Mesin")
            DataGridView1.Columns(1).Width = 200
            DataGridView1.Columns(1).Visible = True


            isTambahKolomKdJnsBiayaProduksi = False
    ...
    Private Sub Get_Data()
        ...
        Try
            ...
            SQL = "select a.Id_Work_Center, a.Keterangan From EMI_Master_Work_Center a "
                ...
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            SQL = "select Id_Jenis_Biaya,b.Kode_Jenis_Biaya_Produksi,Id_Work_Center,sum(Jumlah) as total From EMI_Actual_Biaya_Produksi a, Emi_Jenis_Biaya_Produksi b "
                            SQL = SQL & "where  a.Kode_Perusahaan = b.Kode_Perusahaan and a.Id_Jenis_Biaya = b.Id_Jenis_Biaya_Produksi "
                            SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' and MONTH(a.tanggal) = '" & arrBulan.Item(CmbBulan.SelectedIndex) & "' and YEAR(a.Tanggal) = '" & CmbTahun.Text & "' "
                            SQL = SQL & "and Id_Work_Center = '" & .Rows(i).Item("id_work_center") & "' "
                            SQL = SQL & "group by Id_Jenis_Biaya,b.Kode_Jenis_Biaya_Produksi,Id_Work_Center "
                            ...
                            Using DS1 = BindingTrans(SQL)
                                If DS1.Tables("MyTable").Rows.Count <> 0 Then
                                    For z As Integer = 0 To DS1.Tables("MyTable").Rows.Count - 1
                                        ...
                                        For j = ColNum To DataGridView1.Columns.Count - 1
                                            If DS1.Tables("MyTable").Rows(z).Item("Kode_Jenis_Biaya_Produksi") = DataGridView1.Columns(j).HeaderText Then
    ...
    Private Sub Kosong_DGVWorkCenter()
        ...
        '=================================================
        For i As Integer = DataGridView1.Columns.Count - 1 To ColDinamis Step -1
        ...
        Try
            ...
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    DataGridView1.Columns.Add(dr("kode_jenis_biaya_produksi"), dr("kode_jenis_biaya_produksi"))
                    DataGridView1.Columns(ColNum).Width = 130
                    DataGridView1.Columns(ColNum).ReadOnly = True
                    DataGridView1.Columns(ColNum).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                    ColNum += 1
                Loop
            End Using
            ...
            SQL = "select a.No_Faktur,b.Id_Work_Center, c.Keterangan as Mesin  "
            SQL = SQL & "from Emi_Transaksi_Work_Center a, Emi_Transaksi_Work_Center_Detail b, EMI_Master_Work_Center c, emi_master_routing d,  "
            SQL = SQL & "emi_master_routing_detail e where a.Kode_Perusahaan = b.Kode_Perusahaan and b.Kode_Perusahaan = c.Kode_Perusahaan  "
            SQL = SQL & "and b.Kode_Perusahaan = d.Kode_Perusahaan and d.Kode_Perusahaan = e.Kode_Perusahaan and a.No_Faktur = b.No_Faktur  "
            SQL = SQL & "and b.Id_Work_Center = c.Id_Work_Center and b.Id_Routing = d.Id_Routing and d.Id_Routing = e.Id_Routing and b.Id_Work_Center = e.Id_Work_Center "
            ...
            SQL = SQL & "group by  a.No_Faktur,b.Id_Work_Center , c.Keterangan  "
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            DataGridView1.Rows(row).Cells(item_DGVWork_IDWorkCenter).Value = .Rows(i).Item("Id_Work_Center")
                            DataGridView1.Rows(row).Cells(item_DGVWork_Mesin).Value = .Rows(i).Item("Mesin")
                            ...
                            SQL = SQL & "order by b.Id_Work_Center"
                            Using Ds1 = BindingTrans(SQL)
                                If Ds1.Tables("MyTable").Rows.Count <> 0 Then
                                    For j As Integer = 0 To Ds1.Tables("MyTable").Rows.Count - 1

                                        For k As Integer = ColDinamis To DataGridView1.Columns.Count - 1

                                            If Ds1.Tables("MyTable").Rows(j).Item("Jenis_Biaya") = DataGridView1.Columns(k).HeaderText Then
                                                DataGridView1.Rows(row).Cells(k).Value = Ds1.Tables("MyTable").Rows(j).Item("Total")

                                                If isDataRelease Then
                                                    DataGridView1.Rows(row).Cells(k).ReadOnly = True
                                                Else
                                                    DataGridView1.Rows(row).Cells(k).ReadOnly = False
                                                End If
                                                Exit For
                                            End If
                                        Next
                        ...
                        Using Ds2 = BindingTrans(SQL)
                            If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                                For j As Integer = 0 To Ds2.Tables("MyTable").Rows.Count - 1
                                    ...
                                    DataGridView1.Rows(row).Cells(item_DGVWork_IDWorkCenter).Value = Ds2.Tables("MyTable").Rows(j).Item("Id_Work_Center")

                                    DataGridView1.Rows(row).Cells(item_DGVWork_Mesin).Value = Ds2.Tables("MyTable").Rows(j).Item("Mesin")

                                    For k As Integer = ColDinamis To DataGridView1.Columns.Count - 1
                                        DataGridView1.Rows(row).Cells(k).Value = "0"
                                        If isDataRelease Then
                                            DataGridView1.Rows(row).Cells(k).ReadOnly = True
                                        Else
                                            DataGridView1.Rows(row).Cells(k).ReadOnly = False

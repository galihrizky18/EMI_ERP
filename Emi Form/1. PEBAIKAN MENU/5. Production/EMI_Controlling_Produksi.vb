Public Class EMI_Controlling_Produksi
	...
	Dim LvGR_BatchValidasi As String
	...
	Dim ItemGR_BatchValidasi As Integer = 13
	...
	Private Sub EMI_Controlling_Produksi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		Lv_GI.Columns.Clear()
		Lv_GI.Columns.Add("No Faktur", 145, HorizontalAlignment.Left) '0
		Lv_GI.Columns.Add("No PO", 0, HorizontalAlignment.Left) '1
		Lv_GI.Columns.Add("Keterangan", 220, HorizontalAlignment.Left) '2
		Lv_GI.Columns.Add("Tanggal Produksi", 120, HorizontalAlignment.Center) '3
		Lv_GI.Columns.Add("Jam Produksi", 100, HorizontalAlignment.Center) '4
		Lv_GI.Columns.Add("Kode Barang", 130, HorizontalAlignment.Left) '5
		Lv_GI.Columns.Add("Nama", 220, HorizontalAlignment.Left) '6
		Lv_GI.Columns.Add("Qty Produksi", 130, HorizontalAlignment.Right) '7
		Lv_GI.Columns.Add("Total Batch", 100, HorizontalAlignment.Center) '8
		Lv_GI.Columns.Add("Batch Berjalan", 100, HorizontalAlignment.Center) '9
		Lv_GI.Columns.Add("Routing", 140, HorizontalAlignment.Left) '10
		Lv_GI.Columns.Add("Total Dosing", 120, HorizontalAlignment.Right) '11
		Lv_GI.View = View.Details

		Lv_GI.Columns(8).DisplayIndex = 2
		Lv_GI.Columns(9).DisplayIndex = 3

		Lv_GR.Columns.Clear()
		Lv_GR.Columns.Add("No Faktur", 145, HorizontalAlignment.Left) '0
		Lv_GR.Columns.Add("No PO", 0, HorizontalAlignment.Left) '1
		Lv_GR.Columns.Add("Keterangan", 220, HorizontalAlignment.Left) '2
		Lv_GR.Columns.Add("Tanggal Produksi", 120, HorizontalAlignment.Center) '3
		Lv_GR.Columns.Add("Jam Produksi", 100, HorizontalAlignment.Center) '4
		Lv_GR.Columns.Add("Kode Barang", 130, HorizontalAlignment.Left) '5
		Lv_GR.Columns.Add("Nama", 220, HorizontalAlignment.Left) '6
		Lv_GR.Columns.Add("Qty Produksi", 130, HorizontalAlignment.Right) '7
		Lv_GR.Columns.Add("Total Batch", 100, HorizontalAlignment.Center) '8
		Lv_GR.Columns.Add("Batch Berjalan", 100, HorizontalAlignment.Center) '9
		Lv_GR.Columns.Add("Routing", 140, HorizontalAlignment.Left) '10
		Lv_GR.Columns.Add("Total GR", 120, HorizontalAlignment.Right) '11
		Lv_GR.Columns.Add("Status GI", 120, HorizontalAlignment.Center) '12
		Lv_GR.Columns.Add("Batch Validasi", 100, HorizontalAlignment.Center) '13
		Lv_GR.View = View.Details

		Lv_GR.Columns(12).DisplayIndex = 1
		Lv_GR.Columns(8).DisplayIndex = 2
		Lv_GR.Columns(13).DisplayIndex = 3
		Lv_GR.Columns(9).DisplayIndex = 4
		Lv_GR.Columns(2).DisplayIndex = 5
	...
	Private Sub GetDataLvGR(ByVal index As Integer)
		...
		LvGR_BatchValidasi = Lv_GR.Items(index).SubItems(ItemGR_BatchValidasi).Text
	...
	Private Sub LoadGR(Optional ByVal page As Integer = 1)
		...
		Try
			...
			If asal = "VALIDASI HPP" Then
				FilterSql = FilterSql & "and a.No_Transaksi = '" & NoSplit & "' "
			End If
			If Cmb_FilterGR.SelectedIndex <> -1 Then
				If Cmb_FilterGR.SelectedIndex = 3 Then
					If Cmb_Filter_Status_GI.SelectedIndex <> 0 Then
						FilterSql = FilterSql & "and a.flag_hasil_Produksi_GI " & ArrFilterCmbStatusGI(Cmb_Filter_Status_GI.SelectedIndex).Sql & " "
					End If
				Else
					FilterSql = FilterSql & "and " & arrCari_GR.Item(Cmb_FilterGR.SelectedIndex) & "  like  '%" & Trim(Txt_FilterGR.Text) & "%' "
			...
			Lv_GR.Items.Clear()
			SQL = $"
				select a.No_Transaksi, a.no_Po, c.Keterangan, a.Tgl_Produksi, a.Jam_Produksi, a.Kode_Barang, b.Nama, a.Jumlah, a.Satuan,
					isnull(a.Qty_Batch, 0) As Qty_PerBatch, d.Id_Routing, d.Keterangan as routing, a.flag_hasil_Produksi_GI,
					isnull(Produksi.Jumlah_Batch, 0) as Jumlah_Batch, isnull(produksi.Total_dosing, 0) as Total_dosing, ISNULL(Produksi.Jumlah_Validasi, 0) AS Jumlah_Validasi
				from Emi_Split_Production_Order a
					inner join barang b on a.Kode_Perusahaan = b.Kode_Perusahaan and a.Kode_Stock_Owner = b.Kode_Stock_Owner and a.Kode_Barang = b.Kode_Barang
 					inner join EMI_Order_Produksi c on a.Kode_Perusahaan = c.Kode_Perusahaan and a.No_PO = c.No_Faktur
					inner join emi_master_routing d on c.Kode_Perusahaan = d.Kode_Perusahaan and c.Id_Routing = d.Id_Routing
					outer apply (
						select COUNT(y.no_transaksi) AS Jumlah_Batch, SUM(y.Jumlah_Terpakai) AS Total_dosing,
							   SUM(CASE WHEN y.Flag_Validasi = 'Y' THEN 1 ELSE 0 END) AS Jumlah_Validasi
						from Emi_Production_Results x
						inner join Emi_Production_Results_HPP y on x.kode_perusahaan = y.kode_perusahaan and x.No_Transaksi = y.No_Transaksi
						where x.kode_perusahaan = a.kode_perusahaan
							and x.No_Production_Order = a.No_Transaksi
							and y.Tanggal IS NOT NULL
					) AS Produksi
				where a.Status is null and c.Status is null
				and a.Flag_Produksi = 'Y'
				and a.flag_hasil_Produksi_GR is null
				and (a.flag_hasil_Produksi_GI is null or a.flag_hasil_Produksi_GI = 'Y')
				and a.Kode_Perusahaan = '{KodePerusahaan}'
				{FilterSql}
				order by a.Tgl_Produksi, a.Jam_Produksi
				...
				With Ds.Tables("MyTable")
					If .Rows.Count <> 0 Then
						For i As Integer = 0 To .Rows.Count - 1
							...
							Dim TotBatch As Double = Val(HilangkanTanda(If(General_Class.CekNULL(.Rows(i).Item("Jumlah_Batch")) = "", 0, .Rows(i).Item("Jumlah_Batch"))))
							Dim TotBatchValidasi As Double = Val(HilangkanTanda(If(General_Class.CekNULL(.Rows(i).Item("Jumlah_Validasi")) = "", 0, .Rows(i).Item("Jumlah_Validasi"))))
							Lv.SubItems.Add($"{TotBatchValidasi} / {TotBatch}")

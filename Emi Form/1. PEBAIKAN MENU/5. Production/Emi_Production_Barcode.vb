...
Public Class Emi_Production_Barcode
	...
	Dim LockButton As Boolean = False
	...
	Private Sub Cmb_Batch_Srap_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Cmb_Batch_Srap.SelectedIndexChanged
		If Cmb_Batch_Srap.Items.Count = 0 Then Exit Sub

		If Cmb_Batch_Srap.SelectedIndex >= 0 Then
			CmbSisaProduksi.Enabled = True
		Else
			CmbSisaProduksi.Enabled = False
		End If
		CmbSisaProduksi.SelectedIndex = -1
	...
	Private Sub kosong()
		...
		Cmb_Batch_Srap.SelectedIndex = -1
		...
		Try
			...
			'=     LOAD BATCH     =
			'======================
			Cmb_Tahapan.Items.Clear() : Cmb_Batch_Srap.Items.Clear()
			...
			SQL = $"
				...
				FROM Emi_Split_Production_Order_Detail_Packaging_Packing_Set
				WHERE No_Faktur = '{Txt_NoSplit.Text.Trim}'
				ORDER BY Deskripsi
			"
	...
	Private Sub get_packaging()
		...
		Try
			...
			SQL = SQL & $"and b.deskripsi = '{CMB_PackingSet.Text.Trim}'"
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
		If Txt_NoSplit.Text.Trim.Length = 0 Then Exit Sub
		If LockButton Then Exit Sub

		If TxtJmlScrap.Text.Trim.Length <> 0 And Val(TxtJmlScrap.Text) <> 0 Then
			If Cmb_Batch_Srap.SelectedIndex = -1 Then
				MessageBox.Show("Batch scrap harus di isi!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Cmb_Batch_Srap.Focus()
				Exit Sub
			ElseIf CmbSisaProduksi.SelectedIndex = -1 Then
				MessageBox.Show("Sisa Produksi harus di isi!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				CmbSisaProduksi.Focus()
				Exit Sub
			End If
	...
	Private Sub Simpan_Data(ByVal isNormalSave As Boolean)
		...
		Try
			...
			If Val(HilangkanTanda(Txt_Jumlah.Text)) <> 0 Or Val(TxtJmlScrap.Text) <> 0 Then

				Dim SelectedBatch As String = ""
				If Val(Txt_Jumlah.Text) <> 0 Then
					SelectedBatch = Cmb_Tahapan.Text.Trim
				ElseIf Val(TxtJmlScrap.Text) <> 0 Then
					SelectedBatch = Cmb_Batch_Srap.Text.Trim
				End If

				'=============================================
				'=     CEK APAKAH BATCH SUDAH DIVALIDASI     =
				'=============================================
				Dim HasValidation As Boolean = False
				Dim NoTransaksiResult As String = ""
				SQL = $"
					...
					and a.No_Production_Order = '{Txt_NoSplit.Text.Trim}'
					and b.Proses = '{SelectedBatch}'
				"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
						If General_Class.CekNULL(Dr("Flag_Validasi")) = "Y" Then
							HasValidation = True
							NoTransaksiResult = Dr("No_Transaksi")
						Else
							HasValidation = False
						End If
					Else
				...
				If HasValidation Then
					MessageBox.Show($"Batch {SelectedBatch} Sudah Divalidasi {vbCrLf}Tidak Bisa Melakukan GR 1 Terhadap Batch {SelectedBatch}", Judul, MessageBoxButtons.OK, MessageBoxIcon.Warning)
					CloseTrans()
					CloseConn()
					Exit Sub

#Region "KODE JIKA RESET STATUS VALIDASI"

					'If (MessageBox.Show($"Batch {SelectedBatch} Sudah Divalidasi {vbCrLf}Yakin ingin melanjutkan transaksi?", Judul, MessageBoxButtons.YesNo, MessageBoxIcon.Question)) = vbYes Then
					...
					'		VALUES
					'			('{KodePerusahaan}', '{NoTransaksiResult}', '{Txt_NoSplit.Text.Trim}', '{SelectedBatch}', 'INVALIDASI',
					'			'{UserID}', '{Format(tgl_skg, "yyyy-MM-dd")}', '{Format(tgl_skg, "HH:mm:ss")}')
					'	"
					'	ExecuteTrans(SQL)

					'	SQL = $"
					'		update Emi_Production_Results_HPP set Flag_Validasi = NULL,
					'		UserID_Validasi = NULL, Tanggal_Validasi = NULL, Jam_Validasi = NULL
					'		where Kode_Perusahaan = '{KodePerusahaan}'
					'		and No_Transaksi = '{NoTransaksiResult}'
					'		and Proses = '{SelectedBatch}'
					'	"
					'	ExecuteTrans(SQL)
					'Else
					'	CloseTrans()
...
#Region "Generate Barcode FG"
			...
			If Val(HilangkanTanda(Txt_Jumlah.Text)) <> 0 Then
				...
				SQL = "delete N_EMI_Barcode_Label_Barcode_GR_1 where Kode_Perusahaan = '" & KodePerusahaan & "' and UserID = '" & UserID & "' "
				ExecuteTrans(SQL)
				...
				'=================================
				SQL = $"
					select Kode_Perusahaan from N_EMI_Barcode_Label_Barcode_GR_1 where Kode_Perusahaan = '{KodePerusahaan}' and Kode_Unik_Print = '{kode_unik_print}'
				"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
						Dr.Close()
						CloseTrans()
						CloseConn()
						MessageBox.Show("Harap Ulangi Transaksi, Kode Unik Barcode FG Sudah Ada", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
						Exit Sub
					End If
				End Using
				...
				SQL = SQL & "Proses, Tahap, Jumlah, Satuan, Troli, Nomor, id_routing, routing, Kode_unik_print, UserID)  "
				...
				SQL = SQL & "'" & Txt_Troli.Text & "', '" & Total_GR1_Cetak & "', '" & ID_Routing & "', '" & Routing & "', '" & kode_unik_print & "', '" & UserID & "') "
				ExecuteTrans(SQL)
			...
			If Val(TxtJmlScrap.Text) <> 0 Then
				...
				Using dss = BindingTrans(SQL)
					For ind = 0 To dss.Tables("MyTable").Rows.Count - 1
						...
						SQL = SQL & "Satuan_Barang, Batch_Number, Id_Warehouse, Nomor_Pallet, proses, serial_number, Urut_HPP, Lokasi_Gudang_Sisa, Nomor, Batch) values "
						...
						SQL = SQL & "'" & arrSoGudangSisa(Cmb_Lokasi_Gudang_Sisa.SelectedIndex) & "', '" & TotalCountScrap & "', " & Val(HilangkanTanda(Cmb_Batch_Srap.Text.Trim)) & ") "
						ExecuteTrans(SQL)
				...
				SQL = "delete N_EMI_Barcode_Label_Barcode_GR_1_Scrap where Kode_Perusahaan = '" & KodePerusahaan & "' and UserID = '" & UserID & "' "
				ExecuteTrans(SQL)
				...
				'=================================
				SQL = $"
					select Kode_Perusahaan from N_EMI_Barcode_Label_Barcode_GR_1_Scrap where Kode_Perusahaan = '{KodePerusahaan}' and Kode_Unik_Print = '{kode_unik_print_scrap}'
				"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
						Dr.Close()
						CloseTrans()
						CloseConn()
						MessageBox.Show("Harap Ulangi Transaksi, Kode Unik Barcode Scrap Sudah Ada", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
						Exit Sub
					End If
				End Using
				...
				SQL = SQL & "Proses, Jumlah, Satuan, Nomor, id_routing, routing, Kode_unik_print, UserID)  "
				...
				SQL = SQL & "'" & TotalCountScrap & "', '" & ID_Routing & "', '" & Routing & "', '" & kode_unik_print_scrap & "', '" & UserID & "') "
				ExecuteTrans(SQL)
...
#End Region
		...
		If Val(HilangkanTanda(Txt_Jumlah.Text)) <> 0 Then
			...
			Try
				...
				SQL = "select * from N_EMI_Barcode_Label_Barcode_GR_1 where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_Barang='" & Txt_KdBarang.Text & "' and Kode_Unik_Print = '" & kode_unik_print & "' and UserID = '" & UserID & "' "
				Using Ds = BindingTrans(SQL)
					If Ds.Tables("MyTable").Rows.Count <> 0 Then
						...
						If printerDitemukan Then
							...
							CrDoc.RecordSelectionFormula = "{N_EMI_Barcode_Label_Barcode_GR_1.Kode_Perusahaan} = '" & KodePerusahaan & "' and {N_EMI_Barcode_Label_Barcode_GR_1.Kode_Barang} = '" & Txt_KdBarang.Text & "' and {N_EMI_Barcode_Label_Barcode_GR_1.Kode_Unik_Print} = '" & kode_unik_print & "' and {N_EMI_Barcode_Label_Barcode_GR_1.UserID} = '" & UserID & "' "
		...
		If Val(TxtJmlScrap.Text) <> 0 Then
			...
			Try
				...
				SQL = "select * from N_EMI_Barcode_Label_Barcode_GR_1_Scrap where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_Barang='" & arrKdBarangSrap(CmbSisaProduksi.SelectedIndex) & "' and Kode_Unik_Print = '" & kode_unik_print_scrap & "' and UserID = '" & UserID & "' "
				Using Ds = BindingTrans(SQL)
					If Ds.Tables("MyTable").Rows.Count <> 0 Then
						...
						If printerDitemukan Then
							...
							CrDoc.SetDataSource(Ds)
							CrDoc.SetDatabaseLogon(CUserId, CPassword, CServer, CDatabase)
							CrDoc.RecordSelectionFormula = "{N_EMI_Barcode_Label_Barcode_GR_1_Scrap.Kode_Perusahaan} = '" & KodePerusahaan & "' and {N_EMI_Barcode_Label_Barcode_GR_1_Scrap.Kode_Barang} = '" & arrKdBarangSrap(CmbSisaProduksi.SelectedIndex) & "' and {N_EMI_Barcode_Label_Barcode_GR_1_Scrap.Kode_Unik_Print} = '" & kode_unik_print_scrap & "' and {N_EMI_Barcode_Label_Barcode_GR_1_Scrap.UserID} = '" & UserID & "'   "
							CrDoc.PrintOptions.PrinterName = PrinterBarcode

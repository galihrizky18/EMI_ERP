...
#Region "=== DATA STRUCTURES ==="
	...
	Public Class ColumnFormat
		...
		Public Property Locked As Boolean = True
	...
	Public Class ExportConfig
		...
		Public Property ProtectSheet As Boolean = False
		Public Property ProtectPassword As String = ""
...
#Region "=== CORE ENGINE ==="
	...
	Private Shared Sub PerformExport(xlApp As Excel.Application,
		...
		Try
			...
			' AutoFit harus dieksekusi SEBELUM kolom disembunyikan agar setting hidden tidak tertimpa
			...
			' =========================================================
			' EKSEKUSI HIDE & UNLOCK KOLOM SECARA MASIF
			' =========================================================
			If config.ColumnFormats IsNot Nothing Then
				For Each cf In config.ColumnFormats
					Try
						' Gunakan ColumnIndex + 1 karena kolom Excel dimulai dari index 1
						Dim targetExcelColIndex As Integer = cf.ColumnIndex + 1
						Dim colRange As Excel.Range = CType(xlWorkSheet.Columns(targetExcelColIndex), Excel.Range)

						If cf.Hidden Then
							colRange.Hidden = True
						End If

						If Not cf.Locked Then
							colRange.Locked = False
						End If
					Catch ex As Exception
						' Abaikan error format minor
					End Try
				Next
			End If

			' =========================================================
			' EKSEKUSI PROTEKSI SHEET EXCEL
			' =========================================================
			If config.ProtectSheet Then
				xlWorkSheet.Protect(If(String.IsNullOrEmpty(config.ProtectPassword), Type.Missing, config.ProtectPassword),
	...
	Private Shared Sub WriteDataRows(ws As Excel.Worksheet,
		...
		For c As Integer = 0 To colCount - 1
			...
			If colFormatMap.ContainsKey(c) Then
				...
				' Eksekusi Hide Kolom dan Unlock Cell Input
				If colFormatMap(c).Hidden Then
					colRange.EntireColumn.Hidden = True
				End If
				colRange.Locked = colFormatMap(c).Locked
			Else
				' Default Excel: Kunci semua cell yang tidak dikonfigurasi

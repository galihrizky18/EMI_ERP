Public Class Laporan_HPP
    ...
    Private Sub Laporan_Purchase_Requisition_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LvSupp.Location = New Point(104, 164)
        LvBarang.Location = New Point(104, 192)
        Me.Size = New Size(625, 292)

        Tgl1.Value = Now.Date : Tgl2.Value = Now.Date
        TxtKdSupp.Text = "" : TxtNamaSupp.Text = ""
        TxtKdBrg.Text = "" : TxtNmBrg.Text = ""

        Cmb_Jenis.Items.Clear() : Cmb_Jenis.Text = ""
    ...
    Private Sub TxtKdSupp_TextChanged(sender As Object, e As EventArgs) Handles TxtKdSupp.TextChanged
        If TxtKdSupp.Text.Trim.Length = 0 Then
            LvSupp.Visible = False
            Me.Size = New Size(625, 292) : Exit Sub
        Else
            LvSupp.Visible = True
            Me.Size = New Size(625, 360)
        End If
...
#Region "Kode Lama"
        ...
        'SF = "{hpp_import.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
        'SF = SF & "{hpp_import.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "#"

        'If Not TxtKdSupp.Text.Trim.ToUpper = "SELURUH" Then
        '    SQL = SQL & "And a.kode_supplier = '" & TxtKdSupp.Text & "'"
        '    SF = SF & "And {hpp_import.kode_supplier} = '" & TxtKdSupp.Text & "'"
        'End If

        'If Not TxtKdBrg.Text.Trim.ToUpper = "SELURUH" Then
        '    SQL = SQL & "And b.kode_barang = '" & TxtKdBrg.Text & "'"
        '    SF = SF & "And {detail_hpp_import.kode_barang} = '" & TxtKdBrg.Text & "'"
        'End If

#End Region


        If Cmb_Jenis.Text = "LOKAL" Then
            SQL = "select * from vw_Laporan_HPP_Local "
        ...
        ElseIf Cmb_Jenis.Text = "IMPORT" Then

            SQL = "select * from vw_Laporan_HPP_Import "
            SQL = SQL & "where tanggal between '" & Format(Tgl1.Value, "yyyy-MM-dd") & "' and '" & Format(Tgl2.Value, "yyyy-MM-dd") & "'"

            SF = "{vw_Laporan_HPP_Import.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
            SF = SF & "{vw_Laporan_HPP_Import.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

            If Not TxtKdSupp.Text.Trim.ToUpper = "SELURUH" Then
                SQL = SQL & "And kode_supplier = '" & TxtKdSupp.Text & "'"
                SF = SF & "And {vw_Laporan_HPP_Import.kode_supplier} = '" & TxtKdSupp.Text & "' "
            End If

            If Not TxtKdBrg.Text.Trim.ToUpper = "SELURUH" Then
                SQL = SQL & "And kode_barang = '" & TxtKdBrg.Text & "'"
                SF = SF & "And {vw_Laporan_HPP_Import.kode_barang} = '" & TxtKdBrg.Text & "' "
            End If
        ...
        Using MyDS As DataSet = Binding(SQL)
            With MyDS.Tables(0)
                If .Rows.Count <> 0 Then
                    ...
                    If Cmb_Jenis.Text = "LOKAL" Then
                        CrDoc = New Laporan_HPP_Local_Rpt
                    ElseIf Cmb_Jenis.Text = "IMPORT" Then
                        CrDoc = New Laporan_HPP_Import_Rpt
                    End If

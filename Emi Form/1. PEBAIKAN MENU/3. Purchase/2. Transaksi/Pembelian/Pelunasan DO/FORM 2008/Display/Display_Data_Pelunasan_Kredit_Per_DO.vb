...
Public Class Display_Data_Pelunasan_Kredit_Per_DO
        ...
        Try
            ...
            Using Ds = BindingTrans(SQL)
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    Dim CrDoc As New Faktur_Pelunasan_DO    'Nama file CR
                    CrDoc.SetDataSource(Ds)
                    CrDoc.SetDatabaseLogon(CUserId, CPassword, CServer, CDatabase)
                    CrDoc.PrintOptions.PrinterName = PrinterName
                    CrDoc.RecordSelectionFormula = "{val_do.Kode_Perusahaan} = '" & KodePerusahaan & "' and {val_do.no_val} = '" & ListView1.FocusedItem.Text & "'"
    ...
    Private Sub Display_Data_Pembelian_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ListView2.Columns.Add("Nama Customer", 300, HorizontalAlignment.Left)
        ListView2.Columns.Add("Jumlah", 120, HorizontalAlignment.Right)
        ...
        ComboBox2.Items.Add("No Faktur") : Arr2.Add("d.no_faktur")
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            SQL = SQL & "from penjualan a, customers b, val_do c, detail_val_do d, do_new e where "
    ...
    Private Sub BatalkanTransaksiToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BatalkanTransaksiToolStripMenuItem.Click
        ...
        If tanya = vbYes Then
            ...
            Try
                ...
                If CekButtonRole("batal_pelunasan_do_kredit") = "T" Then
                    CloseTrans()
                    CloseConn()
                ...
                SQL = "select status, tanggal, kode_voucher from val_do where "
                SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' and "
                SQL = SQL & "no_val = '" & ListView1.FocusedItem.Text & "'"
                Using Ds = BindingTrans(SQL)
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            ...
                            If General_Class.CekNULL(.Rows(0).Item("status")) = "Y" Then
                                ...
                                Exit Sub
                            ElseIf Format(.Rows(0).Item("tanggal"), "yyyyMM") <> Format(CDate(fmenu.ToolStripStatusLabel3.Text), "yyyyMM") Then
                ...
                SQL = "select a.no_faktur, c.kode_customer, a.no_do, b.no_val, b.byr from "
                SQL = SQL & "do_new a, detail_val_do b, penjualan c where "
                SQL = SQL & "a.kode_perusahaan = b.kode_perusahaan And b.kode_perusahaan = c.kode_perusahaan and "
                    ...
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For i As Integer = 0 To .Rows.Count - 1
                                ...
                                SQL = SQL & "sudah_dilunasi = sudah_dilunasi - " & (.Rows(i).Item("byr")) & " where "
                                SQL = SQL & "no_do = '" & .Rows(i).Item("no_do") & "'  "
                                ExecuteTrans(SQL)

                                SQL = "Update penjualan set flag_lunas = NULL, "
                            ...
                            For index As Integer = 0 To .Rows.Count - 1
                                ...
                                If .Rows(index).Item("hari") >= Persen_Penurunan_Nol Then
                                    ...
                                    For index2 As Integer = 0 To Plafond_Kode.Count - 1
                                        ...
                                        If .Rows(index).Item("hari") >= Plafond_Dari.Item(index2) And _
                                        .Rows(index).Item("hari") <= Plafond_Sampai.Item(index2) And _
                                        cek_member <> Plafond_Kode.Item(index2) Then
                                            ...
                                            Using Ds2 = BindingTrans(SQL)
                                                For index3 As Integer = 0 To Ds2.Tables("MyTable").Rows.Count - 1
                                                    ...
                                                    If plafon >= Ds2.Tables("MyTable").Rows(index3).Item("Plafond_Dari") And _
                                                    plafon <= Ds2.Tables("MyTable").Rows(index3).Item("Plafond_Sampai") And _
                                                    .Rows(index).Item("hari") >= Ds2.Tables("MyTable").Rows(index3).Item("Hari_Dari") And _
                ...
                SQL = "select kode_unik from val_do where kode_perusahaan = '" & KodePerusahaan & "' and no_val = '" & ListView1.FocusedItem.Text & "' "
                Using Ds = BindingTrans(SQL)
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                ...
                SQL = "update val_do set status = 'Y' where kode_perusahaan = '" & KodePerusahaan & "' and "
                SQL = SQL & "no_val = '" & ListView1.FocusedItem.Text & "'"
                ExecuteTrans(SQL)
                ...
                SQL = "insert into log_val_do(kode_perusahaan, no_faktur, tanggal, jam, userid, jenis) values("
    ...
    Private Sub ListView1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListView1.SelectedIndexChanged
        ...
        Try
            ...
            If pertama = 1 Then
                ...
                SQL = "select e.no_do, e.no_faktur, a.tanggal, a.tgl_jatuh_tempo, a.kode_customer, b.nama, d.byr "
                SQL = SQL & "from penjualan a, customers b, val_do c, detail_val_do d, do_new e where "
                SQL = SQL & "a.kode_perusahaan = b.kode_perusahaan and b.kode_perusahaan = c.kode_perusahaan and "
                SQL = SQL & "c.kode_perusahaan = d.kode_perusahaan and d.kode_perusahaan = e.kode_perusahaan and a.kode_customer = b.kode_customer and "

...
Public Class EMI_PO_Pembelian_Sub_Barang_Lain
    ...
    Dim cellPO_JumlahPO As Integer = 4
    Dim cellPO_Sisa As Integer = 5
    Dim cellPO_Jumlah As Integer = 6
    Dim cellPO_Satuan As Integer = 7
    Dim cellPO_Harga_SB As Integer = 8
    Dim cellPO_Jumlah_SB As Integer = 9
    Dim cellPO_Satuan_SB As Integer = 10
    Dim cellPO_NoPenawaran As Integer = 11
    Dim cellPO_ID As Integer = 12
    Dim cellPO_Total As Integer = 13
    Dim cellPO_Urut As Integer = 14
    Dim cellPO_PR As Integer = 15
    Dim cellTempoPembayaran As Integer = 16
    Dim cellJatuhTempo As Integer = 17
    Dim cellFakPenawaran As Integer = 18
    ...
    Private Sub get_no_faktur()
        TxtPO_NoFaktur.Text = fPO_EMI & arrInisialFaktur.Item(CmbPO_Lokasi.SelectedIndex) & "-" & Format(DtpPO_Tgl.Value, "MM/yy") & "-" &
                                     General_Class.Get_Last_Number2("emi_pembelian_PO_Barang_Lain", "no_faktur", Jumlah_Digit,
    ...
    Public Sub Ambil_Data_Induk()

        'If Txt_Faktur_Induk.Text.Trim.Length = 0 Then
        '    MessageBox.Show("NO Faktur Induk tidak Ditermukan", "Transaksi Purchase Order Existing", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        '    Exit Sub
        'End If
        ...
        Try
            ...
            SQL = SQL & "sum(b.Jumlah) as Berat, a.Ekspedisi, a.Biaya, a.Total_MUA, a.Grand_Sebelum_PPN, a.PPN, (a.Grand_Sebelum_PPN * (a.ppn / 100)) as Total_PPN, a.Grand "
            SQL = SQL & "from EMI_Pembelian_PO_Induk_Barang_Lain a, EMI_Pembelian_PO_Detail_Induk_Barang_Lain b, Suppliers c "
            ...
            SQL = SQL & "and a.No_Faktur = '" & Txt_Faktur_Induk.Text & "' "
            ...
            SQL = SQL & "a.Ekspedisi, a.Biaya, a.Total_MUA, a.Grand_Sebelum_PPN, a.PPN, a.Grand "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then

                    Txt_Faktur_Induk.Text = Dr("No_Faktur")
                    DtpPO_Tgl.Value = Dr("Tanggal_Release")
                    DtpPO_ETD.Value = Dr("ETD_Simulasi")
                    TxtPO_NoNota.Text = Dr("Keterangan")
                    TxtPO_KdSupplier.Text = Dr("Kode_Supplier")
                    TxtPO_NmSupplier.Text = Dr("Nama_Supplier")
                    CmbPO_MataUang.Text = Dr("Mata_Uang")
                    CmbPO_JnsBayar.SelectedIndex = arrPembayaran.IndexOf(Dr("Jenis_Pembayaran"))
                    cmbJenisPengiriman.SelectedItem = Dr("Tempo_Pembayaran")
                    txtJatuhTempo.Text = Dr("Lama_Pembayaran")

                    'TxtPO_Total.Text = Format(Dr("Total_MUA"), "N2")
                    'TxtPO_TotalSblmPPN.Text = Format(Dr("Grand_Sebelum_PPN"), "N2")

                    'If General_Class.CekNULL(Dr("PPN")) = "" Then
                    '    ChkPO_PPN.Checked = False
                    '    TxtPO_PersenPPN.Text = ""
                    '    TxtPO_NilaiPPN.Text = ""
                    'Else
                    '    ChkPO_PPN.Checked = True
                    '    TxtPO_PersenPPN.Text = Dr("PPN")
                    '    TxtPO_NilaiPPN.Text = Format(Dr("Total_PPN"), "N2")
                    'End If
                    'TxtPO_Berat.Text = Format(Dr("Berat"), "N2")
                    'TxtPO_GrandTotal.Text = Dr("Grand")
                Else
                    Dr.Close()
                    CloseConn()
        ...
        Try
            ...
            SQL = "select a.No_Faktur, b.Kode_Stock_Owner, b.Kode_Barang, c.Nama, b.Harga as Harga_Satuan_Besar, b.Satuan, b.Harga_Barang, b.Satuan_Barang, b.No_Penawaran, d.No_Urut_PR, a.Tempo_Pembayaran, a.Lama_Pembayaran, b.Jumlah, "
            SQL = SQL & "ISNULL(( select (b.Jumlah - sum(x.Jumlah)) from emi_pembelian_PO_Barang_Lain z, emi_pembelian_po_detail_Barang_Lain x where z.Kode_Perusahaan = a.Kode_Perusahaan and z.Kode_Perusahaan = x.Kode_Perusahaan "
            SQL = SQL & "and z.No_Faktur_Induk = a.No_Faktur "
            SQL = SQL & "and z.No_Faktur = x.No_Faktur "
            SQL = SQL & "and x.Kode_Stock_Owner = b.Kode_Stock_Owner and x.Kode_Barang = b.Kode_Barang "
            SQL = SQL & "), b.Jumlah) as Sisa "
            SQL = SQL & "from EMI_Pembelian_PO_Induk_Barang_Lain a, EMI_Pembelian_PO_Detail_Induk_Barang_Lain b, Barang_Lain c, EMI_Pembelian_PO_Det_Induk_Barang_Lain d "
            ...
            SQL = SQL & "and b.No_Faktur = d.No_Faktur and b.Kode_Stock_Owner = d.Kode_Stock_Owner and b.Kode_Barang = d.Kode_Barang "
            SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and a.Status is null "
            SQL = SQL & "and a.No_Faktur = '" & Txt_Faktur_Induk.Text & "' "
            ...
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    ...
                    LvPO_DataPO.Rows.Add(1)

                    LvPO_DataPO.Rows(index).Cells(cellPO_Lokasi).Value = Dr("Kode_Stock_Owner")
                    LvPO_DataPO.Rows(index).Cells(cellPO_KdBarang).Value = Dr("Kode_Barang")
                    LvPO_DataPO.Rows(index).Cells(cellPO_NmBarang).Value = Dr("Nama")
                    LvPO_DataPO.Rows(index).Cells(cellPO_Harga).Value = Format(Dr("Harga_Satuan_Besar"), "N2")

                    LvPO_DataPO.Rows(index).Cells(cellPO_JumlahPO).Value = Format(Dr("Jumlah"), "N2")
                    LvPO_DataPO.Rows(index).Cells(cellPO_Sisa).Value = Format(Dr("Sisa"), "N2")

                    LvPO_DataPO.Rows(index).Cells(cellPO_Jumlah).Value = Format(0, "N2") 'user isi

                    LvPO_DataPO.Rows(index).Cells(cellPO_Satuan).Value = Dr("Satuan")
                    LvPO_DataPO.Rows(index).Cells(cellPO_Harga_SB).Value = Dr("Harga_Barang")

                    LvPO_DataPO.Rows(index).Cells(cellPO_Jumlah_SB).Value = 0 ' generate berdasaerkan jumlah

                    LvPO_DataPO.Rows(index).Cells(cellPO_Satuan_SB).Value = Dr("Satuan_Barang")
                    LvPO_DataPO.Rows(index).Cells(cellPO_NoPenawaran).Value = Dr("No_Penawaran")
                    LvPO_DataPO.Rows(index).Cells(cellPO_ID).Value = "T"

                    LvPO_DataPO.Rows(index).Cells(cellPO_Total).Value = Format(0, "N2") 'TOtal auto generate jumlah * harga
                    LvPO_DataPO.Rows(index).Cells(cellPO_Urut).Value = ""
                    LvPO_DataPO.Rows(index).Cells(cellPO_PR).Value = Dr("No_Urut_PR")
                    LvPO_DataPO.Rows(index).Cells(cellTempoPembayaran).Value = Dr("Tempo_Pembayaran")
                    LvPO_DataPO.Rows(index).Cells(cellJatuhTempo).Value = Dr("Lama_Pembayaran")
                    LvPO_DataPO.Rows(index).Cells(cellFakPenawaran).Value = Dr("No_Penawaran")

                    LvPO_DataPO.Rows(index).Cells(cellPO_Jumlah).Style.BackColor = Color.LightGray
    ...
    Private Sub BtnPO_Simpan_Click(sender As Object, e As EventArgs) Handles BtnPO_Simpan.Click
        ...
        Try
            ...
            If BtnPO_Simpan.Tag = "&Simpan" Then
                ...
                SQL = SQL & "'" & KodePerusahaan & "', '" & TxtPO_NoFaktur.Text & "', '" & Txt_Faktur_Induk.Text & "', '" & TxtPO_NoNota.Text & "', "
                ...
                For index = 0 To LvPO_DataPO.Rows.Count - 1
                    ...
                    SQL = "select no_faktur from emi_pembelian_po_detail_Barang_Lain where "
                    ...
                    Using Dr = OpenTrans(SQL)
                        If Dr.Read Then
                            ...
                            SQL = "update emi_pembelian_po_detail_Barang_Lain set "
                        ...
                        Else
                            ...
                            SQL = "insert into emi_pembelian_po_detail_Barang_Lain(Kode_Perusahaan, No_Faktur, Kode_Stock_Owner, "
                    ...
                    SQL = "insert into EMI_Pembelian_PO_Det_Barang_Lain(Kode_Perusahaan, No_Faktur, Kode_Stock_Owner, "
                    SQL = SQL & "Kode_Barang, Jumlah, Satuan, Harga, Nilai_Barang, Satuan_Barang, Harga_Barang, "
                ...
                '    'update po pembelian
                '    SQL = "update  emi_pembelian_PO_Barang_Lain set "
                ...
                '    SQL = "delete from emi_pembelian_po_detail_Barang_Lain where kode_perusahaan = '" & KodePerusahaan & "' and no_faktur = '" & TxtPO_NoFaktur.Text & "' "
                '    ExecuteTrans(SQL)

                '    'delete det nya

                '    SQL = "delete from EMI_Pembelian_PO_Det_Barang_Lain where kode_perusahaan = '" & KodePerusahaan & "' and no_faktur = '" & TxtPO_NoFaktur.Text & "' "
                '    ExecuteTrans(SQL)
                ...
                '        SQL = "select no_faktur from emi_pembelian_po_detail_Barang_Lain where kode_perusahaan = '" & KodePerusahaan & "' "
                '        SQL = SQL & "and no_faktur = '" & TxtPO_NoFaktur.Text & "'  "
                '        SQL = SQL & "and kode_stock_owner = '" & lvPO_Lokasi & "' "
                '        SQL = SQL & "and kode_barang = '" & lvPO_KdBarang & "' "
                '        SQL = SQL & "and satuan = '" & lvPO_Satuan & "' "
                ...
                '            If Dr.Read Then
                '                Dr.Close()
                '                SQL = "update emi_pembelian_po_detail_Barang_Lain set jumlah = jumlah + " & HilangkanTanda(lvPO_Jumlah) & ", "
                '                SQL = SQL & "nilai_barang = nilai_barang + " & lvPO_Jumlah_SB & ", "
                '                SQL = SQL & "total = total +   " & HilangkanTanda(lvPO_Total) & " "
                '                SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
                '                SQL = SQL & "and no_faktur = '" & TxtPO_NoFaktur.Text & "' "
                ...
                '===============================================
                '=     CEK APAKAH PO INDUK SUDAH TERPENUHI     =
                '===============================================
                SQL = "select a.No_Faktur, sum(b.Jumlah) as Jumlah_PO, "
                SQL = SQL & "ISNULL((select SUM(x.jumlah) from emi_pembelian_PO_Barang_Lain z, emi_pembelian_po_detail_Barang_Lain x "
                SQL = SQL & "where z.Kode_Perusahaan = x.Kode_Perusahaan and z.No_Faktur = x.No_Faktur "
                SQL = SQL & "and z.No_Faktur_Induk = a.No_Faktur ), 0) as Jumlah_SubPO "
                SQL = SQL & "from EMI_Pembelian_PO_Induk_Barang_Lain a, EMI_Pembelian_PO_Detail_Induk_Barang_Lain b "
                ...
                Using Ds = BindingTrans(SQL)
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For i As Integer = 0 To .Rows.Count - 1
                                If Val(HilangkanTanda(.Rows(i).Item("Jumlah_PO"))) = Val(HilangkanTanda(.Rows(i).Item("Jumlah_SubPO"))) Then

                                    SQL = "update EMI_Pembelian_PO_Induk_Barang_Lain set Flag_Selesai_SubPO = 'Y' where Kode_Perusahaan = '" & KodePerusahaan & "' and No_Faktur = '" & Txt_Faktur_Induk.Text & "' "
                                    ExecuteTrans(SQL)
                                End If
                            Next
                        End If
        ...
        Me.Close()
        EMI_PO_Pembelian_Display_Sub_Barang_Lain.BtnRefresh_Click(Button2, e)
    End Sub
    ...
    Private Sub cmb_pr_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_pr.SelectedIndexChanged
        ...
        Try
            ...
            txtSatuanSisa.Text = ""
            txtSisaPr.Text = ""
            SQL = "Select a.Jumlah - isnull((Select sum(y.Jumlah) from emi_pembelian_PO_Barang_Lain x, EMI_Pembelian_PO_Det_Barang_Lain y   "
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            ...
            SQL = "select status,selesai,flag_release from emi_pembelian_PO_Barang_Lain "
            SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and no_faktur = '" & TxtPO_NoFaktur.Text & "' "
            Using dr = OpenTrans(SQL)
                If dr.Read Then
            ...
            SQL = "update emi_pembelian_PO_Barang_Lain set flag_release = 'Y', "
            ...
            If flag_kategori_Supplier = "Y" Then
                ...
                Using dr = OpenTrans(SQL)
                    If dr.Read Then
                        arrInisialFakturSubmitPO = dr("inisial_faktur_barang_lain")
                    Else
                        dr.Close()
                        CloseTrans()
                        CloseConn()
                ...
                SQL = SQL & "from detail_rencana_order a, Barang_Lain b, rencana_order c where "
        ...
        Try
            ...
            'SQL = "select a.No_Faktur from emi_pembelian_PO_Barang_Lain a, emi_pembelian_po_detail_Barang_Lain b, Barang_Lain c, suppliers d "
                ...
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    With Ds.Tables(0)
                        ...
                        With A_Place_For_Printing2
                            ...
                            CrDoc.RecordSelectionFormula = " {View_Laporan_PO2_Barang_Lain.Kode_Perusahaan} = '" & KodePerusahaan & "' and {View_Laporan_PO2_Barang_Lain.No_Faktur} = '" & no_Faktur_Sementara & "'"
    ...
    Private Sub TxtPO_NoFaktur_TextChanged(sender As Object, e As EventArgs) Handles TxtPO_NoFaktur.TextChanged, Txt_Faktur_Induk.TextChanged

    End Sub

    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click

        SD_Sub_PO_Barang_Lain.ShowDialog()
    ...
    Public Sub TxtPO_NoFaktur_Leave(sender As Object, e As EventArgs) Handles TxtPO_NoFaktur.Leave, Txt_Faktur_Induk.Leave
        ...
        '    SQL = SQL & "from EMI_Pembelian_PO_Det_Barang_Lain a, Barang_Lain b "
        '    SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan "

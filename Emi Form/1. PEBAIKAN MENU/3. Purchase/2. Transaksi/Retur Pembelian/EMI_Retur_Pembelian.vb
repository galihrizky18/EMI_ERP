...
Public Class EMI_Retur_Pembelian
    ...
    Dim dgv_IsiPerBags, dgv_SatuanIsiBags, dgv_TglProd, dgv_TglExp, dgv_KetWarna, dgv_Barcode, dgv_FlagBlokSN, dgv_JnsTransfer, dgv_NoLoading, dgv_Mobil As String
    ...
    Dim dgv_Rekap_Noloading, dgv_Rekap_Mobil As String
    ...
    Dim dgv_detail_Warna, dgv_detail_JenisKemasan, dgv_detail_TglProduksi, dgv_detail_TglExpired, dgv_detail_JenisKualitas, dgv_detail_Barcode, dgv_detail_JnsTransfer, dgv_detail_NoLoading, dgv_detail_Mobil As String
    ...
    Dim itemDGVNoLoading As Integer = 25
    Dim itemDGVMobil As Integer = 26
    ...
    Dim itemDgvRekap_NoLoading As Integer = 11
    Dim itemDgvRekap_Mobil As Integer = 12
    ...
    Dim itemDgvDetail_NoLoading As Integer = 18
    Dim itemDgvDetail_Mobil As Integer = 19
    ...
    Private Sub get_grid_view(ByVal index As Integer)
        ...
        dgv_NoLoading = DGV_Data_TF.Rows(index).Cells(itemDGVNoLoading).Value
        dgv_Mobil = DGV_Data_TF.Rows(index).Cells(itemDGVMobil).Value
    End Sub
    ...
    Private Sub get_grid_view_Rekap(ByVal index As Integer)
        ...
        dgv_Rekap_Noloading = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_NoLoading).Value
        dgv_Rekap_Mobil = Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_Mobil).Value
    End Sub
    ...
    Private Sub get_grid_view_Detail(ByVal index As Integer)
        ...
        dgv_detail_NoLoading = Dgv_DataDetail.Rows(index).Cells(itemDgvDetail_NoLoading).Value
        dgv_detail_Mobil = Dgv_DataDetail.Rows(index).Cells(itemDgvDetail_Mobil).Value
    ...
    Public Sub kosong()
        ...
        Try
            ...
            DGV_Data_TF.Columns(itemDGVMobil).DisplayIndex = 0
            DGV_Data_TF.Columns(itemDgvKodeBarang).DisplayIndex = 1
            DGV_Data_TF.Columns(itemDgvNama).DisplayIndex = 2
            DGV_Data_TF.Columns(itemDgvSatuan).DisplayIndex = 3
            DGV_Data_TF.Columns(itemDGVTglExp).DisplayIndex = 4
            DGV_Data_TF.Columns(itemDGVTglProd).DisplayIndex = 5
            DGV_Data_TF.Columns(itemDGVBarcode).DisplayIndex = 6
            DGV_Data_TF.Columns(itemDgvCheckBox).DisplayIndex = 7
            DGV_Data_TF.Columns(itemDgvJumlah).DisplayIndex = 8
            DGV_Data_TF.Columns(itemDgvBags).DisplayIndex = 9

            Dgv_DataRekap.Columns(itemDgvRekap_Mobil).DisplayIndex = 0
            Dgv_DataRekap.Columns(itemDgvRekap_KdBarang).DisplayIndex = 1
            Dgv_DataRekap.Columns(itemDgvRekap_NmBarang).DisplayIndex = 2
            ...
            SQL = "select kode_stock_owner, inisial_faktur from Stock_Owner"
    ...
    Private Sub Get_Data()
        If TxtNoPembelian.Text.Trim.Length = 0 Then
            Exit Sub
        ElseIf CmbMobil.SelectedIndex = -1 Or CmbBarang.SelectedIndex = -1 Then
            Exit Sub
        End If
        ...
        Try
            ...
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    ...
                    DGV_Data_TF.Rows(rows).Cells(itemDgvSatuan).Value = General_Class.CekNULL(Dr("satuan_besar"))
                    ...
                    DGV_Data_TF.Rows(rows).Cells(itemDGVNoLoading).Value = Dr("No_Pembelian_Loading")
                    DGV_Data_TF.Rows(rows).Cells(itemDGVMobil).Value = Dr("Mobil")
    ...
    Private Sub DGV_Data_TF_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellEndEdit
        ...
        If DGV_Data_TF.CurrentRow.Cells(itemDgvCheckBox).Value = "True" Then
            ...
            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = DGV_Data_TF.CurrentRow.Cells(itemDgvStockBags).Value
            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = DGV_Data_TF.CurrentRow.Cells(itemDgvGoodStock).Value
    ...
    Private Sub get_no_faktur()
        Dim FPro_Results As String = "RP"

        TxtNo_Transaksi.Text = FPro_Results & arrInisialFaktur.Item(Cmb_Lokasi.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy") & "-" &
                          General_Class.Get_Last_Number2("Emi_Retur_Pembelian", "no_transaksi", 5,
                          "Kode_perusahaan", KodePerusahaan,
                          "And", "substring(no_transaksi,1," & Len(FPro_Results) + Len(arrInisialFaktur.Item(Cmb_Lokasi.SelectedIndex)) + 6 & ")", FPro_Results & arrInisialFaktur.Item(Cmb_Lokasi.SelectedIndex) & "-" & Format(tgl_skg, "MM/yy"))
    End Sub
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        Try
            ...
            Dim akun_selisih_biaya As String = ""

            SQL = "select inisial_faktur, Hutang_Supplier, Akun_Selisih_Bahan from stock_owner "
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    ...
                    akun_selisih_biaya = Dr("Akun_Selisih_Bahan")
                Else
                    Dr.Close()
                    CloseTrans()
                    CloseConn()
            ...
            SQL = "insert into Emi_Retur_Pembelian (kode_perusahaan, No_Transaksi, Kode_Stock_Owner, Tanggal, Jam, UserID, Keterangan, No_Pembelian, Kode_Voucher) Values "
            SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(TxtNo_Transaksi.Text) & "', '" & Cmb_Lokasi.Text & "', "
            SQL = SQL & "'" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & tgl_skg.ToString("HH:mm:ss") & "', '" & UserID & "', "
            SQL = SQL & "'" & TxtKeterangan.Text & "', '" & TxtNoPembelian.Text & "', '" & Kode_voucher & "')"
            ExecuteTrans(SQL)
            ...
            For i As Integer = 0 To Dgv_DataRekap.Rows.Count - 1
                ...
                SQL = "insert into Emi_Retur_Pembelian_Detail(Kode_Perusahaan, No_Transaksi, Kode_Barang, Total, Satuan, "
                SQL = SQL & "Total_Barang, Satuan_Barang, Total_Bags, No_Loading, Lokasi) values "
                ...
                SQL = SQL & "'" & HilangkanTanda(dgv_Rekap_JumlahBags) & "', '" & dgv_Rekap_Noloading & "', '" & dgv_Rekap_lokasi & "' )"
                ExecuteTrans(SQL)

                Dim x_ident_current As Integer = 0
                SQL = "select IDENT_CURRENT('Emi_Retur_Pembelian_Detail') as urutan"
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                ...
                SQL = "select kode_Perusahaan from Emi_Retur_Pembelian_Detail where "
                SQL = SQL & "kode_Perusahaan='" & KodePerusahaan & "' and "
                SQL = SQL & "No_Transaksi='" & TxtNo_Transaksi.Text & "' and "
                ...
                For j As Integer = 0 To Dgv_DataDetail.Rows.Count - 1
                    ...
                    If dgv_detail_lokasi = dgv_Rekap_lokasi And dgv_detail_KdBarang = dgv_Rekap_KdBarang Then
                        ...
                        SQL = "insert into Emi_Retur_Pembelian_Det(Kode_Perusahaan, No_Transaksi, Id_Wms, No_Pallet, "
                        SQL = SQL & "Serial_Number, Jumlah, Jumlah_Barang, Jumlah_Bags, Warna, Urut_Retur_Pembelian) values( "
...
#Region "Potong Stock"
                        ...
                        Dim Harga_PO As Double = 0
                        'Dim jumlahAkhir As Double = Val(dgv_GoodStock) - Val(dgv_Jumlah)
                        SQL = "select Harga from emi_pembelian a, emi_pembelian_detail b where "
                        SQL = SQL & "a.kode_perusahaan=b.kode_perusahaan and a.No_Faktur=b.no_faktur and a.status is null "
                        SQL = SQL & "and b.kode_perusahaan='" & KodePerusahaan & "' and b.no_faktur='" & TxtNoPembelian.Text & "' "
                        SQL = SQL & " And b.no_loading ='" & dgv_Rekap_Noloading & "' and b.Kode_Barang='" & dgv_detail_KdBarang & "' "
                        Using dr = OpenTrans(SQL)
                            If dr.Read Then
                                Harga_PO = dr("Harga")
                            Else
                                ...
                                CloseTrans()
                                CloseConn()
                                MessageBox.Show("Pembelian Barang " & Nama & " tidak ditemukan!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                Exit Sub
                            End If
                        End Using
                        ...
                        Using dr = OpenTrans(SQL)
                            If dr.Read Then
                                ...
                                Dim nilai_persediaan As Double = Math.Round(GoodTemp * harga, 0)
                                Dim nilai_hutang As Double = Math.Round(GoodTemp * Harga_PO, 0)
                                Dim nilai_selisih As Double = nilai_persediaan - nilai_hutang
                                ...
                                Using Dr4 = OpenTrans(SQL)
                                    If Dr4.Read Then
                                        Dr4.Close()
                                        'update
                                        SQL = "update detail_jurnal set kredit = kredit+ " & nilai_persediaan & " where "
                                        SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' and "
                                        SQL = SQL & "kode_voucher = '" & Kode_voucher & "' and "
                                        SQL = SQL & "kode_master_acc + kode_acc + kode_detail_acc = '" & akun_persediaan & "'  "
                                        SQL = SQL & "and kredit <> 0"
                                    ...
                                    Else
                                        ...
                                        SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_persediaan, 1),
                                                  ...
                                                  KodePerusahaan, KodeProyek, "Persediaan " & "; " & TxtNo_Transaksi.Text, "0", nilai_persediaan, pagenumber, dgv_detail_lokasi, Bahasa_Pilihan, Ket_Cost_Center_HO)
                                        ExecuteTrans(SQL)
                                ...
                                Using Dr4 = OpenTrans(SQL)
                                    ...
                                    Else
                                        ...
                                        SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_hutang, 1),
                                                  ...
                                                  KodePerusahaan, KodeProyek, "Hutang " & "; " & TxtNo_Transaksi.Text, nilai_hutang, "0", pagenumber, dgv_detail_lokasi, Bahasa_Pilihan, Ket_Cost_Center_HO)
                                        ExecuteTrans(SQL)
                                        pagenumber = pagenumber + 1
                                ...
                                If nilai_selisih <> 0 Then
                                    SQL = "select kode_perusahaan from detail_jurnal where kode_perusahaan = '" & KodePerusahaan & "' and "
                                    SQL = SQL & "kode_voucher = '" & Kode_voucher & "' and "
                                    SQL = SQL & "kode_master_acc + kode_acc + kode_detail_acc = '" & akun_selisih_biaya & "' "
                                    SQL = SQL & "and debit <> 0 "
                                    Using Dr4 = OpenTrans(SQL)
                                        If Dr4.Read Then
                                            Dr4.Close()
                                            'update
                                            SQL = "update detail_jurnal set debit = debit+ " & nilai_selisih & " where "
                                            SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' and "
                                            SQL = SQL & "kode_voucher = '" & Kode_voucher & "' and "
                                            SQL = SQL & "kode_master_acc + kode_acc + kode_detail_acc = '" & akun_selisih_biaya & "'  "
                                            SQL = SQL & "and debit <> 0"
                                        ...
                                        Else
                                            Dr4.Close()
                                            'insert
                                            SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_selisih_biaya, 1),
                                                      Strings.Mid(akun_selisih_biaya, 2, 1),
                                                      Strings.Mid(Ganti(akun_selisih_biaya), 3),
                                                      KodePerusahaan, KodeProyek, "Selisih Persediaan " & "; " & TxtNo_Transaksi.Text, nilai_selisih, "0", pagenumber, dgv_detail_lokasi, Bahasa_Pilihan, Ket_Cost_Center_HO)
                                            ExecuteTrans(SQL)
                                            pagenumber = pagenumber + 1
...
#End Region
    ...
    Private Sub Btn_SimpanSementara_Click(sender As Object, e As EventArgs) Handles Btn_SimpanSementara.Click
        ...
        Try
            ...
            For row As Integer = 0 To DGV_Data_TF.RowCount - 1
                ...
                SQL = "select Kode_perusahaan from Emi_Retur_Pembelian_Temp "
                SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and "
                SQL = SQL & "No_Transaksi = '" & UserID & "' and "
                SQL = SQL & "No_Pembelian = '" & TxtNoPembelian.Text & "' and "
                SQL = SQL & "No_Pembelian_Loading = '" & dgv_NoLoading & "' and "
                SQL = SQL & "Barcode = '" & dgv_Barcode & "' "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        CloseConn()
                        MessageBox.Show("Data sudah Pernah di Input . . ! !", "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    End If
                End Using

                SQL = "insert into Emi_Retur_Pembelian_Temp ( Kode_Perusahaan, No_Transaksi, No_Pembelian, No_Pembelian_Loading, Lokasi, KdBarang, Serial_Number, Nm_Barang, IdWarehouse, KdRakAwal, IdPalletAwal, Jumlah, "
        ...
        LoadDataTab(UserID, TxtNoPembelian.Text)
        Get_Data()

    End Sub

    Private Sub LoadDataTab(ByVal NoTransaksi As String, ByVal NoPembelian As String)

        Dim DgvTab3_Rows As Integer = 0
        Dim DgvTab2_Rows As Integer = 0
        Dim JnsTransfer As String = ""

        Dim rows_rekap As Integer = 0
        Try
            OpenConn()
            ...
            SQL = "select a.Kode_Perusahaan, a.No_Transaksi, a.No_Pembelian, a.No_Pembelian_Loading, a.Lokasi, a.KdBarang, a.Serial_Number, a.Nm_Barang, a.IdWarehouse, a.KdRakAwal, a.IdPalletAwal, a.Jumlah, a.Jumlah_Bags,  "
            SQL = SQL & "a.KdRak_Tujuan, a.IdWarehpuse_Tujuan, a.Warna, a.JenisKemasan, a.TglProduksi, a.TglExpired, a.JenisKualitas, a.Barcode, a.Jenis_Transfer, a.Satuan, a.Satuan_Kecil, a.urut_Material_Req, "
            SQL = SQL & "(b.No_SJ + ' - ' + b.No_Plat + ' - ' + b.Driver) as Mobil "
            SQL = SQL & "from Emi_Retur_Pembelian_Temp a, emi_pembelian_loading b "
            SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan "
            SQL = SQL & "and a.No_Pembelian_Loading = b.No_Faktur "
            SQL = SQL & "and a.kode_perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and a.No_Transaksi = '" & NoTransaksi & "' "
            SQL = SQL & "and a.No_Pembelian = '" & NoPembelian & "' "
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        ...
                        Dim TotalTFBags As Double = 0
                        Dim OtoMaterialReq As String = ""
                        Dim NoLoading As String = ""
                        Dim Mobil As String = ""
                        For i As Integer = 0 To .Rows.Count - 1

                            Lokasi = .Rows(i).Item("Lokasi")
                            Kdbarang = .Rows(i).Item("KdBarang")
                            NmBarang = .Rows(i).Item("Nm_Barang")
                            Satuan = .Rows(i).Item("Satuan")
                            SatuanKecil = .Rows(i).Item("Satuan_Kecil")
                            OtoMaterialReq = .Rows(i).Item("urut_Material_Req")
                            NoLoading = .Rows(i).Item("No_Pembelian_Loading")
                            Mobil = .Rows(i).Item("Mobil")

                            Dgv_DataDetail.Rows.Add(1)
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_lokasi).Value = .Rows(i).Item("Lokasi")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_KdBarang).Value = .Rows(i).Item("KdBarang")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_SN).Value = .Rows(i).Item("Serial_Number")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_NmBarang).Value = .Rows(i).Item("Nm_Barang")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_IDWarehouseAwal).Value = .Rows(i).Item("IdWarehouse")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_KodeRakAwal).Value = .Rows(i).Item("KdRakAwal")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_IDPalletAwal).Value = .Rows(i).Item("IdPalletAwal")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Jumlah).Value = .Rows(i).Item("Jumlah")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JumlahBags).Value = .Rows(i).Item("Jumlah_Bags")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_KodeRakTujuan).Value = .Rows(i).Item("KdRak_Tujuan")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_IDWarehouseTujuan).Value = .Rows(i).Item("IdWarehpuse_Tujuan")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Warna).Value = .Rows(i).Item("Warna")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JenisKemasan).Value = .Rows(i).Item("JenisKemasan")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_TglProduksi).Value = Format(.Rows(i).Item("TglProduksi"), "dd MMM yyyy")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_TglExpired).Value = Format(.Rows(i).Item("TglExpired"), "dd MMM yyyy")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JenisKualitas).Value = .Rows(i).Item("JenisKualitas")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Barcode).Value = .Rows(i).Item("Barcode")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JnsTransfer).Value = .Rows(i).Item("Jenis_Transfer")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_NoLoading).Value = .Rows(i).Item("No_Pembelian_Loading")
                            Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Mobil).Value = .Rows(i).Item("Mobil")

                            JnsTransfer = dgv_JnsTransfer
                            TotalTF = Val(HilangkanTanda(.Rows(i).Item("Jumlah")))
                            TotalTFBags = Val(HilangkanTanda(.Rows(i).Item("Jumlah_Bags")))
                            ...
                            For index = 0 To Dgv_DataRekap.Rows.Count - 1
                                ...
                                If dgv_Rekap_Noloading = NoLoading And dgv_Rekap_KdBarang = Kdbarang Then
                                    ada_data = True

                                    Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_Jumlah).Value = Val(HilangkanTanda(dgv_Rekap_JumlahBersih)) + TotalTF & " " & Satuan
                                    Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_JumlahBags).Value = Val(HilangkanTanda(dgv_Rekap_JumlahBags)) + TotalTFBags
                                    Dgv_DataRekap.Rows(index).Cells(itemDgvRekap_JumlahBersih).Value = Val(HilangkanTanda(dgv_Rekap_JumlahBersih)) + TotalTF
                            ...
                            If ada_data = False Then
                                Dgv_DataRekap.Rows.Add(1)
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_lokasi).Value = Lokasi
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_KdBarang).Value = Kdbarang
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_NmBarang).Value = NmBarang
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_Jumlah).Value = TotalTF & " " & Satuan
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_JumlahBags).Value = TotalTFBags
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_JumlahBersih).Value = TotalTF
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_Satuan).Value = Satuan
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_SatuanKecil).Value = SatuanKecil
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_Oto).Value = OtoMaterialReq
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_JnsTransfer).Value = JnsTransfer
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_OtoGen).Value = ""
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_NoLoading).Value = NoLoading
                                Dgv_DataRekap.Rows(rows_rekap).Cells(itemDgvRekap_Mobil).Value = Mobil
    ...
    Private Sub TxtNoPembelian_Leave(sender As Object, e As EventArgs) Handles TxtNoPembelian.Leave
        If TxtNoPembelian.Text.Trim.Length = 0 Then
            Exit Sub
        End If
        ...
        Try
            ...
            Dim No_PO As String = ""
            SQL = "select No_PO from emi_pembelian "
            SQL = SQL & "where kode_perusahaan='" & KodePerusahaan & "' and No_Faktur='" & TxtNoPembelian.Text & "' "
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    No_PO = dr("No_PO")
                ...
                Else
                    ...
                    TxtNoPembelian.Text = ""
                    TxtNoPembelian.Enabled = True
                    MessageBox.Show("Nomor Pembelian Tidak di temukan . . ! ! ", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using

            CmbMobil.Items.Clear()
            CmbMobil.Items.Add("-- Seluruh --")
            ArrMobil.Clear()
            ArrMobil.Add("")
            SQL = "Select distinct No_sj+' - '+No_plat+' - '+driver as Keterangan, a.No_Faktur "
            SQL = SQL & "From emi_pembelian_loading a, emi_pembelian_loading_detail b Where "
            SQL = SQL & "a.Kode_Perusahaan = b.kode_perusahaan And a.no_faktur = b.no_faktur "
            SQL = SQL & "and b.kode_perusahaan='" & KodePerusahaan & "' and b.no_PO ='" & No_PO & "' and a.status is null "
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    CmbMobil.Items.Add(dr("Keterangan"))
                    ArrMobil.Add(dr("No_Faktur"))
                Loop
            End Using

            CmbBarang.Items.Clear()
            CmbBarang.Items.Add("-- Seluruh --")
            ArrBarang.Clear()
            ArrBarang.Add("")
            SQL = "Select distinct b.Kode_Barang, b.Kode_Barang+' - '+c.nama as Keterangan "
            SQL = SQL & "From emi_pembelian_loading a, emi_pembelian_loading_detail b, barang c Where "
            SQL = SQL & "a.Kode_Perusahaan = b.kode_perusahaan And a.no_faktur = b.no_faktur "
            SQL = SQL & "And b.kode_barang=c.kode_barang And b.Kode_Stock_Owner=c.Kode_Stock_Owner "
            SQL = SQL & "and b.kode_perusahaan='" & KodePerusahaan & "' and b.no_PO ='" & No_PO & "' and a.status is null "
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    CmbBarang.Items.Add(dr("Keterangan"))
                    ArrBarang.Add(dr("Kode_Barang"))
                Loop
            End Using
    ...
    Private Sub TxtNoPembelian_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtNoPembelian.KeyPress
        If e.KeyChar = Chr(13) Then CmbMobil.Focus()
    End Sub

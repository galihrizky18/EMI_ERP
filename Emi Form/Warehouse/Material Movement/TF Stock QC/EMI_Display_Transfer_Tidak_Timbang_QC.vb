...
Public Class EMI_Display_Transfer_Tidak_Timbang_QC
    ...
    Private Sub Btn_TimbangFloorScale_Click(sender As Object, e As EventArgs) Handles Btn_TimbangFloorScale.Click
        ...
        Dim GetDataKodeTransfer, GetDataLokasi, GetDataKdBrg, GetDataNmBrg, GetDataSNAwal, GetDataBeratBagi, GetJumlahBags, GetDataCount, GetDataJumlahPotong, GetDataSatuanKecil, GetDataSatuanBesar As String
        Dim GetDataIdWmsAwal, GetDataIdWmsTujuan, GetDataPalletAwal, GetDataPalletTujuan, GetDataWarna, GetDataSoAwal, GetDataSoTujuan, GetDataUrutQcDetail, GetDataUrutQCDet As String
        Dim SN As String = ""
        ...
        Try
            ...
            Txt_ScanBarcode.Text = "1111096-0120M9B311224-BN9TL5XI1J"
            ...
            SQL = "select a.No_Faktur, a.Lokasi, b.Kode_Barang, d.Nama, c.Serial_Number_Awal, c.Berat_Bagi, c.Jumlah_Bags, c.Count, c.Total as Jumlah_Potong, b.Satuan_Barang, b.Satuan, "
            SQL = SQL & "c.Id_Wms_Awal, c.Id_Wms_Tujuan, c.No_Pallet_Awal, c.No_Pallet_Tujuan, a.SO_Awal, a.SO_Tujuan, c.Warna, b.Urut_Oto, c.Urut_Oto as Urut_QC_Det "
            SQL = SQL & "from Tf_Stock_QC a, Tf_Stock_QC_Detail b, Tf_Stock_QC_det c, barang d "
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    ...
                    GetDataSNAwal = Dr("Serial_Number_Awal")
                    GetDataBeratBagi = HilangkanTanda(Format(Dr("Berat_Bagi"), "N2"))
                    GetDataCount = HilangkanTanda(Dr("Count"))
                    GetDataSatuanKecil = Dr("Satuan_Barang")
                    GetDataSatuanBesar = Dr("Satuan")
                    GetDataJumlahPotong = Dr("Jumlah_Potong")
                    GetJumlahBags = Dr("Jumlah_Bags")
                    GetDataUrutQcDetail = Dr("urut_oto")
                    GetDataUrutQCDet = Dr("Urut_QC_Det")

                    GetDataIdWmsAwal = Dr("Id_Wms_Awal")
                    GetDataIdWmsTujuan = Dr("Id_Wms_Tujuan")
                    GetDataPalletAwal = Dr("No_Pallet_Awal")
                    GetDataPalletTujuan = Dr("No_Pallet_Tujuan")
                    GetDataWarna = Dr("Warna")
                    GetDataSoAwal = Dr("SO_Awal")
                    GetDataSoTujuan = Dr("SO_Tujuan")
                Else
                    Dr.Close()
                    CloseTrans()
                    CloseConn()
            ...
            'SQL = SQL & "'" & GetDataSatuanKecil & "', '" & GetDataJumlahPotong & "' ) as hasil"
            ...
            SQL = "select round(dbo.get_hpp(serial_number) * " & GetDataJumlahPotong & ", 2) as rp_persediaan_min from barang_sn where "
            SQL = SQL & "Kode_Stock_Owner='" & GetDataSoAwal & "' and Kode_Barang='" & GetDataKdBrg & "' "
            SQL = SQL & "and Serial_Number='" & GetDataSNAwal & "'"
            Using dr = OpenTrans(SQL)
                If dr.Read Then
            ...
            'POTONG BARANG
            Dim Nama As String = ""
            SQL = "select Nama,round(good_stock,2) as good_stock,Jumlah_Bags from Barang where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_Stock_Owner='" & GetDataSoAwal & "' "
            SQL = SQL & "and Kode_Barang='" & GetDataKdBrg & "' "
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    Nama = dr("nama")
                    If dr("good_stock") < GetDataJumlahPotong Then
                        dr.Close()
                        CloseTrans()
                        CloseConn()
                    ...
                    Else
                        dr.Close()
                        SQL = "update barang set Good_Stock = Good_Stock - " & GetDataJumlahPotong & ", Jumlah_Bags = Jumlah_Bags - " & GetJumlahBags & " "
            ...
            'POTONG BARANG SN
            SQL = "select round(jumlah,2) as jumlah,Jumlah_Bags from Barang_SN where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_Stock_Owner='" & GetDataSoAwal & "' "
            SQL = SQL & "and Kode_Barang='" & GetDataKdBrg & "' "
            SQL = SQL & "and Serial_Number='" & GetDataSNAwal & "'"
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    If dr("jumlah") < GetDataJumlahPotong Then
                        dr.Close()
                        CloseTrans()
                        CloseConn()
                        MessageBox.Show("Proses tidak dapat dilanjutkan karena akan membuat stock " & Nama & " menjadi negatif.", Judul, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    ...
                    Else
                        dr.Close()
                        SQL = "update barang_sn set jumlah = jumlah - " & GetDataJumlahPotong & ", Jumlah_Bags = Jumlah_Bags - " & GetJumlahBags & " "
                        SQL = SQL & "where Kode_Stock_Owner='" & GetDataSoAwal & "' and Kode_Barang='" & GetDataKdBrg & "' "
                        SQL = SQL & "and Serial_Number='" & GetDataSNAwal & "'"
                        ExecuteTrans(SQL)
                    End If
                Else
            ...
            SQL = SQL & "and a.Serial_Number='" & GetDataSNAwal & "' "
            SQL = SQL & "and a.Jumlah <> 0 "
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
            ...
            SQL = SQL & "select Kode_Perusahaan, '" & GetDataSoTujuan & "', Kode_Barang, '" & SN_Baru & "', '" & GetDataJumlahPotong & "', " & GetJumlahBags & ", "
            SQL = SQL & "Tgl_Expired, Tgl_Produksi, Stock_PO, Stock_Inquiry, '" & GetDataIdWmsTujuan & "', id_Susunan , Qr_Code, '" & newKodeUnikBerjalan & "', "
            SQL = SQL & "Kode_Unik_Asal, '" & GetDataPalletTujuan & "', batch_number, '" & warnaLama & "' "
            SQL = SQL & "from Barang_SN "
            SQL = SQL & "where Kode_Perusahaan='" & KodePerusahaan & "' "
            SQL = SQL & "and Kode_Stock_Owner='" & GetDataSoAwal & "' "
            SQL = SQL & "and Kode_Barang='" & GetDataKdBrg & "' "
            SQL = SQL & "and Serial_Number='" & GetDataSNAwal & "' "
            ExecuteTrans(SQL)
            ...
            'TAMBAH BARANG
            SQL = "update barang set Good_Stock= Good_Stock + " & GetDataJumlahPotong & ", Jumlah_Bags = Jumlah_Bags + " & GetJumlahBags & " "
            SQL = SQL & "where Kode_Perusahaan='" & KodePerusahaan & "' and Kode_Stock_Owner='" & GetDataSoTujuan & "' "
            SQL = SQL & " and Kode_Barang='" & GetDataKdBrg & "'"
            ExecuteTrans(SQL)
            ...
            SQL = SQL & "isnull((select round(sum(Jumlah_Bags), 2) from Barang_sn y "
            SQL = SQL & "where a.kode_Barang=y.kode_Barang and a.Kode_Stock_Owner=y.kode_Stock_Owner and a.kode_Perusahaan=y.kode_Perusahaan ), 0) as jumlah_bags_sn "
            SQL = SQL & "FROM barang a WHERE a.Kode_Stock_Owner = '" & GetDataSoTujuan & "' "
            SQL = SQL & "AND a.Kode_Barang = '" & GetDataKdBrg & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
            SQL = SQL & "group by a.kode_Barang, a.Kode_Stock_Owner, a.kode_Perusahaan "
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
...
#Region "JURNAL"
            ...
            SQL = SQL & "and b.kode_stock_owner = '" & GetDataSoAwal & "' and b.Kode_Barang='" & GetDataKdBrg & "' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
            ...
            SQL = SQL & "from Barang b,EMI_Group_Jenis a where a.Kode_Perusahaan = b.Kode_Perusahaan "
            SQL = SQL & "and a.Id_Group_Jenis = b.Id_Group_Jenis and b.Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and b.kode_stock_owner = '" & GetDataSoTujuan & "' and b.Kode_Barang='" & GetDataKdBrg & "' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
...
#End Region
            ...
            SQL = SQL & "and urut_oto = '" & GetDataUrutQCDet & "'"
            ExecuteTrans(SQL)
            ...
            SQL = SQL & "('" & KodePerusahaan & "', '" & GetDataKodeTransfer & "', '" & GetDataUrutQCDet & "', '" & GetDataPalletTujuan & "', '" & SN_Baru & "', "
            SQL = SQL & "'" & GetDataJumlahPotong & "', '" & UserID & "', '" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & tgl_skg.ToString("HH:mm:ss") & "')"
            ExecuteTrans(sql)

            SQL = "update Tf_Stock_QC_Detail set  "
            SQL = SQL & "Flag_Timbang = 'Y' "
            SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and urut_oto = '" & GetDataUrutQcDetail & "' "
            ExecuteTrans(SQL)
        ...
        Try
            ...
            '=     CETAK BARCODE     =
            '=========================
            SQL = "select Kode_Perusahaan from Cetak_TransferStock_QC where Kode_Perusahaan='" & KodePerusahaan & "' and kode_unik_print='" & kode_unik_print & "'"
            Using Ds2 = BindingTrans(SQL)
                If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                    ...
                    With A_Place_For_Printing2
                        ...
                        CrDoc.RecordSelectionFormula = "{Cetak_TransferStock_QC.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Cetak_TransferStock_QC.kode_unik_print} = '" & kode_unik_print & "' and {Cetak_TransferStock.batch} = '" & batchLama & "' "
                    ...
                    'CrDoc.SetDataSource(Ds2)
                    'CrDoc.SetDatabaseLogon(CUserId, CPassword, CServer, CDatabase)
                    'CrDoc.RecordSelectionFormula = "{Cetak_TransferStock_QC.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Cetak_TransferStock_QC.kode_unik_print} = '" & kode_unik_print & "' and {Cetak_TransferStock.batch} = '" & .Rows(i).Item("Batch_Number") & "' "
            ...
            SQL = SQL + "where Kode_Perusahaan = '" & KodePerusahaan & "' and No_Faktur = '" & GetDataKodeTransfer & "' and Selesai is null"
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    CloseConn()
                    kosong()
            ...
            SQL = "select Kode_Perusahaan from Vw_Tf_Stock_Detail_QC where Kode_Perusahaan = '" & KodePerusahaan & "' and No_Faktur = '" & GetDataKodeTransfer & "'"
            Using Ds = BindingTrans(SQL)
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    ...
                    With A_Place_For_Printing2
                        ...
                        CrDoc.SetDatabaseLogon(CUserId, CPassword, CServer, CDatabase)
                        CrDoc.PrintOptions.PrinterName = ""
                        CrDoc.RecordSelectionFormula = "{Vw_Tf_Stock_Detail_QC.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Vw_Tf_Stock_Detail_QC.No_Faktur}='" & GetDataKodeTransfer & "' "
    ...
    Private Sub Popup_Timbang_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Label1.Text = "Display - List Transfer Stock QC (Tidak Timbang)"
        ...
        Try
            ...
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    ...
                    Lvw.SubItems.Add(Format(dr("Total_Besar"), "N2"))

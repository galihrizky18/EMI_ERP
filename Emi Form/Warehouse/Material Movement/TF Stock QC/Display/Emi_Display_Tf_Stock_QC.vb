...
#Region "POOTONG STOCK"
            ...
            SQL = SQL & "AND Kode_Stock_Owner = '" & SoAwal & "' and kode_barang = '" & kodeBarang & "' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
            ...
            If Stock_SblmPotong <> Stock_SN_SblmPotong Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock {kodeBarang} pada Gudang {SoAwal} Tidak Sesuai Sebelum Dipotong", Judul,
            ...
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    ...
                    Else
                        ...
                        SQL = "insert into N_EMI_LOG_Transaksi_Validasi_Split_Stock_QC "
                        ...
                        SQL &= $"values ('{KodePerusahaan}', '{NoFaktur}', '{Format(tgl_skg, "yyyy-MM-dd")}', '{Format(tgl_skg, "HH:mm:ss")}', "
                        SQL &= $"'POTONG STOCK BARANG', '{SoAwal}', '{kodeBarang}', '-', '{Stock_SblmPotong}', 0, '{totalPotong}', 0) "
                        ExecuteTrans(SQL)
            ...
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    ...
                    Else
                        ...
                        SQL &= $"(Kode_Perusahaan, No_Transaksi, Tanggal, Jam, Action, Kode_Stock_Owner, Kode_Barang, Serial_Number, Jumlah_Awal, Bags_Awal, Jumlah_Update, Bags_Update) "
                        SQL &= $"values ('{KodePerusahaan}', '{NoFaktur}', '{Format(tgl_skg, "yyyy-MM-dd")}', '{Format(tgl_skg, "HH:mm:ss")}', "
                        SQL &= $"'POTONG STOCK BARANG SN', '{SoAwal}', '{kodeBarang}', '{SN_Awal}', '{Stock_SblmPotong}', 0, '{totalPotong}', 0) "
                        ExecuteTrans(SQL)
            ...
            If Stock_Setelah_Potong <> Stock_SN_Setelah_Potong Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock {kodeBarang} pada Gudang {SoAwal} Tidak Sesuai Setelah Di Potong", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If

            If Math.Round((Stock_SblmPotong - Stock_Setelah_Potong), 4) <> Math.Round(totalPotong, 4) Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock Sebelum dan Sesudah Di potong Pada Gudang {SoAwal}, Barang {kodeBarang} Tidak Sesuai Setelah Di Potong", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If

            If Math.Round((Stock_SN_SblmPotong - Stock_SN_Setelah_Potong), 4) <> Math.Round(totalPotong, 4) Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock Sebelum dan Sesudah Di potong Pada Gudang {SoAwal}, Barang {kodeBarang} Tidak Sesuai Setelah Di Potong", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
...
#Region "TAMBAH STOCK"
            ...
            SQL = SQL & "AND Kode_Stock_Owner = '" & SoTujuan & "' and kode_barang = '" & kodeBarang & "' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
            ...
            If Stock_Sebelum_Insert <> Stock_SN_Sebelum_Insert Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock {kodeBarang} pada Gudang {SoTujuan} Tidak Sesuai Sebelum Diinsert", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If
            ...
            For i As Integer = 0 To count - 1
                ...
                SQL &= $"(Kode_Perusahaan, No_Transaksi, Tanggal, Jam, Action, Kode_Stock_Owner, Kode_Barang, Serial_Number, Jumlah_Awal, Bags_Awal, Jumlah_Update, Bags_Update) "
                SQL &= $"values ('{KodePerusahaan}', '{NoFaktur}', '{Format(tgl_skg, "yyyy-MM-dd")}', '{Format(tgl_skg, "HH:mm:ss")}', "
                SQL &= $"'INSERT STOCK BARANG SN', '{SoTujuan}', '{kodeBarang}', '{SN_Baru}', '{Stock_Sebelum_Insert}', 0, '{beratBagi}', 0) "
                ExecuteTrans(SQL)
            ...
            SQL &= $"(Kode_Perusahaan, No_Transaksi, Tanggal, Jam, Action, Kode_Stock_Owner, Kode_Barang, Serial_Number, Jumlah_Awal, Bags_Awal, Jumlah_Update, Bags_Update) "
            SQL &= $"values ('{KodePerusahaan}', '{NoFaktur}', '{Format(tgl_skg, "yyyy-MM-dd")}', '{Format(tgl_skg, "HH:mm:ss")}', "
            SQL &= $"'INSERT STOCK BARANG SN', '{SoTujuan}', '{kodeBarang}', '-', '{Stock_Sebelum_Insert}', 0, '{totalTambahStock}', 0) "
            ExecuteTrans(SQL)
            ...
            If Stock_Setelah_Insert <> Stock_SN_Setelah_Insert Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock {kodeBarang} pada Gudang {SoTujuan} Tidak Sesuai Setelah Diinsert", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If
            If Math.Round((Stock_Setelah_Insert - Stock_Sebelum_Insert), 4) <> Math.Round(totalTambahStock, 4) Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock Sebelum dan Sesudah Di Insert Pada Gudang {SoTujuan}, Barang {kodeBarang} Tidak Sesuai Setelah Di Potong", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If

            If Math.Round((Stock_SN_Setelah_Insert - Stock_SN_Sebelum_Insert), 4) <> Math.Round(totalTambahStock, 4) Then
                CloseTrans()
                CloseConn()
                MessageBox.Show($"Jumlah Stock Sebelum dan Sesudah Di Insert Pada Gudang {SoTujuan}, Barang {kodeBarang} Tidak Sesuai Setelah Di Potong", Judul,
                                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
...
#Region "JURNAL"
            ...
            SQL = Get_Detail_Jurnal(Kode_voucher, Strings.Left(akun_persediaan_tujuan, 1),
                     ...
                     KodePerusahaan, KodeProyek, "Persedian " & NoFaktur, nilai_persediaan_min, "0", pagenumber, SoTujuan, Bahasa_Pilihan, Ket_Cost_Center_HO)
            ExecuteTrans(SQL)
            pagenumber = pagenumber + 1

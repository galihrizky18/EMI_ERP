...
Public Class N_EMI_TF_Material_To_Material_Display
    ...
    Private Sub kosong()
        ...
        Cmb_Filter_ParamLain.Items.Add("No Transaksi") : arrCari.Add("a.kode_transfer")
    ...
    Private Sub Intial_ListView_Stock()
        ...
        Lv_Stock_Detail.Columns.Add("Kode Barang Awal", 120, HorizontalAlignment.Left)
        Lv_Stock_Detail.Columns.Add("Qr Code Awal", 300, HorizontalAlignment.Left)
        ...
        Lv_Stock_Detail.Columns.Add("Kode Barang Akhir", 120, HorizontalAlignment.Left)
        Lv_Stock_Detail.Columns.Add("Qr Code Akhir", 300, HorizontalAlignment.Left)
        Lv_Stock_Detail.Columns.Add("Jumlah Akhir", 130, HorizontalAlignment.Right)
    ...
    Private Sub Lv_Stock_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Lv_Stock.SelectedIndexChanged
        ...
        Try
            ...
            SQL = "Select a.kode_transfer, a.kode_barang_awal, c.qr_code+'-'+c.kode_unik_berjalan as Qr, jumlah_barang, b.satuan_barang, "
            SQL = SQL & "b.jumlah_bags, a.kode_barang_tujuan, jumlah_akhir, b.flag_validasi, "

            SQL = SQL & "isnull((select x.qr_code+'-'+x.kode_unik_berjalan from barang_sn x where "
            SQL = SQL & "x.kode_perusahaan = b.kode_perusahaan And x.serial_number = b.serial_number_akhir),'-') as Qr_Akhir "

            SQL = SQL & "From emi_tf_material_to_material a, emi_tf_material_to_material_detail b, barang_sn c Where "
            SQL = SQL & "a.kode_perusahaan = b.kode_perusahaan And a.kode_transfer = b.kode_transfer And "
            SQL = SQL & "b.kode_perusahaan = c.kode_perusahaan And b.serial_number_awal = c.serial_number and "
            SQL = SQL & "a.kode_perusahaan = '" & KodePerusahaan & "' and a.kode_transfer = '" & Lv_Stock.FocusedItem.Text & "' "
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    Dim lv As New ListViewItem

                    lv = Lv_Stock_Detail.Items.Add(Dr("kode_transfer"))
                    lv.SubItems.Add(Dr("kode_barang_awal"))
                    lv.SubItems.Add(Dr("Qr"))
                    lv.SubItems.Add(Format(Dr("jumlah_barang"), "N2") + " " + Dr("satuan_barang"))
                    lv.SubItems.Add(Format(Dr("jumlah_bags"), "N2"))

                    lv.SubItems.Add(Dr("kode_barang_tujuan"))
                    If General_Class.CekNULL(Dr("flag_validasi")) = "Y" Then
                        lv.BackColor = Color.LightGreen
                        lv.SubItems.Add(Dr("Qr_Akhir"))
                        lv.SubItems.Add(Format(Dr("jumlah_akhir"), "N2") + " " + Dr("satuan_barang"))
                    Else
    ...
    Private Sub BtnBarangMasuk_Cari_Click(sender As Object, e As EventArgs) Handles BtnBarangMasuk_Cari.Click
        ...
        Try
            ...
            SQL = "select a.kode_transfer, a.Keterangan, a.Tanggal, a.jam, a.userid, "
            SQL = SQL & "a.Kode_Stock_Owner, a.total, a.Satuan, a.total_bags, a.status "
            SQL = SQL & "from emi_tf_material_to_material a where "
    ...
    Private Sub BatalPengeluaranBahanBakarToolStripMenuItem_Click(sender As Object, e As EventArgs)
    ...
    Private Sub CetakUlangBarodeToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CetakUlangBarodeToolStripMenuItem.Click
        If Lv_Stock_Detail.Items.Count = 0 Then Exit Sub

        Dim selectedIndex As Integer = Lv_Stock_Detail.FocusedItem.Index
        Dim QrCode As String = Lv_Stock_Detail.Items(selectedIndex).SubItems(6).Text
        Dim KdTransfer As String = Lv_Stock_Detail.FocusedItem.Text
        ...
        Try
            ...
            '======================================
            SQL = "select Kode_Perusahaan from EMI_TF_Material_To_Material "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Transfer = '" & KdTransfer & "' and status = 'Y' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    Dr.Close()
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show("Cetak Ulang Barcode Tidak dapat Dilakukan karena No Faktur ini Sudah Dibatalkan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using
            ...
            If QrCode = "-" Then
                MessageBox.Show("Data Belum di validasi . .!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                CloseConn()
                Exit Sub
            End If
            ...
            Using Dr = OpenTrans(SQL)
                ...
                Else
                    Dr.Close()
                    MessageBox.Show("Data QR tidak ditemukan!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    CloseConn()
                    Exit Sub
                End If
            End Using
        ...
        Try
            ...
            SQL = SQL & "from Cetak_MaterialtoMaterial a "
            SQL = SQL & "where a.Kode_Perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "and a.Kode_Unik_Print = '" & kode_unik_print & "' "
            Using Ds = BindingTrans(SQL)
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    CrDoc = New N_EMI_CR_TF_Material_To_Material_Barcode

                    'With A_Place_For_Printing2
                    '    CrDoc.SetDataSource(Ds)
                    '    CrDoc.SetDatabaseLogon(CUserId, CPassword, CServer, CDatabase)
                    '    CrDoc.PrintOptions.PrinterName = ""
                    '    CrDoc.RecordSelectionFormula = "{Cetak_MaterialtoMaterial.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Cetak_MaterialtoMaterial.kode_unik_print} = '" & kode_unik_print & "'"
                    ...
                    '        CloseConn()
                    '        MessageBox.Show("Kertas Tidak DiTemukan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    '        Exit Sub
                    '    End If
    ...
    Private Sub BatalTransferMaterialToMaterialToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles BatalTransferMaterialToMaterialToolStripMenuItem.Click
        If Lv_Stock.Items.Count = 0 Or Lv_Stock.FocusedItem Is Nothing Then Exit Sub
        ...
        Try
            ...
            Dim JudulNotif As String = "Pembatalan Material To Material"
            Dim NoFaktur As String = Lv_Stock.FocusedItem.Text

            '====================
            '=     CEK ROLE     =
            '====================
            If CekButtonRole("Batal_Material_To_Material") = "T" Then
                CloseTrans()
                CloseConn()
            ...
            '========================================
            SQL = "select Kode_Perusahaan from EMI_TF_Material_To_Material "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Transfer = '" & NoFaktur & "' and status = 'Y' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    Dr.Close()
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show(JudulNotif & " Tidak dapat Dilakukan karena No Faktur ini Sudah Dibatalkan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using
            ...
            '========================================================
            SQL = "select a.Kode_Perusahaan from EMI_TF_Material_To_Material a, EMI_TF_Material_To_Material_Detail b "
            SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan and a.Kode_Transfer = b.Kode_Transfer "
            SQL = SQL & "and a.Status is null "
            SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' and a.Kode_Transfer = '" & NoFaktur & "' and b.Flag_Validasi is null "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    Dr.Close()
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show(JudulNotif & " Tidak dapat Dilakukan karena Barang Belum Divalidasi Sepenuhnya", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using
            ...
            '============================================
            SQL = "select a.Kode_Perusahaan, a.Kode_Transfer, a.Kode_Stock_Owner, a.Kode_Barang_Tujuan, b.Jumlah_Barang, b.Satuan_Barang, b.Serial_Number_Akhir, "
            SQL = SQL & "isnull((( (select Jumlah_Barang_Akhir from EMI_Master_Flever z where z.Kode_Perusahaan = a.Kode_Perusahaan and z.Kode_Barang_Min = a.Kode_Barang_Awal and z.Kode_Barang_Plus = a.Kode_Barang_Tujuan) / "
            SQL = SQL & "(select Jumlah_Barang_Awal from EMI_Master_Flever z where z.Kode_Perusahaan = a.Kode_Perusahaan and z.Kode_Barang_Min = a.Kode_Barang_Awal and z.Kode_Barang_Plus = a.Kode_Barang_Tujuan) "
            SQL = SQL & ") * b.Jumlah_Barang ), 0) as Jumlah_Transfer, "
            SQL = SQL & "isnull((select z.Jumlah from Barang_SN z where b.kode_perusahaan = z.kode_perusahaan and a.Kode_Stock_Owner = z.Kode_Stock_Owner and a.Kode_Barang_Tujuan = z.Kode_Barang and b.Serial_Number_Akhir = z.Serial_Number), 0) as Jumlah_SN, "
            SQL = SQL & "b.Jumlah_Bags, isnull((select z.Jumlah_Bags from Barang_SN z where b.kode_perusahaan = z.kode_perusahaan and a.Kode_Stock_Owner = z.Kode_Stock_Owner and a.Kode_Barang_Tujuan = z.Kode_Barang and b.Serial_Number_Akhir = z.Serial_Number), 0) as Jumlah_Bags_SN "
            ...
            SQL = SQL & "and a.Status is null "
            SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "'  "
            SQL = SQL & "and a.Kode_Transfer = '" & NoFaktur & "' "
            SQL = SQL & "and b.Flag_Validasi = 'Y' "
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        For i As Integer = 0 To .Rows.Count - 1
                            If .Rows(i).Item("Jumlah_Transfer") <> .Rows(i).Item("Jumlah_SN") Or .Rows(i).Item("Jumlah_Bags") <> .Rows(i).Item("Jumlah_Bags_SN") Then
                                CloseTrans()
                                CloseConn()
            ...
            '========================================
            Dim HasData As Boolean = False
            Dim TglTranfer As DateTime
            SQL = "select Tanggal from EMI_TF_Material_To_Material where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Transfer = '" & NoFaktur & "' and status is null "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    HasData = True
                    TglTranfer = Dr("Tanggal")
                Else
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show(JudulNotif & " Tidak dapat Dilakukan karena Data Ditemukan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using

            If HasData Then
                If CekSudahTutupSaldo(TglTranfer) = "Y" Then
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show(JudulNotif & " Tidak dapat Dilakukan karena Data Sudah Tutup Saldo", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End If
            ...
            '=     CEK BULAN     =
            '=====================
            If Not tgl_skg.Month = TglTranfer.Month Then
                CloseTrans()
                CloseConn()
            ...
            '=     ROLLBACK DATA     =
            '=========================
            SQL = "select a.Kode_Transfer, a.Kode_Stock_Owner, a.Kode_Barang_Awal, a.Kode_Barang_Tujuan, b.Serial_Number_Awal, b.Serial_Number_Akhir, b.Jumlah_Barang, "
            SQL = SQL & "isnull((( (select Jumlah_Barang_Akhir from EMI_Master_Flever z where z.Kode_Perusahaan = a.Kode_Perusahaan and z.Kode_Barang_Min = a.Kode_Barang_Awal and z.Kode_Barang_Plus = a.Kode_Barang_Tujuan) / "
            SQL = SQL & "(select Jumlah_Barang_Awal from EMI_Master_Flever z where z.Kode_Perusahaan = a.Kode_Perusahaan and z.Kode_Barang_Min = a.Kode_Barang_Awal and z.Kode_Barang_Plus = a.Kode_Barang_Tujuan) "
            SQL = SQL & ") * b.Jumlah_Barang ), 0) as Jumlah_Transfer, b.Jumlah_Bags, b.Satuan_Barang, b.Kode_Voucher "
            SQL = SQL & "from EMI_TF_Material_To_Material a, EMI_TF_Material_To_Material_Detail b "
            SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan "
            SQL = SQL & "and a.Kode_Transfer = b.Kode_Transfer "
            SQL = SQL & "and a.status is null "
                ...
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            Dim KdBarang_Awal As String = .Rows(i).Item("Kode_Barang_Awal")
                            Dim KdBarang_Tujuan As String = .Rows(i).Item("Kode_Barang_Tujuan")
                            Dim SN_Awal As String = .Rows(i).Item("Serial_Number_Awal")
                            Dim SN_Akhir As String = .Rows(i).Item("Serial_Number_Akhir")
                            Dim JumlahKecil As Double = .Rows(i).Item("Jumlah_Barang")
                            Dim JumlahTransfer As Double = .Rows(i).Item("Jumlah_Transfer")
                            Dim JumlahBags As Double = .Rows(i).Item("Jumlah_Bags")
                            Dim SatuanBarang As String = .Rows(i).Item("Satuan_Barang")
                            ...
                            SQL = SQL & "and Kode_Barang = '" & KdBarang_Tujuan & "' and Serial_Number = '" & SN_Akhir & "' "
                            Using Ds1 = BindingTrans(SQL)
                                If Ds1.Tables("MyTable").Rows.Count <> 0 Then

                                    If Val(HilangkanTanda(Ds1.Tables("MyTable").Rows(0).Item("Jumlah"))) < JumlahTransfer Or Val(HilangkanTanda(Ds1.Tables("MyTable").Rows(0).Item("Jumlah_Bags"))) < JumlahBags Then
                                        CloseTrans()
                                        CloseConn()
                                        MessageBox.Show("Terjadi Kesahalaan saat Rollback Barang", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                        Exit Sub
                                    Else

                                        SQL = "update Barang_SN set Jumlah = Jumlah - " & JumlahTransfer & ", Jumlah_Bags = Jumlah_Bags - " & JumlahBags & " "
                                        SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Stock_Owner = '" & KdSo & "' and Kode_Barang = '" & KdBarang_Tujuan & "' "
                                        SQL = SQL & "and Serial_Number = '" & SN_Akhir & "'"
                                        ExecuteTrans(SQL)
                                ...
                                Else
                                    CloseTrans()
                                    CloseConn()
                                    MessageBox.Show("Kode Barang Sn Tidak Ditemukan", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                    Exit Sub
                                End If
                            End Using
                            ...
                            SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' and kode_Stock_Owner = '" & KdSo & "'  "
                            SQL = SQL & "and kode_barang = '" & KdBarang_Tujuan & "' "
                            Using Ds1 = BindingTrans(SQL)
                                If Ds1.Tables("MyTable").Rows.Count <> 0 Then

                                    If Val(HilangkanTanda(Ds1.Tables("MyTable").Rows(0).Item("Good_Stock"))) < JumlahTransfer Or Val(HilangkanTanda(Ds1.Tables("MyTable").Rows(0).Item("Jumlah_Bags"))) < JumlahBags Then
                                        CloseTrans()
                                        CloseConn()
                                        MessageBox.Show("Terjadi Kesahalaan saat Rollback Barang", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                        Exit Sub
                                    Else

                                        SQL = "update Barang set Good_Stock = Good_Stock - " & JumlahTransfer & ", Jumlah_Bags = Jumlah_Bags - " & JumlahBags & " "
                                        SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Stock_Owner = '" & KdSo & "' and Kode_Barang = '" & KdBarang_Tujuan & "' "
                                        ExecuteTrans(SQL)
                                ...
                                Else
                                    CloseTrans()
                                    CloseConn()
                                    MessageBox.Show("Kode Barang Tidak Ditemukan", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                    Exit Sub
                                End If
                            End Using
                            ...
                            SQL = SQL & "where a.kode_Barang=y.kode_Barang and a.Kode_Stock_Owner=y.kode_Stock_Owner and a.kode_Perusahaan=y.kode_Perusahaan ), 0) as jumlah_bags_sn "
                            SQL = SQL & "FROM barang a WHERE a.Kode_Stock_Owner = '" & KdSo & "' "
                            SQL = SQL & "AND a.Kode_Barang = '" & KdBarang_Tujuan & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
                            SQL = SQL & "group by a.kode_Barang, a.Kode_Stock_Owner, a.kode_Perusahaan "
                            Using Ds2 = BindingTrans(SQL)
                                If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                                    If Ds2.Tables("MyTable").Rows(0).Item("good_stock") <> Ds2.Tables("MyTable").Rows(0).Item("Jumlah_sn") Or Ds2.Tables("MyTable").Rows(0).Item("jumlah_bags_barang") <> Ds2.Tables("MyTable").Rows(0).Item("jumlah_bags_sn") Then
                                        CloseTrans()
                                        CloseConn()
                                        MessageBox.Show("Terjadi Kesalahan . . ! !", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                        Exit Sub
                                    End If
                                Else
                                    CloseTrans()
                                    CloseConn()
                                    MessageBox.Show("Data tidak ditemukan . . ! !", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                    Exit Sub
                                End If
                            End Using
                            ...
                            SQL = "select Jumlah, Jumlah_Bags from Barang_SN "
                            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Stock_Owner = '" & KdSo & "' "
                            SQL = SQL & "and Kode_Barang = '" & KdBarang_Awal & "' and Serial_Number = '" & SN_Awal & "' "
                            Using Ds1 = BindingTrans(SQL)
                                If Ds1.Tables("MyTable").Rows.Count <> 0 Then

                                    SQL = "update Barang_SN set Jumlah = Jumlah + " & JumlahKecil & ", Jumlah_Bags = Jumlah_Bags + " & JumlahBags & " "
                                    SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Stock_Owner = '" & KdSo & "' and Kode_Barang = '" & KdBarang_Awal & "' "
                            ...
                            SQL = "select Good_Stock, Jumlah_Bags from Barang "
                            SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' and kode_Stock_Owner = '" & KdSo & "'  "
                            SQL = SQL & "and kode_barang = '" & KdBarang_Awal & "' "
                            Using Ds1 = BindingTrans(SQL)
                                If Ds1.Tables("MyTable").Rows.Count <> 0 Then
                            ...
                            SQL = SQL & "where a.kode_Barang=y.kode_Barang and a.Kode_Stock_Owner=y.kode_Stock_Owner and a.kode_Perusahaan=y.kode_Perusahaan ), 0) as jumlah_bags_sn "
                            SQL = SQL & "FROM barang a WHERE a.Kode_Stock_Owner = '" & KdSo & "' "
                            SQL = SQL & "AND a.Kode_Barang = '" & KdBarang_Awal & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
                            SQL = SQL & "group by a.kode_Barang, a.Kode_Stock_Owner, a.kode_Perusahaan "
                            Using Ds2 = BindingTrans(SQL)
                                If Ds2.Tables("MyTable").Rows.Count <> 0 Then
                                    If Ds2.Tables("MyTable").Rows(0).Item("good_stock") <> Ds2.Tables("MyTable").Rows(0).Item("Jumlah_sn") Or Ds2.Tables("MyTable").Rows(0).Item("jumlah_bags_barang") <> Ds2.Tables("MyTable").Rows(0).Item("jumlah_bags_sn") Then
                            ...
                            Using Ds1 = BindingTrans(SQL)
                                ...
                                Else
                                    CloseTrans()
                                    CloseConn()
                                    MessageBox.Show("Data Jurnal Tidak Ditemukan . . ! !", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                    Exit Sub
                                End If
                            End Using
                    ...
                    Else
                        CloseTrans()
                        CloseConn()
                        MessageBox.Show("Data Transfer Barang Tidak Ditemukan", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    End If
                End With
            End Using
            ...
            '=========================================
            '=     UPDATE TF MATERIAL 2 MATERIAL     =

            SQL = "select Kode_Perusahaan from EMI_TF_Material_To_Material "
            SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Transfer = '" & NoFaktur & "' and status is null "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then

                    Dr.Close()
                    SQL = "update EMI_TF_Material_To_Material set Status = 'Y', UserID_Batal = '" & UserID & "', Tanggal_Batal = '" & Format(tgl_skg, "yyyy-MM-dd") & "', Jam_Batal = '" & Format(tgl_skg, "HH:mm:ss") & "' "
                    SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' and Kode_Transfer = '" & NoFaktur & "' and status is null"
                    ExecuteTrans(SQL)
                Else
                    CloseTrans()
                    CloseConn()
                    MessageBox.Show("Kode Transfer Material To Material Tidak Ditemukan . . ! !", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using
            ...
            CloseTrans()
            CloseConn()
            MessageBox.Show(JudulNotif & " Berhasil Dibatalkan", JudulNotif, MessageBoxButtons.OK, MessageBoxIcon.Information)
        Catch ex As Exception
            CloseTrans()
            CloseConn()
            MessageBox.Show(ex.Message)

Public Class N_EMI_Transaksi_Transfer_Stock_Sementara_Timbang
    ...
    Private scanStart As DateTime
    Private scanEnd As DateTime
    Private inputCount As Integer = 0
    Private isValidScan As Boolean = False
    ...
    Private Sub Popup_Timbang_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Lv_List_Barang.Columns.Add("Kode Transfer", 0, HorizontalAlignment.Left).DisplayIndex = 0 '0
            '  Lv_List_Barang.Columns.Add(Base_Language.Lang_Global_Lokasi, 130, HorizontalAlignment.Left) '1
            Lv_List_Barang.Columns.Add("SO Awal", 140, HorizontalAlignment.Left) '1
            Lv_List_Barang.Columns.Add("SO Akhir", 0, HorizontalAlignment.Left) '2
            Lv_List_Barang.Columns.Add(Base_Language.Lang_Global_KodeBarang, 140, HorizontalAlignment.Left) '3
            Lv_List_Barang.Columns.Add(Base_Language.Lang_Global_NamaBarang, 0, HorizontalAlignment.Left) '4
            Lv_List_Barang.Columns.Add("Total Input", 130, HorizontalAlignment.Right) '7
            Lv_List_Barang.Columns.Add("Satuan Input", 100, HorizontalAlignment.Center) '8
            Lv_List_Barang.Columns.Add("Total", 130, HorizontalAlignment.Right) '5
            Lv_List_Barang.Columns.Add(Base_Language.Lang_Global_Satuan, 100, HorizontalAlignment.Center) '6
            Lv_List_Barang.Columns.Add("Lokasi RAK", 180, HorizontalAlignment.Left) '9
            Lv_List_Barang.Columns.Add("barangSn", 0, HorizontalAlignment.Left) '10
            Lv_List_Barang.Columns.Add("Barcode ", 250, HorizontalAlignment.Left) '11
            Lv_List_Barang.View = View.Details

            Lv_List_Barang.Columns(11).DisplayIndex = 2
    ...
    Private Sub Btn_TimbangFloorScale_Click(sender As Object, e As EventArgs) Handles Btn_TimbangFloorScale.Click
        ...
        Dim Data_Barcode As New List(Of (No_Faktur As String, Serial_Number As String, Keterangan As String))
        ...
        Try
            ...
            SQL = "select a.No_Faktur, c.Serial_Number_Awal, d.Blok_SN, a.Keterangan "
            SQL &= $"from N_EMI_Transaksi_Transfer_Stock_Sementara a "
            SQL &= $"inner join N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b on a.Kode_Perusahaan = b.Kode_Perusahaan and a.No_Faktur = b.No_Faktur and b.Flag_Timbang = 'Y' "
            SQL &= $"inner join N_EMI_Transaksi_Transfer_Stock_Sementara_Det c on b.Kode_Perusahaan = c.Kode_Perusahaan and b.No_Faktur = c.No_Faktur and b.Urut_Oto = c.Urut_TF and c.selesai is null "
            SQL &= $"inner join barang_sn d on c.Kode_Perusahaan = d.Kode_Perusahaan and a.SO_Awal = d.Kode_Stock_Owner and b.Kode_Barang = d.Kode_Barang and c.Serial_Number_Awal = d.Serial_Number "
            SQL &= $"where a.Kode_Perusahaan = '{KodePerusahaan}' "
            SQL &= $"and a.status is NULL "
            SQL &= $"and (d.Qr_Code+'-'+d.Kode_Unik_Berjalan) = '{Txt_ScanBarcode.Text}' "
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    ...
                    Data_Barcode.Add((No_Faktur:=Dr("No_Faktur"), Serial_Number:=Dr("Serial_Number_Awal"), Keterangan:=Dr("Keterangan")))
        ...
        '--------------------------------------------------------------- 
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.kosong()
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.CmbJenisTimbang.SelectedItem = "TRANSFER STOCK"
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtBarcode.Text = Txt_ScanBarcode.Text

        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txtUrutOto.Text = GetDataUrutOto
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txtKodeTransfer.Text = GetDataKodeTransfer
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtKdBarang.Text = GetDataKdBrg
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_lokasi.Text = GetDataLokasi
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_barang.Text = GetDataNmBrg
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_Barang_SN.Text = GetDataBrgSN
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_Jml_Estimasi.Text = GetDataJmlEstimasi
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtJumlahBags.Text = GetDataJumlahBags
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtBeratBags.Text = GetDataBeratBags & " " & GetDataSatuanBeratBags
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Txt_Berat_Bags_Bersih.Text = GetDataBeratBags
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtSatuan_FloorScale.Text = GetDataSatuanBeratBesar  

        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Txt_SatuanKecil.Text = GetDataSatuanKecil
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Txt_Sisa_Request.Text = SisaRequest

        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.arrSelectedSN.Clear()
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.arrSelectedFaktur.Clear()
        Dim cmb = N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Cmb_No_Faktur
        cmb.BeginUpdate()
        cmb.Items.Clear()
        For Each item In Data_Barcode
            cmb.Items.Add($"{item.No_Faktur} - {item.Keterangan}")
            N_EMI_SD_Transaksi_Transfer_Stock_Timbang.arrSelectedSN.Add(item.Serial_Number)
            N_EMI_SD_Transaksi_Transfer_Stock_Timbang.arrSelectedFaktur.Add(item.No_Faktur)
        Next
        cmb.EndUpdate()
        ...
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Cmb_No_Faktur.SelectedIndex = -1
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Cmb_No_Faktur_SelectedIndexChanged(e, New EventArgs)
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Btn_Refresh.Visible = False
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.UNIX.Visible = False
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.GetSisaTransfer()
        'N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Cmb_No_Faktur.Focus()
        N_EMI_SD_Transaksi_Transfer_Stock_Timbang.ShowDialog()
    ...
    Private Sub get_transfer_stock()
        Try
            ...
            SQL = "Select a.so_awal, b.kode_Barang, sum(c.jumlah) as jumlah, b.satuan, b.Satuan_Barang,  "
            SQL = SQL & "isnull((select x.Labeling_WMS_Position from View_Warehouse_Position x where "
            SQL = SQL & "x.Kode_Perusahaan = a.Kode_Perusahaan And x.Id_WMS_Warehouse_Position = c.Id_Wms_Awal), null) As Rak_Awal, "
            SQL = SQL & "isnull(( select case when r.Flag_Dasar IS NULL THEN (sum(c.Jumlah) / r.Nilai) "
            SQL = SQL & "else (sum(c.Jumlah) * isnull(z.Nilai, 1)) end as Hasil "
            SQL = SQL & "from N_EMI_Master_Satuan r "
            SQL = SQL & "left join N_EMI_Master_Satuan z ON r.Kode_Perusahaan = z.Kode_Perusahaan and r.Kode_Barang = z.Kode_Barang and z.Flag_Dasar = 'Y' "
            SQL = SQL & "where r.Kode_Perusahaan = a.Kode_Perusahaan and r.Kode_Barang = b.Kode_Barang and r.Satuan = c.Satuan_Input "
            SQL = SQL & "), 0) as Jumlah_Input, isnull(c.Satuan_Input, '-') as Satuan_Input, "
            SQL = SQL & "(e.Qr_Code+'-'+e.Kode_Unik_Berjalan) as Barcode "
            SQL = SQL & "From N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, barang d, Barang_SN e "
            ...
            SQL = SQL & "And a.so_awal=d.kode_stock_Owner "
            SQL = SQL & "And b.kode_Perusahaan=d.Kode_Perusahaan "
            SQL = SQL & "And a.status Is null And b.Flag_Timbang ='Y' and c.selesai is null "
            SQL = SQL & "and a.Kode_Perusahaan = e.Kode_Perusahaan and a.SO_Awal = e.Kode_Stock_Owner and b.Kode_Barang = e.Kode_Barang and c.Serial_Number_Awal = e.Serial_Number "
            SQL = SQL & "group by a.so_awal, b.kode_Barang, d.nama, b.satuan, b.Satuan_Barang, a.kode_perusahaan, c.satuan_input, c.Id_Wms_Awal, (e.Qr_Code+'-'+e.Kode_Unik_Berjalan) "
            SQL = SQL & "order by a.SO_Awal, b.Kode_Barang "
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    Dim Lvw As ListViewItem

                    Lvw = Lv_List_Barang.Items.Add("X")
    ...
    Private Sub ListView2_DoubleClick(sender As Object, e As EventArgs) Handles Lv_List_Barang.DoubleClick
        ...
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txtKodeTransfer.Text = LvKodeTransfer
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_lokasi.Text = LvSoAwal
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_barang.Text = LvNamaBarang
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.TxtKdBarang.Text = LvKodeBarang
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_Barang_SN.Text = LvSn
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.txt_Jml_Estimasi.Text = LvTotal
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.Txt_SatuanKecil.Text = LvSatuanBarang
        ''N_EMI_SD_Transaksi_Transfer_Stock_Timbang.CmbJenisTimbang.SelectedItem = "TRANSFER STOCK"
...
#End Region
    ...
    Private Sub Txt_ScanBarcode_KeyDown(sender As Object, e As KeyEventArgs) Handles Txt_ScanBarcode.KeyDown
        'If e.Control OrElse e.Shift OrElse e.Alt Then
        '    e.SuppressKeyPress = True
        'End If
    End Sub

    Private Sub Txt_ScanBarcode_KeyUp(sender As Object, e As KeyEventArgs) Handles Txt_ScanBarcode.KeyUp
        If e.KeyCode = Keys.Enter Then
            scanEnd = DateTime.Now

            Dim duration = (scanEnd - scanStart).TotalMilliseconds

            Btn_TimbangFloorScale_Click(Me, Nothing)


            'If duration < 200 AndAlso inputCount >= 6 Then
            '    isValidScan = True
            '    'MessageBox.Show("SCAN VALID: " & Txt_ScanBarcode.Text)

            '    If Txt_ScanBarcode.Text.Trim.Length <> 0 Then
            '        'Btn_TimbangFloorScale_Click(Me, Nothing)
            '    End If
            'Else
            '    Txt_ScanBarcode.Clear()
            'End If

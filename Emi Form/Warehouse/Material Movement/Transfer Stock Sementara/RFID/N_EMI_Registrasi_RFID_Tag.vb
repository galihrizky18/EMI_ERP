...
Public Class N_EMI_Registrasi_RFID_Tag
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
       ...
       String.IsNullOrWhiteSpace(GeneratedLabel) Then
        ...
        Try
            ...
            'Qr_Code.Image = Generate_QR(Txt_RFIDTag.Text.Trim)
            'Dim Qr_CodePath As String = System.IO.Path.Combine(My.Computer.FileSystem.SpecialDirectories.Temp, "RFIDTag" & Txt_RFIDTag.Text.Trim & ".jpg")
            'Qr_Code.Image.Save(Qr_CodePath, System.Drawing.Imaging.ImageFormat.Jpeg)
            'Dim Qr_CodeFileStream As FileStream = New FileStream(Qr_CodePath, FileMode.Open, FileAccess.Read)
            'Dim QR_CodeFileSize As Integer = Qr_CodeFileStream.Length
            'Dim QR_CodeRawData As Byte() = New Byte(QR_CodeFileSize) {}
            'Qr_CodeFileStream.Read(QR_CodeRawData, 0, QR_CodeFileSize)
            'Qr_CodeFileStream.Close()
            ...
            Cmd.Parameters.Add("@Tanggal", SqlDbType.Date).Value = Date.Today
            Cmd.Parameters.Add("@Jam", SqlDbType.Char, 8).Value = Date.Now.ToString("HH:mm:ss")
            Cmd.CommandText = SQL
            Cmd.ExecuteNonQuery()
    ...
    Public Function Generate_QRRFIDTag(ByVal isi As String) As Bitmap
        ...
        Dim result As Bitmap = qr.Write(isi)
    ...
    Private Sub CetakRFID(RFIDTag As String)
        Try
            ...
            Using Ds = BindingTrans(SQL)
                ...
                'Dim frm As New A_Place_For_Printing()
                'frm.CrystalReportViewer1.ReportSource = CrDoc

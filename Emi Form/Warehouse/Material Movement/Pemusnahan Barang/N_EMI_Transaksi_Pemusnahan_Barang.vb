...
Public Class N_EMI_Transaksi_Pemusnahan_Barang
    ...
    Private Sub get_grid_view(ByVal index As Integer)
        ...
        dgv_Jumlah = If(General_Class.CekNULL(DGV_Data_TF.Rows(index).Cells(itemDgvJumlah).Value) = "", "0", Val(HilangkanTanda(DGV_Data_TF.Rows(index).Cells(itemDgvJumlah).Value)))
        ...
        dgv_IsiPerBags = If(General_Class.CekNULL(DGV_Data_TF.Rows(index).Cells(itemDGVIsiPerBags).Value) = "", "0", Val(HilangkanTanda(DGV_Data_TF.Rows(index).Cells(itemDGVIsiPerBags).Value)))
    ...
    Public Sub kosong()
        ...
        Try
            ...
            SQL = SQL & "kode_perusahaan = '" & KodePerusahaan & "' and aktif = 'Y' and flag_waste='Y' "
    ...
    Private Sub DGV_Data_TF_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellClick
        ...
        Try
            ...
            SQL = "select Blok_SN from Barang_SN where Kode_Perusahaan = '" & KodePerusahaan & "' "
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    If General_Class.CekNULL(Dr("Blok_SN")) = "Y" Then
                        DGV_Data_TF.CurrentRow.DefaultCellStyle.BackColor = Color.DarkRed
                        DGV_Data_TF.CurrentRow.DefaultCellStyle.ForeColor = Color.White
                    Else
                        DGV_Data_TF.Rows(currentRow).DefaultCellStyle.BackColor = Color.White
                        DGV_Data_TF.Rows(currentRow).DefaultCellStyle.ForeColor = Color.Black
                    End If
                End If
            End Using
    ...
    Private Sub DGV_Data_TF_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellEndEdit
        ...
        If DGV_Data_TF.CurrentRow.Cells(itemDgvCheckBox).Value = "True" Then
            ...
            Try
                ...
                SQL = SQL & "and c.Serial_Number_Awal ='" & DGV_Data_TF.CurrentRow.Cells(itemDgvSerialNumber).Value & "' "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        Dr.Close()
                        CloseConn()
                        Me.BeginInvoke(New MethodInvoker(Sub()
                                                             MessageBox.Show("Barang Sn belum di timbang", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                                         End Sub))
                ...
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        If General_Class.CekNULL(Dr("Blok_SN")) = "Y" Then
                            ...
                            DGV_Data_TF.CurrentRow.DefaultCellStyle.ForeColor = Color.White
                            Me.BeginInvoke(New MethodInvoker(Sub()
                                                                 MessageBox.Show("Data Tidak Bisa Digunakan karena SN telahg Di Blok . . ! !", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                                             End Sub))
        ...
        Else
            ...
            DGV_Data_TF.CurrentRow.DefaultCellStyle.ForeColor = Color.Black
            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = ""
            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = ""
    ...
    Private Sub TxtKd_Barang_TextChanged(sender As Object, e As EventArgs) Handles TxtKd_Barang.TextChanged, Txt_SO.TextChanged, Txt_SatuanPermintaan.TextChanged, Txt_JumlahPermintaan.TextChanged, Txt_OtoMaterial_req.TextChanged
        If asal <> "Emi_Display_Request_Material" Then
            ...
            If Not TxtKd_Barang.Text.Trim.Count = 0 Then
                Lv_DetBarang.Location = New Point(37, 237)
                Lv_DetBarang.Visible = True
            Else
                Lv_DetBarang.Location = New Point(1278, 237)
        ...
        Try
            ...
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    ...
                    If General_Class.CekNULL(Dr("Blok_SN")) = "Y" Then
                        DGV_Data_TF.Rows(rows).DefaultCellStyle.BackColor = Color.DarkRed
                        DGV_Data_TF.Rows(rows).DefaultCellStyle.ForeColor = Color.White
                    End If
            ...
            For i As Integer = 0 To Dgv_DataRekap.Rows.Count - 1
                ...
                For j As Integer = 0 To Dgv_DataDetail.Rows.Count - 1
                    ...
                    If dgv_detail_lokasi = dgv_Rekap_lokasi And dgv_detail_KdBarang = dgv_Rekap_KdBarang Then
                            ...
                            With Ds.Tables("MyTable")
                                If .Rows.Count <> 0 Then
                                    ...
                                    For Index As Integer = 0 To .Rows.Count - 1
                                        ...
                                        SQL = "select dbo.ubah_satuan('" & KodePerusahaan & "', 'masa','" & dgv_detail_KdBarang & "', '" & dgv_Rekap_SatuanKecil & "',"
                                        SQL = SQL & "'" & dgv_Rekap_Satuan & "', '" & HilangkanTanda(JumlahInsert) & "' ) as hasil"
                                        Using Dr1 = OpenTrans(SQL)
                                            If Dr1.Read Then
                                                If General_Class.CekNULL(Dr1("hasil")) = "" Then
                                                    Dr1.Close()
                        ...
                        '=     BLOK SN     =
                        '===================
                        'SQL = "select kode_stock_owner from Barang_SN where kode_stock_owner = '" & arrSO(CmbSO_Asal.SelectedIndex) & "' "
                        'SQL = SQL & "and kode_barang = '" & dgv_detail_KdBarang & "' and serial_number = '" & dgv_detail_SN & "' "
                        'Using Ds1 = BindingTrans(SQL)
                        '    With Ds1.Tables("MyTable")
                        '        If .Rows.Count <> 0 Then

                        '            SQL = "update Barang_SN set Blok_SN = 'Y' where kode_stock_owner = '" & arrSO(CmbSO_Asal.SelectedIndex) & "' "
                        '            SQL = SQL & "and kode_barang = '" & dgv_detail_KdBarang & "' and serial_number = '" & dgv_detail_SN & "' "
                        '            ExecuteTrans(SQL)
        ...
        Try
            ...
            Using Ds = BindingTrans(SQL)
                If Ds.Tables("MyTable").Rows.Count <> 0 Then
                    ...
                    '    CrDoc.RecordSelectionFormula = "{N_EMI_View_Transfer_Waste.Kode_Perusahaan} = '" & KodePerusahaan & "' and {N_EMI_View_Transfer_Waste.No_Faktur}='" & Trim(TxtNo_Transaksi.Text) & "' "
    ...
    Private Sub Btn_SimpanSementara_Click(sender As Object, e As EventArgs) Handles Btn_SimpanSementara.Click
        ...
        Try
            ...
            For row As Integer = 0 To DGV_Data_TF.RowCount - 1
                ...
                SQL = SQL & "WHERE a.kode_Perusahaan ='" & KodePerusahaan & "' "
                SQL = SQL & "and a.Kode_Stock_Owner='" & arrSO(CmbSO_Asal.SelectedIndex) & "' "
                SQL = SQL & "group by a.id_wms_warehouse_position "
                ...
                Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_Jumlah).Value = Val(HilangkanTanda(dgv_Jumlah))
                Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JumlahBags).Value = Val(HilangkanTanda(dgv_JmlhBags))
                ...
                Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_IDWarehouseTujuan).Value = available_Id_Warehouse
        ...
        If hasDataSelected Then
            ...
            Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_Jumlah).Value = Val(HilangkanTanda(TxtTotalTransfer.Text)) & " " & TxtSatuan.Text
            Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_JumlahBags).Value = Val(HilangkanTanda(TxtTotalTransferBags.Text))
            Dgv_DataRekap.Rows(DgvTab2_Rows).Cells(itemDgvRekap_JumlahBersih).Value = Val(HilangkanTanda(TxtTotalTransfer.Text))
...
#End Region
    ...
    Private Sub TxtKd_Barang_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtKd_Barang.KeyPress
        If e.KeyChar = Chr(13) Then Lv_DetBarang.Focus()
    End Sub

...
Public Class Jf_Master_Karyawan2_Display
    ...
    Private Sub Kosong()
        Try
            ...
            SQL = SQL & "as flag from HRIS_Transaksi_Karyawan_Approval_new b "
            SQL = SQL & "where b.No_Faktur = dbo.HRIS_Transaksi_Karyawan.No_Faktur) as flag_app "
            ...
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    ...
                    Lvw.SubItems.Add(General_Class.CekNULL(dr("flag_app")))
                Loop
            End Using
        ...
        TxtGapokAju.Text = "" : TxtGapokAju.Enabled = False
        ...
        TxtTunjanganAju.Text = "" : TxtTunjanganAju.Enabled = False
    ...
    Private Sub TxtNoFaktur_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtNoFaktur.Leave
        ...
        Try
            ...
            SQL = "Select a.*,c.Range1 as RangeGP1,c.range2 as RangeGP2,d.range1 as RangeTJ1,d.range2 as RangeTJ2, e.Gaji_Pokok As GapokAju "
            ...
            SQL = SQL & "left outer join HRIS_Transaksi_Rekrutmen_Gaji as e on a.kode_perusahaan = e.kode_perusahaan and a.kode_calon = e.kode_calon "
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    ...
                    If IsDBNull(Dr("RangeGP1")) Then
                        TxtKodeCalon.Text = ""
                    Else
                        TxtKodeCalon.Text = Dr("Kode_Calon")
                    End If
                    ...
                    If IsDBNull(Dr("GapokAju")) Then
                        TxtGapok.Text = "0"
                    Else
                        TxtGapok.Text = Dr("GapokAju")
                    End If
                    ...
                    If IsDBNull(Dr("GapokAju")) Then
                        TxtGapokAju.Text = ""
                    Else
                        TxtGapokAju.Text = Format(Dr("GapokAju"), "N0")
                    End If
            ...
            'SQL = "select Level_Approval from HRIS_UserID_Approval_Karyawan where UserID_Approval = '" & UserID & "' "
            'Using Dr = OpenTrans(SQL)
            '    If Dr.Read Then
            ...
            'StrAda = "T"
            'SQL = "SELECT a.Kode_Komponen FROM HRIS_Transaksi_Karyawan_Tunjangan a "
            ...
            ''If StrAda = "Y" Then
            'SQL = "Select a.Kode_Komponen, b.Keterangan, a.Tunjangan, e.range1 as RangeTJ1, e.range2 as RangeTJ2, f.Tunjangan As TunjanganAju "
            ...
            'SQL = SQL & "Left Join HRIS_Transaksi_Rekrutmen_Tunjangan f ON a.Kode_Perusahaan = f.Kode_Perusahaan AND c.Kode_Calon = f.Kode_Calon AND e.Kode_Komponen = f.Kode_Komponen) "
            'SQL = SQL & "Where a.Kode_Perusahaan = '" & KodePerusahaan & "' and "
            'SQL = SQL & "a.No_Faktur = '" & TxtNoFaktur.Text.Trim & "'"
            ''Else
            ''    SQL = "Select a.Kode_Komponen, 0 As Tunjangan, b.Keterangan, e.range1 as RangeTJ1, e.range2 as RangeTJ2, a.Tunjangan As TunjanganAju "
            ...
            ''    SQL = SQL & "Left Join HRIS_Range_Tunjangan e on d.id_golongan_sub_golongan = e.id_golongan_sub_golongan and d.id_level = e.id_level and a.Kode_Komponen = e.Kode_Komponen) "
            ''    SQL = SQL & "Where a.Kode_Perusahaan = '" & KodePerusahaan & "' and "
            ''    SQL = SQL & "a.Kode_Calon = '" & TxtKodeCalon.Text.Trim & "'"
            ''End If
            'Using Ds = OpenTrans(SQL)
            '    Do While Ds.Read
            '        Dim Lvw As ListViewItem
            ...
            '        Lvw.SubItems.Add(Format(Ds("TunjanganAju"), "N0"))
            '    Loop
            'End Using
            'Call Proses_StrCari()
    ...
    Private Sub BtnSimpan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSimpan.Click
        ...
        Try
            ...
            Dim flag_ada As String = ""

            SQL = "select no_faktur from HRIS_Transaksi_Karyawan_Approval_New where kode_Perusahaan = '" & KodePerusahaan & "' and "
            SQL = SQL & "no_faktur = '" & TxtNoFaktur.Text & "'"
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    flag_ada = "Y"
                Else
            ...
            If flag_ada = "T" Then
                Dim hasil As Integer = Approval_Hierarchy("Approval Karyawan", idlevel.Item(CmbLevel.SelectedIndex), iddivisi.Item(CmbDivisi.SelectedIndex))
                ...
                Else
                    ...
                    For zz As Integer = 0 To Data_User_App.Count - 1
                        SQL = "INSERT INTO HRIS_Transaksi_Karyawan_Approval_New(Kode_Perusahaan,No_Faktur,"
                        SQL = SQL & "UserID_Approval, Level_Hierarchy) "
                        SQL = SQL & "VALUES('" & KodePerusahaan & "', '" & TxtNoFaktur.Text & "', "
                        SQL = SQL & "'" & Data_User_App.Item(zz) & "', '" & urutan & "')"
                        ExecuteTrans(SQL)
        ...
        Try
            ...
            'SQL = "select Level_Approval,UserID_Approval,Flag_Approval from HRIS_Transaksi_Karyawan_Approval where "
            'SQL = SQL & "No_Faktur = '" & TxtNoFaktur.Text & "' and UserID_Approval = '" & UserID & "' "
            ...
            'Using Dr = OpenTrans(SQL)
            '    If Dr.Read Then
            '        If Dr("UserID_Approval") = UserID And General_Class.CekNULL(Dr("Flag_Approval")) = "" Then
            '            If Dr("Level_Approval") = "1" Then
            '                Dr.Close()
            ...
            '            SQL = "update HRIS_Transaksi_Karyawan_Approval set Flag_Approval = 'Y',"
            '            SQL = SQL & "Tanggal_Approval = '" & Format(tgl_skg, "yyyy-MM-dd") & "',"
            '            SQL = SQL & "Jam_Approval = '" & Format(tgl_skg, "HH:mm:ss") & "' where "
            '            SQL = SQL & "Kode_Perusahaan = '" & KodePerusahaan & "' and No_Faktur = '" & TxtNoFaktur.Text & "' "
            '            SQL = SQL & "and UserID_Approval = '" & UserID & "' "
            '            ExecuteTrans(SQL)

            '        ElseIf Dr("UserID_Approval") = UserID And General_Class.CekNULL(Dr("Flag_Approval")) = "Y" Then
            '            Dr.Close()
            '            CloseTrans()
            '            CloseConn()
            ...
            'Using Dr = OpenTrans(SQL)
            '    Do While Dr.Read
            '        If General_Class.CekNULL(Dr("flag_approval")) = "Y" Then
            '            flag_simpan = "Y"
            '        Else
            '            flag_simpan = "T"
            '            Exit Do
            ...
            SQL = "select top(1) userid_approval, urut_oto from HRIS_Transaksi_Karyawan_Approval_New where kode_Perusahaan = '" & KodePerusahaan & "' and "
            SQL = SQL & "no_faktur = '" & TxtNoFaktur.Text & "' and flag_approval is null order by level_hierarchy asc"
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    If dr("userid_approval").ToString.ToUpper = UserID.ToUpper Then

                        SQL = "update HRIS_Transaksi_Karyawan_Approval_New set Flag_approval = 'Y', tanggal_approval = '" & Format(tgl_skg, "yyyy-MM-dd") & "', "
                        SQL = SQL & "jam_approval = '" & Format(tgl_skg, "HH:mm:ss") & "' where urut_oto = '" & dr("urut_oto") & "' "
                        dr.Close()
                        ExecuteTrans(SQL)
                    Else
                        dr.Close()
                        CloseConn()
                        CloseConnSQL()
                        MessageBox.Show("Anda tidak memiliki akses untuk melakukan approval transaksi ini!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    End If
                Else
                    dr.Close()
                    CloseConn()
                    CloseConnSQL()
                    MessageBox.Show("Approval tidak ditemukan!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Sub
                End If
            End Using

            'kalau sdh di approval semua
            Dim sudah_selesai_approval As String = ""
            SQL = "select top(1) userid_approval, urut_oto from HRIS_Transaksi_Karyawan_Approval_New where kode_Perusahaan = '" & KodePerusahaan & "' and "
            SQL = SQL & "no_faktur = '" & TxtNoFaktur.Text & "' and flag_approval is null order by level_hierarchy asc"
            Using dr = OpenTrans(SQL)
                If dr.Read Then
                    sudah_selesai_approval = "T"
                Else
                    sudah_selesai_approval = "Y"
                End If
            End Using

            If sudah_selesai_approval = "Y" Then
                ...
                '    SQL = "select count(Id) as id from HRIS_Transaksi_Karyawan_Approval where No_Faktur = '" & TxtNoFaktur.Text.Trim & "'"
                '    Using Dr = OpenTrans(SQL)
                '        If Dr.Read Then
                '            level = Dr("id") + 1
                '        End If
                ...
                '    SQL = "select a.UserID_Approval,b.ID_Level_Jabatan,b.ID_Divisi_Sub_Divisi,d.Keterangan,e.pin "
                '    SQL = SQL & "from HRIS_UserID_Approval_Karyawan a,Karyawan b,HRIS_Level_Jabatan c,HRIS_Level d,Users e "
                ...
                '    SQL = SQL & "and a.Jenis_Approval = 'Approval Rekrutmen' and a.UserID_Approval = e.UserID "
                ...
                '                    Dim fpin As String = .Rows(h).Item("pin")

                '                    SQLSQL = "select User_ID from HRIS_Transaksi_Karyawan_Approval where No_Faktur = '" & TxtNoFaktur.Text.Trim & "' "
                ...
                '                                        SQLSQL = "insert into HRIS_Transaksi_Karyawan_Approval(No_Faktur,User_ID,ID_Level,"
                '                                        SQLSQL = SQLSQL & "ID_Divisi,Level_Hierarchy,pin,kode_unik) values('" & TxtNoFaktur.Text.Trim & "',"
                '                                        SQLSQL = SQLSQL & "'" & fuser & "','" & LLevelJabatan.Item(CmbLevel.SelectedIndex) & "',"
                ...
                '                                        'SQLSQL = SQLSQL & "'" & fuser & "',"
                '                                        'SQLSQL = SQLSQL & "'" & level & "','" & Kd_unik2 & "','" & Kd_unik & "')"
                '                                        'ExecuteTransSQL(SQLSQL)
                ...
                For i As Integer = 0 To LvTunjangan.Items.Count - 1
                    SQL = "Insert into HRIS_Transaksi_Karyawan_Tunjangan(Kode_Perusahaan,No_Faktur,Kode_Karyawan,Kode_Komponen,Tunjangan) "
                    SQL = SQL & "Values('" & KodePerusahaan & "','" & TxtNoFaktur.Text & "','" & TxtKodeKaryawan.Text & "',"
                    SQL = SQL & "'" & LvTunjangan.Items(i).Text.ToUpper & "'," & HilangkanTanda(LvTunjangan.Items(i).SubItems(2).Text) & ")"
                    ExecuteTrans(SQL)
                Next i
                ...
                SQL = "Update HRIS_Transaksi_Karyawan Set Flag_OK = 'Y', Tanggal_OK = '" & Format(tgl_skg, "yyyy-MM-dd") & "' "
                SQL = SQL & "Where kode_perusahaan = '" & KodePerusahaan & "' and no_faktur = '" & TxtNoFaktur.Text & "'"
                ExecuteTrans(SQL)
                ...
                SQL = SQL & "no_induk_karyawan,id_perusahaan,pendidikan_terakhir,agama,"
                SQL = SQL & "kode_divisi,tgl_lahir,jenis_kelamin,bank,nama_rekening,"
                SQL = SQL & "id_level_jabatan,id_divisi_sub_divisi, kepala)"
                ...
                'insert ke tabel HRIS_Kirim_WA_Karyawan_Baru
                arrUserId.Clear()
                SQL = "select a.UserID,b.HP from Karyawan a,Users b "
                SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan and a.UserID = b.UserID and a.Flag_BOD = 'Y' "
                Using Ds = BindingTrans(SQL)
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For h As Integer = 0 To .Rows.Count - 1

                                arrUserId.Add(.Rows(h).Item("UserID"))

                                SQL = "INSERT INTO HRIS_Kirim_WA_Karyawan_Baru(Kode_Perusahaan,No_Faktur,User_ID,HP) VALUES("
                                SQL = SQL & "'" & KodePerusahaan & "','" & TxtNoFaktur.Text & "','" & .Rows(h).Item("UserID") & "',"
                                SQL = SQL & "'" & .Rows(h).Item("HP") & "')"
                                ExecuteTrans(SQL)
                            Next
                        End If
                    End With
                ...
                SQL = "select a.No_Faktur,a.Tanggal,a.Nama,c.Keterangan as level,d.Keterangan as jabatan,c.ID_Level,c.Level_Hierarchy,f.ID_Divisi,"
                SQL = SQL & "f.Keterangan as divisi,g.Keterangan as subdivisi,a.ID_Level_Jabatan,a.ID_Divisi_Sub_Divisi "
                SQL = SQL & "from HRIS_Transaksi_Karyawan a,HRIS_Level_Jabatan b,HRIS_Level c,HRIS_Jabatan d,"
                SQL = SQL & "HRIS_Divisi_Sub_Divisi e,HRIS_Divisi f,HRIS_Sub_Divisi g "
                SQL = SQL & "where a.ID_Level_Jabatan = b.ID_Level_Jabatan and b.ID_Level = c.ID_Level and b.ID_Jabatan = d.ID_Jabatan "
                SQL = SQL & "and a.ID_Divisi_Sub_Divisi = e.ID_Divisi_Sub_Divisi and e.ID_Divisi = f.ID_Divisi and e.ID_Sub_Divisi = g.ID_Sub_Divisi "
                SQL = SQL & "and a.Flag_Ok = 'Y' and a.No_Faktur = '" & TxtNoFaktur.Text & "' "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        idDivisi = Dr("ID_Divisi")
                    End If
                End Using

                SQL = "select k.UserID,e.HP from Karyawan k,HRIS_Level_Jabatan a,HRIS_Level b,"
                SQL = SQL & "HRIS_Divisi_Sub_Divisi c,HRIS_Divisi d,Users e "
                SQL = SQL & "where k.ID_Level_Jabatan = a.ID_Level_Jabatan and a.ID_Level = b.ID_Level "
                SQL = SQL & "and k.ID_Divisi_Sub_Divisi = c.ID_Divisi_Sub_Divisi and c.ID_Divisi = d.ID_Divisi "
                SQL = SQL & "and k.UserID = e.UserID and d.ID_Divisi = '" & idDivisi & "' "
                Using Ds = BindingTrans(SQL)
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For h As Integer = 0 To .Rows.Count - 1
                                Dim flag_user As String = "T"
                                For a As Integer = 0 To arrUserId.Count - 1
                                    If arrUserId.Item(a) = .Rows(h).Item("UserID") Then
                                        flag_user = "T"
                                        Exit For
                                    Else
                                        flag_user = "Y"
    ...
    Private Sub Kirim_Wa()
        Try
            ...
            SQL = "select No_Faktur,User_ID,HP from HRIS_Kirim_WA_Karyawan_Baru where Kode_Perusahaan = '" & KodePerusahaan & "' and  Flag_Notif is null "
            Using Ds = BindingTrans(SQL)
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        For h As Integer = 0 To .Rows.Count - 1
                            Dim Nnama As String = ""
                            Dim Ndivisi As String = ""
                            Dim Ndept As String = ""
                            Dim NLevel As String = ""
                            Dim idLevel_Hierarchy As String = ""
                            Dim idDivisi As String = ""
                            SQL = "select a.No_Faktur,a.Tanggal,a.Nama,c.Keterangan as level,d.Keterangan as jabatan,c.ID_Level,c.Level_Hierarchy,f.ID_Divisi,"
                            SQL = SQL & "f.Keterangan as divisi,g.Keterangan as subdivisi,a.ID_Level_Jabatan,a.ID_Divisi_Sub_Divisi "
                            SQL = SQL & "from HRIS_Transaksi_Karyawan a,HRIS_Level_Jabatan b,HRIS_Level c,HRIS_Jabatan d,"
                            ...
                            SQL = SQL & "where a.ID_Level_Jabatan = b.ID_Level_Jabatan and b.ID_Level = c.ID_Level and b.ID_Jabatan = d.ID_Jabatan "
                            SQL = SQL & "and a.ID_Divisi_Sub_Divisi = e.ID_Divisi_Sub_Divisi and e.ID_Divisi = f.ID_Divisi and e.ID_Sub_Divisi = g.ID_Sub_Divisi "
                            SQL = SQL & "and a.Flag_Ok = 'Y' and a.No_Faktur = '" & .Rows(h).Item("No_Faktur") & "' "
                            Using Dr = OpenTrans(SQL)
                                If Dr.Read Then
                                    Nnama = Dr("Nama")
                                    Ndivisi = Dr("divisi")
                                    Ndept = Dr("subdivisi")
                                    NLevel = Dr("level") & " - " & Dr("jabatan")
                                    idLevel_Hierarchy = Dr("Level_Hierarchy")
                                    idDivisi = Dr("ID_Divisi")

                                    Dim postdata As String = JSON_Notif_Karyawan_Baru(.Rows(h).Item("HP"), Nnama, Ndivisi, Ndept, NLevel)

                                    Dim WebRequest As HttpWebRequest
                                    WebRequest = HttpWebRequest.Create(Url_WA_Business)
                                    ...
                                    WebRequest.Headers.Add("Authorization", "Bearer " & Token_WA_Business)
                                    ...
                                    If responseFromServer.Contains("accepted") Then
                                        sukses = "Y"
                                    Else
                                        sukses = "T"
                                    End If
                                    ...
                                    If sukses = "Y" Then
                                        ...
                                        SQL = "update HRIS_Kirim_WA_Karyawan_Baru set Flag_Notif = 'Y' where "
                                        SQL = SQL & "Kode_Perusahaan = '" & KodePerusahaan & "' and "
                                        SQL = SQL & "No_Faktur = '" & .Rows(h).Item("No_Faktur") & "' and "
                                        SQL = SQL & "User_ID = '" & .Rows(h).Item("User_ID") & "' "
                                        ExecuteTrans(SQL)
                                        'Else
                                        '    SQL = "update HRIS_Kirim_WA_Karyawan_Baru set Flag_Notif = 'G' where "
                                        '    SQL = SQL & "Kode_Perusahaan = '" & KodePerusahaan & "' and "
                                        '    SQL = SQL & "No_Faktur = '" & .Rows(h).Item("No_Faktur") & "' and "
                                        '    SQL = SQL & "User_ID = '" & .Rows(h).Item("User_ID") & "' "
                                        '    ExecuteTrans(SQL)
    ...
    Private Sub TxtCari1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtCari1.TextChanged
        ...
        Try
            ...
            'SQL = "select a.Kode_Komponen, a.Keterangan , c.range1 as RangeTJ1,c.range2 as RangeTJ2, d.Tunjangan As TunjanganAju "
            ...
            'SQL = SQL & "inner Join HRIS_Transaksi_Rekrutmen_Tunjangan d ON a.Kode_Perusahaan = d.Kode_Perusahaan AND a.Kode_Komponen = d.Kode_Komponen) "
            ...
            'SQL = SQL & "c.id_golongan_sub_golongan in (select id_golongan_sub_golongan from HRIS_Level_Jabatan where id_level_jabatan = '" & LLevelJabatan.Item(CmbLevel.SelectedIndex) & "') And "
            'SQL = SQL & "d.Kode_Calon = '" & TxtKodeCalon.Text & "' "
            ...
            SQL = "select kode_komponen, keterangan from HRIS_Komponen_Gaji where jenis = 'I' and perhitungan = 'T'"
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    ...
                    'If IsDBNull(Dr("TunjanganAju")) Then
                    '    Lvw2.SubItems.Add("0")
                    'Else
                    '    Lvw2.SubItems.Add(Format(Dr("TunjanganAju"), "N0"))
                    'End If
                Loop
            End Using
    ...
    Private Sub BtOK1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtOK1.Click
        ...
        Try
            ...
            SQL = "select a.Kode_Komponen, a.Keterangan, c.range1 as RangeTJ1,c.range2 as RangeTJ2, d.Tunjangan As TunjanganAju "
            SQL = SQL & "from ((HRIS_Komponen_Gaji a Left Join HRIS_Range_Tunjangan c ON a.Kode_Perusahaan = c.Kode_Perusahaan AND a.Kode_Komponen = c.Kode_Komponen) "
            SQL = SQL & "Left Join HRIS_Transaksi_Rekrutmen_Tunjangan d ON a.Kode_Perusahaan = d.Kode_Perusahaan AND a.Kode_Komponen = c.Kode_Komponen) "
    ...
    Private Sub LvCari_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles LvCari.DoubleClick
        ...
        Dim TunjanganAju As Double = LvCari.FocusedItem.SubItems(4).Text

        TxtCari1.Text = Kode
        TxtRangeTJ1.Text = StrRange
        TxtRangeTJ2.Text = StrRange2
        TxtTunjanganAju.Text = Format(TunjanganAju, "N0")
        TxtNilaiT.Focus()
    ...
    Private Sub LvKaryawan_ChangeUICues(sender As Object, e As UICuesEventArgs) Handles LvKaryawan.ChangeUICues

Public Class EMI_Controlling_Produksi
    ...
    Private Sub LoadLvGI()
        Try
            ...
            SQL = SQL & "d.Keterangan, d.Id_Routing, isnull(Qty_PerBatch,0) as Qty_PerBatch "
                ...
                With Ds.Tables("MyTable")
                    If .Rows.Count <> 0 Then
                        For i As Integer = 0 To .Rows.Count - 1
                            ...
                            Dim JumlahBatch As Double = Val(HilangkanTanda(.Rows(i).Item("Jumlah_KG"))) / .Rows(i).Item("Qty_PerBatch")
    ...
    Private Sub GetDataGR(ByVal index As Integer)
        ...
        LvGR_NoFaktur = Lv_GI.Items(index).SubItems(itemGR_NoFaktur).Text
        LvGR_TglProduksi = Lv_GI.Items(index).SubItems(itemGR_TglProduksi).Text
        LvGR_Jam = Lv_GI.Items(index).SubItems(itemGR_Jam).Text
        LvGR_NmBarang = Lv_GI.Items(index).SubItems(itemGR_NmBarang).Text
        LvGR_Jmlh = Lv_GI.Items(index).SubItems(itemGR_Jmlh).Text
        LvGR_JnsProduksi = Lv_GI.Items(index).SubItems(itemGR_JnsProduksi).Text
        LvGR_JmlhBatch = Lv_GI.Items(index).SubItems(itemGR_JmlhBatch).Text
        LvGR_BatchDosing = Lv_GI.Items(index).SubItems(itemGR_BatchDosing).Text
        LvGR_KdBarang = Lv_GI.Items(index).SubItems(itemGR_KdBarang).Text
        LvGR_Satuan = Lv_GI.Items(index).SubItems(itemGR_Satuan).Text
        LvGR_JmlhKg = Lv_GI.Items(index).SubItems(itemGR_JmlhKG).Text
        LvGR_SatuanKg = Lv_GI.Items(index).SubItems(itemGR_SatuanKg).Text
        LvGR_NoPO = Lv_GI.Items(index).SubItems(itemGR_NoPO).Text
    ...
    Private Sub Lv_GR_DoubleClick(sender As Object, e As EventArgs) Handles Lv_GR.DoubleClick
        ...
        Try
            ...
            EMI_Hasil_ProductionFG.TxtNoSplit.Text = LvGR_NoFaktur

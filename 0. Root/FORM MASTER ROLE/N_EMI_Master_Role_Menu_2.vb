...
Public Class N_EMI_Master_Role_Menu_2
		...
		Try
			...
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					Lv = Lv_Users.Items.Add(If(General_Class.CekNULL(Dr("UserName")) = "", "-", Dr("UserName")))
	...
	Private Sub LoadLvRole()
		...
		Lv_Users.Columns.Add("Username", 125, HorizontalAlignment.Left)
		Lv_Users.Columns.Add("UserID", 0, HorizontalAlignment.Left)
	...
	Private Sub Lv_Role_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Lv_Role.SelectedIndexChanged
		...
		Try
			...
			SQL = "select b.Form, a.ButtonName, a.Kategori, a.Keterangan "
	...
	Private Sub Btn_ShowMenus_Click(sender As Object, e As EventArgs) Handles Btn_ShowMenus.Click
		'Cmb_Reference.SelectedIndex = -1 : Cmb_Reference.Text = ""

		Dim hasCheckedData As Boolean = False
		For i As Integer = 0 To Lv_Users.Items.Count - 1
			If Lv_Users.Items(i).Checked Then
				hasCheckedData = True
				Exit For
			End If
		Next
		If Not hasCheckedData Then
			MessageBox.Show("Pilih Dahulu Minimal 1 User", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			Lv_Role.Items.Clear()
			Exit Sub
		End If
	...
	Private Sub Btn_Save_Click(sender As Object, e As EventArgs) Handles Btn_Save.Click
		...
		Try
			...
			Dim listUserChecked As New List(Of String)
			For Each DataUserCheked As ListViewItem In Lv_Users.Items
				If DataUserCheked.Checked Then
					Dim CurrentUserLoop As String = DataUserCheked.SubItems(1).Text
					listUserChecked.Add(CurrentUserLoop.Trim)
				End If
			Next
			Dim listUserCheckedWrap As String = $"'{String.Join("', '", listUserChecked)}'"

			SQL = "delete from RoleMainMenus where userid in (" & listUserCheckedWrap & ") "
			ExecuteTrans(SQL)

			SQL = "delete from RoleMenus where userid in (" & listUserCheckedWrap & ")"
			ExecuteTrans(SQL)

			SQL = "delete from RoleSubMenu where userid in (" & listUserCheckedWrap & ")"
			ExecuteTrans(SQL)

			SQL = "delete from RoleSubMenuLv1 where userid in (" & listUserCheckedWrap & ")"
			ExecuteTrans(SQL)

			SQL = "delete from RoleSubMenuLv2 where userid in (" & listUserCheckedWrap & ")"
			ExecuteTrans(SQL)

			SQL = "delete from RoleSubMenuLv3 where userid in (" & listUserCheckedWrap & ")"
			ExecuteTrans(SQL)
...
#Region "METODE BARU DENGAN BULK INSERT"
			...
			Dim tmp_mainmenu As New List(Of (user As String, mainmenuid As String))
			Dim tmp_menu As New List(Of (user As String, menuid As String))
			Dim tmp_submenu As New List(Of (user As String, submenuid As String))
			Dim tmp_submenu1 As New List(Of (user As String, submenulv1id As String))
			Dim tmp_submenu2 As New List(Of (user As String, submenulv2id As String))
			Dim tmp_submenu3 As New List(Of (user As String, submenulv3id As String))
				...
				If Lv_Role.Items(i).Checked Then
					...
					For Each DataUserCheked As ListViewItem In Lv_Users.Items
						If DataUserCheked.Checked Then
							...
							If Not String.IsNullOrWhiteSpace(dataL3) AndAlso
								Not tmp_submenu3.Any(Function(x) x.user = CurrentUserLoop AndAlso x.submenulv3id = dataL3) Then
								dtSL3.Rows.Add("RoleSL3_" & GenerateUniqueID(i, 6, IndexUser), CurrentUserLoop, KodePerusahaan, dataL3)
								tmp_submenu3.Add((CurrentUserLoop, dataL3))
							End If

							Dim dataL2 As String = Lv_Role.Items(i).SubItems(itemSubMenuLv2IDRole).Text
							If Not String.IsNullOrWhiteSpace(dataL2) AndAlso
								Not tmp_submenu2.Any(Function(x) x.user = CurrentUserLoop AndAlso x.submenulv2id = dataL2) Then
								dtSL2.Rows.Add("RoleSL2_" & GenerateUniqueID(i, 5, IndexUser), CurrentUserLoop, KodePerusahaan, dataL2)
								tmp_submenu2.Add((CurrentUserLoop, dataL2))
							End If

							Dim dataL1 As String = Lv_Role.Items(i).SubItems(itemSubMenuLv1IDRole).Text
							If Not String.IsNullOrWhiteSpace(dataL1) AndAlso
								Not tmp_submenu1.Any(Function(x) x.user = CurrentUserLoop AndAlso x.submenulv1id = dataL1) Then
								dtSL1.Rows.Add("RoleSL1_" & GenerateUniqueID(i, 4, IndexUser), CurrentUserLoop, KodePerusahaan, dataL1)
								tmp_submenu1.Add((CurrentUserLoop, dataL1))
							End If

							Dim dataSub As String = Lv_Role.Items(i).SubItems(itemSubMenuIDRole).Text
							If Not String.IsNullOrWhiteSpace(dataSub) AndAlso
								Not tmp_submenu.Any(Function(x) x.user = CurrentUserLoop AndAlso x.submenuid = dataSub) Then
								dtS.Rows.Add("RoleS_" & GenerateUniqueID(i, 3, IndexUser), CurrentUserLoop, KodePerusahaan, dataSub)
								tmp_submenu.Add((CurrentUserLoop, dataSub))
							End If

							Dim dataMenu As String = Lv_Role.Items(i).SubItems(itemMenuIDRole).Text
							If Not String.IsNullOrWhiteSpace(dataMenu) AndAlso
								Not tmp_menu.Any(Function(x) x.user = CurrentUserLoop AndAlso x.menuid = dataMenu) Then
								dtM.Rows.Add("RoleM_" & GenerateUniqueID(i, 2, IndexUser), CurrentUserLoop, KodePerusahaan, dataMenu)
								tmp_menu.Add((CurrentUserLoop, dataMenu))
							End If

							Dim dataMainMenu As String = Lv_Role.Items(i).SubItems(itemMainMenuIDRole).Text
							If Not String.IsNullOrWhiteSpace(dataMainMenu) AndAlso
								Not tmp_mainmenu.Any(Function(x) x.user = CurrentUserLoop AndAlso x.mainmenuid = dataMainMenu) Then
								dtMM.Rows.Add("RoleMM_" & GenerateUniqueID(i, 1, IndexUser), CurrentUserLoop, KodePerusahaan, dataMainMenu)
								tmp_mainmenu.Add((CurrentUserLoop, dataMainMenu))
							End If
			...
			If listMenuUnCheck.Count <> 0 Then
				...
				SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
				SQL = SQL & "and UserID in (" & listUserCheckedWrap & ") "
...
#End Region
	...
	Private Sub Btn_SaveRoleButton_Click(sender As Object, e As EventArgs) Handles Btn_SaveRoleButton.Click
		...
		Try
			...
			For Each Data As ListViewItem In Lv_RoleButton.Items
				...
				For Each DataUserCheked As ListViewItem In Lv_Users.Items
					If DataUserCheked.Checked = False Then Continue For

					Dim DataRow As DataRow = Dt_Bulk.NewRow
					DataRow("Kode_Perusahaan") = KodePerusahaan
					DataRow("UserID") = DataUserCheked.SubItems(1).Text
	...
	Private Sub LoadAllRoleMenus(ByVal Optional UserIDReference As String = "")
		...
		Try
			...
			SQL = SQL & "a.urut, a.MenuOrder, a.SubMenuOrder, a.SubMenuLv1Order, a.SubMenuLv2Order, a.SubMenuLv3Order, form, "

			If Not String.IsNullOrEmpty(UserIDReference) Then
				SQL = SQL & "CASE "
			...
			Else
				SQL = SQL & "'Not Access' AS StatusAccess "
				SQL = SQL & "FROM CTE_A a "
			End If
	...
	Public Function GenerateUniqueID(ByVal UrutMenu As Integer, ByVal LevelMenu As Integer, ByVal indexUser As Integer) As String
		Dim ticksStr As String = DateTime.Now.Ticks.ToString()
		Dim timePart As String = ticksStr.Substring(ticksStr.Length - 10)

		Dim finalID As String = timePart & indexUser & UrutMenu & LevelMenu
		If finalID.Length > 10 Then
			Return finalID.Substring(finalID.Length - 10)
		Else
			Return finalID
		End If
	End Function
	...
	Private Sub Lv_Users_MouseMove(sender As Object, e As MouseEventArgs) Handles Lv_Users.MouseMove, Lv_Role.MouseMove, Lv_RoleButton.MouseMove

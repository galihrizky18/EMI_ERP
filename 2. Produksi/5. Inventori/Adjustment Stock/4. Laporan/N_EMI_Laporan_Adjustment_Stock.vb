Public Class N_EMI_Laporan_Adjustment_Stock
	...
	Private Sub BtnCetak_Click(sender As Object, e As EventArgs) Handles BtnCetak.Click
		...
		Try
			...
			SF = "{N_EMI_View_Laporan_Adjustment_Stock.Kode_Perusahaan} = '" & KodePerusahaan & "' "
			SF = SF & "and {N_EMI_View_Laporan_Adjustment_Stock.Tanggal} >= #" & Format(Tgl1.Value, "yyyy-MM-dd") & "# and "
			SF = SF & "{N_EMI_View_Laporan_Adjustment_Stock.Tanggal} <= #" & Format(Tgl2.Value, "yyyy-MM-dd") & "# "

			If Not Rd_Status_Seluruh.Checked Then

				If Rd_Status_Validasi.Checked Then
					Filter &= "and Flag_Validation_Accounting = 'Y' "
					SF = SF & "And {N_EMI_View_Laporan_Adjustment_Stock.Flag_Validation_Accounting} = 'Y' "
				ElseIf Rd_Status_Belum_Validasi.Checked Then
					Filter &= "and Flag_Validation_Accounting is null "
					SF = SF & "And IsNull({N_EMI_View_Laporan_Adjustment_Stock.Flag_Validation_Accounting}) "
				End If
			...
			If Cmb_KdSO.SelectedIndex <> 0 Then
				...
				SF = SF & "And {N_EMI_View_Laporan_Adjustment_Stock.Kode_Stock_Owner} = '" & Cmb_KdSO.Text.Trim & "' "
			End If

			If Not Txt_No_Faktur.Text.ToUpper.Trim = OpsiSeluruh.ToUpper.Trim Then
				SQL = SQL & "and No_Faktur = '" & Txt_No_Faktur.Text.Trim & "' "
				SF = SF & "And {N_EMI_View_Laporan_Adjustment_Stock.No_Faktur} = '" & Txt_No_Faktur.Text.Trim & "'"
			End If
			...
			If Cmb_Lain.SelectedIndex <> 0 Then
				If Txt_Lain.Text.ToUpper.Trim <> OpsiSeluruh.ToUpper.Trim Then
					...
					SF = SF & "And {N_EMI_View_Laporan_Adjustment_Stock." & Kolom & "} = '" & Txt_Lain.Text & "'"
				End If
			End If

			SQL = $"
				select kode_perusahaan
				from N_EMI_View_Laporan_Adjustment_Stock
				where kode_perusahaan = '{KodePerusahaan}'
			...
			Using DS = BindingTrans(SQL)
				With DS.Tables("MyTable")
					If .Rows.Count <> 0 Then
						...
						Dim CrDoc As New N_EMI_CR_Laporan_Adjustment_Stock

...
Public Class N_EMI_Transaksi_Trial_Request_Material
	...
	Dim isFormula As Boolean = False

	Dim LvKodeBarang, LvNamaBarang, LvJumlah, LvSatuan, LvKet, LvLokasiGudangRequest As String
	...
	Dim item_Lokasi_Gudang_Request As Integer = 5
	...
	Private Sub Get_Isi_Listview(ByVal i As Integer)
		...
		LvLokasiGudangRequest = LvBarang.Items(i).SubItems(item_Lokasi_Gudang_Request).Text
	End Sub
	...
	Private Sub TxtKodeBarang_TextChanged(sender As Object, e As EventArgs) Handles TxtKodeBarang.TextChanged
		...
		Try
			...
			SQL = SQL & "and a.kode_stock_owner = '" & LSOReq(CmbSO_Req.SelectedIndex) & "' "
	...
	Private Sub TxtKodeBarang_Leave(sender As Object, e As EventArgs) Handles TxtKodeBarang.Leave
		...
		If CmbSO_Req.SelectedIndex = -1 Then Exit Sub
	...
	Private Sub BtnOK_Click(sender As Object, e As EventArgs) Handles BtnOK.Click
		...
		ElseIf TxtSatuan.Text.Trim.Length = 0 Then
			MessageBox.Show("Kode barang harus Valid!", "Perhatian", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			TxtKodeBarang.Focus() : Exit Sub
		End If
		...
		If Val(HilangkanTanda(TxtJlh.Text)) > Val(HilangkanTanda(Txt_Stock.Text)) Then
			MessageBox.Show("Jumlah barang tidak boleh melebihi stock gudang!", "Perhatian", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			TxtJlh.Focus() : Exit Sub
		End If
		...
		Lv.SubItems.Add(LSOSup(CmbSo_Sup.SelectedIndex).ToString.Trim)
	...
	Private Sub CmbSo_Sup_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbSo_Sup.SelectedIndexChanged
		If CmbSo_Sup.Items.Count = 0 Or CmbSo_Sup.SelectedIndex = -1 Then Exit Sub

		If LSOSup(CmbSo_Sup.SelectedIndex).ToString.ToUpper.Trim = LSOReq(CmbSO_Req.SelectedIndex).ToString.ToUpper.Trim Then
		...
		'If CmbSo_Sup.SelectedIndex = -1 Then
		'	Btn_Get_Forumla.Enabled = True
		'Else
		'	Btn_Get_Forumla.Enabled = False
		...
		If CmbSo_Sup.SelectedIndex <> -1 Then
			...
			Try
				...
				SQL &= $"and Kode_Barang = '{TxtKodeBarang.Text.Trim}' and Kode_Stock_Owner = '{LSOSup(CmbSo_Sup.SelectedIndex)}' "
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
	...
	Private Sub BtnSimpan_Click(sender As Object, e As EventArgs) Handles BtnSimpan.Click
		...
		Try
			...
			Dim Gudang_Supplier As String = "-"
			Dim FlagTrial As String = ""
			If isFormula Then
				'Gudang_Supplier = "-"
				FlagTrial = $"'Y'"
			Else
				'Gudang_Supplier = LSOSup(CmbSo_Sup.SelectedIndex)
				FlagTrial = $"NULL"
			End If
			...
			'=     INSERT INDUK     =
			'========================
			SQL = "insert into Emi_Material_Requisition_General (Kode_Perusahaan, No_Faktur, Kode_Stock_Owner_Req, Kode_Stock_Owner_Sup, Keterangan, Total, Tanggal, Jam, UserId, No_Transaksi_Formula, Flag_Trial_Formula) values "
			SQL = SQL & "('" & KodePerusahaan & "', '" & TxtNoFaktur.Text.Trim & "', '" & LSOReq(CmbSO_Req.SelectedIndex) & "', '" & Gudang_Supplier & "', "
			SQL = SQL & "'" & Txt_Ket.Text & "', '" & HilangkanTanda(TxtTotal.Text) & "', '" & Format(tgl_skg, "yyyy-MM-dd") & "', "
			SQL = SQL & "'" & Format(tgl_skg, "HH:mm:ss") & "', '" & UserID & "', " & NoTransaksiFormula & ", " & FlagTrial & ")"
			ExecuteTrans(SQL)
			...
			For i As Integer = 0 To LvBarang.Items.Count - 1
				...
				'Dim Gudang_Supplier_Bahan As String = "-"
				Dim Gudang_Supplier_Bahan As String = LvLokasiGudangRequest.Trim
				'If isFormula Then
				'	Gudang_Supplier_Bahan = LvLokasiGudangRequest.Trim
				'Else
				'	Gudang_Supplier_Bahan = "-"
				'End If

				'=========================
				'=     INSERT DETAIL     =
				'=========================
				SQL = "insert into Emi_Material_Requisition_General_Detail (Kode_Perusahaan, No_Faktur, Kode_Barang, Jumlah, Satuan, Jumlah_Barang, Satuan_Barang, Keterangan, Gudang_Asal) values "
				...
				SQL = SQL & "'" & LvSatuan & "', '" & HilangkanTanda(LvJumlah) & "', '" & LvSatuan & "', '" & LvKet & "', '" & Gudang_Supplier_Bahan.Trim & "')"
				ExecuteTrans(SQL)
	...
	Public Sub ShowBahanByFormula(ByVal NoFormula As String, ByVal JumlahKebutuhan As String, ByVal SatuanKebutuhan As String)
		...
		Try
			...
			SQL &= $") * b.jumlah "
			SQL &= $"), 0) as Jumlah_Kebutuhan, e.kode_Stock_Owner as Gudang_Asal_Bahan "
			SQL &= $"from Emi_Transaksi_Formulator a "
			SQL &= $"inner join EMI_Transaksi_Formulator_Detail_Bahan b on a.kode_perusahaan = b.Kode_Perusahaan and a.no_faktur = b.No_Faktur "
			SQL &= $"inner join Barang c on b.Kode_Perusahaan = c.Kode_Perusahaan and b.Kode_Stock_Owner = c.Kode_Stock_Owner and b.Kode_Barang = c.Kode_Barang "
			...
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					Lv.SubItems.Add(Dr("Gudang_Asal_Bahan").ToString.Trim)
				Loop
			End Using

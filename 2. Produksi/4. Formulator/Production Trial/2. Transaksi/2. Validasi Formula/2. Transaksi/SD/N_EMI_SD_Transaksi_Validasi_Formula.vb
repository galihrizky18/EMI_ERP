Public Class N_EMI_SD_Transaksi_Validasi_Formula
	...
	Public Sub Transaksi_Formulator_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		If TxtFormulator_NoFaktur.Text.Trim.Length > 0 Then
			TxtFormulator_NoFaktur_Leave(TxtFormulator_NoFaktur, EventArgs.Empty)
		End If
	End Sub
		...
		Try
			...
			SQL = $"
				...
				SELECT a.No_Faktur,
					   ...
					   ISNULL(
							   cast(dbo.get_hpp(sn.Serial_Number) as FLOAT),
							   d.Estimasi_Harga
					   ) AS Est_HPP,

					   ISNULL(
							   (
								   (b.Berat * (c.Persentase / 100.0))
									   / 1000
								   ),
							   0
					   )
				...
				FROM Emi_Transaksi_Formulator a
					 ...
					 LEFT JOIN LastHPP sn
							   ON c.Kode_Perusahaan = sn.Kode_Perusahaan
								   AND c.Kode_Barang = sn.Kode_Barang
								   AND sn.rn = 1
				WHERE a.Kode_Perusahaan = '{KodePerusahaan}'
				  AND a.Status IS NULL
				  AND a.No_Faktur = '{No_Faktur}'
			...
			Dim SQL_Packaging As String = $"
				WITH cte_bahan AS (
						...
						ISNULL(
							(
								...
								WHERE x.Kode_Perusahaan = a.Kode_Perusahaan
								  and x.kode_barang = a.kode_bahan
					...
					INNER JOIN Barang b
						ON a.Kode_Perusahaan = b.Kode_Perusahaan and a.Kode_Bahan = b.Kode_Barang

					GROUP BY a.Kode_Perusahaan, a.Kode_Bahan, a.Kode_Barang, a.Jumlah_Barang, b.Estimasi_Harga
				)
				...
				INNER JOIN cte_bahan p
					ON f.Kode_Perusahaan = p.Kode_Perusahaan and f.Kode_Barang = p.Kode_Barang
				INNER JOIN Barang brg
					ON p.Kode_Perusahaan = brg.Kode_Perusahaan and p.Kode_Bahan = brg.Kode_Barang
	...
	Private Function Cek_HPP_Formula(NoFormula As String) As Boolean
		Try
			...
			'					  and z.kode_barang = c.kode_barang
			'					  AND z.blok_sn IS NULL
			...
			SQL = $"
				...
				WHERE a.Kode_Perusahaan = '{KodePerusahaan}'
				  AND a.Status IS NULL
				  AND a.No_Faktur = '{NoFormula}'
				ORDER BY d.Nama;
			"
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					adaDataBahanBaku = True
					Dim hppVal As Decimal = Val(Dr("Est_HPP"))
					If hppVal <= 0 Then
						listError.Add($"  - HPP Bahan Baku: {Dr("Kode_Bahan")} - {Dr("Nama_Bahan")} (HPP = 0)")
					End If
				Loop
			End Using
			...
			SQL = $"
				WITH cte_bahan AS (
					...
					FROM Barang_Detail_Bahan_Penolong a
					INNER JOIN Barang b ON a.Kode_Perusahaan = b.Kode_Perusahaan and a.Kode_Bahan = b.Kode_Barang
					GROUP BY a.Kode_Perusahaan, a.Kode_Bahan, a.Kode_Barang, a.Jumlah_Barang, b.Estimasi_Harga, b.Nama
				)
				SELECT
					p.Kode_Bahan,
					brg.Nama AS Nama_Bahan,

...
Public Class N_EMI_Transaksi_Trial_Penyediaan_Bahan_Baku
	...
	Dim Lv_LvBahan_KdSo, Lv_LvBahan_KdBahan, Lv_LvBahan_NmBahan, Lv_LvBahan_Jumlah, Lv_LvBahan_Satuan, Lv_LvBahan_Jumlah_Gram, Lv_LvBahan_Satuan_Gram As String
	...
	Dim Item_LvBahan_Jumlah_Gram As Integer = 5
	Dim Item_LvBahan_Satuan_Gram As Integer = 6
...
#End Region
	...
	Private Sub N_EMI_Transaksi_Trial_Penyediaan_Bahan_Baku_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		Lv_Detail_Bahan.Columns.Add("Satuan", 0, HorizontalAlignment.Center) '4
		Lv_Detail_Bahan.Columns.Add("Jumlah Gram", 130, HorizontalAlignment.Right) '5
		Lv_Detail_Bahan.Columns.Add("Satuan Gram", 0, HorizontalAlignment.Center) '6
	...
	Private Sub GetDetailBahan(ByVal index As Integer)
		...
		Lv_LvBahan_Jumlah_Gram = Lv_Detail_Bahan.Items(index).SubItems(Item_LvBahan_Jumlah_Gram).Text
		Lv_LvBahan_Satuan_Gram = Lv_Detail_Bahan.Items(index).SubItems(Item_LvBahan_Satuan_Gram).Text
	End Sub
	...
	Private Sub Btn_Get_Detail_Bahan_Click(sender As Object, e As EventArgs) Handles Btn_Get_Detail_Bahan.Click
		...
		Try
			...
			SQL = "SELECT a.No_Transaksi, e.Kode_Stock_Owner, b.Kode_Barang, c.Nama as Nama_Barang, b.Satuan, "
			SQL &= $"ISNULL(kebutuhan.Total, 0) as JumlahKebutuhan, "
			SQL &= $"ISNULL( "
			SQL &= $"CASE "
			SQL &= $"WHEN a.Satuan = 'Gram' THEN ISNULL(kebutuhan.Total, 0) "
			SQL &= $"WHEN dp.jenis_pengali = 'KALI' THEN ISNULL(kebutuhan.Total, 0) * dp.Nilai_Pengali "
			SQL &= $"WHEN dp.jenis_pengali = 'BAGI' THEN ISNULL(kebutuhan.Total, 0) / NULLIF(dp.Nilai_Pengali, 0) "
			SQL &= $"END, 0 ) AS Jumlah_Gram, 'Gram' as Satuan_Gram "
			SQL &= $"FROM N_EMI_Transaksi_Trial_Split_Production_Order a "
			SQL &= $"INNER JOIN N_EMI_Transaksi_Trial_Split_Production_Order_Detail_Bahan b ON a.Kode_Perusahaan = b.Kode_Perusahaan AND a.No_Transaksi = b.No_Faktur "
			SQL &= $"INNER JOIN barang c ON b.Kode_Perusahaan = c.Kode_Perusahaan AND b.Kode_Stock_Owner = c.Kode_Stock_Owner AND b.Kode_Barang = c.Kode_Barang "
			SQL &= $"INNER JOIN EMI_Kategori_Gudang_PerLokasi d ON c.kode_perusahaan = d.kode_perusahaan AND c.id_kategori_gudang = d.id_kategori_gudang "
			...
			SQL &= $"OUTER APPLY ( "
			SQL &= $"SELECT TOP 1 "
			SQL &= $"((a.Qty_Batch * {TotalBatch}) /  "
			SQL &= $"(SELECT TOP 1 dbo.ubah_satuan(z.Kode_Perusahaan, 'masa', z.Kode_Barang, r.Satuan_Hasil, x.satuan, r.Hasil)	"
			SQL &= $"FROM Emi_Transaksi_Formulator r "
			SQL &= $"WHERE r.Kode_Perusahaan = x.Kode_Perusahaan AND r.No_Faktur = x.No_Faktur) "
			SQL &= $") * x.jumlah AS Total "
			SQL &= $"FROM N_EMI_Transaksi_Trial_Order_Produksi z "
			SQL &= $"INNER JOIN EMI_Transaksi_Formulator_Detail_Bahan x ON z.Kode_Perusahaan = x.Kode_Perusahaan AND z.Kode_Formula = x.No_Faktur "
			SQL &= $"WHERE z.Status IS NULL "
			SQL &= $"AND z.Kode_Perusahaan = a.Kode_Perusahaan "
			SQL &= $"AND z.No_Faktur = a.No_PO "
			SQL &= $"AND x.Kode_Barang = b.Kode_Barang "
			SQL &= $") kebutuhan "
			SQL &= $"OUTER APPLY ( "
			SQL &= $"SELECT TOP 1 Nilai_Pengali, jenis_pengali "
			SQL &= $"FROM emi_satuan_detail_perhitungan  "
			SQL &= $"WHERE Kode_Perusahaan = a.Kode_Perusahaan "
			SQL &= $"AND Satuan_Awal = b.Satuan  "
			SQL &= $"AND satuan_akhir = 'Gram') dp "
			SQL &= $"WHERE a.Status IS NULL "
			SQL &= $"AND a.Kode_Perusahaan = '{KodePerusahaan}' "
			SQL &= $"AND a.No_Transaksi = '{Txt_No_Split.Text.Trim}' "
			SQL &= $"ORDER BY b.Kode_Barang "
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					Lv.SubItems.Add($"{Format(Dr("JumlahKebutuhan"), "N4")} {Dr("Satuan")}")
					Lv.SubItems.Add(Dr("Satuan"))
					'Lv.SubItems.Add(Format(Dr("Jumlah_Gram"), "N4"))
					Lv.SubItems.Add($"{Format(Dr("Jumlah_Gram"), "N4")} {Dr("Satuan_Gram")}")
					Lv.SubItems.Add(Dr("Satuan_Gram"))
				Loop
			End Using
...
#Region "Kode Lama"
			...
			'SQL &= $"select COUNT(DISTINCT z.batch) from N_EMI_Transaksi_Trial_Penyediaan_Bahan_Baku z where a.Kode_Perusahaan = z.kode_perusahaan "

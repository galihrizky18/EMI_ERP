...
Public Class N_EMI_SD_Transaksi_Transfer_Stock_Timbang
    Dim arrcari As New ArrayList
    Dim Jenis = "Transaksi_Timbang_Kosong"
    Dim Random As New Random()
    Dim ReadThread As Thread

    Public Property arrSelectedSN As New ArrayList
    Public Property arrSelectedFaktur As New ArrayList
    ...
    Dim TAGDEFAULTRFIDDD As String = ""
    ...
    Private Sub Transaksi_Timbang_Unloading_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    If General_Class.CekNULL(Dr("Flag_Cold_Storage_RFID_Mati")) = "Y" Then
                        ...
                        End If
                        RFID_Non_Aktif = True
                        TAGDEFAULTRFIDDD = Dr("Tag_RFID_Default")
                    Else
        ...
        txtUrutOto.Text = ""
        Txt_Sisa_Request.Text = ""

        If Cmb_No_Faktur.Items.Count > 1 Then
            'Cmb_No_Faktur.SelectedItem = Nothing
        Else
            Cmb_No_Faktur.SelectedIndex = 0
            Cmb_No_Faktur_SelectedIndexChanged(e, New EventArgs)
        End If
    ...
    Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
        ...
        If Cmb_No_Faktur.SelectedIndex = -1 Then
            MessageBox.Show("No Faktur Belum dipilih . . ! !", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Cmb_No_Faktur.DroppedDown = True
            Cmb_No_Faktur.Focus()
            Exit Sub
        End If
...
#End Region
        ...
        ElseIf CmbJenisTimbang.Text.Trim.ToUpper = "TRANSFER STOCK" Then
            ...
            Dim IsRequestMaterialProduksi As Boolean = False
            ...
            Try
                ...
                '==================================
                '=     CEK APAKAH RM PRODUKSI     =
                '==================================
                SQL = "select isnull(b.Urut_Material_Requisition_Convert, 0) as Urut_Material_Requisition_Convert "
                ...
                SQL &= $"and a.No_Faktur = '{arrSelectedFaktur(Cmb_No_Faktur.SelectedIndex)}' "
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then

                        If Val(HilangkanTanda(Dr("Urut_Material_Requisition_Convert"))) <> 0 Then
                            IsRequestMaterialProduksi = True
                        End If
                ...
                ' =========================
                ' 1. Ambil No_Faktur_Order
                ' =========================
                SQL = $"
                        SELECT distinct c.No_Faktur_Order, c.batch
                        FROM N_EMI_Transaksi_Transfer_Stock_Sementara_Detail a
                        JOIN Emi_Material_Requisition_Det_Convert b
                            ON b.Kode_Perusahaan = a.Kode_Perusahaan
                            AND b.Urut_Oto = a.Urut_Material_Requisition_Convert
                        JOIN Emi_Material_Requisition c
                            ON c.Kode_Perusahaan = b.Kode_Perusahaan
                            AND c.No_Faktur = b.No_Faktur
                        WHERE a.No_Faktur = '{arrSelectedFaktur(Cmb_No_Faktur.SelectedIndex)}'
                          AND a.Kode_Perusahaan = '{KodePerusahaan}'"
                Using Dr = OpenTrans(SQL)
                    If Dr.Read Then
                        SelectedSplit = Dr("No_Faktur_Order")
                        SelectedBatch = Dr("batch")
                    Else
                        CloseConn()
            ...
            Try
                ...
                'If IsRequestMaterialProduksi Then
                '    Using frm As New N_EMI_SD_Pairing_RFID_Tags_Timbang_Floor_Scale()
                '        frm.SelectedSplit = SelectedSplit
                '        frm.SelectedBatch = SelectedBatch
                '        If frm.ShowDialog() <> DialogResult.OK Then Exit Sub
                '        tags = frm.SelectedRFIDTags
                '    End Using

                '    If tags Is Nothing OrElse tags.Count = 0 Then
                '        MessageBox.Show("Minimal harus ada 1 tag RFID untuk di pairing!", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                '        Exit Sub
                '    End If
                'End If
                ...
                SQL = "Select c.serial_number from N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Det b, barang_sn c where "
                ...
                SQL = SQL & "and a.No_Faktur = '" & arrSelectedFaktur(Cmb_No_Faktur.SelectedIndex) & "' "
                Using dr = OpenTrans(SQL)
                    Do While dr.Read
                        ada_data = True
                ...
                For Indxx = 0 To arr_Sn.Count - 1
                    ...
                    SQL = SQL & "From N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, barang d Where "
                    ...
                    'Nilai_Toleransi_Min = Val(HilangkanTanda(GetDataJmlEstimasi)) - (Val(HilangkanTanda(GetDataJmlEstimasi)) * (Persen_Toleransi_Min / 100))
                    'Nilai_Toleransi_Max = Val(HilangkanTanda(GetDataJmlEstimasi)) + (Val(HilangkanTanda(GetDataJmlEstimasi)) * (Persen_Toleransi_Max / 100))


                    'If Val(HilangkanTanda(txt_Jumlah_Timbang.Text)) < Nilai_Toleransi_Min Then
                    '    CloseTrans()
                    '    CloseConn()
                    '    MessageBox.Show("Jumlah Kurang Dari Jumlah Request", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    '    txt_Jumlah_Timbang.Text = ""
                    '    Exit Sub
                    'ElseIf Val(HilangkanTanda(txt_Jumlah_Timbang.Text)) > Nilai_Toleransi_Max Then
                    '    CloseTrans()
                    '    CloseConn()
                    ...
                    If IsRequestMaterialProduksi Then
                        If RFID_Non_Aktif Then
                            ...
                            Dim NoFakturOrder As String = Nothing
                            Dim NoBatchh As String = ""
                            ' =========================
                            ' 1. Ambil No_Faktur_Order
                            ' =========================
                            SQL = $"
                                ...
                                JOIN Emi_Material_Requisition c
                                    ON c.Kode_Perusahaan = b.Kode_Perusahaan
                                    AND c.No_Faktur = b.No_Faktur
                                WHERE a.No_Faktur = '{GetDataKodeTransfer}'
                                  AND a.Kode_Perusahaan = '{KodePerusahaan}'"

                            Using Dr = OpenTrans(SQL)
                                If Dr.Read() Then
                                    NoFakturOrder = Dr("No_Faktur_Order").ToString()
                                    NoBatchh = Dr("batch").ToString()
                                Else
                                    Dr.Close()
                                    CloseTrans()
                                    CloseConn()
                                    ...
                                    MessageBox.Show(
                                        "Data nomor split purchase order tidak ditemukan!",
                                        Judul,
                                    ...
                                    Exit Sub
                                End If
                            End Using   ' 🔥 DataReader PASTI TERTUTUP DI SINI

                            ' =========================
                            ' 3. CEK APAKAH SUDAH ADA TAG
                            ' =========================
                            SQL = $"select 1 from N_EMI_Pairing_RFID where No_Split_Production_Order = '{NoFakturOrder}' and batch = '{NoBatchh}' "
                            Using Dr = OpenTrans(SQL)
                                If Not Dr.Read Then
                                    Dr.Close()
                                    ...
                                    Cmd.Parameters.AddWithValue("@RFID_Tag", TAGDEFAULTRFIDDD)
                    ...
                    SQL = SQL & "from N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, N_EMI_Transaksi_Transfer_Stock_Sementara_Det c "
...
#Region "POTONG STOCK"
                    ...
                    SQL = "select a.Nama, a.Kode_Barang, "
                    SQL = SQL & "round(((a.good_stock) - isnull(( select sum(y.Jumlah) from N_EMI_Transaksi_Transfer_Stock_Sementara z "
                    ...
                    SQL = SQL & "), 0) "
                    SQL = SQL & "), 4) as good_stock, "
                    SQL = SQL & "a.Jumlah_Bags "
                    SQL = SQL & "from Barang a "
                    SQL = SQL & "where a.Kode_Perusahaan='" & KodePerusahaan & "' and a.Kode_Stock_Owner='" & GetSoAwal & "' "
                    ...
                    SQL = SQL & "and not z.No_Faktur = '" & arrSelectedFaktur(Cmb_No_Faktur.SelectedIndex) & "' "
                    SQL = SQL & "), 0) "
                    SQL = SQL & "), 4) as jumlah, "
                    SQL = SQL & "a.Jumlah_Bags "
...
#End Region
                    ...
                    SQL = "insert into N_EMI_Transaksi_Transfer_Stock_Sementara_Det2(kode_perusahaan, No_faktur, Urut_Det, No_Pallet, "
                    ...
                    SQL = SQL & "'" & GetPalletTujuan & "', NULL, '" & nilai_kecildetail & "', '" & Val(HilangkanTanda(TxtJumlahBagsDetail.Text)) & "', "
                    SQL = SQL & "'" & UserID & "', '" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & Format(tgl_skg, "HH:mm:ss") & "', "
                    SQL = SQL & "NULL) "
                    ExecuteTrans(SQL)

                    SQL = "update N_EMI_Transaksi_Transfer_Stock_Sementara_Det set  "
                    SQL = SQL & "Selesai = 'Y' "
                    SQL = SQL & "where kode_perusahaan = '" & KodePerusahaan & "' "
...
#Region "Request Material"
                ...
                If isDataRequest Then
                    ...
                    SQL = SQL & "select 1 from N_EMI_Transaksi_Transfer_Stock_Sementara_Det2 z where z.kode_perusahaan = c.Kode_Perusahaan and z.no_faktur = c.No_Faktur and z.Urut_Det = c.Urut_Oto) "
                    Using Dr = OpenTrans(SQL)
                        If Not Dr.Read Then
                            isLastData = True
                        End If
...
#End Region
        ...
        ElseIf CmbJenisTimbang.Text.Trim.ToUpper = "TRANSFER STOCK" Then
            ...
            'cetaktransfer(kode_unik_print)
            N_EMI_Transaksi_Transfer_Stock_Sementara_Timbang.kosong()
            Me.Close()
    ...
    Public Sub Cmb_No_Faktur_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Cmb_No_Faktur.SelectedIndexChanged
        If Cmb_No_Faktur.Items.Count = 0 Then Exit Sub
        ...
        Try
            ...
            SQL = SQL & "ISNULL(( select sum(w.jumlah) from N_EMI_Transaksi_Transfer_Stock_Sementara_Detail y, N_EMI_Transaksi_Transfer_Stock_Sementara_Det z, N_EMI_Transaksi_Transfer_Stock_Sementara_Det2 w "
            ...
            SQL = SQL & "And b.Kode_Barang=d.Kode_Barang And a.so_awal=d.kode_stock_Owner And b.kode_Perusahaan=d.Kode_Perusahaan "
            SQL = SQL & "And a.status Is null And b.Flag_Timbang ='Y' and c.selesai is null "
            SQL = SQL & "And c.Serial_Number_Awal = '" & arrSelectedSN(Cmb_No_Faktur.SelectedIndex) & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
            SQL = SQL & "and a.No_Faktur = '" & arrSelectedFaktur(Cmb_No_Faktur.SelectedIndex) & "' "
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    txtKodeTransfer.Text = Dr("no_faktur")
                    txt_lokasi.Text = Dr("SO_Awal")
                    TxtKdBarang.Text = Dr("Kode_Barang")
                    txt_barang.Text = Dr("nama")
                    txt_Barang_SN.Text = Dr("Serial_Number_Awal")
                    txt_Jml_Estimasi.Text = Format(Dr("jumlah"), "N2")
                    TxtJumlahBags.Text = Format(Dr("Jumlah_Bags"), "N2")
                    Txt_SatuanKecil.Text = Dr("Satuan_Barang")
                    TxtBeratBags.Text = $"{Format(Dr("Berat_Bags"), "N2")} {Dr("Satuan_Berat_Bags")}"
                    Txt_Berat_Bags_Bersih.Text = Format(Dr("Berat_Bags"), "N2")
                    TxtSatuan_FloorScale.Text = Dr("satuan")
                    txtUrutOto.Text = Dr("urut_oto")
                    Txt_Sisa_Request.Text = Format(Val(HilangkanTanda(Dr("Jumlah_Kebutuhan")) - Val(HilangkanTanda(Dr("Total_TF")))), "N2")
                Else
                    CloseTrans()
                    CloseConn()

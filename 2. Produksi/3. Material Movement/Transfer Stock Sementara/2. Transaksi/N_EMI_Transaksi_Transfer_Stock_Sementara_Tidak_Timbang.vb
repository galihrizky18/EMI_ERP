...
Public Class N_EMI_Transaksi_Transfer_Stock_Sementara_Tidak_Timbang
    ...
    Private Sub Btn_TimbangFloorScale_Click(sender As Object, e As EventArgs) Handles Btn_TimbangFloorScale.Click
        ...
        Try
            ...
            Dim Data_Baracde As New List(Of (No_Faktur As String, Serial_Number As String))
            ...
            SQL = "Select a.No_Faktur, c.serial_number "
            SQL &= $"from N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Det b, barang_sn c  "
            SQL &= $"where a.kode_perusahaan = b.kode_perusahaan And a.no_faktur = b.no_faktur "
            SQL &= $"And a.status Is null And b.selesai Is null  "
            SQL &= $"And b.kode_perusahaan=c.kode_Perusahaan And b.serial_number_awal=c.serial_number "
            SQL &= $"And c.kode_perusahaan='{KodePerusahaan}' "
            SQL &= $"and (c.qr_code+'-'+kode_unik_berjalan) = '{Txt_ScanBarcode.Text}' "
            Using dr = OpenTrans(SQL)
                Do While dr.Read
                    ada_data = True
                    'arr_Sn.Add(dr("serial_number"))
                    Data_Baracde.Add((No_Faktur:=dr("No_Faktur"), Serial_Number:=dr("serial_number")))
                Loop
            End Using
            ...
            For Indxx = 0 To Data_Baracde.Count - 1

                Dim newKodeUnikBerjalan As String = Generate_Random_Kode(10)
                Dim Id_Jenis_Kategori_Produksi As String = ""

                Dim NoFakturScan As String = Data_Baracde(Indxx).No_Faktur
                Dim SerialNumberScan As String = Data_Baracde(Indxx).Serial_Number
                ...
                SQL = SQL & "and a.Serial_Number ='" & SerialNumberScan & "' "
                    ...
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For i As Integer = 0 To .Rows.Count - 1
                                ...
                                SQL = SQL & "N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, barang d, Stock_Owner_Gudang e "
                                ...
                                SQL = SQL & "and a.kode_perusahaan = e.kode_perusahaan and a.so_tujuan = e.kode_stock_owner "
                                SQL = SQL & "And c.Serial_Number_Awal = '" & .Rows(i).Item("serial_number") & "' and a.Kode_Perusahaan='" & KodePerusahaan & "' "
                                SQL = SQL & "and a.No_Faktur = '" & NoFakturScan & "' "
                                Using Dr = OpenTrans(SQL)
                                    If Dr.Read Then
                ...
                SQL = SQL & "From N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, barang d "
                SQL = SQL & "Where a.kode_Perusahaan = b.kode_Perusahaan And a.no_faktur = b.no_faktur And "
                SQL = SQL & "b.kode_Perusahaan = c.kode_Perusahaan And b.no_faktur = c.no_faktur And b.Urut_Oto = c.urut_TF "
                SQL = SQL & "And b.Kode_Barang=d.Kode_Barang And a.so_awal=d.kode_stock_Owner And b.kode_Perusahaan=d.Kode_Perusahaan "
                SQL = SQL & "And a.status Is null And b.Flag_Timbang ='T' and c.selesai is null "
...
#End Region
                ...
                SQL = SQL & "'" & GetPalletTujuan & "', NULL, '" & nilai_kecildetail & "', "
                SQL = SQL & "'" & UserID & "', '" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & Format(tgl_skg, "HH:mm:ss") & "', "
                SQL = SQL & "NULL, '" & GetJumlahBags & "') "
                ExecuteTrans(SQL)
...
#Region "Request Material"
                ...
                If isDataRequest Then
                    ...
                    SQL = SQL & "from N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, "
                    SQL = SQL & "N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, N_EMI_Transaksi_Transfer_Stock_Sementara_Det2 d "
                    SQL = SQL & "where a.Kode_Perusahaan = b.Kode_Perusahaan and b.Kode_Perusahaan = c.Kode_Perusahaan and c.Kode_Perusahaan = d.Kode_Perusahaan "
                    SQL = SQL & "and a.No_Faktur = b.No_Faktur "
                    SQL = SQL & "and b.No_Faktur = c.No_Faktur and b.Urut_Oto = c.Urut_TF "
...
#End Region
            ...
            CloseConn()
            CloseConn9()
            MessageBox.Show("Scan Barcode Berhasil", Judul, MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub Popup_Timbang_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            'Label1.Text = "Display - List Transfer Stock Sementara (Tidak Timbang)"
            ...
            Lv_Detail.Columns.Add("No Faktur", 130, HorizontalAlignment.Left)
    ...
    Private Sub Lv_List_Barang_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Lv_List_Barang.SelectedIndexChanged
        ...
        Try
            ...
            Dim SelectedBarcode As String = Lv_List_Barang.FocusedItem.SubItems(11).Text

            Lv_Detail.Items.Clear()
            SQL = "Select a.No_Faktur,c.Kode_Barang, b.Jumlah, b.Satuan_Input "
            SQL &= $"from N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Det b, barang_sn c  "
            SQL &= $"where a.kode_perusahaan = b.kode_perusahaan And a.no_faktur = b.no_faktur "
            SQL &= $"And a.status Is null And b.selesai Is null  "
            SQL &= $"And b.kode_perusahaan=c.kode_Perusahaan And b.serial_number_awal=c.serial_number "
            SQL &= $"And c.kode_perusahaan = '{KodePerusahaan}' "
            SQL &= $"and (c.qr_code+'-'+kode_unik_berjalan) = '{SelectedBarcode}' "
            Using Dr = OpenTrans(SQL)
                Do While Dr.Read
                    Dim Lv As ListViewItem
                    Lv = Lv_Detail.Items.Add(Dr("No_Faktur"))
                    Lv.SubItems.Add($"{Format(Dr("Jumlah"), "N4")} {Dr("Satuan_Input")}")
    ...
    Private Sub get_transfer_stock()
        Try
            ...
            SQL = "Select a.so_awal, b.kode_Barang, d.nama, sum(c.jumlah) as jumlah, b.satuan, b.Satuan_Barang, "
            ...
            SQL = SQL & "isnull(c.Satuan_Input, '-') as Satuan_Input, "
            SQL = SQL & "isnull((select x.Labeling_WMS_Position from View_Warehouse_Position x where x.Kode_Perusahaan = a.Kode_Perusahaan And x.Id_WMS_Warehouse_Position = c.Id_Wms_Awal), null) As Rak_Awal, "
            SQL = SQL & "(e.Qr_Code+'-'+e.Kode_Unik_Berjalan) as Barcode "
            SQL = SQL & "From N_EMI_Transaksi_Transfer_Stock_Sementara a, N_EMI_Transaksi_Transfer_Stock_Sementara_Detail b, "
            SQL = SQL & "N_EMI_Transaksi_Transfer_Stock_Sementara_Det c, barang d, barang_sn e  "
            SQL = SQL & "Where a.kode_Perusahaan = b.kode_Perusahaan And a.no_faktur = b.no_faktur And b.kode_Perusahaan = c.kode_Perusahaan And b.no_faktur = c.no_faktur And b.Urut_Oto = c.urut_TF "
            SQL = SQL & "And b.Kode_Barang=d.Kode_Barang And a.so_awal=d.kode_stock_Owner And b.kode_Perusahaan=d.Kode_Perusahaan "
            SQL = SQL & "and a.kode_perusahaan = '" & KodePerusahaan & "' "
            SQL = SQL & "And a.status Is null And b.Flag_Timbang ='T' and c.selesai is null "
            SQL = SQL & "and c.Kode_Perusahaan = e.Kode_Perusahaan and a.SO_Awal = e.Kode_Stock_Owner and b.Kode_Barang = e.Kode_Barang and c.Serial_Number_Awal = e.Serial_Number "
            SQL = SQL & "group by a.so_awal, b.kode_Barang, d.nama, b.satuan, b.Satuan_Barang, "
            SQL = SQL & "a.kode_perusahaan, c.satuan_input, c.Id_Wms_Awal, (e.Qr_Code+'-'+e.Kode_Unik_Berjalan) "

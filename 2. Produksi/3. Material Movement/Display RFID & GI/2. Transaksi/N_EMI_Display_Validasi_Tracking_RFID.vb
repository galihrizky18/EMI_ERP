Public Class N_EMI_Display_Validasi_Tracking_RFID
	...
	Private Sub OpenValidasiDialog()
		isDialogOpen = True
		...
		Try
			...
			SQL = $"
				;with DataRFID as (
					...
					count(case when a.Lokasi_Pairing = 'COLD_STORAGE' and (a.Lokasi_IN = 'MIXER_POUCH_IN' or a.Lokasi_IN = 'MIXER_CAN_IN') then 1 end) as Tot_In_Box_Cold_Storage,
					count(case when a.Lokasi_Pairing = 'PREMIX' and (a.Lokasi_IN = 'MIXER_POUCH_IN' or a.Lokasi_IN = 'MIXER_CAN_IN') then 1 end) as Tot_In_Box_Premix
				from DataRFID a
						...
						For i As Integer = 0 To .Rows.Count - 1
							...
							If hasGudangColdStorage Then
								...
								SQL = $"
									...
									and No_Split_Production_Order = '{SelectedSplit}'
									and batch = '{SelectedBatch}'
									and (Lokasi_IN = 'MIXER_POUCH_IN' or Lokasi_IN = 'MIXER_CAN_IN')
									and Lokasi_Pairing = 'COLD_STORAGE'
								"
								Using Dr = OpenTrans(SQL)
									If Not Dr.Read Then

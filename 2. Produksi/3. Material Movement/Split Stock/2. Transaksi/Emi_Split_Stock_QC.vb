...
Public Class Emi_Split_Stock_QC
	...
	Dim dgv_detail_Warna, dgv_detail_JenisKemasan, dgv_detail_TglProduksi, dgv_detail_TglExpired, dgv_detail_JenisKualitas, dgv_detail_Pembagi, dgv_detail_HasilBagi, dgv_detail_Barcode, dgv_detail_JnsTransfer, dgv_detail_SatuanInput As String
	...
	Dim itemDGVJmlhBagi As Integer = 22
	Dim itemDGVHslBagi As Integer = 23
	Dim itemDGVBarcode As Integer = 24
	Dim itemDGVFlagBlokSN As Integer = 25
	Dim itemDGVJnsTransfer As Integer = 26
	...
	Dim itemDgvDetai_JenisKualitas As Integer = 15
	Dim itemDgvDetai_Pembagi As Integer = 16
	Dim itemDgvDetai_HasilBagi As Integer = 17
	Dim itemDgvDetai_Barcode As Integer = 18
	Dim itemDgvDetail_JnsTransfr As Integer = 19
	Dim itemDgvDetail_SatuanInput As Integer = 20

	'Dim itemDgvIDWareHouseTujuan As Integer = 10

	Dim isProduction As Boolean = False

	Private Sub Transfer_Stock_3_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
	...
	Private Sub Initial_List_View()
		...
		Lv_DetBarang.Columns.Add("Kode SO", 140, HorizontalAlignment.Left)
		Lv_DetBarang.Columns.Add("Kode Barang", 150, HorizontalAlignment.Left)
		Lv_DetBarang.Columns.Add("Nama", 250, HorizontalAlignment.Center)
		Lv_DetBarang.Columns.Add("Stock", 120, HorizontalAlignment.Right)
		...
		Lv_DetBarang.Columns.Add("Jumlah Bags", 90, HorizontalAlignment.Right)
		Lv_DetBarang.Columns.Add("Satuan", 0, HorizontalAlignment.Center)
	...
	Private Sub get_grid_view(ByVal index As Integer)
		...
		dgv_jmlhBagi = DGV_Data_TF.Rows(index).Cells(itemDGVJmlhBagi).Value
		dgv_hslBagi = DGV_Data_TF.Rows(index).Cells(itemDGVHslBagi).Value
	...
	Private Sub get_grid_view_Detail(ByVal index As Integer)
		...
		dgv_detail_JenisKualitas = Dgv_DataDetail.Rows(index).Cells(itemDgvDetai_JenisKualitas).Value
		dgv_detail_Pembagi = Dgv_DataDetail.Rows(index).Cells(itemDgvDetai_Pembagi).Value
		dgv_detail_HasilBagi = Dgv_DataDetail.Rows(index).Cells(itemDgvDetai_HasilBagi).Value
		dgv_detail_Barcode = Dgv_DataDetail.Rows(index).Cells(itemDgvDetai_Barcode).Value
		dgv_detail_JnsTransfer = Dgv_DataDetail.Rows(index).Cells(itemDgvDetail_JnsTransfr).Value
		dgv_detail_SatuanInput = Dgv_DataDetail.Rows(index).Cells(itemDgvDetail_SatuanInput).Value
	...
	Public Sub kosong()
		...
		Try
			...
			DGV_Data_TF.Columns(itemDGVHslBagi).DisplayIndex = 17
			DGV_Data_TF.Columns(itemDgvBags).DisplayIndex = 18
			DGV_Data_TF.Columns(itemDGVJmlhBagi).DisplayIndex = 19
			DGV_Data_TF.Columns(itemDgvJumlah).DisplayIndex = 20
			DGV_Data_TF.Columns(itemDgvRakTujuan).DisplayIndex = 21

			Dgv_DataDetail.Columns(itemDgvDetai_Barcode).DisplayIndex = 6
			Dgv_DataDetail.Columns(itemDgvDetai_Pembagi).DisplayIndex = 8
			Dgv_DataDetail.Columns(itemDgvDetai_HasilBagi).DisplayIndex = 8

			Dgv_DataRekap.Columns(itemDgvRekap_JumlahBags).DisplayIndex = 3
			Dgv_DataRekap.Columns(itemDgvRekap_SatuanKecil).DisplayIndex = 4
			Dgv_DataRekap.Columns(itemDgvRekap_Jumlah).DisplayIndex = 5
			Dgv_DataRekap.Columns(itemDgvRekap_Satuan).DisplayIndex = 6
	...
	Private Sub DGV_Data_TF_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellClick
		...
		ElseIf currentCell = itemDGVHslBagi Then
			DGV_Data_TF.Rows(currentRow).DefaultCellStyle.BackColor = Color.White
			Dim cellKuantity As String = HilangkanTanda(DGV_Data_TF.CurrentCell.Value)
	...
	Private Sub DGV_Data_TF_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellEndEdit
		...
		If DGV_Data_TF.CurrentRow.Cells(itemDgvCheckBox).Value = "True" Then
			...
			If currentColumn = itemDgvBags OrElse currentColumn = itemDgvJumlah OrElse currentColumn = itemDGVJmlhBagi OrElse currentColumn = itemDGVHslBagi Then
				If Not IsNumeric(cellValue) Then
					DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Format(0, "N4")
					DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(0, "N4")
					DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value = Format(0, "N4")
					DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value = Format(0, "N4")
					GetGrandTotal()
					Exit Sub
				End If
			End If

			If Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value)) < 0 Or
					Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value)) < 0 Or
					Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value)) < 0 Or
					Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value)) < 0 Then

				MessageBox.Show("Jumlah Tidak Boleh Minus", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Format(0, "N4")
				DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(0, "N4")
				DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value = Format(0, "N4")
				DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value = Format(0, "N4")
			...
			'    DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).ReadOnly = False
			'    DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Style.BackColor = Color.LightGray
			'    DGV_Data_TF.CurrentRow.Cells(itemDgvHasilBagi).Style.BackColor = Color.LightGray
			...
			'    If currentColumn = itemDgvBags Then
			...
			'            Dim hslBagi As Double = valueJumlah / Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value))
			'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(valueJumlah, "N4")

			'            DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value = Format(hslBagi, "N4")
			'        End If

			'    ElseIf currentColumn = itemDGVHslBagi Then
			'        If Val(DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value) <> 0 Then

			'            Dim hasilJumlah As Double = Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value)) * Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value))
			'            Dim JumlahBags As Double = hasilJumlah / isiPerbags

			'            DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(hasilJumlah, "N4")
			'            DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Format(JumlahBags, "N4")
			...
			'==================================
			'=     HITUNG JUMLAH CONVERT      =
			'==================================
			Dim JumlahConvert As Double = 0
			Dim JumlahStockDB As Double = 0
			Dim JumlahStockBagsDB As Double = 0
			Try
				OpenConn()

				SQL = "select isnull((" & HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value) & " * Nilai), 0) as Hasil "
				SQL = SQL & "from N_EMI_Master_Satuan where Kode_Perusahaan = '" & KodePerusahaan & "' "
				SQL = SQL & "and Kode_Barang = '" & TxtKd_Barang.Text & "' and Satuan = '" & Cmb_Satuan_Barang.Text & "'"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
				...
				SQL = "select Round(Jumlah, 4) as Jumlah, Round(Jumlah_Bags, 4) as Jumlah_Bags from barang_sn where kode_perusahaan = '" & KodePerusahaan & "' "
				SQL = SQL & "and kode_stock_owner = '" & arrSO(CmbSO_Asal.SelectedIndex) & "' and kode_barang = '" & TxtKd_Barang.Text & "' "
				SQL = SQL & "and serial_number = '" & DGV_Data_TF.CurrentRow.Cells(itemDgvSerialNumber).Value & "'"
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
						JumlahStockDB = Val(HilangkanTanda(Dr("Jumlah")))
						JumlahStockBagsDB = Val(HilangkanTanda(Dr("Jumlah_Bags")))
					End If
				End Using
			...
			DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value = Format(JumlahConvert, "N4")

			DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).ReadOnly = False
			DGV_Data_TF.CurrentRow.Cells(itemDgvBags).ReadOnly = False
			DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).ReadOnly = True
			DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).ReadOnly = True
			DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Style.BackColor = Color.LightGray
			DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Style.BackColor = Color.LightGray
			DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Style.BackColor = Color.White
			...
			If jumlahInput > JumlahStockDB Then
				MessageBox.Show("Jumlah Tidak Boleh Melebihi Stock ", "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(0, "N4")
				DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Format(0, "N4")
				DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value = Format(0, "N4")
			...
			'    SQL = SQL & "and Kode_Barang = '" & DGV_Data_TF.CurrentRow.Cells(itemDgvKodeBarang).Value & "' "
			'    Using Dr = OpenTrans(SQL)
			'        If Dr.Read Then

			'            Dim JenisKemasan As String = Dr("jenis_kemasan")
			'            Dim IsiPerBags As String = Dr("Isi_Per_Bags")
			'            Dim Satuan As String = Dr("Satuan")

			'            Dr.Close()

			'            If JenisKemasan.ToString.ToUpper = "ORIGINAL BAGS" Then

			'                Dim Isi_Per_Bags As Double = 0
			'                SQL = "select dbo.ubah_satuan('" & KodePerusahaan & "', 'masa','" & DGV_Data_TF.CurrentRow.Cells(itemDgvKodeBarang).Value & "', "
			'                SQL = SQL & "'" & Satuan & "', '" & DGV_Data_TF.CurrentRow.Cells(itemDgvSatuan).Value & "', '" & HilangkanTanda(IsiPerBags) & "' ) as hasil "
			'                Using Dr2 = OpenTrans(SQL)
			'                    If Dr2.Read Then
			'                        Isi_Per_Bags = Dr2("hasil")
			'                    Else
			'                        Dr2.Close()
			'                        CloseConn()
			...
			'                Dim JumlahBags As Double = jumlahInput / Isi_Per_Bags
			'                DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = Math.Floor(JumlahBags)
		...
		Else
			...
			DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).ReadOnly = True

			DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Style.BackColor = Color.White
			DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Style.BackColor = Color.White
			DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Style.BackColor = Color.White
			DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Style.BackColor = Color.White
		End If

		Dim currentColumnIndex As Integer = DGV_Data_TF.CurrentCell.ColumnIndex

		'If currentColumnIndex = itemDGVJmlhBagi Then
		'    If DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value <> 0 Then
		'        If DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = 0 Or DGV_Data_TF.CurrentRow.Cells(itemDgvBags).Value = 0 Then
		'            MessageBox.Show("Isi Dahulu Jumlah dan Bags", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
		'            DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value = Format(0, "N4")
		'            Exit Sub
		'        End If
		'    End If
		...
		If currentColumnIndex = itemDgvBags OrElse currentColumnIndex = itemDgvJumlah OrElse currentColumnIndex = itemDGVJmlhBagi OrElse currentColumnIndex = itemDGVHslBagi Then

			Dim currentCell As Integer = DGV_Data_TF.CurrentCell.ColumnIndex
			Dim currentRow As Integer = DGV_Data_TF.CurrentRow.Index

			Dim jumlahValue As Object = HilangkanTanda(DGV_Data_TF.Rows(currentRow).Cells(currentCell).Value)

			If jumlahValue IsNot Nothing AndAlso IsNumeric(jumlahValue) Then
				If currentColumnIndex = itemDgvBags Or currentColumnIndex = itemDGVHslBagi Then
					DGV_Data_TF.CurrentRow.Cells(currentCell).Value = Convert.ToDecimal(jumlahValue).ToString("N4")
				Else
					DGV_Data_TF.CurrentRow.Cells(currentCell).Value = Convert.ToDecimal(jumlahValue).ToString("N4")
				End If
			Else
				If currentColumnIndex = itemDgvBags Or currentColumnIndex = itemDGVHslBagi Then
					DGV_Data_TF.CurrentRow.Cells(currentCell).Value = Format(0, "N4")
				Else
					DGV_Data_TF.CurrentRow.Cells(currentCell).Value = Format(0, "N4")
				End If
			End If
	...
	Private Sub HitungHasilBagi()
		If DGV_Data_TF.Rows.Count = 0 Then Exit Sub

		Dim currentColumnIndex As Integer = DGV_Data_TF.CurrentCell.ColumnIndex

		Dim jumlah As Double = 0
		Dim jumlahBagi As Double = Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVJmlhBagi).Value))
		Dim hasilBagi As Double = Val(HilangkanTanda(DGV_Data_TF.CurrentRow.Cells(itemDGVHslBagi).Value))

		If currentColumnIndex = itemDGVHslBagi Or currentColumnIndex = itemDgvBags Then

			jumlah = Val(HilangkanTanda(Format(hasilBagi * jumlahBagi, "N4")))
			DGV_Data_TF.CurrentRow.Cells(itemDgvJumlah).Value = Format(jumlah, "N4")
	...
	'FUNCTION HANDLE
	Private Sub TxtKd_Barang_TextChanged(sender As Object, e As EventArgs) Handles TxtKd_Barang.TextChanged, Txt_SO.TextChanged, Txt_SatuanPermintaan.TextChanged, Txt_OtoMaterial_req.TextChanged, Txt_JumlahPermintaan.TextChanged
		If asal <> "Emi_Display_Request_Material" Then
			...
			If Not TxtKd_Barang.Text.Trim.Count = 0 Then
				Lv_DetBarang.Location = New Point(37, 286)
				Lv_DetBarang.Visible = True
			Else
				Lv_DetBarang.Location = New Point(1278, 286)
			...
			Try
				...
				SQL = "select top(20) a.kode_stock_owner, a.kode_barang, a.nama, a.good_stock, a.Satuan, b.satuan as satuan_display, ISNULL(a.Jumlah_Bags, 0) as Jumlah_Bags, "
	...
	Public Sub Btn_Insert_Click(sender As Object, e As EventArgs) Handles Btn_GetData.Click
		...
		Try
			...
			SQL = SQL & "from view_warehouse_position a "
			...
			SQL = SQL & "and Kode_Barang = '" & TxtKd_Barang.Text & "' and Flag_Dasar = 'Y' "
			Using Dr = OpenTrans(SQL)
				If Dr.Read Then
					DGV_Data_TF.Columns(itemDGVJmlhBagi).HeaderText = $"Nilai Bags ({Dr("Satuan")})"
					DGV_Data_TF.Columns(itemDgvJumlah).HeaderText = $"Total Split ({Dr("Satuan")})"
				Else
					CloseConn()
			...
			SQL = SQL & " b.Jenis_Kemasan, b.Isi_Per_Bags, b.Satuan_Isi_Bags, a.Tgl_Expired, a.Tgl_Produksi "
			...
			SQL = SQL & " a.Id_Warehouse = c.Id_WMS_Warehouse_Position "
			...
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					DGV_Data_TF.Rows(rows).Cells(itemDGVIsiPerBags).Value = If(General_Class.CekNULL(Dr("Isi_Per_Bags")) = "", 0, General_Class.CekNULL(Dr("Isi_Per_Bags")))
					...
					DGV_Data_TF.Rows(rows).Cells(itemDgvBags).Value = Format(0, "N4")
					DGV_Data_TF.Rows(rows).Cells(itemDgvJumlah).Value = Format(0, "N4")
					DGV_Data_TF.Rows(rows).Cells(itemDGVJmlhBagi).Value = Format(0, "N4")
					DGV_Data_TF.Rows(rows).Cells(itemDGVHslBagi).Value = Format(0, "N4")
					DGV_Data_TF.Rows(rows).Cells(itemDGVBarcode).Value = General_Class.CekNULL(Dr("Barcode"))
					...
					DGV_Data_TF.Rows(rows).Cells(itemDGVJmlhBagi).ReadOnly = True
					DGV_Data_TF.Rows(rows).Cells(itemDGVHslBagi).ReadOnly = True
					DGV_Data_TF.Rows(rows).Cells(itemDgvJumlah).ReadOnly = True
	...
	Private Sub Btn_Simpan_Click(sender As Object, e As EventArgs) Handles Btn_Simpan.Click
		...
		Try
			...
			'=====================================
			'=     INSERT PARENT TF STOCK QC     =
			'=====================================
			SQL = "insert into Tf_Stock_QC (Kode_Perusahaan, No_Faktur, SO_Awal, SO_Tujuan, Tanggal, Jam, UserID, Jenis_Transfer, Lokasi, Keterangan) values "
			SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(TxtNo_Transaksi.Text) & "', '" & arrSO(CmbSO_Asal.SelectedIndex) & "', '" & lokasi_tujuan & "', "
			SQL = SQL & "'" & Format(tgl_skg, "yyyy-MM-dd") & "', '" & tgl_skg.ToString("HH:mm:ss") & "', '" & UserID & "', '" & CmbJnsTransfer.SelectedItem & "', '" & Cmb_Lokasi.Text & "', "
			SQL = SQL & "'" & TxtKeterangan.Text.ToString & "')"
			ExecuteTrans(SQL)
			...
			For i As Integer = 0 To Dgv_DataRekap.Rows.Count - 1
				...
				Dim nilai_kecil_jumlah_bersih As Double = Val(HilangkanTanda(dgv_Rekap_JumlahBersih))
				'SQL = "select dbo.ubah_satuan('" & KodePerusahaan & "', 'masa','" & TxtKd_Barang.Text & "', '" & TxtSatuan.Text & "',"
				'SQL = SQL & "'" & TxtSatuanKecil.Text & "', '" & c & "' ) as hasil"
				...
				'        nilai_kecil_jumlah_bersih = Val(HilangkanTanda(Format(Dr1("hasil"), "N4")))
				'    Else
				'        Dr1.Close()
				'        CloseTrans()
				'        CloseConn()
				...
				'SQL = "insert into Tf_Stock_QC_Detail (Kode_Perusahaan, Kode_Transfer, SO_Awal, SO_Tujuan ,Kode_Barang, "
				'SQL = SQL & "Tanggal, Jam, Jenis_Transfer, Total, Satuan, Keterangan, Total_Barang, Satuan_Barang, userid, Total_Transfer_Bags, urut_material_requisition_convert, Lokasi) values "
				'SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(TxtNo_Transaksi.Text) & "', '" & arrSO(CmbSO_Asal.SelectedIndex) & "', "
				'SQL = SQL & "'" & lokasi_tujuan & "', "
				'SQL = SQL & "'" & dgv_Rekap_KdBarang & "', '" & tgl_skg & "', '" & tgl_skg.ToString("HH:mm:ss") & "', '" & CmbJnsTransfer.SelectedItem & "', "
				'SQL = SQL & "'" & HilangkanTanda(dgv_Rekap_JumlahBersih) & "', '" & TxtSatuan.Text & "', '" & TxtKeterangan.Text.ToString & "', "
				'SQL = SQL & " '" & nilai_kecil & "', '" & TxtSatuanKecil.Text.ToString & "', '" & UserID & "', '" & HilangkanTanda(dgv_Rekap_JumlahBags) & "', '" & Txt_OtoMaterial_req.Text & "', "
				...
				For j As Integer = 0 To Dgv_DataDetail.Rows.Count - 1
					...
					If dgv_detail_lokasi = dgv_Rekap_lokasi And dgv_detail_KdBarang = dgv_Rekap_KdBarang Then
						...
						'======================================
						SQL = "select a.Kode_Perusahaan from Tf_Stock_QC a, Tf_Stock_QC_det b  where "
						...
						'=      UBAH SATUAN KE SATUAN DASAR     =
						'========================================
						Dim nilai_kecil_detail As Double = 0
						Dim Satuan_Kecil_Detail As String = ""
						...
						SQL = SQL & "from N_EMI_Master_Satuan a "
						SQL = SQL & "left join N_EMI_Master_Satuan z ON a.Kode_Perusahaan = z.Kode_Perusahaan and a.Kode_Barang = z.Kode_Barang and z.Flag_Dasar = 'Y' "
						SQL = SQL & "where a.Kode_Perusahaan = '" & KodePerusahaan & "' and a.Kode_Barang = '" & dgv_detail_KdBarang & "' and a.Satuan = '" & dgv_detail_SatuanInput & "' "
						Using Dr = OpenTrans(SQL)
							If Dr.Read Then
								nilai_kecil_detail = Dr("Hasil")
								Satuan_Kecil_Detail = Dr("Satuan_Dasar")
							Else
								nilai_kecil_detail = 0
								Satuan_Kecil_Detail = Dr("Satuan_Dasar")
							End If
						End Using

						Dim Nilai_Kecil_Berat_Bagi As Double = Val(HilangkanTanda(dgv_detail_Pembagi))
						'SQL = "select dbo.ubah_satuan('" & KodePerusahaan & "', 'masa','" & dgv_detail_KdBarang & "', '" & TxtSatuan.Text & "',"
						'SQL = SQL & "'" & TxtSatuanKecil.Text & "', '" & HilangkanTanda(dgv_detail_Pembagi) & "' ) as hasil"
						'Using Dr1 = OpenTrans(SQL)
						'    If Dr1.Read Then
						'        If General_Class.CekNULL(Dr1("hasil")) = "" Then
						'            Dr1.Close()
						...
						'        Nilai_Kecil_Berat_Bagi = Val(HilangkanTanda(Format(Dr1("hasil"), "N4")))
						'    Else
						'        Dr1.Close()
						'        CloseTrans()
						'        CloseConn()
						...
						'SQL = "Select Top(1) nomor_urut from dbo.N_EMI_Wharehouse_Position_Fn('" & KodePerusahaan & "', '" & lokasi_tujuan & "', " & dgv_detail_IDWarehouseTujuan & ") where "
						...
						jumlah_pot_stock = Val(HilangkanTanda(dgv_detail_HasilBagi.ToString)) * Val(HilangkanTanda(dgv_detail_Pembagi.ToString))

						Dim x_ident_current As Integer = 0
						SQL = "select IDENT_CURRENT('Tf_Stock_QC_Detail') as urutan"
						Using Dr = OpenTrans(SQL)
							If Dr.Read Then
						...
						SQL = "select kode_perusahaan from Tf_Stock_QC_Detail "
						SQL = SQL & "where Kode_Perusahaan = '" & KodePerusahaan & "' "
						SQL = SQL & "and No_Faktur = '" & Trim(TxtNo_Transaksi.Text) & "' "
						SQL = SQL & "and Urut_Oto = '" & x_ident_current & "'"
						Using Dr = OpenTrans(SQL)
							If Not Dr.Read Then
								Dr.Close()
								CloseTrans()
								CloseConn()
								MessageBox.Show("Terjadi sedikit masalah, Harap Ulangi Transaksi", "Transfer Stock Quality Control", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
								Exit Sub
							End If
						End Using
						...
						'SQL = SQL & "Flag_Pot_Stock, jumlah_pot_Stock, warna, Urut_Tf) values "
						'SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(TxtNo_Transaksi.Text) & "', "
						'SQL = SQL & "'" & dgv_detail_SN & "', " & sn & ", " & dgv_detail_IDWarehouseAwal & ", " & dgv_detail_IDWarehouseTujuan & ", '" & nilai_kecildetail & "', " & HilangkanTanda(dgv_detail_JumlahBags) & ", "
						'SQL = SQL & "'" & TxtSatuanKecil.Text & "','" & dgv_detail_IDPalletAwal & "', " & palletTujuan & ", "
						'SQL = SQL & "" & flag_pot_stock & ", " & jumlah_pot_stock & ", "
						'SQL = SQL & "'" & dgv_detail_Warna & "', '" & x_ident_current & "')"
						'ExecuteTrans(SQL)

						Dim total As Double
						total = Nilai_Kecil_Berat_Bagi * Val(HilangkanTanda(dgv_detail_HasilBagi))
						...
						Using Dr = OpenTrans(SQL)
							If Dr.Read Then
								If Val(HilangkanTanda(Dr("Jumlah"))) < total Then
						...
						SQL = "insert into Tf_Stock_QC_det (Kode_Perusahaan, No_Faktur, Id_Wms_Awal, No_Pallet_Awal, Id_Wms_Tujuan, No_Pallet_Tujuan, Serial_Number_Awal, Berat_Bagi, Jumlah_Bags, Count, total, Satuan_Barang, Warna, Urut_TF, Satuan_Input) values "
						SQL = SQL & "('" & KodePerusahaan & "', '" & Trim(TxtNo_Transaksi.Text) & "', " & dgv_detail_IDWarehouseAwal & ", '" & dgv_detail_IDPalletAwal & "', " & dgv_detail_IDWarehouseTujuan & ", '" & palletTujuan & "', '" & dgv_detail_SN & "', "
						SQL = SQL & "'" & Nilai_Kecil_Berat_Bagi & "', '" & HilangkanTanda(0) & "', '" & HilangkanTanda(dgv_detail_HasilBagi) & "', '" & total & "', '" & TxtSatuanKecil.Text & "', "
						SQL = SQL & "'" & dgv_detail_Warna & "', '" & x_ident_current & "', '" & dgv_detail_SatuanInput & "')"
						ExecuteTrans(SQL)
						...
						If dgv_detail_JenisKemasan.ToUpper = "ORIGINAL BAGS" Or Jenis_Berat = "T" Then
							flag_pot_stock = "'T'"
						End If

						SQL = "update Tf_Stock_QC_Detail set Flag_Timbang = " & flag_pot_stock
						SQL = SQL & " where Kode_Perusahaan = '" & KodePerusahaan & "' "
						SQL = SQL & "and No_Faktur = '" & Trim(TxtNo_Transaksi.Text) & "' "
						SQL = SQL & "and Urut_Oto = '" & x_ident_current & "'"
						ExecuteTrans(SQL)
...
#End Region
		...
		Try
			...
			SQL = "select Kode_Perusahaan from Vw_Tf_Stock_QC where Kode_Perusahaan = '" & KodePerusahaan & "' and No_Faktur = '" & Trim(TxtNo_Transaksi.Text) & "'"
			Using Ds = BindingTrans(SQL)
				If Ds.Tables("MyTable").Rows.Count <> 0 Then
					CrDoc = New Rpt_EMI_Faktur_Transfer_Stock_QC
					kertas = "Faktur"
					...
					With A_Place_For_Printing2
						...
						CrDoc.RecordSelectionFormula = "{Vw_Tf_Stock_QC.Kode_Perusahaan} = '" & KodePerusahaan & "' and {Vw_Tf_Stock_QC.No_Faktur}='" & Trim(TxtNo_Transaksi.Text) & "' "
	...
	Private Sub DGV_Data_TF_CellLeave(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_Data_TF.CellLeave
		...
		ElseIf DGV_Data_TF.CurrentCell.ColumnIndex = itemDGVHslBagi Then
			Dim cellKuantity As String = DGV_Data_TF.CurrentCell.Value
	...
	Private Sub Btn_SimpanSementara_Click(sender As Object, e As EventArgs) Handles Btn_SimpanSementara.Click
		...
		If isProduction Then
			...
			Try
				...
				SQL = "select c.Jumlah, "
				SQL = SQL & "ISNULL(( "
				SQL = SQL & "select sum(w.jumlah) from Tf_Stock_QC x,  Tf_Stock_QC_Detail y, Tf_Stock_QC_Det z, Tf_Stock_QC_Det2 w  "
		...
		For row As Integer = 0 To DGV_Data_TF.RowCount - 1
			...
			If String.IsNullOrWhiteSpace(dgv_Jumlah) Or String.IsNullOrWhiteSpace(dgv_JmlhBags) Or String.IsNullOrWhiteSpace(dgv_jmlhBagi) Then
				MessageBox.Show("Jumlah atau Bags Harus Di Input", "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Exit Sub
			End If

			If dgv_jmlhBagi = 0 Then
				MessageBox.Show("Jumlah Bagi tidak boleh 0", "Transfer Stock", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Exit Sub
			End If
			...
			If Val(HilangkanTanda(dgv_Jumlah)) < 0 Or Val(HilangkanTanda(dgv_JmlhBags)) < 0 Or Val(HilangkanTanda(dgv_jmlhBagi)) < 0 Then
				MessageBox.Show("Jumlah Tidak Boleh Minus", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
				Exit Sub
			End If
		...
		For row As Integer = 0 To DGV_Data_TF.RowCount - 1
			...
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetai_JenisKualitas).Value = dgv_KetWarna
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetai_Pembagi).Value = dgv_jmlhBagi
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetai_HasilBagi).Value = dgv_hslBagi
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetai_Barcode).Value = dgv_Barcode
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_JnsTransfr).Value = dgv_JnsTransfer
			Dgv_DataDetail.Rows(DgvTab3_Rows).Cells(itemDgvDetail_SatuanInput).Value = Cmb_Satuan_Barang.Text
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
		...
		Emi_Display_Request_Material.asal = "Split_Stock"
	...
	Private Sub Btn_ListGeneral_Click(sender As Object, e As EventArgs) Handles Btn_ListGeneral.Click
		...
		Emi_Display_Request_General.asal = "Split_Stock"

...
Public Class EMI_Transfer_Quality_QC
    ...
    Dim Lv_KdSo, Lv_KdBarang, Lv_NmBarang, Lv_JumlahStock, Lv_JumlahBags, Lv_Satuan, Lv_Rak, Lv_Sn, Lv_QrCode, Lv_Batch, Lv_IdWarehouse, Lv_NoPallet, Lv_Warna, Lv_TglExp, Lv_TglMsk, Lv_MetodePengeluaran, Lv_KodeUnikBerjalan, Lv_Barcode As String


    Dim item_KdSo As Integer = 0
    Dim item_KdBarang As Integer = 1
    Dim item_NmBarang As Integer = 2
    Dim item_JumlahStock As Integer = 3
    Dim item_JumlaBags As Integer = 4
    Dim item_Satuan As Integer = 5
    Dim item_Rak As Integer = 6
    Dim item_Sn As Integer = 7
    Dim item_QrCode As Integer = 8
    Dim item_Batch As Integer = 9
    Dim item_IdWarehouse As Integer = 10
    Dim item_NoPallet As Integer = 11
    ...
    Dim item_Barcode As Integer = 17
    ...
    Private Sub IntialListView()
        ...
        Lv_Data.Columns.Add("Rak", 120, HorizontalAlignment.Left)

        'Hide
        Lv_Data.Columns.Add("Serial_Number", 0, HorizontalAlignment.Center)
        Lv_Data.Columns.Add("Qr_Code", 0, HorizontalAlignment.Center)
        ...
        Lv_Data.Columns.Add("Barcode", 0, HorizontalAlignment.Center)
    ...
    Private Sub Get_LvData(ByVal index As Integer)
        ...
        Lv_Sn = Lv_Data.Items(index).SubItems(item_Sn).Text
    ...
    Private Sub Btn_Insert_Click(sender As Object, e As EventArgs) Handles Btn_Insert.Click
        ...
        Try
            ...
            Using Dr = OpenTrans(SQL)
                If Dr.Read Then
                    ...
                    If General_Class.CekNULL(Dr("Blok_SN")) = "Y" Then
                        Dr.Close()
                        CloseConn()
                        MessageBox.Show("SN Pada Pallet di Block, Transfer Quality dibatalkan", Judul, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Kosong()
                        Exit Sub
                    End If

                    Data_KdSo = General_Class.CekNULL(Dr("Kode_Stock_Owner"))
                    Data_KdBarang = General_Class.CekNULL(Dr("Kode_Barang"))
                    Data_NmBarang = General_Class.CekNULL(Dr("Nama"))
                    Data_JumlahStock = General_Class.CekNULL(Dr("Jumlah"))
                    Data_JumlahBags = General_Class.CekNULL(Dr("Jumlah_Bags"))
                    Data_Satuan = General_Class.CekNULL(Dr("satuan"))
                    Data_Rak = General_Class.CekNULL(Dr("Rak"))
                    Data_Sn = General_Class.CekNULL(Dr("Serial_Number"))
                    Data_QrCode = General_Class.CekNULL(Dr("Qr_Code"))
                    Data_Batch = General_Class.CekNULL(Dr("Batch_Number"))
                    Data_IdWarehouse = General_Class.CekNULL(Dr("Id_Warehouse"))
                    Data_NoPallet = General_Class.CekNULL(Dr("Nomor_Pallet"))
                    Data_Warna = General_Class.CekNULL(Dr("Warna"))
            ...
            If Not arrWarna(Cmb_KualitasAwal.SelectedIndex) = Data_Warna Then
                CloseConn()
                MessageBox.Show("Kualitas Pallet Tidak Sama Dengan Kualitas Awal", JudulMessage, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Txt_ScanBarcode.Text = ""
                Txt_ScanBarcode.Focus()
                Exit Sub
            End If
            ...
            Dim Rows As Integer = Lv_Data.Items.Count + 1
            Dim Lv As New ListViewItem
            Lv = Lv_Data.Items.Add(Data_KdSo)
            Lv.SubItems.Add(Data_KdBarang)
            Lv.SubItems.Add("X")
            Lv.SubItems.Add(Data_JumlahStock)
            Lv.SubItems.Add(Data_JumlahBags)
            Lv.SubItems.Add(Data_Satuan)
            Lv.SubItems.Add(Data_Rak)
            Lv.SubItems.Add(Data_Sn)
            Lv.SubItems.Add(Data_QrCode)
            Lv.SubItems.Add(Data_Batch)
            Lv.SubItems.Add(Data_IdWarehouse)
            Lv.SubItems.Add(Data_NoPallet)
            Lv.SubItems.Add(Data_Warna)
            ...
            Lv.SubItems.Add($"{Data_QrCode}-{Data_KodeUnikBerjalan}")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Try
            ...
            For i As Integer = 0 To Lv_Data.Items.Count - 1
                ...
                SQL = "select distinct b.Kode_Stock_Owner, b.Kode_Barang, b.Serial_Number "
                SQL &= $"from barang a "
                    ...
                    With Ds.Tables("MyTable")
                        If .Rows.Count <> 0 Then
                            For j As Integer = 0 To .Rows.Count - 1
                                ...
                                SQL = SQL & "id_Pallet_Awal, Id_Warehouse_Akhir, id_Pallet_Akhir, Warna_Awal, Warna_Akhir, Urut_TF) values "
                                ...
                                SQL = SQL & "'" & arrWarna(Cmb_KualitasAkhir.SelectedIndex) & "', '" & x_urut_TF_Quality_Detail & "')"
                                ExecuteTrans(SQL)

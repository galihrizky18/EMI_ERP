...
#End Region
			...
			'SQL = SQL & "isnull(( select z.Keterangan from Stock_Owner_Gudang z where a.kode_perusahaan = z.kode_perusahaan "
			'SQL = SQL & "AND ((a.Flag_Trial_Formula = 'Y' AND b.Gudang_Asal = z.Kode_Stock_Owner) "
			'SQL = SQL & "OR "
			'SQL = SQL & "(a.Flag_Trial_Formula IS NULL and a.Kode_Stock_Owner_Req = z.Kode_Stock_Owner ))), '-') as Lokasi, "
			...
			SQL = SQL & "and a.Kode_Perusahaan = '" & KodePerusahaan & "' "
			SQL = SQL & "and a.Kode_Stock_Owner_Req = '" & Lokasi_Req & "' "
			'SQL = SQL & "AND b.Gudang_Asal = '" & Lokasi_Sup & "' and b.Gudang_Asal = c.Kode_Stock_Owner "
			SQL = SQL & "AND (( b.Gudang_Asal = '" & Lokasi_Sup & "' and b.Gudang_Asal = c.Kode_Stock_Owner) "
			SQL = SQL & "OR  "
			SQL = SQL & "(a.Flag_Trial_Formula IS NULL AND a.Kode_Stock_Owner_Sup = '" & Lokasi_Sup & "' and a.Kode_Stock_Owner_Sup = c.Kode_Stock_Owner)) "

			'SQL = SQL & "AND ((a.Flag_Trial_Formula = 'Y' AND b.Gudang_Asal = '" & Lokasi_Sup & "' and b.Gudang_Asal = c.Kode_Stock_Owner) "
			'SQL = SQL & "OR  "
			'SQL = SQL & "(a.Flag_Trial_Formula IS NULL AND a.Kode_Stock_Owner_Sup = '" & Lokasi_Sup & "' and a.Kode_Stock_Owner_Sup = c.Kode_Stock_Owner)) "
			SQL = SQL & "order by a.Tanggal, b.No_Faktur, b.Kode_Barang "
	...
	Private Sub Lv_Data_DoubleClick(sender As Object, e As EventArgs) Handles Lv_Data.DoubleClick
		...
		If asal = "TF_Stock" Then
					...
					With Ds.Tables("MyTable")
						If .Rows.Count <> 0 Then
							For i As Integer = 0 To .Rows.Count - 1
								...
								Transfer_Stock_3.TxtStockDisplay.Text = Format(.Rows(i).Item("good_stock"), "N4") + " KG"
			...
			Transfer_Stock_3.TxtjmlPermintaanDisplay.Text = Format(Val(HilangkanTanda(JumlahRequest)), "N4") + " " + Lv_Satuan
			Transfer_Stock_3.TxtjmlPermintaanBersih.Text = HilangkanTanda(Format(Val(HilangkanTanda(JumlahRequest)), "N4"))
		...
		ElseIf asal = "Split_Stock" Then
					...
					With Ds.Tables("MyTable")
						If .Rows.Count <> 0 Then
							For i As Integer = 0 To .Rows.Count - 1
								...
								Emi_Split_Stock_QC.TxtStockDisplay.Text = Format(.Rows(i).Item("good_stock"), "N4") + " KG"
			...
			Emi_Split_Stock_QC.TxtjmlPermintaanDisplay.Text = Format(Val(HilangkanTanda(JumlahRequest)), "N4") + " " + Lv_Satuan
			Emi_Split_Stock_QC.TxtjmlPermintaanBersih.Text = HilangkanTanda(Format(Val(HilangkanTanda(JumlahRequest)), "N4"))

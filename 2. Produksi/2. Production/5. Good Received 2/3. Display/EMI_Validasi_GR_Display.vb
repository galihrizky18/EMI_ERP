...
#End Region
							...
							Else
								...
								SQL = "select distinct b.Id_Routing, b.Keterangan as Routing "
								SQL = SQL & "from EMI_Order_Produksi a "
								SQL = SQL & "inner join EMI_Master_Routing b on a.Kode_Perusahaan = b.Kode_Perusahaan and a.Id_Routing = b.Id_Routing "
								SQL = SQL & "inner join Emi_Split_Production_Order c on a.Kode_Perusahaan = c.Kode_Perusahaan and a.No_Faktur = c.No_PO "
								SQL = SQL & "where a.Kode_Perusahaan = '" & KodePerusahaan & "' "
								SQL = SQL & "and c.No_Transaksi in ( "
								SQL = SQL & "select x.No_Transaksi from Emi_Split_Production_Order x where x.status is null and x.No_Transaksi in ( "
								SQL = SQL & "select distinct z.No_Split from Emi_Production_Results_Validation_Detail z where z.No_Transaksi = '" & selectedFaktur & "' )) "
								Using Dr = OpenTrans(SQL)
									If Dr.Read Then
										idRouting = Dr("Id_Routing")
										Routing = Dr("Routing")
					...
					If .Rows.Count <> 0 Then
						For i As Integer = 0 To .Rows.Count - 1
							...
							If Jenis.ToUpper = "FINISHED GOOD" Then
								...
								So_Tujuan = If(General_Class.CekNULL(.Rows(i).Item("Serial_Number_Akhir")) = "", .Rows(i).Item("Kode_Stock_Owner_awal"), .Rows(i).Item("Kode_Stock_Owner_Tujuan"))
								...
								End Using
							Else
								So_Tujuan = .Rows(i).Item("Kode_Stock_Owner_Tujuan")

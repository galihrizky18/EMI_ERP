Public Class N_EMI_SD_Validasi_GR_3
	...
	Dim arrSelectedBarcode As New List(Of (QRCode As String, KdUnikberjalan As String))

	'Public arrBarcodeFromParent As New List(Of Dictionary(Of String, String))
	Public arrBarcodeFromParent As New List(Of (QRCode As String, KdUnikberjalan As String))
		...
		Try
			...
			Using Dr = OpenTrans(SQL)
				Do While Dr.Read
					...
					Dim foundMatch As Boolean = arrBarcodeFromParent.Any(Function(item)
																			 Return (item.QRCode = Dr("Qr_Code").ToString()) AndAlso
		   item.KdUnikberjalan.Trim().Equals(Dr("Kode_Unik_Berjalan").ToString().Trim(), StringComparison.OrdinalIgnoreCase)
																		 End Function)
	...
	Private Sub Btn_Tambah_Click(sender As Object, e As EventArgs) Handles Btn_Tambah.Click
		...
		For i As Integer = 0 To Lv_Data.Items.Count - 1
			If Lv_Data.Items(i).Checked = True Then
				...
				Dim Dict As New List(Of (QrCode As String, KdUnikBerjalan As String))

				arrSelectedBarcode.Add((Lv_QrCode, Lv_KdUnikBerjalan))

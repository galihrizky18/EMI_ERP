...
Public Class N_EMI_Transaksi_Validasi_GR_3
	...
	'Public arrBarcodeFromSD As New List(Of (QrCode As String, KdUnikBerjalan As String, BatchNumber As String))
	Public arrBarcodeFromSD As New List(Of (QrCode As String, KdUnikBerjalan As String))
	...
	Private Sub EMI_Validasi_GR_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		Try
			...
			SQL = SQL & "and Flag_GR3_Finish_Goods = 'Y' "
		...
		LvBarcode.Columns.Add("Barcode", 230, HorizontalAlignment.Left)
		LvBarcode.Columns.Add("Jenis", 120, HorizontalAlignment.Center)
	...
	Private Sub Btn_Scan_Click(sender As Object, e As EventArgs) Handles Btn_Scan.Click
		...
		If Cmb_Barcode.SelectedIndex = 0 Then
			...
			Try
				...
				SQL &= $") select No_GR2, Kode_Stock_Owner_Tujuan, Kode_Barang, Nama_Barang, Jenis, Qr_Code, Kode_Unik_Berjalan, Tgl_Produksi, Tgl_Expired, UserID, sum(Jumlah) as Jumlah, "
				SQL &= $"Satuan, Warna, Kualitas, Barcode, Status_Split "
				SQL &= $"from cte "
				SQL &= $"group by No_GR2, Kode_Stock_Owner_Tujuan, Kode_Barang, Nama_Barang, Jenis, Qr_Code, Kode_Unik_Berjalan, Tgl_Produksi, Tgl_Expired, UserID, "
				SQL &= $"Satuan, Warna, Kualitas, Barcode, Status_Split "
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then
		...
		Else
			...
			Dim dataBarcode As New List(Of (QRCode As String, KdUnikberjalan As String))
			For i As Integer = 0 To Lv_DataPallet.Items.Count - 1
				Get_LvPallet_Data(i)

				dataBarcode.Add((LvPallet_QR, LvPallet_KdUnikBerjalan))
			Next
...
#Region "Jurnal 1"
					  ...
					  KodePerusahaan, KodeProyek, ket_packaging & "; " & Temp_NoTransksi, "0", sumPackaging, pagenumber, fso, Bahasa_Pilihan, Ket_Cost_Center_HO)
						...
						Dim selisih_pembulatan As Double = Math.Round(total_hpp - (sumPackaging + SumHPPAwal), 0)
	...
	Public Sub LoadFromSD()
		...
		Try
			...
			For i As Integer = 0 To arrBarcodeFromSD.Count - 1
				...
				Dim SelectedQrCode As String = arrBarcodeFromSD.Item(i).QrCode
				Dim SelectedKdUnikBerjalan As String = arrBarcodeFromSD.Item(i).KdUnikBerjalan
				'Dim SelectedBatchNumber As String = arrBarcodeFromSD.Item(i).BatchNumber
				...
				SQL &= $"and b.Jenis = 'Finished Good' "
				SQL &= $"and c.Jumlah <> 0 "
				SQL &= $"and c.Qr_Code = '{SelectedQrCode}' and c.Kode_Unik_Berjalan = '{SelectedKdUnikBerjalan}' "
				'SQL &= $"and c.Batch_Number = '{SelectedBatchNumber}' "
				SQL &= $"group by a.kode_perusahaan, a.No_Transaksi, b.No_Split, b.Kode_Stock_Owner_Tujuan, b.Kode_Barang, d.Nama, b.Jenis, c.Qr_Code, "
				SQL &= $"c.Kode_Unik_Berjalan, c.Batch_Number, c.Tgl_Produksi, c.Tgl_Expired, a.UserID, b.Satuan, b.Warna, e.Keterangan, b.Tahap "
				SQL &= $") "
				'SQL &= $"select No_GR2, Kode_Stock_Owner_Tujuan, Kode_Barang, Nama_Barang, Jenis, Qr_Code, Kode_Unik_Berjalan, Tgl_Produksi, Tgl_Expired, UserID, sum(Jumlah) as Jumlah, "
				...
				'SQL &= $"group by No_GR2, Kode_Stock_Owner_Tujuan, Kode_Barang, Nama_Barang, Jenis, Qr_Code, Kode_Unik_Berjalan, Tgl_Produksi, Tgl_Expired, UserID, "
				'SQL &= $"Satuan, Warna, Kualitas, Barcode, Status_Split "
				SQL &= $"select a.No_GR2, a.Kode_Stock_Owner_Tujuan, a.Kode_Barang, a.Nama_Barang, a.Jenis, a.Qr_Code, a.Kode_Unik_Berjalan, a.Tgl_Produksi, "
				SQL &= $"a.Tgl_Expired, a.UserID, (sum(a.Jumlah) - isnull(b.Jumlah, 0)) as Jumlah, "
				SQL &= $"a.Satuan, a.Warna, a.Kualitas, a.Barcode, a.Status_Split "
				SQL &= $"from cte a "
				SQL &= $"left join ( select z.Kode_Perusahaan, z.No_Transaksi_GR2, z.Qr_Code, z.Kode_Unik_Berjalan, z.Jumlah "
				SQL &= $"from N_EMI_Validation_GR3_Temp z "
				SQL &= $"where z.No_Transaksi = '{TxtNo_Transaksi.Text.Trim}' "
				SQL &= $"and z.UserID = '{UserID}' "
				SQL &= $"and z.Kode_Unik_Transaksi = '{Kode_Unik_Transaksi}' "
				SQL &= $") b on a.No_GR2 = b.No_Transaksi_GR2 and a.Qr_Code = b.Qr_Code and a.Kode_Unik_Berjalan = b.Kode_Unik_Berjalan "
				SQL &= $"group by a.No_GR2, a.Kode_Stock_Owner_Tujuan, a.Kode_Barang, a.Nama_Barang, b.Jumlah, a.Jenis, a.Qr_Code, a.Kode_Unik_Berjalan, a.Tgl_Produksi, "
				SQL &= $"a.Tgl_Expired, a.UserID, a.Satuan, a.Warna, a.Kualitas, a.Barcode, a.Status_Split "
				Using Dr = OpenTrans(SQL)
					If Dr.Read Then

...
Public Class N_EMI_Laporan_Final_GI_GR_Main_Baru_Thread
	...
	Private Async Function LoadData_Thread(Filter As Boolean) As Task
		...
		Using cts As New CancellationTokenSource()
			...
			Try
				...
				'===============================================S
				Dim t1 = Task.Run(Function() GetDataRekapAsync(token, Filter), token)
				Dim t2 = Task.Run(Function() LoadDataDetailAsync(token, Filter), token)
				Dim t3 = Task.Run(Function() LoadDataDetailSplitAsync(token, Filter), token)

				' --- 3. TUNGGU KETIGA TASK SELESAI ---
				Await Task.WhenAll(t1, t2, t3)

				ProgressBar1.Style = ProgressBarStyle.Continuous
				ProgressBar1.Value = 100

				'RenderDgvRekap(t1.Result)
				'RenderDgvDetail(t2.Result)
				'RenderDgvDetailSplit(t3.Result)
...
#Region "Fungsi Load Tanpa Thread"
...
#Region "FUNGSI LOAD DENGAN THREAD"
	...
	Private Function GetDataRekapAsync(token As CancellationToken, isFilter As Boolean)
		...
		Try
			...
			' --- 2. LOGIC PEMBENTUKAN SQL_Rekap ---
			Dim SQL_Rekap As String = "select no_po, No_split, Tgl_Produksi, Jam_Produksi, Nama_Routing, Keterangan, Kode_Barang, Nama, Jumlah, Berat_GI, Jumlah_Dosing, "
			SQL_Rekap = SQL_Rekap & "Pro_Reject_KG, Qc_Reject_KG, Warehouse_Reject_KG, Tot_Reject_KG, "
			SQL_Rekap = SQL_Rekap & "ScrapGR1_KG, ScrapGR2_KG, ScrapGR3_KG, ScrapTotal_KG, "
			SQL_Rekap = SQL_Rekap & "WasteGR1_KG, WasteGR2_KG, WasteGR3_KG, WasteTotal_KG, Loss_Production_Final_GR, "
			SQL_Rekap = SQL_Rekap & "NilaiGRFinal_KG, NilaiGRFinal_Pcs, Tanggal_GI, Jam_GI, "
			SQL_Rekap = SQL_Rekap & "Reject_Final_Persen, ScrapTotal_Persen, WasteTotal_Persen, Loss_Production_Final_GR_Persen, NilaiGRFinal_Persen, GR_Inspection_KG, "
			SQL_Rekap = SQL_Rekap & "GR_Inspection_PCS, GR_Inspection_Persen, Status, Jumlah_Scrap_Packaging, Flag_Preservative "
			SQL_Rekap = SQL_Rekap & "from Laporan_Akhir_GIGR_Rekap2 "
			SQL_Rekap = SQL_Rekap & "where kode_perusahaan = '" & KodePerusahaan & "' "
			SQL_Rekap = SQL_Rekap & "and Tgl_Produksi between '" & Format(tgl1Value, "yyyy-MM-dd") & "' and '" & Format(tgl2Value.AddDays(1), "yyyy-MM-dd") & "' "

			If isFilter Then
				If Not idRoutingText.ToUpper = OpsiSeluruh.ToUpper Then
					SQL_Rekap = SQL_Rekap & "and Id_Routing = '" & idRoutingText & "' "
				End If
				If Not kdBarangText.ToUpper = OpsiSeluruh.ToUpper Then
					SQL_Rekap = SQL_Rekap & "and Kode_Barang = '" & kdBarangText & "' "
				End If
				If noSplitText.Trim.Length > 0 AndAlso noSplitText.ToUpper <> OpsiSeluruh.ToUpper Then
					SQL_Rekap = SQL_Rekap & "and no_split like '%" & noSplitText & "%' "
				End If
				If Not cmbIndex = 0 Then
					SQL_Rekap = SQL_Rekap & "and Group_Jenis = '" & arrJenis(cmbIndex) & "' "
				End If
			End If
			SQL_Rekap = SQL_Rekap & "order by no_split, Tgl_Produksi, Jam_Produksi "

			' --- 3. GET DATA DARI DATABASE ---
			Using Ds = BindingTrans_Thread(SQL_Rekap)
...
#Region "Komen"
				...
				Me.Invoke(Sub()
							  Try
								  Dgv_Rekap.SuspendLayout()
								  Dgv_Rekap.Rows.Clear()
								  If dt.Rows.Count > 0 Then
									  Dgv_Rekap.RowCount = dt.Rows.Count

									  For i As Integer = 0 To dt.Rows.Count - 1

										  If i Mod 100 = 0 Then token.ThrowIfCancellationRequested()

										  Dim row As DataRow = dt.Rows(i)
										  Dim dgvRow As DataGridViewRow = Dgv_Rekap.Rows(i)

										  dgvRow.Cells(0).Value = row("no_po")
										  dgvRow.Cells(1).Value = row("No_split")
										  dgvRow.Cells(2).Value = row("Kode_Barang")
										  dgvRow.Cells(3).Value = row("Nama")
										  dgvRow.Cells(4).Value = Format(row("Tgl_Produksi"), "dd MMM yyyy")
										  dgvRow.Cells(5).Value = row("Jam_Produksi")
										  dgvRow.Cells(6).Value = row("Nama_Routing")
										  dgvRow.Cells(7).Value = row("Keterangan")
										  dgvRow.Cells(8).Value = If(General_Class.CekNULL(row("Jumlah")) = "", 0, Format(row("Jumlah"), DigitDecimal))
										  dgvRow.Cells(9).Value = If(General_Class.CekNULL(row("Berat_GI")) = "", 0, Format(row("Berat_GI"), "N0"))
										  dgvRow.Cells(10).Value = If(General_Class.CekNULL(row("Jumlah_Dosing")) = "", 0, Format(row("Jumlah_Dosing"), "N4"))
										  dgvRow.Cells(11).Value = If(General_Class.CekNULL(row("Pro_Reject_KG")) = "", 0, Format(row("Pro_Reject_KG"), "N4"))
										  dgvRow.Cells(12).Value = If(General_Class.CekNULL(row("Qc_Reject_KG")) = "", 0, Format(row("Qc_Reject_KG"), "N4"))
										  dgvRow.Cells(13).Value = If(General_Class.CekNULL(row("Warehouse_Reject_KG")) = "", 0, Format(row("Warehouse_Reject_KG"), "N4"))
										  dgvRow.Cells(14).Value = If(General_Class.CekNULL(row("Tot_Reject_KG")) = "", 0, Format(row("Tot_Reject_KG"), "N4"))
										  dgvRow.Cells(15).Value = If(General_Class.CekNULL(row("ScrapGR1_KG")) = "", 0, Format(row("ScrapGR1_KG"), "N4"))
										  dgvRow.Cells(16).Value = If(General_Class.CekNULL(row("ScrapGR2_KG")) = "", 0, Format(row("ScrapGR2_KG"), "N4"))
										  dgvRow.Cells(17).Value = If(General_Class.CekNULL(row("ScrapGR3_KG")) = "", 0, Format(row("ScrapGR3_KG"), "N4"))
										  dgvRow.Cells(18).Value = If(General_Class.CekNULL(row("ScrapTotal_KG")) = "", 0, Format(row("ScrapTotal_KG"), "N4"))
										  dgvRow.Cells(19).Value = If(General_Class.CekNULL(row("WasteGR1_KG")) = "", 0, Format(row("WasteGR1_KG"), "N4"))
										  dgvRow.Cells(20).Value = If(General_Class.CekNULL(row("WasteGR2_KG")) = "", 0, Format(row("WasteGR2_KG"), "N4"))
										  dgvRow.Cells(21).Value = If(General_Class.CekNULL(row("WasteGR3_KG")) = "", 0, Format(row("WasteGR3_KG"), "N4"))
										  dgvRow.Cells(22).Value = If(General_Class.CekNULL(row("WasteTotal_KG")) = "", 0, Format(row("WasteTotal_KG"), "N4"))
										  dgvRow.Cells(23).Value = If(General_Class.CekNULL(row("Loss_Production_Final_GR")) = "", 0, Format(row("Loss_Production_Final_GR"), "N4"))
										  dgvRow.Cells(24).Value = If(General_Class.CekNULL(row("GR_Inspection_KG")) = "", 0, Format(row("GR_Inspection_KG"), "N4"))
										  dgvRow.Cells(25).Value = If(General_Class.CekNULL(row("GR_Inspection_PCS")) = "", 0, Format(row("GR_Inspection_PCS"), DigitDecimal))
										  dgvRow.Cells(26).Value = If(General_Class.CekNULL(row("NilaiGRFinal_KG")) = "", 0, Format(row("NilaiGRFinal_KG"), "N4"))
										  dgvRow.Cells(27).Value = If(General_Class.CekNULL(row("NilaiGRFinal_Pcs")) = "", 0, Format(row("NilaiGRFinal_Pcs"), DigitDecimal))
										  dgvRow.Cells(28).Value = If(General_Class.CekNULL(row("Reject_Final_Persen")) = "", 0, Format(row("Reject_Final_Persen"), "N4"))
										  dgvRow.Cells(29).Value = If(General_Class.CekNULL(row("ScrapTotal_Persen")) = "", 0, Format(row("ScrapTotal_Persen"), "N4"))
										  dgvRow.Cells(30).Value = If(General_Class.CekNULL(row("WasteTotal_Persen")) = "", 0, Format(row("WasteTotal_Persen"), "N4"))
										  dgvRow.Cells(31).Value = If(General_Class.CekNULL(row("Loss_Production_Final_GR_Persen")) = "", 0, Format(row("Loss_Production_Final_GR_Persen"), "N4"))
										  dgvRow.Cells(32).Value = If(General_Class.CekNULL(row("GR_Inspection_Persen")) = "", 0, Format(row("GR_Inspection_Persen"), "N4"))
										  dgvRow.Cells(33).Value = If(General_Class.CekNULL(row("NilaiGRFinal_Persen")) = "", 0, Format(row("NilaiGRFinal_Persen"), "N4"))
										  dgvRow.Cells(34).Value = If(General_Class.CekNULL(row("Tanggal_GI")) = "", "-", Format(row("Tanggal_GI"), "dd MMM yyyy"))
										  dgvRow.Cells(35).Value = If(General_Class.CekNULL(row("Jam_GI")) = "", "-", row("Jam_GI"))
										  dgvRow.Cells(36).Value = If(General_Class.CekNULL(row("Jumlah_Scrap_Packaging")) = "", "-", Format(row("Jumlah_Scrap_Packaging"), "N4"))
										  dgvRow.Cells(37).Value = If(General_Class.CekNULL(row("Flag_Preservative")) = "", "-", row("Flag_Preservative"))
										  dgvRow.Cells(38).Value = If(General_Class.CekNULL(row("Status")) = "", "-", row("Status"))

										  If Not General_Class.CekNULL(row("Status")) = "" Then
											  If row("Status") = "PRODUCTION" Then
												  dgvRow.Cells(29).Style.BackColor = Color.LightYellow
											  ElseIf row("Status") = "INSPECTION" Then
												  dgvRow.Cells(29).Style.BackColor = Color.LightBlue
											  ElseIf row("Status") = "WAREHOUSE" Then
												  dgvRow.Cells(29).Style.BackColor = Color.LightGray
											  Else
												  dgvRow.Cells(29).Style.BackColor = Color.White
											  End If
										  Else
											  dgvRow.Cells(29).Style.BackColor = Color.White
										  End If
...
#End Region
	...
	Private Function LoadDataDetailAsync(token As CancellationToken, Optional ByVal isFilter As Boolean = False)
		Dim DigitDecimal As String = ""
		Dim tgl1Value, tgl2Value As Date
		Dim idRoutingText As String = ""
		Dim kdBarangText As String = ""
		...
		Try
	       ...
	       "
			...
			Using Ds = BindingTrans_Thread(SQL_Detail)
				Dim dt As DataTable = Ds.Tables("MyTable")
				token.ThrowIfCancellationRequested()
				'Return Ds.Tables("MyTable").Copy()
...
#Region "Komen"
				...
				Me.Invoke(Sub()
							  Try
								  Dgv_Detail2.SuspendLayout()
								  Dgv_Detail2.Rows.Clear()

								  If dt.Rows.Count > 0 Then
									  Dim BoldFont As New Font(Dgv_Detail2.Font, FontStyle.Bold)
									  Dgv_Detail2.RowCount = dt.Rows.Count
									  ...
									  For i As Integer = 0 To dt.Rows.Count - 1
										  ...
										  Dim row As DataRow = dt.Rows(i)
										  Dim dgvRow As DataGridViewRow = Dgv_Detail2.Rows(i)
										  ...
										  dgvRow.Cells(7).Value = row("Keterangan")
										  dgvRow.Cells(8).Value = If(General_Class.CekNULL(row("Jumlah")) = "", 0, Format(row("Jumlah"), DigitDecimal))
										  dgvRow.Cells(9).Value = row("batch")
										  dgvRow.Cells(10).Value = If(General_Class.CekNULL(row("Berat_GI")) = "", 0, Format(row("Berat_GI"), "N0"))
										  dgvRow.Cells(11).Value = If(General_Class.CekNULL(row("Jumlah_Dosing")) = "", 0, Format(row("Jumlah_Dosing"), "N4"))
										  dgvRow.Cells(12).Value = If(General_Class.CekNULL(row("WasteGR1_KG")) = "", 0, Format(row("WasteGR1_KG"), "N4"))
										  dgvRow.Cells(13).Value = If(General_Class.CekNULL(row("WasteGR1_Persen")) = "", 0, Format(row("WasteGR1_Persen"), "N4"))
										  dgvRow.Cells(14).Value = If(General_Class.CekNULL(row("Reject_GR1_KG")) = "", 0, Format(row("Reject_GR1_KG"), "N4"))
										  dgvRow.Cells(15).Value = If(General_Class.CekNULL(row("ScrapGR1_KG")) = "", 0, Format(row("ScrapGR1_KG"), "N4"))
										  dgvRow.Cells(16).Value = If(General_Class.CekNULL(row("Loss_Production")) = "", 0, Format(row("Loss_Production"), "N4"))
										  dgvRow.Cells(17).Value = If(General_Class.CekNULL(row("Loss_Production_Persen")) = "", 0, Format(row("Loss_Production_Persen"), "N4"))
										  dgvRow.Cells(18).Value = If(General_Class.CekNULL(row("NilaiGR1_KG")) = "", 0, Format(row("NilaiGR1_KG"), "N4"))
										  dgvRow.Cells(19).Value = If(General_Class.CekNULL(row("NilaiGR1_Pcs")) = "", 0, Format(row("NilaiGR1_Pcs"), DigitDecimal))
										  dgvRow.Cells(20).Value = If(General_Class.CekNULL(row("GR1_StockSementara")) = "", 0, Format(row("GR1_StockSementara"), "N4"))
										  dgvRow.Cells(21).Value = row("WaktuGR1")
										  dgvRow.Cells(22).Value = If(General_Class.CekNULL(row("WasteGR2_Pcs")) = "", 0, Format(row("WasteGR2_Pcs"), DigitDecimal))
										  dgvRow.Cells(23).Value = If(General_Class.CekNULL(row("WasteGR2_Persen")) = "", 0, Format(row("WasteGR2_Persen"), "N4"))
										  dgvRow.Cells(44).Value = If(General_Class.CekNULL(row("Persentase_Waste_Terhadap_GR2")) = "", 0, Format(row("Persentase_Waste_Terhadap_GR2"), "N4"))

										  dgvRow.Cells(24).Value = If(General_Class.CekNULL(row("Reject_GR2_PCS")) = "", 0, Format(row("Reject_GR2_PCS"), DigitDecimal))
										  dgvRow.Cells(25).Value = If(General_Class.CekNULL(row("Reject_GR2_KG")) = "", 0, Format(row("Reject_GR2_KG"), "N4"))
										  dgvRow.Cells(26).Value = If(General_Class.CekNULL(row("ScrapGR2_PCS")) = "", 0, Format(row("ScrapGR2_PCS"), DigitDecimal))
										  dgvRow.Cells(27).Value = If(General_Class.CekNULL(row("ScrapGR2_KG")) = "", 0, Format(row("ScrapGR2_KG"), "N4"))
										  dgvRow.Cells(28).Value = If(General_Class.CekNULL(row("NilaiGR2_Pcs")) = "", 0, Format(row("NilaiGR2_Pcs"), DigitDecimal))
										  dgvRow.Cells(29).Value = If(General_Class.CekNULL(row("GR2_StockSementara")) = "", 0, Format(row("GR2_StockSementara"), DigitDecimal))
										  dgvRow.Cells(30).Value = row("WaktuGR2")
										  dgvRow.Cells(31).Value = If(General_Class.CekNULL(row("WasteGR3_Pcs")) = "", 0, Format(row("WasteGR3_Pcs"), DigitDecimal))
										  dgvRow.Cells(32).Value = If(General_Class.CekNULL(row("WasteGR3_Persen")) = "", 0, Format(row("WasteGR3_Persen"), "N4"))

										  dgvRow.Cells(33).Value = If(General_Class.CekNULL(row("Reject_GR3_PCS")) = "", 0, Format(row("Reject_GR3_PCS"), DigitDecimal))
										  dgvRow.Cells(34).Value = If(General_Class.CekNULL(row("Reject_GR3_KG")) = "", 0, Format(row("Reject_GR3_KG"), "N4"))
										  dgvRow.Cells(35).Value = If(General_Class.CekNULL(row("ScrapGR3_PCS")) = "", 0, Format(row("ScrapGR3_PCS"), DigitDecimal))
										  dgvRow.Cells(36).Value = If(General_Class.CekNULL(row("ScrapGR3_KG")) = "", 0, Format(row("ScrapGR3_KG"), "N4"))
										  dgvRow.Cells(37).Value = If(General_Class.CekNULL(row("NilaiGRFinal_Pcs")) = "", 0, Format(row("NilaiGRFinal_Pcs"), DigitDecimal))
										  dgvRow.Cells(38).Value = If(General_Class.CekNULL(row("GR3_StockSementara")) = "", 0, Format(row("GR3_StockSementara"), DigitDecimal))
										  dgvRow.Cells(39).Value = row("WaktuGR3")
										  dgvRow.Cells(40).Value = Format(row("Tanggal_GI"), "dd MMM yyyy")
										  dgvRow.Cells(41).Value = row("Jam_GI")
										  dgvRow.Cells(42).Value = Format(row("Jumlah_Scrap_Packaging"), "N4")
										  dgvRow.Cells(43).Value = row("Flag_Preservative")
										  dgvRow.Cells(45).Value = If(General_Class.CekNULL(row("GR2_Stock_Belum_Validasi")) = "", 0, Format(row("GR2_Stock_Belum_Validasi"), DigitDecimal))

									  Next

								  End If
							  Finally
								  Dgv_Detail2.ResumeLayout()
							  End Try
...
#End Region
	...
	Private Function LoadDataDetailSplitAsync(token As CancellationToken, Optional ByVal isFilter As Boolean = False)
		Dim DigitDecimal As String = ""
		Dim tgl1Value, tgl2Value As Date
		Dim idRoutingText As String = ""
		Dim kdBarangText As String = ""
		...
		Try
	       ...
	       "
			...
			Using Ds = BindingTrans_Thread(SQL_DetailSplit)
				Dim dt As DataTable = Ds.Tables("MyTable")
				token.ThrowIfCancellationRequested()
				'Return Ds.Tables("MyTable").Copy()
...
#Region " Komen"
				...
				Me.Invoke(Sub()
							  ...
							  Try
								  Dgv_Detail2_Split.SuspendLayout()
								  Dgv_Detail2_Split.Rows.Clear()

								  If dt.Rows.Count > 0 Then
									  Dim BoldFont As New Font(Dgv_Detail2_Split.Font, FontStyle.Bold)
									  Dgv_Detail2_Split.RowCount = dt.Rows.Count

									  For i As Integer = 0 To dt.Rows.Count - 1
										  If i Mod 100 = 0 Then token.ThrowIfCancellationRequested()

										  Dim row As DataRow = dt.Rows(i)
										  Dim dgvRow As DataGridViewRow = Dgv_Detail2_Split.Rows(i)
										  ...
										  dgvRow.Cells(4).Value = Format(row("Tgl_Produksi"), "dd MMM yyyy")
										  dgvRow.Cells(5).Value = row("Jam_Produksi")
										  dgvRow.Cells(6).Value = Format(row("Tanggal_GI"), "dd MMM yyyy")
										  dgvRow.Cells(7).Value = row("Jam_GI")
										  dgvRow.Cells(8).Value = row("Nama_Routing")
										  dgvRow.Cells(9).Value = row("Keterangan")
										  dgvRow.Cells(10).Value = If(General_Class.CekNULL(row("Jumlah")) = "", 0, Format(row("Jumlah"), DigitDecimal))
										  dgvRow.Cells(11).Value = If(General_Class.CekNULL(row("Berat_GI")) = "", 0, Format(row("Berat_GI"), "N4"))
										  dgvRow.Cells(12).Value = If(General_Class.CekNULL(row("Jumlah_Dosing")) = "", 0, Format(row("Jumlah_Dosing"), "N4"))
										  dgvRow.Cells(13).Value = If(General_Class.CekNULL(row("WasteGR1_KG")) = "", 0, Format(row("WasteGR1_KG"), "N4"))
										  dgvRow.Cells(14).Value = If(General_Class.CekNULL(row("WasteGR1_Persen")) = "", 0, Format(row("WasteGR1_Persen"), "N4"))
										  dgvRow.Cells(15).Value = If(General_Class.CekNULL(row("Reject_GR1_KG")) = "", 0, Format(row("Reject_GR1_KG"), "N4"))
										  dgvRow.Cells(16).Value = If(General_Class.CekNULL(row("ScrapGR1_KG")) = "", 0, Format(row("ScrapGR1_KG"), "N4"))
										  dgvRow.Cells(17).Value = If(General_Class.CekNULL(row("Loss_Production")) = "", 0, Format(row("Loss_Production"), "N4"))
										  dgvRow.Cells(18).Value = If(General_Class.CekNULL(row("Loss_Production_Persen")) = "", 0, Format(row("Loss_Production_Persen"), "N4"))
										  dgvRow.Cells(19).Value = If(General_Class.CekNULL(row("NilaiGR1_KG")) = "", 0, Format(row("NilaiGR1_KG"), "N4"))
										  dgvRow.Cells(20).Value = If(General_Class.CekNULL(row("NilaiGR1_Pcs")) = "", 0, Format(row("NilaiGR1_Pcs"), DigitDecimal))
										  dgvRow.Cells(21).Value = If(General_Class.CekNULL(row("GR1_StockSementara")) = "", 0, Format(row("GR1_StockSementara"), "N4"))
										  dgvRow.Cells(22).Value = row("WaktuGR1")

										  dgvRow.Cells(23).Value = If(General_Class.CekNULL(row("WasteGR2_Pcs")) = "", 0, Format(row("WasteGR2_Pcs"), DigitDecimal))
										  dgvRow.Cells(24).Value = If(General_Class.CekNULL(row("WasteGR2_Persen")) = "", 0, Format(row("WasteGR2_Persen"), "N4"))
										  dgvRow.Cells(25).Value = If(General_Class.CekNULL(row("Persentase_Waste_Terhadap_GR2")) = "", 0, Format(row("Persentase_Waste_Terhadap_GR2"), DigitDecimal))
										  dgvRow.Cells(26).Value = If(General_Class.CekNULL(row("Reject_GR2_PCS")) = "", 0, Format(row("Reject_GR2_PCS"), DigitDecimal))
										  dgvRow.Cells(27).Value = If(General_Class.CekNULL(row("Reject_GR2_KG")) = "", 0, Format(row("Reject_GR2_KG"), "N4"))
										  dgvRow.Cells(28).Value = If(General_Class.CekNULL(row("ScrapGR2_Pcs")) = "", 0, Format(row("ScrapGR2_Pcs"), DigitDecimal))
										  dgvRow.Cells(29).Value = If(General_Class.CekNULL(row("ScrapGR2_KG")) = "", 0, Format(row("ScrapGR2_KG"), "N4"))
										  dgvRow.Cells(30).Value = If(General_Class.CekNULL(row("NilaiGR2_Pcs")) = "", 0, Format(row("NilaiGR2_Pcs"), DigitDecimal))
										  dgvRow.Cells(31).Value = If(General_Class.CekNULL(row("GR2_StockSementara")) = "", 0, Format(row("GR2_StockSementara"), DigitDecimal))
										  dgvRow.Cells(32).Value = If(General_Class.CekNULL(row("GR2_Stock_Belum_Validasi")) = "", 0, Format(row("GR2_Stock_Belum_Validasi"), DigitDecimal))
										  dgvRow.Cells(33).Value = row("WaktuGR2")

										  dgvRow.Cells(34).Value = If(General_Class.CekNULL(row("WasteGR3_Pcs")) = "", 0, Format(row("WasteGR3_Pcs"), DigitDecimal))
										  dgvRow.Cells(35).Value = If(General_Class.CekNULL(row("WasteGR3_Persen")) = "", 0, Format(row("WasteGR3_Persen"), "N4"))
										  dgvRow.Cells(36).Value = If(General_Class.CekNULL(row("Reject_GR3_PCS")) = "", 0, Format(row("Reject_GR3_PCS"), DigitDecimal))
										  dgvRow.Cells(37).Value = If(General_Class.CekNULL(row("Reject_GR3_KG")) = "", 0, Format(row("Reject_GR3_KG"), "N4"))
										  dgvRow.Cells(38).Value = If(General_Class.CekNULL(row("ScrapGR3_Pcs")) = "", 0, Format(row("ScrapGR3_Pcs"), DigitDecimal))
										  dgvRow.Cells(39).Value = If(General_Class.CekNULL(row("ScrapGR3_KG")) = "", 0, Format(row("ScrapGR3_KG"), "N4"))
										  dgvRow.Cells(40).Value = If(General_Class.CekNULL(row("NilaiGRFinal_Pcs")) = "", 0, Format(row("NilaiGRFinal_Pcs"), DigitDecimal))
										  dgvRow.Cells(41).Value = If(General_Class.CekNULL(row("GR3_StockSementara")) = "", 0, Format(row("GR3_StockSementara"), DigitDecimal))
										  dgvRow.Cells(42).Value = row("WaktuGR3")
										  dgvRow.Cells(43).Value = If(General_Class.CekNULL(row("Jumlah_Scrap_Packaging")) = "", 0, Format(row("Jumlah_Scrap_Packaging"), "N4"))
										  dgvRow.Cells(44).Value = If(General_Class.CekNULL(row("Flag_Preservative")) = "", "", row("Flag_Preservative"))

									  Next

								  End If
							  Finally
								  Dgv_Detail2_Split.ResumeLayout()
...
#End Region
	...
	Private Sub SetStyleDGV()

		'=====================
		'=     DGV REKAP     =
		'=====================
		Dgv_Rekap.Columns(11).DefaultCellStyle.BackColor = Color.FromArgb(255, 206, 117)
		Dgv_Rekap.Columns(12).DefaultCellStyle.BackColor = Color.FromArgb(255, 206, 117)
		Dgv_Rekap.Columns(13).DefaultCellStyle.BackColor = Color.FromArgb(255, 206, 117)
		Dgv_Rekap.Columns(14).DefaultCellStyle.BackColor = Color.FromArgb(255, 206, 117)
		Dgv_Rekap.Columns(15).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Rekap.Columns(16).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Rekap.Columns(17).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Rekap.Columns(18).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Rekap.Columns(19).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Rekap.Columns(20).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Rekap.Columns(21).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Rekap.Columns(22).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Rekap.Columns(23).DefaultCellStyle.BackColor = Color.LightGreen
		Dgv_Rekap.Columns(24).DefaultCellStyle.BackColor = Color.LightCoral
		Dgv_Rekap.Columns(25).DefaultCellStyle.BackColor = Color.LightCoral
		Dgv_Rekap.Columns(26).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Rekap.Columns(27).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Rekap.Columns(28).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(29).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(30).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(31).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(32).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(33).DefaultCellStyle.BackColor = Color.LightCyan
		Dgv_Rekap.Columns(36).DefaultCellStyle.BackColor = Color.FromArgb(244, 208, 111)
		Dgv_Rekap.Columns(37).DefaultCellStyle.BackColor = Color.FromArgb(188, 158, 130)

		Dim BoldFontRekap As New Font(Dgv_Rekap.Font, FontStyle.Bold)
		Dgv_Rekap.Columns(32).DefaultCellStyle.Font = BoldFontRekap

		'======================
		'=     DGV DETAIL     =
		'======================
		Dgv_Detail2.Columns(12).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(13).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(14).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(15).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(16).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(17).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(18).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(19).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(20).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2.Columns(21).DefaultCellStyle.BackColor = Color.FromArgb(252, 105, 108)
		Dgv_Detail2.Columns(22).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(23).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(24).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(25).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(26).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(27).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(28).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(29).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(44).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(45).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2.Columns(30).DefaultCellStyle.BackColor = Color.FromArgb(181, 230, 162)
		Dgv_Detail2.Columns(31).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(32).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(33).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(34).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(35).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(36).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(37).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(38).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2.Columns(39).DefaultCellStyle.BackColor = Color.White
		Dgv_Detail2.Columns(42).DefaultCellStyle.BackColor = Color.FromArgb(244, 208, 111)
		Dgv_Detail2.Columns(43).DefaultCellStyle.BackColor = Color.FromArgb(188, 158, 130)

		Dim BoldFontDetail As New Font(Dgv_Detail2.Font, FontStyle.Bold)
		Dgv_Detail2.Columns(20).DefaultCellStyle.Font = BoldFontDetail
		Dgv_Detail2.Columns(29).DefaultCellStyle.Font = BoldFontDetail
		Dgv_Detail2.Columns(38).DefaultCellStyle.Font = BoldFontDetail

		'============================
		'=     DGV DETAIL SPLIT     =
		'============================

		Dgv_Detail2_Split.Columns(13).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(14).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(15).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(16).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(17).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(18).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(19).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(20).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(21).DefaultCellStyle.BackColor = Color.LightYellow
		Dgv_Detail2_Split.Columns(22).DefaultCellStyle.BackColor = Color.FromArgb(252, 105, 108)
		Dgv_Detail2_Split.Columns(23).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(24).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(25).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(26).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(27).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(28).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(29).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(30).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(31).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(32).DefaultCellStyle.BackColor = Color.LightBlue
		Dgv_Detail2_Split.Columns(33).DefaultCellStyle.BackColor = Color.FromArgb(181, 230, 162)
		Dgv_Detail2_Split.Columns(34).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(35).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(36).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(37).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(38).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(39).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(40).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(41).DefaultCellStyle.BackColor = Color.LightGray
		Dgv_Detail2_Split.Columns(42).DefaultCellStyle.BackColor = Color.White
		Dgv_Detail2_Split.Columns(43).DefaultCellStyle.BackColor = Color.FromArgb(244, 208, 111)
		Dgv_Detail2_Split.Columns(44).DefaultCellStyle.BackColor = Color.FromArgb(188, 158, 130)

		Dim BoldFontDetailSplit As New Font(Dgv_Detail2_Split.Font, FontStyle.Bold)
		Dgv_Detail2_Split.Columns(22).DefaultCellStyle.Font = BoldFontDetailSplit
		Dgv_Detail2_Split.Columns(32).DefaultCellStyle.Font = BoldFontDetailSplit
		Dgv_Detail2_Split.Columns(42).DefaultCellStyle.Font = BoldFontDetailSplit

	End Sub

	Private Sub RenderDgvRekap(dt As DataTable)
	...
	Private Sub RenderDgvDetail(dt As DataTable)
		If dt Is Nothing OrElse dt.Rows.Count = 0 Then Exit Sub
	...
	Private Sub RenderDgvDetailSplit(dt As DataTable)
		If dt Is Nothing OrElse dt.Rows.Count = 0 Then Exit Sub
